...
fn print_usage() {
    println!(
        "Usage: cargo bench -p nimbus-engine --bench libsql-replica-provider-benchmarks -- [--markdown <path>] [--workload <slug>] [--local-cache-encryption <disabled|temp-master-key-file>] [--libsql-url <url>] [--libsql-auth-token <token>] [--libsql-admin-url <url>] [--libsql-admin-auth-header <header>]"
    );
}
...
impl BenchmarkLane {
    ...
    pub(super) fn warmup_rounds(self) -> usize {
        match self {
            Self::SteadyState => read_round_override(
                "NIMBUS_LIBSQL_REPLICA_BENCH_STEADY_WARMUP_ROUNDS",
                STEADY_STATE_WARMUP_ROUNDS,
            ),
            Self::ColdStart => read_round_override(
                "NIMBUS_LIBSQL_REPLICA_BENCH_COLD_WARMUP_ROUNDS",
                COLD_START_WARMUP_ROUNDS,
            ),
            Self::ReplicaOperational => read_round_override(
                "NIMBUS_LIBSQL_REPLICA_BENCH_OPERATIONAL_WARMUP_ROUNDS",
    ...
    pub(super) fn measure_rounds(self) -> usize {
        match self {
            Self::SteadyState => read_round_override(
                "NIMBUS_LIBSQL_REPLICA_BENCH_STEADY_MEASURE_ROUNDS",
                STEADY_STATE_MEASURE_ROUNDS,
            ),
            Self::ColdStart => read_round_override(
                "NIMBUS_LIBSQL_REPLICA_BENCH_COLD_MEASURE_ROUNDS",
                COLD_START_MEASURE_ROUNDS,
            ),
            Self::ReplicaOperational => read_round_override(
                "NIMBUS_LIBSQL_REPLICA_BENCH_OPERATIONAL_MEASURE_ROUNDS",
