...
mod scenarios;
#[path = "postgres_provider_benchmarks/suite.rs"]
mod suite;
#[path = "postgres_provider_benchmarks/support.rs"]
mod support;
...
#[path = "postgres_provider_benchmarks/workloads.rs"]
mod workloads;
...
const BENCH_POSTGRES_URL_ENV: &str = "NIMBUS_BENCH_POSTGRES_URL";
...
fn print_usage() {
    println!(
        "Usage: cargo bench -p nimbus-engine --bench postgres-provider-benchmarks -- [--markdown <path>] [--workload <slug>] [--postgres-url <connection-string>] [--rtt-ms <delay>]"
    );
}
...
impl BenchmarkLane {
    ...
    fn notes(self) -> &'static str {
        match self {
            ...
            Self::RttSensitive => {
                "compares Postgres loopback against the same engine path through a local injected-latency TCP proxy"
            }
        }
    }
    ...
    fn warmup_rounds(self) -> usize {
        match self {
            Self::SteadyState => read_round_override(
                "NIMBUS_PG_BENCH_STEADY_WARMUP_ROUNDS",
                STEADY_STATE_WARMUP_ROUNDS,
            ),
            Self::ColdStart => read_round_override(
                "NIMBUS_PG_BENCH_COLD_WARMUP_ROUNDS",
                COLD_START_WARMUP_ROUNDS,
            ),
            Self::RttSensitive => {
                read_round_override("NIMBUS_PG_BENCH_RTT_WARMUP_ROUNDS", RTT_WARMUP_ROUNDS)
            }
        }
    }
    ...
    fn measure_rounds(self) -> usize {
        match self {
            Self::SteadyState => read_round_override(
                "NIMBUS_PG_BENCH_STEADY_MEASURE_ROUNDS",
                STEADY_STATE_MEASURE_ROUNDS,
            ),
            Self::ColdStart => read_round_override(
                "NIMBUS_PG_BENCH_COLD_MEASURE_ROUNDS",
                COLD_START_MEASURE_ROUNDS,
            ),
            Self::RttSensitive => {
                read_round_override("NIMBUS_PG_BENCH_RTT_MEASURE_ROUNDS", RTT_MEASURE_ROUNDS)
            }
        }
    }
}
