...
fn print_usage() {
    println!(
        "Usage: cargo bench -p nimbus-engine --bench mysql-provider-benchmarks -- [--markdown <path>] [--workload <slug>] [--mysql-url <connection-string>] [--rtt-ms <delay>]"
    );
}
...
impl BenchmarkLane {
    ...
    fn warmup_rounds(self) -> usize {
        match self {
            Self::SteadyState => read_round_override(
                "NIMBUS_MYSQL_BENCH_STEADY_WARMUP_ROUNDS",
                STEADY_STATE_WARMUP_ROUNDS,
            ),
            Self::ColdStart => read_round_override(
                "NIMBUS_MYSQL_BENCH_COLD_WARMUP_ROUNDS",
                COLD_START_WARMUP_ROUNDS,
            ),
            Self::RttSensitive => {
                read_round_override("NIMBUS_MYSQL_BENCH_RTT_WARMUP_ROUNDS", RTT_WARMUP_ROUNDS)
            }
        }
    }
    ...
    fn measure_rounds(self) -> usize {
        match self {
            Self::SteadyState => read_round_override(
                "NIMBUS_MYSQL_BENCH_STEADY_MEASURE_ROUNDS",
                STEADY_STATE_MEASURE_ROUNDS,
            ),
            Self::ColdStart => read_round_override(
                "NIMBUS_MYSQL_BENCH_COLD_MEASURE_ROUNDS",
                COLD_START_MEASURE_ROUNDS,
            ),
            Self::RttSensitive => {
                read_round_override("NIMBUS_MYSQL_BENCH_RTT_MEASURE_ROUNDS", RTT_MEASURE_ROUNDS)
            }
        }
    }
}
