...
/// own at-rest encryption; Nimbus does not claim to encrypt those.
...
///
/// The key provider determines how Nimbus wraps per-database data-encryption
/// keys (DEKs). The same key provider is used across all Nimbus-owned local
...
pub struct MasterKeyFileConfig {
    ...
    /// permissions. Nimbus reads it at startup but does not store the key
    ...
    pub fn postgres(connection_string: impl Into<String>) -> Self {
        Self {
            ...
            routing: TenantRoutingConfig::SchemaPerTenant {
                metadata_schema: "nimbus_provider".to_string(),
