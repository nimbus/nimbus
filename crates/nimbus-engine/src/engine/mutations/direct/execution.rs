...
impl Engine {
    pub(super) fn apply_mutation_with_mode(
        ...
        let eviction_completion = initiated_eviction.then(|| runtime.eviction_completion());
        // The committer actor is the sole registry/store finalizer. Direct
        // callers retain only the store-independent completion token so they
        // cannot keep redb open while waiting for a replacement runtime.
        drop(runtime);
