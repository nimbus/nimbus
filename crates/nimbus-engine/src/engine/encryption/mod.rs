//! Engine-level encryption management.
//!
//! This module provides key provider initialization, manifest-backed runtime
//! wiring, and encryption diagnostics for the engine layer.
...
use crate::persistence_config::{
    EncryptionConfigDescriptor, EnginePersistenceConfig, LocalEncryptionConfig,
    LocalPersistenceFamily,
};
...
/// Returns a diagnostics-safe descriptor for the engine's encryption state.
#[allow(dead_code)]
pub fn encryption_descriptor(config: &LocalEncryptionConfig) -> EncryptionConfigDescriptor {
    config.descriptor()
}

/// Encryption state for engine status reporting.
...
impl EncryptionStatus {
    /// Creates the encryption status from engine config.
    pub fn from_config(config: &EnginePersistenceConfig) -> Self {
