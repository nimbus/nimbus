...
impl TenantPersistence {
    ...
    delegate_store_method!(fn read_durable_journal_from(&self, sequence: SequenceNumber) -> Result<Vec<TenantEventRecord>>);
    ...
    pub(crate) fn append_durable_records_batch(&self, records: &[TenantEventRecord]) -> Result<()> {
        match_tenant_persistence!(self, |store| store.append_durable_records_batch(records))
    }

    pub(crate) fn apply_durable_records_batch(&self, records: &[TenantEventRecord]) -> Result<()> {
