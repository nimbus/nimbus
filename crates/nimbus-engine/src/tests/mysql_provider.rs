...
async fn mysql_ppsc_seeded_three_route_differential() {
    with_mysql_engine_config(|engine_config, _provider_config| async move {
        exercise_ppsc_provider_three_route_differential(PpscBackend::Mysql, engine_config).await;
    })
    .await;
}
...
async fn typed_mysql_config_supports_async_schema_mutation_journal_and_scheduler_paths() {
    with_mysql_engine_config(|engine_config, _provider_config| async move {
        ...
        let pipeline = engine
            ...
            .expect("MySQL tenants should expose provider pipeline diagnostics");
        assert_eq!(pipeline.adapter, "mysql");
        assert_eq!(pipeline.configured_max_in_flight, 1);
        assert!(pipeline.batch_attempt_count >= 1);
        assert_eq!(
            pipeline.journal_statement_count, pipeline.batch_attempt_count,
            "every MySQL journal batch should use one journal statement"
        );
        assert!(pipeline.journal_record_count >= pipeline.journal_statement_count);
        assert_eq!(pipeline.max_observed_in_flight, 1);
