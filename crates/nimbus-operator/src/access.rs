...
pub const LOCAL_SESSION_COOKIE_NAME: &str = "nimbus_session";
...
#[derive(Debug, Clone, Eq, PartialEq)]
pub struct LocalAdminTokenRotation {
    pub token: LocalAdminTokenRecord,
...
impl LocalServerSecurityState {
    ...
    pub fn record_audit_event(&self, event: LocalServerAuditEvent) -> io::Result<()> {
    ...
    pub fn rotate_and_persist_token_with_outcome(&self) -> io::Result<LocalAdminTokenRotation> {
    ...
    #[cfg(test)]
    pub fn register_session_for_test(&self, session_id: &str) {
        let now = OffsetDateTime::now_utc();
        let mut guard = self
            .inner
            .write()
...
mod tests {
    use super::*;
    use crate::token::{load_local_admin_token, load_or_create_local_admin_token};
