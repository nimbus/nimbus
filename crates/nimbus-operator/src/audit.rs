...
#[derive(Debug)]
pub struct LocalServerAuditLog {
    path: PathBuf,
    write_lock: Mutex<()>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct LocalServerAuditEvent {
    pub route_family: LocalServerRouteFamily,
...
pub struct LocalServerAuditRecord {
    pub ts: String,
...
impl LocalServerAuditLog {
    pub fn new(paths: &LocalServerPaths) -> Self {
        Self {
    ...
    pub fn append(&self, paths: &LocalServerPaths, event: LocalServerAuditEvent) -> io::Result<()> {
...
pub fn tenant_id_from_request(path: &str, headers: &HeaderMap) -> Option<String> {
    tenant_id_from_path(path).or_else(|| firebase_project_id_from_headers(headers))
}

pub fn tenant_id_from_path(path: &str) -> Option<String> {
