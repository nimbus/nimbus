...
tmpdir.refresh();
fs.mkdirSync(fixtures.path('wasi/subdir'), { recursive: true });
const wasmDir = path.join(__dirname, '..', 'wasi', 'wasm');
const nimbusRuntimeTestStdinPath = globalThis.__nimbusRuntimeTestStdinPath;
const nimbusRuntimeTestStdinFd =
  typeof nimbusRuntimeTestStdinPath === 'string' && nimbusRuntimeTestStdinPath.length > 0
    ? fs.openSync(nimbusRuntimeTestStdinPath, 'r')
    : 0;
const wasiPreview1 = new WASI({
  version: 'preview1',
  args: ['foo', '-bar', '--baz=value'],
  env: process.env,
  stdin: nimbusRuntimeTestStdinFd,
  preopens: {
...
function nimbusMaybeRewriteWasiImport(importObject) {
  return importObject;
}

const nimbusMainScriptPromise = (async () => {
  const importObject = nimbusMaybeRewriteWasiImport({
  ...
  if (nimbusRuntimeTestStdinFd > 2) {
    fs.closeSync(nimbusRuntimeTestStdinFd);
  }
});

globalThis.__nimbusMainScriptPromise = nimbusMainScriptPromise;
nimbusMainScriptPromise.then(common.mustCall());
