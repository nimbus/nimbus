...
function fixtureFileURL(...segments) {
  return pathToFileURL(fixturePath(...segments));
}

function readFixtureSync(args, enc) {
  if (Array.isArray(args)) {
    return fs.readFileSync(fixturePath(...args), enc);
  }
  return fs.readFileSync(fixturePath(args), enc);
}

function readFixtureKey(name, enc) {
  return fs.readFileSync(fixturePath('keys', name), enc);
}
...
module.exports = {
  fixturesDir,
  path: fixturePath,
  fileURL: fixtureFileURL,
  readSync: readFixtureSync,
  readKey: readFixtureKey,
  readKeys: readFixtureKeys,
  utf8TestText:
    '永和九年，嵗在癸丑，暮春之初，會於會稽山隂之蘭亭，脩稧事也。' +
  ...
  get utf8TestTextPath() {
    return fixturePath('utf8_test_text.txt');
  },
};
