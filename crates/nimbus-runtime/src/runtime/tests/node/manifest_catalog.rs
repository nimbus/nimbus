...
pub(super) fn load_family_catalogs_from_disk() -> NodeCompatResolvedManifestCatalog {
    ...
    let lane_catalogs = lane_files
        ...
        .map(|file_name| {
            let path = repo_root.join(format!(
                "crates/nimbus-runtime/src/runtime/tests/node_compat_manifests/lanes/{file_name}"
            ));
    ...
    let family_catalogs = family_files
        ...
        .map(|file_name| {
            let path = repo_root.join(format!(
                "crates/nimbus-runtime/src/runtime/tests/node_compat_manifests/fixtures/{file_name}"
            ));
