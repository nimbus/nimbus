...
fn classify_harness_mismatch(rendered_error: &str) -> bool {
    [
        ...
        "Nimbus node_compat harness is missing",
        "op_nimbus_runtime_test_",
    ]
    .iter()
...
try {{
  ...
  const result = await globalThis.__nimbusInvoke();
  ...
  if (processExitCode === 0) {{
    writeResult("pass");
  }} else if (error?.code === "NIMBUS_NODE_COMPAT_SKIP" || error?.__nimbusSkip === true) {{
...
fn node_compat_oracle_entrypoint_emits_fixture_artifact() {
    let lane = std::env::var("NIMBUS_NODE_COMPAT_ORACLE_LANE").expect("oracle lane should be set");
    let fixture =
        std::env::var("NIMBUS_NODE_COMPAT_ORACLE_FIXTURE").expect("oracle fixture should be set");
    let output_root = std::env::var("NIMBUS_NODE_COMPAT_ORACLE_OUTPUT_ROOT")
        .map(PathBuf::from)
        .unwrap_or_else(|_| default_oracle_output_root());
    let node_bin_override = std::env::var("NIMBUS_NODE_COMPAT_ORACLE_NODE_BIN")
        .ok()
