use nimbus_core::{Document, DocumentId, DocumentLocator, Error, Result, TableName};
use nimbus_runtime::{
    HostCallCancellation, HostCallPayload, NimbusRuntimeError, RuntimeAsyncDbDeletePayload,
...
use crate::responses::encode_runtime_core_result;

pub fn dispatch_document_host_call<H>(
    host: &H,
...
pub fn dispatch_document_host_call_cancellable<H>(
    host: &H,
    payload: HostCallPayload,
...
pub async fn dispatch_document_host_call_async<H>(
    host: &H,
    payload: HostCallPayload,
    cancellation: &HostCallCancellation,
) -> std::result::Result<Value, NimbusRuntimeError>
    ...
    H: RuntimeCapabilityHost + ?Sized,
{
    ...
    let fields = match resolve_runtime_input(json_object(payload.fields, "document insert fields"))
    {
        Ok(fields) => fields,
        Err(response) => return response,
    };
    ...
    H: RuntimeCapabilityHost + ?Sized,
{
    ...
    let patch = match resolve_runtime_input(json_object(payload.patch, "document patch payload")) {
        Ok(patch) => patch,
        Err(response) => return response,
    };
    encode_runtime_core_result(
...
fn resolve_runtime_input<T>(
    result: Result<T>,
) -> std::result::Result<T, std::result::Result<Value, NimbusRuntimeError>> {
    match result {
        Ok(value) => Ok(value),
