...
mod tests {
    ...
    fn deterministic_test_case_formats_context_and_repro() {
        ...
        assert_eq!(
            ...
            "subscription bootstrap should recover; case websocket-auth-change-resubscribe [run-to-completion-snapshot]: auth changes force explicit resubscribe for runtime-backed subscriptions Repro: cargo test -p neovex-server convex_websocket_auth_change_drops_active_subscriptions_until_resubscribed -- --nocapture"
        );
    }
}
