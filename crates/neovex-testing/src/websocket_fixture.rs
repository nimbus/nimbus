...
impl WebSocketFixture {
    pub async fn connect_request(
        ...
        let should_handshake_neovex_v2 = should_offer_neovex_v2(request.uri().path());
        if should_handshake_neovex_v2 && !request.headers().contains_key(SEC_WEBSOCKET_PROTOCOL) {
            request.headers_mut().insert(
                SEC_WEBSOCKET_PROTOCOL,
                http::HeaderValue::from_static("neovex.v2"),
            );
        }
        let (mut socket, _) = connect_async(request).await?;
        if should_handshake_neovex_v2 {
            complete_neovex_v2_handshake(&mut socket).await?;
        }
        Ok(Self { socket })
    }
...
fn should_offer_neovex_v2(path: &str) -> bool {
    !path.starts_with("/google.firestore.v1.Firestore/Listen")
}

async fn complete_neovex_v2_handshake(
