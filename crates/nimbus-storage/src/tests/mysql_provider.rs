...
async fn mysql_durable_replay_retires_recreated_table_identity() {
    with_test_provider(|provider, _config| async move {
        ...
        let old_document = super::Document::new(
            table.clone(),
            serde_json::Map::from_iter([("title".to_string(), serde_json::json!("old"))]),
        );
        let new_document = super::Document::new(
            table.clone(),
...
fn mysql_indexed_rank_schema(table: &TableName) -> (TableSchema, IndexDefinition) {
...
fn mysql_ranked_document(table: &TableName, title: &str, rank: u64) -> Document {
    Document::new(
        table.clone(),
        serde_json::Map::from_iter([
...
fn mysql_status_rank_schema(table: &TableName) -> TableSchema {
    TableSchema {
        table: table.clone(),
        fields: vec![
            FieldSchema {
...
fn mysql_status_rank_document(table: &TableName, title: &str, status: &str, rank: u64) -> Document {
    Document::new(
        table.clone(),
        serde_json::Map::from_iter([
            ("title".to_string(), serde_json::json!(title)),
...
fn mysql_historical_read_shape(
    table: &TableName,
    table_id: &TableId,
    schema: &TableSchema,
...
fn mysql_historical_snapshot(sequence: SequenceNumber) -> nimbus_core::HistoricalReadSnapshot {
...
fn mysql_rank_full_scan_oracle_titles(
...
fn mysql_status_rank_full_scan_oracle_titles(
    store: &crate::MySqlTenantStore,
    table_id: &TableId,
    corpus: &[&Document],
    sequence: SequenceNumber,
...
fn mysql_document_titles(documents: &[Document]) -> Vec<&str> {
    documents
        .iter()
        .map(|document| {
            document
...
fn mysql_document_title_strings(documents: &[Document]) -> Vec<String> {
    documents.iter().map(mysql_document_title_string).collect()
}

fn mysql_document_title_string(document: &Document) -> String {
    document
        .fields
        .get("title")
        .and_then(|value| value.as_str())
...
fn mysql_active_table_id_for_diagnostic(
...
async fn document_generated_column_expressions(
