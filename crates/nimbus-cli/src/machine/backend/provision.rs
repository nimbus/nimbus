...
impl ForwardedMachineProvisionAdapter {
    ...
    pub(crate) fn retire_parent_publication(
        &self,
        ...
        {
            return Err(Error::PreconditionFailed(
                "machine publication retirement uses crossed forwarder authority".to_owned(),
            ));
        }
        if retirement.members().is_empty() {
            return self.publication_journal.mark_retired(retirement);
        }
        let plan_id = retirement.members()[0]
            .request()
            .plan_id()
            .ok_or_else(|| {
        ...
        if retirement
            .members()
            .iter()
            .any(|member| member.request().plan_id() != Some(&plan_id))
        {
            return Err(Error::PreconditionFailed(
        ...
        let live = batches.remove(&plan_id);
        let Some(live) = live else {
            drop(batches);
            return super::retire_recovered_parent_publication(
                &self.port_leases,
                &self.publication_journal,
                retirement,
            );
        };
        if live.members != retirement.members() {
            batches.insert(plan_id, live);
            return Err(Error::PreconditionFailed(
                "live parent publication members differ from retirement authority".to_owned(),
            ));
        }
        let bindings = retirement
            .members()
            .iter()
            .map(|member| (member.request().clone(), member.expected_binding().clone()))
            .collect::<Vec<_>>();
        let release = self
            ...
            .and_then(|_| self.publication_journal.mark_retired(retirement));
        if release.is_err() {
            batches.insert(plan_id, live);
        }
        release.map(|_| ())
