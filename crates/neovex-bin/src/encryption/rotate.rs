...
fn rotate_all_manifests(
        ...
        .map_err(|e| Error::Internal(format!("failed to read directory: {e}")))?
    {
        ...
        if path.extension().map(|e| e == "neovex-enc").unwrap_or(false) {
            found_count += 1;
...
fn rotate_sqlite_dek(
    ...
    if let Err(error) = neovex::rekey_encrypted_database_at_path(path, &current_dek, &new_dek) {
...
mod tests {
    ...
    fn infer_new_provider_detects_aws_kms_inputs() {
        let mut command = base_rotate_kek_command();
        command.new_aws_kms_key_id = Some("alias/neovex-prod".to_string());
