...
use clap::Args;
use neovex::{EncryptionConfigDescriptor, LocalPersistenceFamily, ServicePersistenceConfig};
...
fn print_text_status(
    ...
    if enabled {
        ...
        println!("To enable, set --encryption-key-provider or NEOVEX_ENCRYPTION_KEY_PROVIDER.");
    }
}
...
fn print_json_status(
    ...
    let json = serde_json::to_string_pretty(&status).map_err(|e| {
        neovex::Error::Internal(format!("failed to serialize encryption status: {e}"))
    })?;
    println!("{json}");
    Ok(())
}
