...
use super::super::bootstrap::{GUEST_NEOVEX_BIN, GUEST_NEOVEX_SOCKET};
...
use super::{
    DEFAULT_GUEST_NEOVEX_BINARY_ARCHIVE_NAME_ARM64,
    DEFAULT_GUEST_NEOVEX_BINARY_ARCHIVE_NAME_X86_64, DEFAULT_GUEST_NEOVEX_RELEASE_BASE_URL,
    DesiredGuestNeovexBinaryStatus, GUEST_NEOVEX_BINARY_OVERRIDE_ENV,
    GUEST_NEOVEX_RELEASE_BASE_URL_ENV, HTTP_IMAGE_TIMEOUT, LOCAL_GUEST_BINARY_HELP_TEXT,
    ObservedGuestNeovexBinaryStatus, StartupSignalMonitor,
};
...
pub(super) fn requires_host_guest_neovex_sync(config: &MachineConfigRecord) -> bool {
...
fn sync_guest_neovex_binary(
    ...
    super::emit_machine_progress("Ensuring guest neovex binary");
    ...
    if current_hash.as_deref() != Some(desired_hash.as_str()) {
        super::emit_machine_progress("Updating guest neovex binary inside the machine");
...
pub(super) fn ensure_guest_neovex_socket_shell_script() -> String {
    format!(
...
pub(super) fn inspect_desired_guest_neovex_binary(
    ...
    if let Some(path) = env::var_os(GUEST_NEOVEX_BINARY_OVERRIDE_ENV).map(PathBuf::from) {
        ...
        return DesiredGuestNeovexBinaryStatus {
            ...
            source: super::GuestNeovexBinarySourceKind::ExplicitOverride,
    ...
    match guest_neovex_archive_name() {
        Ok(archive_name) => {
            ...
            DesiredGuestNeovexBinaryStatus {
                install_path: PathBuf::from(GUEST_NEOVEX_BIN),
                source: super::GuestNeovexBinarySourceKind::ReleaseAsset,
...
pub(super) fn resolve_guest_neovex_binary(paths: &MachinePaths) -> Result<PathBuf, Error> {
    if let Some(path) = env::var_os(GUEST_NEOVEX_BINARY_OVERRIDE_ENV).map(PathBuf::from) {
        if !path.is_file() {
            return Err(Error::InvalidInput(format!(
                "guest neovex binary override {} from ${GUEST_NEOVEX_BINARY_OVERRIDE_ENV} does not exist",
...
pub(super) fn guest_neovex_archive_name() -> Result<&'static str, Error> {
