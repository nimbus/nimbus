...
use super::super::jwt::{
    JsonWebKeySet, ParsedJwt, normalize_issuer, select_jwk, validate_temporal_claims,
    verify_signature,
};
...
impl ConvexAuthVerifier {
    ...
    async fn verify_token(
        &self,
        token: &str,
    ) -> Result<ResolvedAuthIdentity, ApplicationAuthError> {
        ...
        if verify_token_signature(&parsed, &jwks).is_err() {
            let refreshed_discovery = match &provider {
                ConvexAuthProvider::Oidc(oidc) => {
                    Some(self.refresh_oidc_discovery(&oidc.domain).await?)
                }
                ConvexAuthProvider::CustomJwt(_) => None,
            };
            if let Some(discovery) = refreshed_discovery.as_ref()
                && normalize_issuer(&discovery.issuer) != normalize_issuer(&parsed.claims.issuer)
            {
                return Err(ApplicationAuthError::unauthorized(
                    "token issuer does not match OIDC discovery metadata",
            ...
            let refreshed_jwks_source = provider.jwks_source(refreshed_discovery.as_ref())?;
            let refreshed_jwks = self.refresh_jwks(&refreshed_jwks_source).await?;
            verify_token_signature(&parsed, &refreshed_jwks)?;
        }
