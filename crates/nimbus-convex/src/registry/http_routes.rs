...
impl ConvexRegistry {
    ...
    pub fn resolve_http_route_for_method(
        &self,
        method: &str,
        request_path: &str,
    ) -> Option<&ConvexHttpRouteDefinition> {
    ...
    pub fn has_http_route_for_path(&self, request_path: &str) -> bool {
...
pub fn validate_runtime_http_route(
    ...
    let route_name = route.name.as_deref().ok_or_else(|| {
        NimbusRuntimeError::Contract("runtime http route is missing a route name".to_string())
    })?;
