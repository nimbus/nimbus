...
pub(crate) async fn ui_auth() -> Html<&'static str> {
    Html(
        "<!doctype html><html><head><meta charset=\"utf-8\"><title>Neovex Sign In</title></head><body><main><h1>Neovex</h1><form method=\"post\" action=\"/ui/auth/session\"><label>Local admin token <input type=\"password\" name=\"token\" autocomplete=\"off\" autofocus /></label><button type=\"submit\">Continue</button></form></main></body></html>",
    )
}
...
pub(crate) async fn create_ui_session(
    ...
    let mut response = if accept.is_some_and(|value| value.contains("application/json")) {
        ...
        Html(
            "<!doctype html><html><head><meta charset=\"utf-8\"><title>Neovex Ready</title></head><body><main><p>Session created.</p><p><a href=\"/ui/\">Open Neovex UI</a></p></main></body></html>",
        )
        .into_response()
    };
...
fn ui_shell_html(requested_path: &str) -> String {
    format!(
        "<!doctype html><html><head><meta charset=\"utf-8\"><title>Neovex UI</title></head><body><main><h1>Neovex UI</h1><p>Bootstrap route active.</p><p>Path: {requested_path}</p></main></body></html>"
    )
}
