    ...
    fn unauthorized_message(&self) -> &'static str {
        match self.credential_mode {
            LocalServerCredentialMode::AuthorizationOrAdminHeader => {
                "local admin access requires Authorization: Bearer <token> or X-Neovex-Admin-Token"
            }
            LocalServerCredentialMode::AdminHeaderOnly => {
                "deploy admin access requires X-Neovex-Admin-Token in addition to the deploy bearer token"
            }
        }
    }
}
...
fn extract_admin_header(headers: &HeaderMap) -> Result<Option<ExtractedCredential>, AppError> {
    ...
    let token = value
        .to_str()
        .map_err(|error| {
            AppError::unauthorized(format!("X-Neovex-Admin-Token must be valid UTF-8: {error}"))
        })?
        .trim();
    if token.is_empty() {
        return Err(AppError::unauthorized(
            "X-Neovex-Admin-Token must not be empty",
        ));
    }
