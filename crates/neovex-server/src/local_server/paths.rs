...
fn resolve_linux_paths(env: &BTreeMap<String, OsString>) -> io::Result<LocalServerPaths> {
    ...
    let server_discovery_path = if let Some(runtime_root) = env_path(env, "XDG_RUNTIME_DIR") {
        runtime_root.join("neovex").join("server.json")
    } else {
...
fn resolve_macos_paths(env: &BTreeMap<String, OsString>) -> io::Result<LocalServerPaths> {
    ...
    let server_discovery_path = if let Some(tmpdir) = env_path(env, "TMPDIR") {
        tmpdir.join("neovex").join("server.json")
    } else {
...
fn resolve_windows_paths(env: &BTreeMap<String, OsString>) -> io::Result<LocalServerPaths> {
    ...
    let neovex_root = local_app_data.join("neovex");
    Ok(LocalServerPaths {
        auth_token_path: neovex_root.join("auth").join("token.json"),
        server_discovery_path: neovex_root.join("run").join("server.json"),
        audit_log_path: neovex_root.join("logs").join("access.jsonl"),
    })
}
...
mod tests {
    ...
    fn linux_paths_use_xdg_overrides() {
        ...
        assert_eq!(
            ...
            PathBuf::from("/tmp/data/neovex/auth/token")
        );
        assert_eq!(
            paths.server_discovery_path,
            PathBuf::from("/tmp/runtime/neovex/server.json")
        );
        assert_eq!(
            paths.audit_log_path,
            PathBuf::from("/tmp/state/neovex/logs/access.jsonl")
        );
    }
    ...
    fn linux_paths_fall_back_to_home_convention() {
        ...
        assert_eq!(
            paths.auth_token_path,
            PathBuf::from("/Users/jack/.local/share/neovex/auth/token")
        );
        assert_eq!(
            paths.server_discovery_path,
            PathBuf::from("/Users/jack/.local/state/neovex/run/server.json")
        );
        assert_eq!(
            paths.audit_log_path,
            PathBuf::from("/Users/jack/.local/state/neovex/logs/access.jsonl")
        );
    }
    ...
    fn macos_paths_prefer_tmpdir_for_run_state() {
        let paths = LocalServerPaths::resolve_for_platform(
            ...
            &env(&[
                ("HOME", "/Users/jack"),
                ("TMPDIR", "/private/tmp/neovex-test"),
            ]),
        )
        ...
        assert_eq!(
            paths.auth_token_path,
            PathBuf::from("/Users/jack/Library/Application Support/neovex/auth/token")
        );
        assert_eq!(
            paths.server_discovery_path,
            PathBuf::from("/private/tmp/neovex-test/neovex/server.json")
        );
        assert_eq!(
            paths.audit_log_path,
            PathBuf::from("/Users/jack/Library/Logs/neovex/access.jsonl")
        );
    }
    ...
    fn macos_paths_fall_back_to_application_support_run_state() {
        ...
        assert_eq!(
            paths.server_discovery_path,
            PathBuf::from("/Users/jack/Library/Application Support/neovex/run/server.json")
        );
    }
