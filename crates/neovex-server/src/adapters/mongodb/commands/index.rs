...
use neovex_core::{IndexDefinition, TableName, TableSchema, TenantId};
...
fn get_or_create_schema(
    ...
    match service.get_table_schema(tenant_id, table) {
        Ok(schema) => Ok(schema),
        Err(neovex_core::Error::SchemaNotFound(_)) => Ok(TableSchema {
            table: table.clone(),
            fields: vec![],
            indexes: vec![],
            access_policy: None,
...
mod tests {
    ...
    fn create_schema_with_fields(fixture: &ServiceFixture<Service>, collection: &str) {
        ...
        let schema = TableSchema {
            table,
            fields: vec![
                neovex_core::FieldSchema {
                    name: "name".into(),
                    field_type: neovex_core::FieldType::String,
                    required: false,
                },
                neovex_core::FieldSchema {
                    name: "age".into(),
                    field_type: neovex_core::FieldType::Number,
                    required: false,
                },
            ],
            indexes: vec![],
