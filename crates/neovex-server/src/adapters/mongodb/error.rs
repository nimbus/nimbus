...
impl From<neovex_core::Error> for MongoError {
    fn from(err: neovex_core::Error) -> Self {
        let (ec, msg) = match &err {
            neovex_core::Error::DocumentNotFound(_) => (NAMESPACE_NOT_FOUND, err.to_string()),
            neovex_core::Error::TenantNotFound(_) => (NAMESPACE_NOT_FOUND, err.to_string()),
            neovex_core::Error::SchemaNotFound(_) => (NAMESPACE_NOT_FOUND, err.to_string()),
            neovex_core::Error::AlreadyExists(msg) if msg.contains("document") => {
                (DUPLICATE_KEY, err.to_string())
            }
            neovex_core::Error::AlreadyExists(_) => (NAMESPACE_EXISTS, err.to_string()),
            neovex_core::Error::InvalidInput(_) => (BAD_VALUE, err.to_string()),
            neovex_core::Error::SchemaValidation(_) => (BAD_VALUE, err.to_string()),
            neovex_core::Error::PermissionDenied(_) => (UNAUTHORIZED, err.to_string()),
            neovex_core::Error::Conflict(_) => (WRITE_CONFLICT, err.to_string()),
            neovex_core::Error::Serialization(_) => (BAD_VALUE, err.to_string()),
...
mod tests {
    ...
    fn core_not_found_maps_to_namespace_not_found() {
        let core_err =
            neovex_core::Error::DocumentNotFound(neovex_core::DocumentId::from_str("abc").unwrap());
    ...
    fn core_already_exists_maps_to_namespace_exists() {
        let core_err = neovex_core::Error::AlreadyExists("test".into());
        let mongo_err = MongoError::from(core_err);
        match mongo_err {
    ...
    fn core_invalid_input_maps_to_bad_value() {
        let core_err = neovex_core::Error::InvalidInput("bad field".into());
        let mongo_err = MongoError::from(core_err);
        match mongo_err {
    ...
    fn core_permission_denied_maps_to_unauthorized() {
        let core_err = neovex_core::Error::PermissionDenied("no access".into());
        let mongo_err = MongoError::from(core_err);
        match mongo_err {
    ...
    fn core_conflict_maps_to_write_conflict() {
        let core_err = neovex_core::Error::Conflict("concurrent write".into());
        let mongo_err = MongoError::from(core_err);
        match mongo_err {
    ...
    fn core_internal_maps_to_internal_error() {
        let core_err = neovex_core::Error::Internal("something broke".into());
        let mongo_err = MongoError::from(core_err);
        match mongo_err {
