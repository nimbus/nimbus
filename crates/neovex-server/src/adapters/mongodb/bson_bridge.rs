...
use neovex_core::typed_scalar::TypedScalarValue;
use neovex_core::types::{DocumentId, TableName, Timestamp};
use neovex_core::{Document, SpecialDouble};
...
/// Convert a raw BSON document (bytes) into a Neovex `Document`.
///
...
/// Convert a `bson::Document` into a Neovex `Document`.
...
/// Convert a Neovex `Document` back to a `bson::Document` for wire responses.
...
/// Convert a Neovex `Document` to raw BSON bytes for wire transmission.
...
mod tests {
    ...
    fn roundtrip_simple_document() {
        ...
        let neovex_doc = bson_doc_to_document(&bson_doc, &test_table()).unwrap();
        assert_eq!(
            neovex_doc.get_field("name"),
            Some(&Value::String("Alice".into()))
        );
        assert_eq!(neovex_doc.get_field("age"), Some(&Value::Number(30.into())));
        assert_eq!(neovex_doc.get_field("active"), Some(&Value::Bool(true)));

        let back = document_to_bson_doc(&neovex_doc);
    ...
    fn roundtrip_object_id() {
        ...
        assert_eq!(neovex_doc.id.as_str(), oid.to_hex());
        assert!(neovex_doc.typed_fields.contains_key("_id_type"));
    ...
    fn roundtrip_string_id() {
        ...
        assert_eq!(neovex_doc.id.as_str(), "my-custom-key");
        assert!(!neovex_doc.typed_fields.contains_key("_id_type"));
    ...
    fn roundtrip_integer_id() {
        ...
        assert_eq!(neovex_doc.id.as_str(), "42");
    ...
    fn auto_generates_object_id_when_missing() {
        ...
        assert_eq!(neovex_doc.id.as_str().len(), 24);
        assert!(neovex_doc.typed_fields.contains_key("_id_type"));
        assert!(matches!(
            neovex_doc.typed_fields.get("_id_type"),
    ...
    fn roundtrip_special_doubles() {
        ...
        let neovex_doc = bson_doc_to_document(&bson_doc, &test_table()).unwrap();
        assert!(neovex_doc.typed_fields.contains_key("nan_val"));
        assert!(neovex_doc.typed_fields.contains_key("pos_inf"));
        assert!(neovex_doc.typed_fields.contains_key("neg_inf"));
        assert!(neovex_doc.typed_fields.contains_key("neg_zero"));
        assert!(!neovex_doc.typed_fields.contains_key("normal"));
    ...
    fn roundtrip_binary() {
        ...
        let neovex_doc = bson_doc_to_document(&bson_doc, &test_table()).unwrap();
        assert!(matches!(
            neovex_doc.typed_fields.get("data"),
    ...
    fn roundtrip_regex() {
        ...
        let neovex_doc = bson_doc_to_document(&bson_doc, &test_table()).unwrap();
        assert!(matches!(
            neovex_doc.typed_fields.get("pattern"),
    ...
    fn roundtrip_datetime() {
        ...
        let neovex_doc = bson_doc_to_document(&bson_doc, &test_table()).unwrap();
        assert!(matches!(
            neovex_doc.typed_fields.get("created"),
    ...
    fn roundtrip_mongo_timestamp() {
        ...
        let neovex_doc = bson_doc_to_document(&bson_doc, &test_table()).unwrap();
        assert!(matches!(
            neovex_doc.typed_fields.get("ts"),
    ...
    fn roundtrip_min_max_key() {
        ...
        let neovex_doc = bson_doc_to_document(&bson_doc, &test_table()).unwrap();
        assert!(matches!(
            neovex_doc.typed_fields.get("low"),
            Some(TypedScalarValue::MinKey)
        ));
        assert!(matches!(
            neovex_doc.typed_fields.get("high"),
    ...
    fn roundtrip_nested_document() {
        ...
        let neovex_doc = bson_doc_to_document(&bson_doc, &test_table()).unwrap();
        let addr = neovex_doc.get_field("address").unwrap();
    ...
    fn roundtrip_array() {
        let bson_doc = doc! {
            "_id": ObjectId::new(),
            "tags": ["rust", "mongodb", "neovex"],
        };

        let neovex_doc = bson_doc_to_document(&bson_doc, &test_table()).unwrap();
        let tags = neovex_doc.get_field("tags").unwrap();
        assert_eq!(tags, &serde_json::json!(["rust", "mongodb", "neovex"]));
    ...
    fn roundtrip_null_and_bool() {
        ...
        let neovex_doc = bson_doc_to_document(&bson_doc, &test_table()).unwrap();
        assert_eq!(neovex_doc.get_field("nothing"), Some(&Value::Null));
        assert_eq!(neovex_doc.get_field("flag"), Some(&Value::Bool(true)));
    ...
    fn roundtrip_javascript_code() {
        ...
        let neovex_doc = bson_doc_to_document(&bson_doc, &test_table()).unwrap();
        assert!(matches!(
            neovex_doc.typed_fields.get("validator"),
    ...
    fn document_to_bson_bytes_produces_valid_bson() {
        ...
        let neovex_doc = bson_doc_to_document(&bson_doc, &test_table()).unwrap();
        let bytes = document_to_bson_bytes(&neovex_doc);
    ...
    fn decimal128_roundtrip() {
        ...
        let neovex_doc = bson_doc_to_document(&bson_doc, &test_table()).unwrap();
        assert!(matches!(
            neovex_doc.typed_fields.get("price"),
