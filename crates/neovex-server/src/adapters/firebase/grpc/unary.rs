...
use super::write_stream::{
    decode_neovex_value_from_grpc, encode_document_field_to_grpc, encode_neovex_value_to_grpc,
...
fn rpc_status_from_error(error: neovex_core::Error) -> RpcStatus {
    RpcStatus {
...
pub(super) async fn handle_create_document(
    ...
    let document_path =
        neovex_core::DocumentPath::new(collection_target.collection_path, document_id);
...
struct LoweredRunQueryRequest {
    ...
    parent_document_path: Option<neovex_core::DocumentPath>,
...
fn proto_aggregation_result(
    result: &neovex_core::StructuredAggregationResult,
    ...
    Ok(proto::AggregationResult {
        aggregate_fields: result
            ...
            .iter()
            .map(|(alias, value)| {
                encode_neovex_value_to_grpc(value).map(|value| (alias.clone(), value))
            })
...
fn decode_query_value_from_grpc(value: &proto::Value) -> Result<serde_json::Value, Status> {
    match value.value_type.as_ref() {
        ...
        _ => decode_neovex_value_from_grpc(value),
    }
}
...
fn lower_find_nearest(
    ...
    Ok(FindNearest {
        ...
        query_vector: find_nearest
            ...
            .and_then(decode_neovex_value_from_grpc)?,
