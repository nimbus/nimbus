...
use nimbus_engine::{Service, SubscriptionCleanupHandle, SubscriptionUpdate};
use tokio::sync::mpsc;
...
pub fn open_change_stream(
    ...
    let (_sub_id, cleanup) = registration.into_parts();
...
fn snapshot_to_change_events(
    ...
    let diff = nimbus_core::diff_subscription_snapshots(previous, current);
    let mut events = Vec::new();
...
mod tests {
    ...
    fn resume_token_in_change_event_is_parseable() {
        use nimbus_core::{CommitEntry, SequenceNumber, Timestamp};
        let doc = make_doc(
            "u1",
            serde_json::Map::from_iter([("name".to_string(), json!("Alice"))]),
        );
