...
fn insert_single_document() {
    ...
    let result = insert(&body, &mut test_conn(), &fixture.engine()).unwrap();
...
fn insert_ordered_stops_on_duplicate() {
    ...
    insert(&body1, &mut test_conn(), &fixture.engine()).unwrap();
    ...
    let result = insert(&body2, &mut test_conn(), &fixture.engine()).unwrap();
...
fn insert_missing_collection_returns_error() {
    ...
    let err = insert(&body, &mut test_conn(), &fixture.engine()).unwrap_err();
    match err {
