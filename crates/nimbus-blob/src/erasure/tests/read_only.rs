//! Read-only inspection-handle tests for [`ErasureBlobStore`] (EOW1 +
//! review rounds): identity binding, absent roots, snapshot coherence,
//! stats stability, and refusal semantics. Split from `tests.rs` per the
