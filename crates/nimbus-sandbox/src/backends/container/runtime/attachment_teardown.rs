//! Exact Container host-managed attachment detach and release adapter.
...
use super::{
    ContainerNetworkPublicationMode, ContainerSandboxBackend, ContainerSandboxManifest,
    ContainerStartMode, hostname_for,
};
...
impl ContainerSandboxBackend {
    ...
    fn apply_network_teardown(
        ...
        let adapter = self.attachment_adapter(&context, network_config, &hostname, None);
                ...
                let proof = adapter.detach_host_managed_retained(
                    ...
                    |disposition| {
                        ...
                        self.egress_proxies.stop_for_detach(
                            ...
                            &context.handle.id,
                            context.provision_network_plan.as_ref().ok_or_else(|| {
                                sandbox_crossed("Container compiled network plan")
                            })?,
        ...
        succeeded("container_host_attachment_teardown", &progress.borrow())
    }
    ...
    fn inspect_network_teardown_inner(
        ...
        match inspected.inspect_and_rebase_command(command, provider_observation) {
            Ok(HostManagedAttachmentCommandInspection::ExactTerminalSuccess) => succeeded(
    ...
    fn authenticate_network_teardown_manifest(
        ...
        if self.config.start_mode != ContainerStartMode::Execute
            ...
            || command.provider_registration_key() != CONTAINER_HOST_MANAGED_ATTACHMENT_PROVIDER_KEY
        ...
        {
            return Err(NetworkTeardownAdapterError::crossed(
                "Container forwarded-machine attachment composition",
            ));
        }
        manifest
