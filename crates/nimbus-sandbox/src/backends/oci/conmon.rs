...
mod tests {
    ...
    fn conmon_launch_plan_uses_private_runtime_and_buildah_unshare() {
        ...
        let layout = OciConmonLayout::new("/tmp/nimbus-sandbox-state", &sandbox_id);
        let config = OciConmonConfig {
            conmon_path: Path::new("/usr/bin/conmon").into(),
            runtime_path: Path::new("/usr/libexec/nimbus/crun").into(),
        ...
        let launch_plan = build_launch_plan(
            ...
            &sandbox_id,
            "db",
            Path::new("/tmp/nimbus-bundles/db-01"),
            None,
            &[],
        );
        ...
        assert!(
            launch_plan
                ...
                .args
                .iter()
                .any(|arg| arg == "/usr/libexec/nimbus/crun"),
            "create command should retain the private nimbus crun path"
        );
        assert_eq!(
    ...
    fn conmon_launch_plan_injects_mount_prelude_for_image_backed_sandboxes() {
        ...
        let launch_plan = build_launch_plan(
            ...
            &sandbox_id,
            "db",
            Path::new("/tmp/nimbus-bundles/db-02"),
            Some("db-02-image"),
