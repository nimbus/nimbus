...
fn wrap_unshare_prefixes_existing_command() {
    ...
    let wrapped = buildah.wrap_unshare(
        &CommandSpec::new("/usr/libexec/nimbus/crun")
            .arg("state")
            .arg("sandbox-123"),
    );
...
fn build_materializes_localhost_image_before_creating_working_container() {
    ...
    assert_eq!(built.image_reference, "localhost/nimbus-api");

    let log = fs::read_to_string(log_path).expect("fake buildah log should be readable");
    let lines: Vec<_> = log.lines().collect();
    assert_eq!(
        lines[0],
        "bud -t nimbus-api -f /workspace/Dockerfile /workspace"
    );
    assert_eq!(lines[1], "from --name api-working localhost/nimbus-api");
}
...
fn prepare_built_image_launch_uses_built_image_reference() {
    ...
    assert_eq!(
        ...
        vec![
            "bud -t nimbus-api -f /workspace/Dockerfile /workspace",
            "from --name api-working localhost/nimbus-api",
            "mount api-working",
