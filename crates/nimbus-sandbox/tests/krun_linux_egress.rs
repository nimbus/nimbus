...
fn krun_tenant_grows_onto_a_second_block_when_the_first_is_full() {
    ...
    let s2_command = format!(
        r#"TMP=/nimbus-egress/result.tmp; : > "$TMP"; if timeout 8 wget -T 5 -q -O /tmp/own "http://127.0.0.1:{upstream_port}/allowed" && grep -q grow-upstream-body /tmp/own; then echo own_egress=allowed >> "$TMP"; else echo own_egress=denied >> "$TMP"; fi; mv "$TMP" {RESULT_PATH_IN_GUEST}; sleep 30"#
    );
    ...
    // Positive control + the real proof: the grown-block sandbox reaches its OWN
    // on-link PEP (bound on the grown block's gateway). This is what the grow-egress
    // bug broke — the grown sandbox's veth landed in the wrong block (a shared-cursor
    // IPAM bug) so its PEP was off-link. (The guest is a libkrun/TSI VMM with no
    // normal eth0 IP, so we assert reachability, not a guest address, matching the
    // MTN5 cross-tenant proof.)
