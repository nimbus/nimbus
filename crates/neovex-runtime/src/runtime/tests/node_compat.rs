...
const COMMON_FIXTURES_FIXTURE: &str = include_str!("node_compat_fixtures/test/common/fixtures.js");
...
const PROCESS_BEFORE_EXIT_REENTRY_POSTLUDE: &str = r#"
  if (typeof globalThis.process?.emit === "function") {
    const __neovexInitialExitCode = globalThis.process.exitCode ?? 0;
    const __neovexDeadline = Date.now() + 1000;
    for (let __neovexPass = 0; ; __neovexPass += 1) {
      globalThis.process.emit(
        "beforeExit",
        globalThis.process.exitCode ?? __neovexInitialExitCode,
      );
      if (typeof globalThis.__neovexProcessTicksAndRejections === "function") {
        globalThis.__neovexProcessTicksAndRejections();
      }
      await Promise.resolve();
      await new Promise((resolve) => queueMicrotask(resolve));
      if (typeof globalThis.process?.nextTick === "function") {
      ...
      const __neovexHasMoreWork =
        typeof globalThis.__neovexEventLoopHasMoreWork === "function" &&
        globalThis.__neovexEventLoopHasMoreWork() === true;
      if (!__neovexHasMoreWork) {
        break;
      }
      if (Date.now() >= __neovexDeadline) {
        break;
      }
...
const FORK_CHILD_SETTLE_POSTLUDE: &str = r#"
  if (typeof common.__neovexFlushForkWorkers === "function") {
    await common.__neovexFlushForkWorkers();
  }
"#;
...
import { createRequire as __neovexCreateRequireForProcessExit } from "node:module";

const __neovexRequireForProcessExit =
  __neovexCreateRequireForProcessExit(import.meta.url);
const __neovexProcessExitSentinel = "__NEOVEX_NODE_COMPAT_PROCESS_EXIT__";
...
if (globalThis.process && typeof globalThis.process.reallyExit === "function") {
  globalThis.process.reallyExit = (code) => {
    ...
    __neovexRequireForProcessExit("./test/common/index.js").__neovexAssert?.();
    const exitError = new Error(`${__neovexProcessExitSentinel}:${resolvedCode}`);
    exitError.code = "NEOVEX_NODE_COMPAT_PROCESS_EXIT";
    throw exitError;
  };
}
"#;
...
globalThis.__neovexNodeCompatTerm = "xterm-256color";
...
{
  const __neovexPreludeRequire = createRequire(import.meta.url);
  const __neovexPreludeFs = __neovexPreludeRequire("node:fs");
  const __neovexPreludePath = __neovexPreludeRequire("node:path");
  const __neovexPreludeUrl = __neovexPreludeRequire("node:url");
  const __neovexCompatBundleRoot = __neovexPreludePath.dirname(
    __neovexPreludeUrl.fileURLToPath(import.meta.url),
  );
  __neovexPreludeFs.mkdirSync(
    __neovexPreludePath.join(__neovexCompatBundleRoot, "lib"),
    { recursive: true },
  );
  if (typeof globalThis.process?.chdir === "function") {
    globalThis.process.chdir(__neovexCompatBundleRoot);
  }
}
"#;
const PENDING_DEPRECATION_PRELUDE: &str = r#"
{
  const __neovexNodeProcess = createRequire(import.meta.url)("node:process");
  const existingNodeOptions =
    typeof __neovexNodeProcess.env.NODE_OPTIONS === "string"
      ? __neovexNodeProcess.env.NODE_OPTIONS
      : "";
  if (!existingNodeOptions.split(/\s+/u).includes("--pending-deprecation")) {
    __neovexNodeProcess.env.NODE_OPTIONS = existingNodeOptions.trim().length === 0
      ? "--pending-deprecation"
      : `${existingNodeOptions} --pending-deprecation`;
  }
  globalThis.process ??= __neovexNodeProcess;
}
"#;
...
macro_rules! node22_primary_only_batch_case {
    ($test_relative_path:literal, $fixture_source_path:literal) => {
        NodeCompatBatchEntry {
            test_relative_path: $test_relative_path,
            node20_fixture_source_path: None,
...
// focused Neovex subset so future NLC3 expansion adds manifest rows instead of
// more hand-written Rust test wrappers. Keep both Node20 and Node22 fixture
// roots in one manifest so the primary and validation lanes do not drift.
const CORE_SEMANTICS_BATCH: &[NodeCompatBatchEntry] = &[
    ...
    split_batch_case!(
        ...
        "node20/test/parallel/test-assert-checktag.js",
    ...
    split_batch_case!(
        ...
        "node20/test/parallel/test-console-clear.js",
        "test/parallel/test-console-clear.js"
    ),
    shared_batch_case!(
...
fn write_node_compat_bundle(
    ...
    let compat_exec_setup = match mode {
        NodeCompatBundleMode::Runtime => {
            ...
            format!(
                "const __neovexCompatExecPath = {:?};",
                compat_exec_path.to_string_lossy()
            )
        }
        NodeCompatBundleMode::Oracle => {
            "const __neovexCompatExecPath = globalThis.process?.execPath ?? \"\";".to_string()
        }
    };
    let gc_setup_script = match mode {
        NodeCompatBundleMode::Runtime => {
            r#"const __neovexTestGc = function gc() {
  return globalThis.__neovexSyncHostValue("op_neovex_runtime_test_force_gc");
};
globalThis.gc = __neovexTestGc;
globalThis.global.gc = __neovexTestGc;"#
        }
    ...
    let import_preamble = if should_quiesce_then_require_fixture(test_relative_path) {
        ...
        format!(
            r#"let __neovexImportError = null;
try {{
  await import("./{test_relative_path}");
}} catch (error) {{
  __neovexImportError = error;
}}"#
        )
    ...
    let invoke_import_guard =
        if should_quiesce_then_require_fixture(test_relative_path) && capture_import_error {
            format!(
                r#"  if (typeof globalThis.__neovexProcessTicksAndRejections === "function") {{
    globalThis.__neovexProcessTicksAndRejections();
  }}
  let __neovexImportError = null;
  try {{
    require("./{test_relative_path}");
  }} catch (error) {{
    __neovexImportError = error;
  }}
  if (__neovexImportError) {{
    if ({capture_top_level_skip} &&
        (__neovexImportError?.__neovexSkip ||
         __neovexImportError?.code === "NEOVEX_NODE_COMPAT_SKIP")) {{
      return {{
        ok: true,
        skipped: true,
...
"#
    ...
    let lane_prelude = lane
        .map(|lane| {
            format!(
                "globalThis.__neovexNodeCompatLane = {:?};",
...
"#
        ...
        r#"  if (typeof globalThis.__neovexFlushEmbeddedTests === "function") {
...
{compat_exec_setup}
const __neovexCompatMainScriptPath = new URL(
  "./{test_relative_path}",
  import.meta.url,
).pathname;
...
if (typeof globalThis.process === "object" && globalThis.process !== null) {{
  globalThis.process.execPath = __neovexCompatExecPath;
  if (Array.isArray(globalThis.process.argv)) {{
    if (globalThis.process.argv.length === 0) {{
      globalThis.process.argv.push(__neovexCompatExecPath);
    }} else {{
      globalThis.process.argv[0] = __neovexCompatExecPath;
    }}
    if (globalThis.process.argv.length >= 2) {{
      globalThis.process.argv[1] = __neovexCompatMainScriptPath;
    }} else {{
      globalThis.process.argv.push(__neovexCompatMainScriptPath);
    }}
  }}
}}
{lane_prelude}
...
const NLC9_TRACE_EVENTS_FOUNDATION_BATCH: &[NodeCompatBatchEntry] = &[
    node22_primary_only_batch_case!(
        "test/parallel/test-trace-events-api.js",
...
const NODE24_NLC7_CRYPTO_DH_STATELESS_PREVIEW_WATCHPOINT_BATCH: &[NodeCompatBatchEntry] =
    &[NodeCompatBatchEntry {
        test_relative_path: "test/parallel/test-crypto-dh-stateless.js",
        node20_fixture_source_path: None,
        node22_fixture_source_path: None,
...
const NODE20_NLC7_CRYPTO_DH_VALIDATION_WATCHPOINT_BATCH: &[NodeCompatBatchEntry] =
    &[shared_official_batch_case_with_extra!(
...
const NODE20_NLC7_CRYPTO_AUTHENTICATED_VALIDATION_WATCHPOINT_BATCH: &[NodeCompatBatchEntry] =
    &[NodeCompatBatchEntry {
        test_relative_path: "test/parallel/test-crypto-authenticated.js",
...
#[test]
#[ignore = "Pinned Node20 divergence: official v20.20.2 process.features does not expose the Node22-only `typescript` key that Neovex intentionally keeps in its single Node22-shaped runtime contract"]
...
#[test]
#[ignore = "Pinned later-family dependency: official test-process-finalization.mjs now runs through the Neovex sync subprocess harness, and the only remaining failure is different-registry-per-thread.mjs because worker_threads are still owned by a later compatibility family"]
...
#[test]
#[ignore = "Pinned Node20 divergence: official v20.20.2 PerformanceResourceTiming#toJSON() omits the Node22-era `deliveryType` and `responseStatus` fields that Neovex intentionally keeps in its single Node22-shaped runtime contract"]
...
#[test]
#[ignore = "Pinned Node20 validation divergence: official v20.20.2 test-async-local-storage-exit-does-not-leak.js still expects the old JavaScript AsyncLocalStorage _propagate hook, while the current runtime matches the newer Node22/Node24 implementation shape"]
...
#[test]
#[ignore = "Pinned Node24 preview-only divergence: test-crypto-scrypt.js expects ERR_INCOMPATIBLE_OPTION_PAIR for duplicate short/long option pairs, while the current runtime still throws the older ERR_CRYPTO_SCRYPT_INVALID_PARAMETER shape used by the verified Node22 baseline"]
...
#[test]
#[ignore = "Pinned Node24 preview drift: test-crypto-dh-stateless.js still expects ERR_OSSL_FAILED_DURING_DERIVATION on the invalid X25519 public-key case"]
fn node24_nlc7_crypto_dh_stateless_preview_watchpoint_batch() {
    run_node_compat_watchpoint_entry_batch(
        "node24-nlc7-crypto-dh-stateless-preview-watchpoints",
        NodeCompatLane::Node24,
        NODE24_NLC7_CRYPTO_DH_STATELESS_PREVIEW_WATCHPOINT_BATCH,
    );
}

#[test]
#[ignore = "Pinned Node20 validation divergence: test-crypto-dh.js still expects the older OpenSSL invalid-secret message while the verified Node22 baseline now returns the newer unspecified-validation shape"]
fn node20_nlc7_crypto_dh_validation_watchpoint_batch() {
    run_node_compat_watchpoint_entry_batch(
        "node20-nlc7-crypto-dh-validation-watchpoints",
        NodeCompatLane::Node20,
...
#[test]
#[ignore = "Pinned Node20 validation divergence: test-crypto-authenticated.js still expects the older deprecation-warning ordering without DEP0182"]
fn node20_nlc7_crypto_authenticated_validation_watchpoint_batch() {
    run_node_compat_watchpoint_entry_batch(
        "node20-nlc7-crypto-authenticated-validation-watchpoints",
        NodeCompatLane::Node20,
        NODE20_NLC7_CRYPTO_AUTHENTICATED_VALIDATION_WATCHPOINT_BATCH,
    );
}
...
#[test]
#[ignore = "Pinned Node20 validation divergence: test-https-hwm.js still times out on the current Node20 lane while the Node22/Node24 official files complete"]
...
#[test]
#[ignore = "Pinned Node20 validation divergence: test-tls-connect-hwm-option.js still times out on the current Node20 lane while the Node22/Node24 official files complete"]
...
#[test]
#[ignore = "Pinned Node24 preview-only divergence: test-stream-pipeline.js currently returns an AbortError-style 'The operation was aborted' message where the staged Node24 fixture still expects the inner 'Boom!' pipeline error message"]
...
#[test]
#[ignore = "Pinned Node24 preview divergence: official v24.15.0 test-fs-constants.js expects a newer constant-surface TypeError gate that Neovex has not adopted into the current Node22 contract"]
...
#[test]
#[ignore = "Pinned Node24 preview divergence: official v24.15.0 test-fs-promises-file-handle-dispose.js now also asserts opendir Dir[Symbol.asyncDispose]() close semantics that the current runtime does not yet match"]
...
#[test]
#[ignore = "Pinned Node24 preview divergence: official v24.15.0 test-fs-write-stream.js now also requires fs.close() to be observed when destroying WriteStream directly, while the current Node22 contract still follows the older file semantics"]
...
#[test]
#[ignore = "Pinned Node24 preview divergence: official v24.15.0 test-fs-write-stream-autoclose-option.js now also asserts ERR_INVALID_THIS when probing WriteStream.prototype.autoClose, while the current Node22 contract still follows the older surface"]
...
#[test]
#[ignore = "Pinned Node24 preview divergence: official v24.15.0 test-fs-symlink.js still expects the newer invalid-type ERR_INVALID_ARG_VALUE contract, while the current runtime intentionally keeps the Node22 ERR_FS_INVALID_SYMLINK_TYPE behavior"]
...
#[test]
#[ignore = "Pinned Node24 preview divergence: official v24.15.0 test-fs-opendir.js now also asserts ERR_INVALID_THIS for newer Dir handle receiver checks, while the current runtime intentionally keeps the Node22 directory-handle surface"]
...
#[test]
#[ignore = "Pinned Node24 preview divergence: official v24.15.0 test-fs-promises-watch.js adds maxQueue and overflow option validation that Neovex has not adopted into the current Node22-based fs.watch contract"]
...
#[test]
fn node20_validation_lane_executes_official_core_semantics_subset() {
...
#[test]
fn node22_primary_lane_executes_manifested_core_semantics_subset() {
    run_manifested_subset_for_lane(
        "nlc3-core-semantics",
        NodeCompatLane::Node22,
        CORE_SEMANTICS_BATCH,
...
#[test]
#[ignore = "Future Node24 preview lane: pinned official fixtures are staged for forward visibility, not current support claims or default gating"]
fn node24_preview_lane_executes_manifested_core_semantics_subset() {
    run_manifested_subset_for_lane(
        "nlc3-core-semantics",
        NodeCompatLane::Node24,
        CORE_SEMANTICS_BATCH,
...
#[test]
fn node20_validation_lane_executes_official_process_and_timing_subset() {
    run_manifested_subset_for_lane(
...
#[test]
fn node22_primary_lane_executes_manifested_process_and_timing_subset() {
    run_manifested_subset_for_lane(
        "nlc4-process-and-timing",
        NodeCompatLane::Node22,
        PROCESS_AND_TIMING_BATCH,
...
#[test]
#[ignore = "Future Node24 preview lane: pinned official fixtures are staged for forward visibility, not current support claims or default gating"]
fn node24_preview_lane_executes_manifested_process_and_timing_subset() {
    run_manifested_subset_for_lane(
        "nlc4-process-and-timing",
        NodeCompatLane::Node24,
        PROCESS_AND_TIMING_BATCH,
...
#[test]
fn node20_validation_lane_executes_official_streams_and_local_io_subset() {
    run_manifested_subset_for_lane(
...
#[test]
fn node22_primary_lane_executes_manifested_streams_and_local_io_subset() {
    run_manifested_subset_for_lane(
        "nlc5-streams-and-local-io",
        NodeCompatLane::Node22,
        STREAMS_AND_LOCAL_IO_BATCH,
...
#[test]
#[ignore = "Future Node24 preview lane: pinned official fixtures are staged for forward visibility, not current support claims or default gating"]
fn node24_preview_lane_executes_manifested_streams_and_local_io_subset() {
    run_manifested_subset_for_lane(
        "nlc5-streams-and-local-io",
        NodeCompatLane::Node24,
        STREAMS_AND_LOCAL_IO_BATCH,
...
#[test]
fn node22_primary_lane_executes_manifested_loader_context_subset() {
    run_manifested_subset_for_lane(
        "nlc7-loader-context",
        NodeCompatLane::Node22,
        LOADER_CONTEXT_BATCH,
...
#[test]
#[ignore = "Future Node24 preview lane: pinned official fixtures are staged for forward visibility, not current support claims or default gating"]
fn node24_preview_lane_executes_manifested_loader_context_subset() {
    run_manifested_subset_for_lane(
        "nlc7-loader-context",
        NodeCompatLane::Node24,
        LOADER_CONTEXT_BATCH,
...
fn node_compat_supplementary_process_shape_node20() {
    ...
    assert!(
        detail.contains("v22.0.0-neovex") && detail.contains("/^v20\\./"),
...
fn node_compat_supplementary_process_shape_node24() {
    ...
    assert!(
        detail.contains("v22.0.0-neovex") && detail.contains("/^v24\\./"),
        "node24 supplementary process shape should record the preview-lane version drift: {detail}",
    );
}
