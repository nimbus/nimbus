...
const neovexRuntimeTestStdinPath = globalThis.__neovexRuntimeTestStdinPath;
const neovexRuntimeTestStdinFd =
  typeof neovexRuntimeTestStdinPath === 'string' && neovexRuntimeTestStdinPath.length > 0
    ? fs.openSync(neovexRuntimeTestStdinPath, 'r')
    : 0;
const wasiPreview1 = new WASI({
  version: 'preview1',
  args: ['foo', '-bar', '--baz=value'],
  env: process.env,
  stdin: neovexRuntimeTestStdinFd,
  preopens: {
...
function neovexMaybeRewriteWasiImport(importObject) {
  return importObject;
}

const neovexMainScriptPromise = (async () => {
  const importObject = neovexMaybeRewriteWasiImport({
  ...
  if (neovexRuntimeTestStdinFd > 2) {
    fs.closeSync(neovexRuntimeTestStdinFd);
  }
});

globalThis.__neovexMainScriptPromise = neovexMainScriptPromise;
neovexMainScriptPromise.then(common.mustCall());
