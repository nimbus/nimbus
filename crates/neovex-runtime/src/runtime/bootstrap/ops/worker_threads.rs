...
#[op2]
#[serde]
pub(super) fn op_neovex_worker_bootstrap_state(
    state: &mut OpState,
) -> RuntimeWorkerBootstrapDescriptor {
...
#[op2]
pub(super) fn op_neovex_worker_parent_post_message(
    state: &mut OpState,
...
#[op2]
pub(super) fn op_neovex_worker_parent_post_message_raw(
    state: &mut OpState,
...
#[op2]
#[serde]
pub(super) async fn op_neovex_worker_parent_recv_message(
...
#[op2]
#[serde]
pub(super) fn op_neovex_worker_parent_recv_message_sync(
    state: &mut OpState,
) -> Result<Option<JsMessageData>, MessagePortError> {
    let port = state
        .try_borrow::<InstalledRuntimeWorkerBootstrapState>()
...
pub(crate) fn worker_threads_state_extension(
    ...
    Extension {
        name: "neovex_runtime_worker_threads_state_ext",
...
globalThis.__neovexStartWorkerMessagePump?.();
if (
  globalThis.__neovexWorkerThreadEnv &&
  globalThis.process &&
  typeof globalThis.process === "object"
) {{
  try {{
    Object.defineProperty(globalThis.process, "env", {{
      value: globalThis.__neovexWorkerThreadEnv,
      configurable: true,
      enumerable: true,
      writable: true,
    }});
    ...
    try {{
      globalThis.process.env = globalThis.__neovexWorkerThreadEnv;
