...
import neovexPerfHooksBuiltin from "ext:neovex_node22/perf_hooks_impl.js";
...
globalThis.__neovexPerfHooksBuiltin = neovexPerfHooksBuiltin;
const neovexInternalFsBinding = getNodeInternalBinding("fs");
const {
  ArrayIsArray,
  Float64Array,
...
async function runtimeFsRemove(path, options = undefined) {
  const normalizedOptions = options && typeof options === "object" ? options : {};
  try {
    await globalThis.__neovexAsyncHostValue("op_neovex_runtime_remove", {
      path: runtimeFsPathToString(path),
      recursive: normalizedOptions.recursive === true,
    });
  } catch (error) {
...
function runtimeFsRemoveSync(path, options = undefined) {
  const normalizedOptions = options && typeof options === "object" ? options : {};
  try {
    globalThis.__neovexSyncHostValue("op_neovex_runtime_remove_sync", {
      path: runtimeFsPathToString(path),
      recursive: normalizedOptions.recursive === true,
    });
  } catch (error) {
...
async function runtimeFsChmod(path, mode) {
  try {
    await globalThis.__neovexAsyncHostValue("op_neovex_runtime_chmod", {
      path: runtimeFsPathToString(path),
      mode,
    });
  } catch (error) {
...
async function runtimeFsCopyFile(from, to) {
  try {
    await globalThis.__neovexAsyncHostValue("op_neovex_runtime_copy_file", {
...
function runtimeFsCopyFileSync(from, to) {
  try {
    globalThis.__neovexSyncHostValue("op_neovex_runtime_copy_file_sync", {
      from: runtimeFsPathToString(from),
      to: runtimeFsPathToString(to),
    });
  } catch (error) {
...
async function runtimeFsLink(oldpath, newpath) {
  try {
    await globalThis.__neovexAsyncHostValue("op_neovex_runtime_link", {
...
function runtimeFsLinkSync(oldpath, newpath) {
  try {
    globalThis.__neovexSyncHostValue("op_neovex_runtime_link_sync", {
      oldpath: runtimeFsPathToString(oldpath),
      newpath: runtimeFsPathToString(newpath),
    });
  } catch (error) {
...
async function runtimeFsSymlink(oldpath, newpath, options = undefined) {
  try {
    await globalThis.__neovexAsyncHostValue("op_neovex_runtime_symlink", {
      oldpath: runtimeFsPathToString(oldpath),
      newpath: runtimeFsPathToString(newpath),
...
function runtimeFsSymlinkSync(oldpath, newpath, options = undefined) {
  try {
    globalThis.__neovexSyncHostValue("op_neovex_runtime_symlink_sync", {
      oldpath: runtimeFsPathToString(oldpath),
      newpath: runtimeFsPathToString(newpath),
      file_type: runtimeFsSymlinkFileType(options),
    });
...
async function runtimeFsReadLink(path) {
  try {
    return await globalThis.__neovexAsyncHostValue("op_neovex_runtime_read_link", {
      path: runtimeFsPathToString(path),
    });
  } catch (error) {
    throw runtimeFsMapThrownError(error);
...
function runtimeFsReadLinkSync(path) {
  try {
    return globalThis.__neovexSyncHostValue("op_neovex_runtime_read_link_sync", {
      path: runtimeFsPathToString(path),
    });
  } catch (error) {
    throw runtimeFsMapThrownError(error);
...
async function runtimeFsRename(oldpath, newpath) {
  try {
    await globalThis.__neovexAsyncHostValue("op_neovex_runtime_rename", {
      oldpath: runtimeFsPathToString(oldpath),
      newpath: runtimeFsPathToString(newpath),
    });
  } catch (error) {
...
function runtimeFsRenameSync(oldpath, newpath) {
  try {
    globalThis.__neovexSyncHostValue("op_neovex_runtime_rename_sync", {
      oldpath: runtimeFsPathToString(oldpath),
      newpath: runtimeFsPathToString(newpath),
    });
  } catch (error) {
...
function runtimeFsChmodSync(path, mode) {
  try {
    globalThis.__neovexSyncHostValue("op_neovex_runtime_chmod_sync", {
      path: runtimeFsPathToString(path),
      mode,
    });
  } catch (error) {
...
async function runtimeFsUtime(path, atime, mtime) {
  ...
  try {
    await globalThis.__neovexAsyncHostValue("op_neovex_runtime_utime", {
      path: runtimeFsPathToString(path),
...
function runtimeFsUtimeSync(path, atime, mtime) {
  ...
  const normalizedMtime = runtimeFsToUnixTimeFromEpoch(mtime);
  try {
    globalThis.__neovexSyncHostValue("op_neovex_runtime_utime_sync", {
      path: runtimeFsPathToString(path),
      atime_secs: normalizedAtime.seconds,
      atime_nanos: normalizedAtime.nanoseconds,
      mtime_secs: normalizedMtime.seconds,
...
function seedNodeProcessExecPath(nodeProcess) {
  ...
  const execPath = core.ops.op_neovex_runtime_exec_path();
...
const neovexLoadEnvFilePatched = Symbol("neovex.loadEnvFilePatched");
const neovexLoadEnvOverlaySymbol = Symbol.for("neovex.runtimeEnvOverlay");
...
function seedNodeProcessEnvOverlay(nodeProcess) {
  ...
  if (globalThis[neovexLoadEnvOverlaySymbol] === undefined) {
    Object.defineProperty(globalThis, neovexLoadEnvOverlaySymbol, {
  ...
  return globalThis[neovexLoadEnvOverlaySymbol];
}
...
const neovexFileHandleGcPatched = Symbol("neovex.fileHandleGcPatched");
const neovexFsPromisesLifecyclePatched = Symbol("neovex.fsPromisesLifecyclePatched");
const neovexFsPromisesWatchPatched = Symbol("neovex.fsPromisesWatchPatched");
const neovexOriginalFileHandleFdGetter =
  Object.getOwnPropertyDescriptor(nodeInternalFsFileHandle?.prototype ?? {}, "fd")?.get;

function isNeovexFileHandle(value) {
  return !!(
    value &&
    typeof value === "object" &&
...
async function closeNeovexFileHandle(handle) {
  const closeMethod = handle?.close;
  if (typeof closeMethod !== "function") {
    return undefined;
  }
  if (
    !isNeovexFileHandle(handle) ||
    typeof neovexOriginalFileHandleFdGetter !== "function"
  ) {
    return await Reflect.apply(closeMethod, handle, []);
  }
  const rawFd = Reflect.apply(neovexOriginalFileHandleFdGetter, handle, []);
...
async function statFsReadFileHandle(handle) {
  const bindingFstat = neovexInternalFsBinding?.fstat;
...
function patchNodeFsReadSemantics(nodeProcess) {
  ...
  if (typeof originalRead === "function" && originalRead.__neovexNormalizedLength !== true) {
    ...
    Object.defineProperty(patchedRead, "__neovexNormalizedLength", {
      value: true,
      configurable: true,
      enumerable: false,
      writable: false,
  ...
  if (typeof originalReadSync === "function" && originalReadSync.__neovexNormalizedLength !== true) {
    ...
    Object.defineProperty(patchedReadSync, "__neovexNormalizedLength", {
      value: true,
      configurable: true,
      enumerable: false,
      writable: false,
  ...
  if (
    ...
    originalFileHandleRead.__neovexNormalizedLength !== true
  ) {
    ...
    Object.defineProperty(patchedFileHandleRead, "__neovexNormalizedLength", {
      value: true,
      configurable: true,
      enumerable: false,
      writable: false,
  ...
  if (
    ...
    nodeInternalFsFileHandle?.prototype &&
    nodeInternalFsFileHandle.prototype[neovexFileHandleGcPatched] !== true
  ) {
    ...
    for (const nodeFsPromises of nodeFsPromiseTargets) {
      ...
      if (
        ...
        originalOpen[neovexFileHandleGcPatched] !== true
      ) {
        const patchedOpen = async function () {
          const handle = await Reflect.apply(originalOpen, this, arguments);
          const handleFd = isNeovexFileHandle(handle) &&
              typeof neovexOriginalFileHandleFdGetter === "function"
            ? Reflect.apply(neovexOriginalFileHandleFdGetter, handle, [])
            : undefined;
        ...
        Object.defineProperty(patchedOpen, neovexFileHandleGcPatched, {
          value: true,
          configurable: false,
          enumerable: false,
          writable: false,
    ...
    Object.defineProperty(nodeInternalFsFileHandle.prototype, neovexFileHandleGcPatched, {
      value: true,
      configurable: false,
      enumerable: false,
      writable: false,
  ...
  for (const nodeFsPromises of nodeFsPromiseTargets) {
    if (
      ...
      nodeFsPromises[neovexFsPromisesLifecyclePatched] === true
    ) {
      continue;
    }
    ...
    if (typeof originalReadFile === "function") {
      const patchedReadFile = function (path, options = undefined) {
        if (typeof path === "number" || isNeovexFileHandle(path)) {
    ...
    if (
      ...
      originalWatch[neovexFsPromisesWatchPatched] !== true
    ) {
      ...
      Object.defineProperty(patchedWatch, neovexFsPromisesWatchPatched, {
        value: true,
        configurable: false,
        enumerable: false,
        writable: false,
    ...
    Object.defineProperty(nodeFsPromises, neovexFsPromisesLifecyclePatched, {
      value: true,
      configurable: false,
      enumerable: false,
      writable: false,
...
function seedNodeProcessLoadEnvFile(nodeProcess) {
  ...
  if (originalLoadEnvFile[neovexLoadEnvFilePatched] === true) {
    return;
  }
  ...
  Object.defineProperty(patchedLoadEnvFile, neovexLoadEnvFilePatched, {
    value: true,
    configurable: false,
    enumerable: false,
    writable: false,
...
function createNodeCompatibleSetImmediate(setImmediateImpl) {
  function neovexSetImmediate(callback, ...args) {
  ...
  for (const property of Reflect.ownKeys(setImmediateImpl)) {
    ...
    if (descriptor) {
      Object.defineProperty(neovexSetImmediate, property, descriptor);
    }
  }
  Object.defineProperty(neovexSetImmediate, "name", {
    value: "setImmediate",
    configurable: true,
  });
  return neovexSetImmediate;
}
...
const neovexGlobalEventTarget = new EventTarget();

function seedGlobalEventTargetSurface() {
  const bindings = {
    addEventListener: neovexGlobalEventTarget.addEventListener.bind(neovexGlobalEventTarget),
    removeEventListener: neovexGlobalEventTarget.removeEventListener.bind(neovexGlobalEventTarget),
    dispatchEvent: neovexGlobalEventTarget.dispatchEvent.bind(neovexGlobalEventTarget),
  };
...
function seedWorkerThreadHostSurface(workerBootstrapState) {
  ...
  Object.defineProperty(globalThis, "postMessage", {
    value(message, transferOrOptions = undefined) {
      ...
      if (transferList.length === 0) {
        core.ops.op_neovex_worker_parent_post_message_raw(core.serialize(message));
        return;
      }
      const data = webSerializeJsMessageData(message, transferList);
      core.ops.op_neovex_worker_parent_post_message(data);
    },
    configurable: true,
    enumerable: false,
    writable: true,
  ...
  Object.defineProperty(globalThis, "__neovexStartWorkerMessagePump", {
    value() {
      if (pumpStarted) {
        return;
      }
      ...
      PromiseResolve().then(async () => {
        while (true) {
          currentRecvMessage = core.ops.op_neovex_worker_parent_recv_message();
          maybeUnrefCurrentRecvMessage();
          ...
          for (let index = 0; index < 1000; index += 1) {
            const syncData = core.ops.op_neovex_worker_parent_recv_message_sync();
            if (syncData === null) {
              break;
            }
            if (!hasMessageConsumer()) {
...
const neovexNodeConsoleUpgraded = Symbol("neovex.nodeConsoleUpgraded");
...
function upgradeGlobalConsole(nodeProcess) {
  ...
  if (runtimeConsole[neovexNodeConsoleUpgraded] === true) {
    return;
  }
  ...
  Object.defineProperty(runtimeConsole, neovexNodeConsoleUpgraded, {
    value: true,
    configurable: false,
    enumerable: false,
    writable: false,
...
const neovexWarningHandlerInstalled = Symbol("neovex.warningHandlerInstalled");
...
function seedNodeProcessWarnings(nodeProcess) {
  ...
  if (nodeProcess[neovexWarningHandlerInstalled] === true) {
    return;
  }

  nodeProcess.on("warning", (warning) => safeNodeProcessOnWarning(nodeProcess, warning));
  Object.defineProperty(nodeProcess, neovexWarningHandlerInstalled, {
    value: true,
    configurable: false,
    enumerable: false,
    writable: false,
...
function createNeovexSharedWorkerEnvProxy() {
  return new Proxy(Object.create(null), {
    get(_target, prop) {
      if (typeof prop === "symbol") {
        return undefined;
      }
      return core.ops.op_neovex_runtime_shared_env_get(String(prop));
    },
    ownKeys() {
      return Reflect.ownKeys(core.ops.op_neovex_runtime_shared_env_snapshot());
    },
    getOwnPropertyDescriptor(_target, prop) {
      if (typeof prop === "symbol") {
        return undefined;
      }
      const value = core.ops.op_neovex_runtime_shared_env_get(String(prop));
      if (value === undefined) {
        return undefined;
      }
      return {
    ...
    has(_target, prop) {
      ...
      return core.ops.op_neovex_runtime_shared_env_get(String(prop)) !== undefined;
    },
    set(_target, prop, value) {
      if (typeof prop === "symbol" || typeof value === "symbol") {
        throw new TypeError("Cannot convert a Symbol value to a string");
      }
      core.ops.op_neovex_runtime_shared_env_set(String(prop), String(value));
      return true;
    },
    deleteProperty(_target, prop) {
      ...
      core.ops.op_neovex_runtime_shared_env_delete(String(prop));
      return true;
    },
    defineProperty(_target, prop, attributes) {
      ...
      core.ops.op_neovex_runtime_shared_env_set(
        String(prop),
...
function installNeovexSharedWorkerEnvProxy() {
  ...
  core.ops.op_neovex_runtime_shared_env_seed(snapshot);
  const sharedEnv = createNeovexSharedWorkerEnvProxy();
...
Object.defineProperty(globalThis, "__neovexInstallSharedWorkerEnvProxy", {
  value: installNeovexSharedWorkerEnvProxy,
  configurable: true,
  enumerable: false,
  writable: true,
});
...
Object.defineProperty(globalThis, "__neovexFlushEmbeddedTests", {
  value: flushEmbeddedDenoTests,
  configurable: true,
  enumerable: false,
  writable: false,
});
Object.defineProperty(globalThis, "__neovexProcessTicksAndRejections", {
  value: core.processTicksAndRejections,
  configurable: true,
  enumerable: false,
  writable: false,
});
Object.defineProperty(globalThis, "__neovexEventLoopHasMoreWork", {
...
const workerBootstrapState =
  typeof core.ops.op_neovex_worker_bootstrap_state === "function"
    ? core.ops.op_neovex_worker_bootstrap_state()
    : null;
...
if (typeof internals.__initWorkerThreads === "function") {
  ...
  const workerEnv =
    shouldShareWorkerEnv
      ? createNeovexSharedWorkerEnvProxy()
  ...
  if (
    ...
    Object.defineProperty(globalThis, "__neovexWorkerThreadEnv", {
      value: workerEnv,
      configurable: true,
      enumerable: false,
      writable: true,
