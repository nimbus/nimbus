...
const UID_ENV: &str = "NEOVEX_GUEST_UID";
const GID_ENV: &str = "NEOVEX_GUEST_GID";
...
mod tests {
    ...
    fn read_env_u32_parses_numeric_values() {
        unsafe {
            env::set_var("NEOVEX_TEST_NUMERIC_ENV", "42");
        }
        assert_eq!(
            read_env_u32("NEOVEX_TEST_NUMERIC_ENV").expect("env should parse"),
            42
        );
        unsafe {
            env::remove_var("NEOVEX_TEST_NUMERIC_ENV");
        }
    }
    ...
    fn read_env_u32_rejects_non_numeric_values() {
        unsafe {
            env::set_var("NEOVEX_TEST_INVALID_ENV", "abc");
        }
        let error = read_env_u32("NEOVEX_TEST_INVALID_ENV").expect_err("env should be rejected");
        assert!(
            error.contains("must be a u32"),
            "expected numeric parse error, got: {error}"
        );
        unsafe {
            env::remove_var("NEOVEX_TEST_INVALID_ENV");
        }
    }
}
