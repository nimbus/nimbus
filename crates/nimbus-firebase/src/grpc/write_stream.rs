...
use nimbus_core::{
    ...
    FieldTransformOperation, NumericValue, PrincipalContext, StoredValue, Timestamp, TypedFieldMap,
...
impl Default for WriteStreamRegistry {
    fn default() -> Self {
        Self::new()
    }
}
...
pub(super) fn proto_write_result(
    result: &nimbus_core::AtomicWriteResult,
...
pub(super) fn encode_nimbus_value_to_grpc(value: &JsonValue) -> Result<Value, Status> {
    let firestore_value =
        FirestoreValue::try_from_nimbus_value(value).map_err(firestore_value_status)?;
...
pub(super) fn encode_document_field_to_grpc(
    ...
    match document.typed_value(field_name) {
        ...
        None => encode_nimbus_value_to_grpc(value),
    }
}
