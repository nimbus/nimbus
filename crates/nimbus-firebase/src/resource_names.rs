use nimbus_core::{CollectionName, CollectionPath, DocumentPath};
use thiserror::Error;
...
/// `nimbus-core` raw path primitives from `F0.2`.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct FirestoreDatabaseName {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct FirestoreParentName {
    pub database: FirestoreDatabaseName,
    pub parent_document_path: Option<DocumentPath>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct FirestoreCollectionTarget {
    pub database: FirestoreDatabaseName,
    pub parent_document_path: Option<DocumentPath>,
...
#[derive(Debug, Error)]
pub enum FirestoreResourceNameError {
...
pub fn parse_collection_target(
    ...
    let collection_group =
        CollectionName::new(collection_id.to_string()).map_err(invalid_nimbus_path)?;
...
pub fn decode_rest_document_path(path: &str) -> Result<DocumentPath, FirestoreResourceNameError> {
...
fn invalid_nimbus_path(error: nimbus_core::Error) -> FirestoreResourceNameError {
