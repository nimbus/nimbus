...
#[derive(Debug, Error)]
pub enum FirestoreProtoJsonError {
...
pub fn decode_proto_json_value(value: &Value) -> Result<Value, FirestoreProtoJsonError> {
...
pub fn encode_proto_json_value(value: &Value) -> Result<Value, FirestoreProtoJsonError> {
    FirestoreValue::try_from_nimbus_value(value).map(|value| value.to_proto_json())
}
...
impl FirestoreValue {
    pub fn from_proto_json(value: &Value) -> Result<Self, FirestoreProtoJsonError> {
    ...
    pub fn to_proto_json(&self) -> Value {
        match self {
    ...
    pub fn into_nimbus_value(self) -> Result<Value, FirestoreProtoJsonError> {
        match self {
            Self::Null => Ok(Value::Null),
            Self::Boolean(value) => Ok(Value::Bool(value)),
            Self::Integer(value) => Ok(Value::Number(Number::from(value))),
            Self::Double(value) => value.into_nimbus_value(),
            Self::String(value) => Ok(Value::String(value)),
            Self::Array(values) => values
                .into_iter()
                .map(Self::into_nimbus_value)
                .collect::<Result<Vec<_>, _>>()
                .map(Value::Array),
            Self::Map(fields) => fields
                .into_iter()
                .map(|(key, value)| value.into_nimbus_value().map(|value| (key, value)))
    ...
    pub fn try_from_nimbus_value(value: &Value) -> Result<Self, FirestoreProtoJsonError> {
        match value {
            ...
            Value::Number(value) => {
                ...
                if let Some(value) = value.as_u64() {
                    let value = i64::try_from(value).map_err(|_| {
                        invalid_field(
                            "integerValue",
                            "Nimbus integer exceeds Firestore int64 range".to_string(),
                        )
                    })?;
                    return Ok(Self::Integer(value));
                }
            ...
            Value::Array(values) => values
                .iter()
                .map(Self::try_from_nimbus_value)
                .collect::<Result<Vec<_>, _>>()
                .and_then(|values| {
            ...
            Value::Object(fields) => fields
                .iter()
                .map(|(key, value)| {
                    Self::try_from_nimbus_value(value).map(|value| (key.clone(), value))
                })
...
impl FirestoreDouble {
    ...
    fn into_nimbus_value(self) -> Result<Value, FirestoreProtoJsonError> {
        match self {
...
mod tests {
    ...
    #[test]
    fn firestore_value_converts_supported_nimbus_json_values() {
        let source = json!({
            "name": "Ada",
            "age": 42,
            "score": 4.5,
        ...
        let parsed = FirestoreValue::try_from_nimbus_value(&source)
            .expect("nimbus json should convert to firestore value");
    ...
    fn firestore_value_rejects_nested_arrays() {
        ...
        let nested_nimbus_array = json!([[1, 2, 3]]);
        assert!(FirestoreValue::try_from_nimbus_value(&nested_nimbus_array).is_err());
    }
