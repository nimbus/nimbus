...
pub fn run_query_response_entries(
...
pub fn run_aggregation_query_response_entries(
...
pub fn firestore_document_name(
    database: &resource_names::FirestoreDatabaseName,
...
pub fn serialize_json_lines(entries: &[Value]) -> Result<String> {
    entries
        .iter()
        .map(|entry| {
...
pub fn commit_response_json(outcome: &AtomicWriteBatchOutcome) -> Result<Value> {
...
pub fn batch_write_response_json(outcome: &BatchWriteOutcome) -> Result<Value> {
    Ok(json!({
...
pub fn write_result_json(result: &AtomicWriteResult) -> Result<Value> {
    let mut object = serde_json::Map::new();
...
pub fn format_timestamp(timestamp: Timestamp) -> Result<String> {
