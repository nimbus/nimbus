...
impl InvocationAuth {
    ...
    pub fn to_runtime_payload(&self) -> Value {
        serde_json::to_value(self).expect("InvocationAuth must serialize to a JSON object")
    }

    pub fn into_runtime_payload(self) -> Value {
        serde_json::to_value(self).expect("InvocationAuth must serialize to a JSON object")
    }
}
