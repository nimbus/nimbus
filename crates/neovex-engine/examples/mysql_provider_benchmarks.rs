...
async fn run_suite(
    ...
    cleanup_registered_mysql_providers().await;
    Ok(report)
}
...
async fn create_tenant_service(
    ...
    match backend {
        ...
        MeasuredBackend::MySqlLoopback | MeasuredBackend::MySqlInjectedRtt => {
            ...
            let service = Arc::new(
                Service::new_with_persistence_config(mysql_service_config(
                    control_dir.path(),
                    &provider_config,
                ))
                .await?,
...
async fn cleanup_mysql_provider(config: &MySqlProviderConfig) -> BenchResult<()> {
    benchmark_mysql_cleanup(
...
async fn terminate_benchmark_mysql_connections(config: &MySqlProviderConfig) -> BenchResult<()> {
...
async fn benchmark_mysql_cleanup<F>(context: &str, future: F) -> BenchResult<()>
where
    F: std::future::Future<Output = BenchResult<()>>,
{
    match tokio::time::timeout(
...
fn register_mysql_cleanup(config: &MySqlProviderConfig) {
...
async fn cleanup_registered_mysql_providers() {
...
fn benchmark_mysql_provider_config(
    label: &str,
