...
pub(crate) fn build_trigger_invocation_records(
    ...
    for matched in matches {
        ...
        let event = TriggerEvent::new(
            ...
            candidate.commit,
            TriggerExecutionPrincipal::service(neovex_core::PrincipalContext::system()),
        );
...
mod tests {
    ...
    fn registration(
        ...
        TriggerRegistration::new(
            id,
            event_type,
            neovex_core::DocumentTriggerPattern::from_segments(pattern)
    ...
    fn over_depth_candidates_materialize_terminal_failure_without_running() {
        ...
        assert!(matches!(
            records[0].state,
            neovex_core::TriggerInvocationState::TerminalFailure { attempt: 0, .. }
        ));
    }
}
