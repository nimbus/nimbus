...
//! at `<protected-path>.neovex-enc` that stores the wrapped DEK and metadata.
//!
//! # Wire Format (v1)
//!
//! ```text
...
/// Magic bytes at the start of every manifest file: "NVX\x01" (Neovex v1).
const MANIFEST_MAGIC: [u8; 4] = [b'N', b'V', b'X', 0x01];

/// The file extension for sidecar manifests.
pub const MANIFEST_EXTENSION: &str = ".neovex-enc";
...
impl KeyManifest {
    ...
    /// Reads a manifest for a protected path (appending `.neovex-enc`).
    ...
    fn parse(bytes: &[u8], path: &Path) -> Result<Self, ManifestReadError> {
        ...
        if bytes[..4] != MANIFEST_MAGIC {
            return Err(parse_err("invalid magic bytes (not a neovex-enc manifest)"));
        }
    ...
    pub fn write(&self, manifest_path: &Path) -> Result<(), ManifestWriteError> {
        ...
        let temp_path = manifest_path.with_extension("neovex-enc.tmp");
        {
            let mut file =
    ...
    /// Writes the manifest for a protected path (appending `.neovex-enc`).
...
mod tests {
    ...
    fn manifest_round_trips_through_serialization() {
        let manifest = KeyManifest::new(
            ...
            KeyProviderKind::MasterKeyFile {
                path: "/secure/neovex.key".to_string(),
            },
        ...
        let parsed =
            KeyManifest::parse(&bytes, Path::new("test.neovex-enc")).expect("parse should succeed");
    ...
    fn manifest_path_appends_extension() {
        ...
        assert_eq!(manifest, PathBuf::from("/data/demo.sqlite3.neovex-enc"));
    }
    ...
    fn manifest_rejects_bad_magic() {
        ...
        let result = KeyManifest::parse(&bytes, Path::new("test.neovex-enc"));
    ...
    fn manifest_rejects_truncation() {
        ...
        let result = KeyManifest::parse(truncated, Path::new("test.neovex-enc"));
