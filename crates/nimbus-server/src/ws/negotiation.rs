...
pub(crate) const NIMBUS_PROTOCOL_V2: &str = "nimbus.v2";
...
pub(crate) fn negotiate(headers: &HeaderMap) -> Result<NegotiatedWebSocketProtocol, AppError> {
    ...
    let offered = value
        ...
        .map(str::to_string)
        .collect::<Vec<_>>();
    if offered.iter().any(|entry| entry == NIMBUS_PROTOCOL_V2) {
...
pub(crate) fn configure_upgrade(ws: WebSocketUpgrade) -> WebSocketUpgrade {
    ws.protocols([NIMBUS_PROTOCOL_V2])
}
...
async fn send_hello(socket: &mut WebSocket, hello_context: HelloContext) -> Result<(), ()> {
    let hello = HelloFrame {
        ...
        server: HelloServer {
            ...
            build: option_env!("NIMBUS_BUILD_ID").unwrap_or("unknown"),
        },
