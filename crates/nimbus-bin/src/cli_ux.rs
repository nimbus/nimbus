...
Examples:
  nimbus machine init --now
  nimbus machine init --cpus 4 --memory 4096 team-a";

pub(crate) const MACHINE_START_HELP_EXAMPLES: &str = "\
Examples:
  nimbus machine start
  nimbus machine start --cpus 4 --memory 4096 team-a
  nimbus machine start --quiet team-a";

pub(crate) const MACHINE_STOP_HELP_EXAMPLES: &str = "\
Examples:
  nimbus machine stop
  nimbus machine stop team-a";

pub(crate) const MACHINE_STATUS_HELP_EXAMPLES: &str = "\
Examples:
  nimbus machine status
  nimbus machine status -f json
  nimbus machine status --noheading
  nimbus machine status --quiet";

pub(crate) const MACHINE_LIST_HELP_EXAMPLES: &str = "\
Examples:
  nimbus machine list
  nimbus machine list --noheading
  nimbus machine ls --quiet
  nimbus machine list -f json";

pub(crate) const MACHINE_INFO_HELP_EXAMPLES: &str = "\
Examples:
  nimbus machine info
  nimbus machine info -f json
  nimbus machine info -f yaml";
...
Examples:
  ...
  nimbus machine set --disk-size 40 team-a";

pub(crate) const MACHINE_CP_HELP_EXAMPLES: &str = "\
Examples:
  nimbus machine cp ./local.txt default:/tmp/remote.txt
  nimbus machine cp team-a:/var/log/cloud-init-output.log ./cloud-init.log";
...
Examples:
  nimbus compose config
  nimbus compose config --file ./compose.dev.yaml
        ...
        assert_eq!(
            format_action_block(
                ...
                &[
                    ...
                    format_hint("run `nimbus machine start` to boot the updated image"),
                ]
            ),
            "Machine \"default\" machine OS upgraded successfully\n\
Image: docker://example.com/image@sha256:abc123\n\
Hint: run `nimbus machine start` to boot the updated image\n"
        );
    }
