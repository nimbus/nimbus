...
pub(crate) async fn auto_install_node_dependencies(
    ...
    match node_dependency_install_action(app_dir)? {
        ...
        NodeDependencyInstallAction::Install(state) => {
            ...
            let status = tokio::process::Command::new("npm")
                ...
                .map_err(|e| {
                    io::Error::other(format!(
                        "failed to run npm install in {}: {e}. Install Node.js with npm to use Nimbus authoring flows.",
                        app_dir.display()
                    ))
                })?;
            ...
            if !status.success() {
                return Err(io::Error::other(format!(
                    "npm install failed in {}. Resolve the npm error above, then rerun the Nimbus command or run `npm install` manually.",
                    app_dir.display()
                ))
                .into());
            }
...
pub(crate) fn ensure_node22_runtime_available() -> io::Result<()> {
    ...
    match validate_node_runtime_version(&version)? {
        ...
        NodeRuntimeVersionValidation::NewerThanVerified => {
            let message = format!(
                "Nimbus verifies Node.js {}.x for authoring flows; found Node.js {}. Proceeding with best-effort compatibility.",
    ...
    run_version_command("npm", "--version")
        ...
        .map_err(|error| {
            io::Error::new(
                error.kind(),
                format!(
                    "Nimbus authoring flows require npm on PATH when dependencies must be installed: {error}"
                ),
            )
        })
}
...
fn validate_node_runtime_version(version: &Version) -> io::Result<NodeRuntimeVersionValidation> {
    if version.major < REQUIRED_NODE_MAJOR_VERSION {
        return Err(io::Error::other(format!(
            "Nimbus authoring flows require Node.js {}.x or newer; found Node.js {}.",
            REQUIRED_NODE_MAJOR_VERSION, version
        )));
    }
    if version.major == REQUIRED_NODE_MAJOR_VERSION {
...
mod tests {
  ...
  "devDependencies": {
    "@nimbus/codegen": "^1.0.0"
        ...
        assert_eq!(
            missing,
            vec!["@nimbus/codegen".to_string(), "convex".to_string()]
        );
    }
  ...
  "devDependencies": {
    "@nimbus/codegen": "^1.0.0"
        ...
        std::fs::create_dir_all(temp.path().join("node_modules/@nimbus/codegen")).unwrap();
        std::fs::write(
            temp.path()
                .join("node_modules/@nimbus/codegen/package.json"),
            "{}",
        )
        .unwrap();
