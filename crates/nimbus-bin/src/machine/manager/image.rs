...
pub(super) struct MachineArtifactMetadata {
    ...
    pub(super) nimbus_version: Option<String>,
}
...
pub(super) fn machine_artifact_metadata_from_annotations(
    ...
    MachineArtifactMetadata {
        ...
        nimbus_version: annotation_value(primary, fallback, OCI_ANNOTATION_MACHINE_NIMBUS_VERSION),
    }
}
...
fn log_machine_artifact_metadata(reference: &str, metadata: &MachineArtifactMetadata) {
    if let Some(nimbus_version) = metadata.nimbus_version.as_deref() {
        emit_machine_info(format!(
            "machine image '{reference}' embeds nimbus {nimbus_version}"
        ));
    }
...
fn query_attestations(
    ...
    let response = client
        ...
        .header("User-Agent", "nimbus-machine-manager")
        .send()
        .map_err(|e| format!("{e}"))?;
...
pub(super) fn attestation_repositories_for_reference(
    ...
    if image_repo == NIMBUS_SOURCE_REPO {
        vec![image_repo.to_owned()]
    } else {
        vec![image_repo.to_owned(), NIMBUS_SOURCE_REPO.to_owned()]
    }
}
