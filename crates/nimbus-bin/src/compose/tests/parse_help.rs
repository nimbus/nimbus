...
fn compose_help_uses_shared_template_and_examples() {
    let error = RootCli::try_parse_from(["nimbus", "compose", "--help"])
    ...
    assert!(rendered.contains("nimbus compose config"));
    assert!(rendered.contains("nimbus compose up"));
    assert!(rendered.contains("nimbus compose logs api --follow"));
...
fn compose_leaf_help_uses_shared_template_and_examples() {
    let cases = [
        (
            vec!["nimbus", "compose", "config", "--help"],
            "nimbus compose config --services",
        ),
        (
            vec!["nimbus", "compose", "up", "--help"],
            "nimbus compose up api",
        ),
        (
            vec!["nimbus", "compose", "down", "--help"],
            "nimbus compose down api",
        ),
        (
            vec!["nimbus", "compose", "ps", "--help"],
            "nimbus compose ps --all-tenants",
        ),
        (
            vec!["nimbus", "compose", "inspect", "--help"],
            "nimbus compose inspect api",
        ),
        (
            vec!["nimbus", "compose", "logs", "--help"],
            "nimbus compose logs api --follow",
        ),
        (
            vec!["nimbus", "compose", "top", "--help"],
...
fn parses_compose_config_subcommand() {
    let cli = RootCli::parse_from(["nimbus", "compose", "config", "--file", "stack.yml"]);
...
fn parses_compose_config_services_listing_flag() {
    let cli = RootCli::parse_from(["nimbus", "compose", "config", "--services"]);
    let Some(RootCommand::Compose(compose_command)) = cli.command else {
        panic!("compose subcommand should parse");
    };
...
fn parses_compose_ps_all_tenants_flag() {
    let cli = RootCli::parse_from(["nimbus", "compose", "ps", "--all-tenants"]);
    let Some(RootCommand::Compose(compose_command)) = cli.command else {
        panic!("compose subcommand should parse");
    };
...
fn parses_compose_ps_output_shaping_flags() {
    let cli = RootCli::parse_from(["nimbus", "compose", "ps", "-f", "json", "--noheading"]);
    let Some(RootCommand::Compose(compose_command)) = cli.command else {
        panic!("compose subcommand should parse");
    };
...
fn parses_compose_up_with_optional_service_and_tenant_override() {
    let cli = RootCli::parse_from(["nimbus", "compose", "up", "db", "--tenant", "svc-demo"]);
    let Some(RootCommand::Compose(compose_command)) = cli.command else {
        panic!("compose subcommand should parse");
    };
...
fn parses_compose_down_without_service_uses_default_compose_file() {
    let cli = RootCli::parse_from(["nimbus", "compose", "down"]);
    let Some(RootCommand::Compose(compose_command)) = cli.command else {
        panic!("compose subcommand should parse");
    };
...
fn parses_compose_inspect_with_optional_tenant_override() {
    let cli = RootCli::parse_from(["nimbus", "compose", "inspect", "db", "--tenant", "svc-demo"]);
    let Some(RootCommand::Compose(compose_command)) = cli.command else {
        panic!("compose subcommand should parse");
    };
...
fn parses_compose_inspect_format_flag() {
    let cli = RootCli::parse_from(["nimbus", "compose", "inspect", "db", "-f", "yaml"]);
    let Some(RootCommand::Compose(compose_command)) = cli.command else {
        panic!("compose subcommand should parse");
    };
...
fn parses_compose_logs_with_follow_flag() {
    let cli = RootCli::parse_from(["nimbus", "compose", "logs", "db", "--follow"]);
    let Some(RootCommand::Compose(compose_command)) = cli.command else {
        panic!("compose subcommand should parse");
    };
...
fn parses_compose_top_with_optional_tenant_override() {
    let cli = RootCli::parse_from(["nimbus", "compose", "top", "db", "--tenant", "svc-demo"]);
    let Some(RootCommand::Compose(compose_command)) = cli.command else {
        panic!("compose subcommand should parse");
    };
...
fn parses_compose_top_output_shaping_flags() {
    let cli = RootCli::parse_from(["nimbus", "compose", "top", "db", "-f", "json", "-n"]);
    let Some(RootCommand::Compose(compose_command)) = cli.command else {
        panic!("compose subcommand should parse");
    };
...
fn compose_ps_help_describes_output_shaping_flags() {
    let error = RootCli::try_parse_from(["nimbus", "compose", "ps", "--help"])
        .expect_err("help should short-circuit");
    assert_eq!(error.kind(), clap::error::ErrorKind::DisplayHelp);
    let rendered = error.to_string();
...
fn compose_inspect_help_describes_short_format_flag() {
    let error = RootCli::try_parse_from(["nimbus", "compose", "inspect", "--help"])
        .expect_err("help should short-circuit");
    assert_eq!(error.kind(), clap::error::ErrorKind::DisplayHelp);
    let rendered = error.to_string();
...
fn compose_top_help_describes_output_shaping_flags() {
    let error = RootCli::try_parse_from(["nimbus", "compose", "top", "--help"])
        .expect_err("help should short-circuit");
    assert_eq!(error.kind(), clap::error::ErrorKind::DisplayHelp);
    let rendered = error.to_string();
