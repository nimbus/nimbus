...
/// Exclusive durable ownership of one Nimbus-initiated provider bind attempt.
///
/// The opaque provider attempt is generated by the effect-owning adapter. It
/// is deliberately distinct from stable lease identity: concurrent replays of
/// one request must not be able to report failure or release authority owned
/// by another in-flight attempt.
#[derive(Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct PortBindClaim {
    provider_attempt: NetworkProviderHandle,
}

impl PortBindClaim {
    /// Wrap one attempt-unique, provider-scoped opaque identity.
    pub fn new(provider_attempt: NetworkProviderHandle) -> Self {
        Self { provider_attempt }
    }

    /// Opaque attempt identity interpreted only by the effect adapter.
    pub fn provider_attempt(&self) -> &NetworkProviderHandle {
        &self.provider_attempt
    }
}

impl fmt::Debug for PortBindClaim {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("PortBindClaim")
...
impl PortLeaseBinding {
    ...
    pub(crate) fn provider_registration_matches_claim(&self, claim: &PortBindClaim) -> bool {
        self.provider_handle.provider_id() == claim.provider_attempt().provider_id()
    }
...
mod tests {
    ...
    fn binding_and_failure_diagnostics_redact_provider_attempts() {
        ...
        let endpoint = host_v4_specific(PORT);
        let reservation_claim = NetworkReservationClaim::new(provider_handle(secret));
        let claim = PortBindClaim::new(provider_handle(secret));
        ...
        assert!(!format!("{reservation_claim:?}").contains(secret));
        assert!(!format!("{claim:?}").contains(secret));
        assert!(!format!("{binding:?}").contains(secret));
        assert!(!format!("{failure:?}").contains(secret));
        assert_eq!(
            reservation_claim.coordinator_attempt().expose_to_provider(),
            secret,
            "only the explicit coordinator accessor reveals the reservation identity"
        );
        assert_eq!(
            claim.provider_attempt().expose_to_provider(),
            secret,
            "only the explicit provider accessor reveals the claim identity"
        );
        ...
        let wire =
            serde_json::to_value(&reservation_claim).expect("reservation claim should serialize");
        assert_eq!(
            serde_json::from_value::<NetworkReservationClaim>(wire)
                .expect("reservation claim should deserialize"),
            reservation_claim,
            "durable reservation authority must round-trip exactly"
        );
    }
