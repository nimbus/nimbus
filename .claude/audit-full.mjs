...
const SLICES = [
  ...
  { key: 'js-firebase', paths: ['packages/firebase', 'packages/mongodb', 'packages/dynamodb', 'examples'],
    focus: 'JS drop-in SDKs (firebase, mongodb, dynamodb) + examples. Audit: API parity/correctness vs the real Firebase/Mongo/DynamoDB client surface, NO leaked internal Nimbus APIs, type correctness, example accuracy, no secrets committed in example config.' },
