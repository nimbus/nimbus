...
export type QueryReference<Args, Result> = {
  kind: "query";
  name: string;
...
export type PaginatedQueryReference<Args, Item> = {
...
export type MutationReference<Args, Result> = {
  kind: "mutation";
  name: string;
  visibility?: FunctionVisibility;
...
export type ActionReference<Args, Result> = {
  kind: "action";
  name: string;
  visibility?: FunctionVisibility;
...
export type FunctionReference<Args, Result> =
...
export type InferResult<Ref> = Ref extends PaginatedQueryReference<any, infer Item>
  ? Item
...
export function definePaginatedQuery<Args, Item>(
  name: string,
  resolve: (args: Args) => QueryShape,
): PaginatedQueryReference<Args, Item> {
...
export function validateDeploymentUrl(address: string) {
  if (typeof address !== "string") {
    throw new Error(`Neovex clients require a URL string, received ${typeof address}.`);
  }
  if (!address.includes("://")) {
