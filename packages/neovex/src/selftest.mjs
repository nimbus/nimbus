...
async function bundleModule(relativePath, platform) {
  const outdir = await fs.mkdtemp(path.join(os.tmpdir(), "neovex-package-"));
...
async function typecheckNeovexAuthExtension() {
  const fixtureDir = await fs.mkdtemp(path.join(os.tmpdir(), "neovex-ts-"));
    ...
    JSON.stringify(
      {
        compilerOptions: {
          ...
          paths: {
            ...
            "neovex/react": [reactEntry],
            "neovex/values": [valuesEntry],
          },
        },
        files: ["fixture.ts"],
      },
  ...
  await fs.writeFile(
    ...
    `
import { NeovexHttpClient, NeovexReactClient } from "neovex/browser";
import {
  NeovexProvider,
  NeovexProviderWithAuth,
  NeovexReactClient as ReactClient,
  useNeovex,
  useNeovexAuth,
  useNeovexConnectionState,
  type NeovexAuthState,
} from "neovex/react";
import {
  action,
  httpAction,
...
const _neovexBrowserClient = NeovexHttpClient;
const _nativeHttpClient = new NeovexHttpClient("http://localhost:8080/neovex/demo", {
  skipDeploymentUrlCheck: true,
});
const _reactClient = NeovexReactClient;
const _reactClientAlias = ReactClient;
const _neovexReactClient = new NeovexReactClient("http://localhost:8080/neovex/demo", {
  skipDeploymentUrlCheck: true,
});
const _provider = NeovexProvider;
const _providerWithAuth = NeovexProviderWithAuth;
const _useClient = useNeovex;
const _useAuth = useNeovexAuth;
const _useConnectionState = useNeovexConnectionState;
const _authState = null as NeovexAuthState | null;
