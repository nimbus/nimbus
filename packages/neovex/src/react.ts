...
const NeovexContext = createContext<NeovexReactClient | undefined>(undefined);
const NeovexAuthContext = createContext<{
  isLoading: boolean;
  isAuthenticated: boolean;
} | undefined>(undefined);

export function NeovexProvider(props: {
  client: NeovexReactClient;
  children?: ReactNode;
}) {
  return createElement(
    NeovexContext.Provider,
    { value: props.client },
    props.children,
  );
}

export type NeovexAuthState = {
  isLoading: boolean;
  isAuthenticated: boolean;
};

export function useNeovexAuth(): NeovexAuthState {
  const authState = useContext(NeovexAuthContext);
...
export function NeovexProviderWithAuth(props: {
  client: NeovexReactClient;
  children?: ReactNode;
  useAuth: () => {
    isLoading: boolean;
  ...
  return createElement(
    ...
    createElement(NeovexAuthStateFirstEffect, {
      authProviderAuthenticated,
      fetchAccessToken,
      client: props.client,
    ...
    createElement(NeovexAuthStateLastEffect, {
      authProviderAuthenticated,
      client: props.client,
      setIsClientAuthenticated,
    }),
...
export function useNeovex() {
  const client = useContext(NeovexContext);
  if (client === undefined) {
...
function NeovexAuthStateFirstEffect(props: {
...
function NeovexAuthStateLastEffect(props: {
  authProviderAuthenticated: boolean;
  client: NeovexReactClient;
  setIsClientAuthenticated: Dispatch<SetStateAction<boolean | null>>;
}) {
...
export function useNeovexConnectionState(): ConnectionState {
  const client = useNeovex();
