...
usage() {
  cat <<'EOF'
usage: collect-nimbus-homebrew-cask-proof.sh [options]
...
options:
  ...
  --host-binary <path>           Host macOS nimbus binary to package
  ...
  --cask <token>                 Temporary cask token (default: nimbus-dev)
...
examples:
  ...
  bash scripts/collect-nimbus-homebrew-cask-proof.sh \
    --output-dir /tmp/nimbus-d4a-proof \
    --home /tmp/nimbus-d4a-proof/home \
    --runtime-root /tmp/nimbus-d4a-proof/runtime
EOF
}
...
if [[ -z "${output_dir}" ]]; then
  output_dir="$(mktemp -d "${TMPDIR:-/tmp}/nimbus-homebrew-cask-proof.XXXXXX")"
else
  mkdir -p "${output_dir}"
fi
...
cleanup() {
  ...
  if [[ "${machine_stopped}" -eq 0 && -x "${installed_binary:-}" ]]; then
    local -a cleanup_cmd=(
      env
      -u NIMBUS_MACHINE_GVPROXY
      -u NIMBUS_MACHINE_HELPER_BINARY_DIR
      "HOME=${home_dir}"
...
if [[ ! -x "${host_binary}" ]]; then
  echo "host nimbus binary is not executable at ${host_binary}; build it first or pass --host-binary" >&2
  exit 64
fi

if [[ -n "${guest_binary}" && ! -x "${guest_binary}" ]]; then
  echo "guest Linux nimbus binary override is not executable at ${guest_binary}; build it first or pass a valid --guest-binary" >&2
  exit 64
fi
...
if [[ -z "${host_version}" ]]; then
  echo "failed to determine host nimbus version from ${host_binary} --version" >&2
  exit 64
fi

bundle_root="${output_dir}/bundle-root"
bundle_contents="${bundle_root}/contents"
archive_path="${output_dir}/nimbus_darwin_arm64.tar.gz"
...
capture_command \
  ...
  "${base_cmd[@]}" machine ssh -- /usr/local/bin/nimbus --version
...
guest_proof_cmd=(
  ...
  "NIMBUS_MACHINE_RUNTIME_ROOT=${runtime_root}"
  bash
  "${repo_root}/scripts/collect-nimbus-machine-guest-proof.sh"
  --home
  "${home_dir}"
  --runtime-root
  "${runtime_root}"
...
assert_file_contains \
  "assert.archive_manifest_has_nimbus" \
  "${archive_manifest}" \
  '^nimbus$'
