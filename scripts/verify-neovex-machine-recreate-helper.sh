...
tmp_base="${TMPDIR:-/tmp}"
tmp_base="${tmp_base%/}"
tmp_dir="$(mktemp -d "${tmp_base}/neovex-machine-recreate-verify.XXXXXX")"
...
runtime_root="${NEOVEX_MACHINE_RUNTIME_ROOT:?runtime root must be set}"
config_dir="${home_dir}/.config/neovex/machine/${machine_name}"
state_dir="${home_dir}/.local/state/neovex/machine/${machine_name}"
...
HOME="${home_dir}" bash "${repo_root}/scripts/recreate-neovex-machine.sh" \
  --home "${home_dir}" \
...
for expected_file in \
  "${output_dir}/neovex-machine-stop-command.txt" \
  "${output_dir}/neovex-machine-rm-command.txt" \
  "${output_dir}/neovex-machine-init-command.txt" \
  "${output_dir}/neovex-machine-start-command.txt" \
...
grep -F -- "--image ${image_path}" "${output_dir}/neovex-machine-init-command.txt" >/dev/null
grep -F -- "--volume /Users:/Users" "${output_dir}/neovex-machine-init-command.txt" >/dev/null
grep -F "started" "${output_dir}/neovex-machine-start.txt" >/dev/null
...
echo "verified: neovex machine recreate helper captured deterministic artifacts"
