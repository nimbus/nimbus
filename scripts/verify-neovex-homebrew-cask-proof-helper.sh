...
tmp_base="${TMPDIR:-/tmp}"
tmp_base="${tmp_base%/}"
tmp_dir="$(mktemp -d "${tmp_base}/neovex-homebrew-cask-proof-verify.XXXXXX")"
...
host_binary="${bin_dir}/host-neovex"
...
gvproxy_path="${brew_prefix}/Caskroom/neovex-dev/${host_version}/libexec/gvproxy"
...
OUT
  ...
  ssh)
    ...
    shift
    command_string="$*"
    if [[ "${command_string}" == "/usr/local/bin/neovex --version" ]]; then
      printf 'neovex %s\n' "${host_version}"
    ...
    elif [[ "${command_string}" == *"sha256sum /usr/local/bin/neovex"* ]]; then
      printf 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa  /usr/local/bin/neovex\n'
...
OUT
    elif [[ "${command_string}" == *"systemctl show --no-pager --property=Id,LoadState,UnitFileState,ActiveState,SubState neovex.socket"* ]]; then
      cat <<OUT
Id=neovex.socket
LoadState=loaded
...
SubState=listening
OUT
    elif [[ "${command_string}" == *"systemctl show --no-pager --property=Id,LoadState,UnitFileState,ActiveState,SubState neovex.service"* ]]; then
      cat <<OUT
Id=neovex.service
LoadState=loaded
...
case "${1:-}" in
  ...
  install)
    ...
    ln -sf "${caskroom}/neovex" "${brew_prefix}/bin/${token}"
    ;;
  uninstall)
...
bash "${repo_root}/scripts/collect-neovex-homebrew-cask-proof.sh" \
...
grep -Fq "${brew_prefix}/Caskroom/neovex-dev/${host_version}/neovex" "${output_dir}/cask-symlink.txt" || {
  echo "expected cask symlink to point at fake caskroom payload" >&2
  exit 1
}

grep -Fq "${brew_prefix}/Caskroom/neovex-dev/${host_version}/libexec/gvproxy" "${output_dir}/machine-status-running.txt" || {
  echo "expected machine status to report packaged gvproxy path" >&2
  exit 1
}

grep -Fq "neovex ${host_version}" "${output_dir}/guest-neovex-version.txt" || {
...
echo "verified: neovex homebrew cask proof helper captures the packaged macOS release-asset contract deterministically"
