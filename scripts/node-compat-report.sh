...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/node-compat-report.sh --family <family-id> --slice <slice-id> [--output-root <path>] [--observed-results <path>] [--capture-live]

Examples:
  bash scripts/node-compat-report.sh --family networking --slice dns-net-foundation
  bash scripts/node-compat-report.sh --family networking --slice dns-net-foundation --observed-results /tmp/networking-results.json
  bash scripts/node-compat-report.sh --family networking --slice dns-net-foundation --capture-live
  bash scripts/node-compat-report.sh --family loader-context --slice module-and-async-foundation --output-root target/node-compat
EOF
}
