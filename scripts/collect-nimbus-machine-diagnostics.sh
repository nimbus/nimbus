...
usage() {
  cat <<'EOF'
usage: collect-nimbus-machine-diagnostics.sh [options]

Collect best-effort diagnostics for a Nimbus macOS machine-manager state root.
...
options:
  ...
  --nimbus <path>              Optional nimbus binary for `machine status`
  --ps <path>                  Override ps command path
  --log-lines <count>          Number of log lines to copy from each log
  -h, --help                   Show this help

examples:
  bash scripts/collect-nimbus-machine-diagnostics.sh \
    --home /tmp/nimbus-home \
    --runtime-root /tmp/nimbus \
    --output-dir /tmp/nimbus-machine-diag
EOF
}
...
if [[ -z "${config_root}" ]]; then
  if [[ -n "${XDG_CONFIG_HOME:-}" ]]; then
    config_root="${XDG_CONFIG_HOME%/}/nimbus/machine"
  elif [[ -n "${home_dir}" ]]; then
    config_root="${home_dir%/}/.config/nimbus/machine"
  else
...
if [[ -z "${state_root}" ]]; then
  if [[ -n "${XDG_STATE_HOME:-}" ]]; then
    state_root="${XDG_STATE_HOME%/}/nimbus/machine"
  elif [[ -n "${home_dir}" ]]; then
    state_root="${home_dir%/}/.local/state/nimbus/machine"
  else
...
if [[ -z "${output_dir}" ]]; then
  output_dir="$(mktemp -d "${TMPDIR:-/tmp}/nimbus-machine-diag.XXXXXX")"
else
  mkdir -p "${output_dir}"
fi
...
grep -E 'krunkit|gvproxy|nimbus' "${output_dir}/processes-all.txt" >"${output_dir}/processes-matching.txt" || true
print_line "capture.processes_filtered" "ok path=${output_dir}/processes-matching.txt"

if [[ -n "${nimbus_bin}" ]]; then
  capture_command \
    "capture.nimbus_machine_status" \
