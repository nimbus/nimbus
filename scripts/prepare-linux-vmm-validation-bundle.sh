...
usage() {
  cat <<'EOF'
usage: prepare-linux-vmm-validation-bundle.sh [options]
...
if [[ -n "${crun_source}" ]]; then
  crun_source="$(resolve_existing_dir "${crun_source}")"
fi
if [[ -n "${nimbus_crun_asset}" ]]; then
  nimbus_crun_asset="$(resolve_file_path "${nimbus_crun_asset}")"
  if [[ ! -f "${nimbus_crun_asset}" ]]; then
    echo "nimbus-crun asset not found: ${nimbus_crun_asset}" >&2
    exit 66
  fi
fi
if [[ -n "${nimbus_libkrun_archive}" ]]; then
  nimbus_libkrun_archive="$(resolve_file_path "${nimbus_libkrun_archive}")"
  if [[ ! -f "${nimbus_libkrun_archive}" ]]; then
    echo "nimbus-libkrun archive not found: ${nimbus_libkrun_archive}" >&2
    exit 66
  fi
fi
...
NIMBUS_CRUN_ASSET=${nimbus_crun_asset}
NIMBUS_LIBKRUN_ARCHIVE=${nimbus_libkrun_archive}
STAGE_DIR=${stage_dir}
...
1. \`bash ${lh1_script}\`
2. \`bash ${lh2_script}\` (source diagnostics when \`CRUN_SOURCE\` is set)
