...
def registry_path() -> Path:
    return repo_root() / "tests" / "node-compat" / "canary-registry.json"
...
def build_markdown(summary: dict) -> str:
    ...
    for claim in summary["claim_summaries"]:
        ...
        lines.append(
            ...
            f"`{claim['status']}` | {', '.join(claim['lane_coverage'])} | "
    ...
    if summary["oracle_reports"]:
        ...
        for report in summary["oracle_reports"]:
            lines.append(
                ...
                f"`{report['runtime_state']}` | `{report['oracle_state']}` | "
                f"`{report['drift_class']}` | `{report['node_version']}` | "
