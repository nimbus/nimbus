...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/node_compat/oracle-run.sh --lane <node20|node22|node24> --fixture <test-relative-path> [--output-root <path>] [--node-bin <path>]

Examples:
  bash scripts/node_compat/oracle-run.sh --lane node22 --fixture test/parallel/test-buffer-alloc.js
  bash scripts/node_compat/oracle-run.sh --lane node22 --fixture test/parallel/test-buffer-alloc.js --node-bin /opt/homebrew/bin/node
  bash scripts/node_compat/oracle-run.sh --lane node20 --fixture test/parallel/test-buffer-alloc.js --output-root target/node-compat/oracle
EOF
}
