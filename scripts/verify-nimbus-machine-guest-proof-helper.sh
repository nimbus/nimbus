...
OUT
      ...
      *"findmnt --noheadings --output TARGET,SOURCE,FSTYPE,OPTIONS -T \"/Users\" || stat \"/Users\""*)
        ...
        ;;
      *"curl --silent --show-error --include --unix-socket /run/nimbus/nimbus.sock http://localhost/healthz"*)
        cat <<'OUT'
HTTP/1.1 200 OK
...
OUT
        ;;
      *"curl --silent --show-error --include --unix-socket /run/nimbus/nimbus.sock http://localhost/v1/machine-api/capabilities"*)
        cat <<'OUT'
HTTP/1.1 200 OK
content-type: application/json
...
bash "${repo_root}/scripts/collect-nimbus-machine-guest-proof.sh" \
  --home "${home_dir}" \
