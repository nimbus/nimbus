...
repo_root_override="${NIMBUS_MACHINE_GUEST_BUILD_REPO_ROOT:-}"
...
usage() {
  cat <<'EOF'
usage: build-nimbus-machine-guest-binary.sh [options]

Build the Linux guest `nimbus` binary that macOS developer-machine runs stage
...
automatic local guest-binary source for `nimbus machine start`.
...
options:
  ...
  --cache-root <path>          Cache root for cargo-zigbuild + zig
                               (default: ${TMPDIR:-/tmp}/nimbus-machine-guest-build)
...
examples:
  bash scripts/build-nimbus-machine-guest-binary.sh
  bash scripts/build-nimbus-machine-guest-binary.sh --copy-to /tmp/nimbus-linux-guest/nimbus
EOF
}
...
profile="release"
copy_to=""
cache_root="${NIMBUS_MACHINE_GUEST_BUILD_CACHE_ROOT:-${TMPDIR:-/tmp}/nimbus-machine-guest-build}"
...
built_binary="${repo_root}/target/${target_triple}/${profile}/nimbus"
