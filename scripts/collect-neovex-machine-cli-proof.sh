...
usage() {
  cat <<'EOF'
usage: collect-neovex-machine-cli-proof.sh [options]

Collect an isolated local-binary proof for the shipped `neovex machine ...`
...
options:
  ...
  --guest-binary <path>      Optional `NEOVEX_MACHINE_GUEST_BINARY` override
...
examples:
  bash scripts/collect-neovex-machine-cli-proof.sh \
    --neovex target/debug/neovex \
    --image "$HOME/.local/share/neovex/machine/default/images/default.raw"

  bash scripts/collect-neovex-machine-cli-proof.sh \
    --neovex /opt/homebrew/bin/neovex \
    --guest-binary "$HOME/.cache/neovex/machine/guest-neovex/v0.1.18-neovex_linux_arm64-neovex"
EOF
}
...
if [[ -z "${proof_root}" ]]; then
  proof_root="$(mktemp -d "${TMPDIR:-/tmp}/neovex-machine-cli-proof.XXXXXX")"
else
  mkdir -p "${proof_root}"
fi
...
if [[ -n "${guest_binary_override}" ]]; then
  base_cmd+=("NEOVEX_MACHINE_GUEST_BINARY=${guest_binary_override}")
fi
