...
usage() {
  cat <<'EOF'
usage: collect-neovex-machine-service-proof.sh --compose-file <path> --service <name> [options]

Collect host-side proof for the macOS forwarded machine-API and explicit
`neovex service ...` workflow. This is the checked-in MAC5/MAC6/MAC7 evidence
lane for:
...
examples:
  bash scripts/collect-neovex-machine-service-proof.sh \
...
if [[ -z "${output_dir}" ]]; then
  output_dir="$(mktemp -d "${TMPDIR:-/tmp}/neovex-machine-service-proof.XXXXXX")"
else
  mkdir -p "${output_dir}"
fi
