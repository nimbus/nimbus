...
tmp_base="${TMPDIR:-/tmp}"
tmp_base="${tmp_base%/}"
tmp_dir="$(mktemp -d "${tmp_base}/nimbus-machine-cli-proof-verify.XXXXXX")"
...
bin_dir="${tmp_dir}/bin"
guest_binary_path="${tmp_dir}/guest-nimbus"
...
call_log="${XDG_STATE_HOME}/fake-nimbus-calls.txt"
mkdir -p "$(dirname "${call_log}")"
printf '%s\n' "$*" >> "${call_log}"

expected_version="__EXPECTED_NIMBUS_VERSION__"
...
python3 - <<'PY' "${bin_dir}/nimbus" "${expected_nimbus_version}" "${bin_dir}/script" "${script_log}"
from pathlib import Path
import sys

nimbus_path = Path(sys.argv[1])
expected_version = sys.argv[2]
script_path = Path(sys.argv[3])
script_log = sys.argv[4]

nimbus_path.write_text(
    nimbus_path.read_text().replace("__EXPECTED_NIMBUS_VERSION__", expected_version)
)
...
chmod +x "${bin_dir}/nimbus" "${bin_dir}/script"

bash "${repo_root}/scripts/collect-nimbus-machine-cli-proof.sh" \
  --machine proofbox \
  --root "${proof_root}" \
...
call_log="${proof_root}/state/fake-nimbus-calls.txt"
...
grep -F "nimbus.bin                         ${bin_dir}/nimbus" "${summary_file}" >/dev/null
...
grep -F "nimbus ${expected_nimbus_version}" "${output_dir}/host-nimbus-version.txt" >/dev/null
...
grep -F "NIMBUS_MACHINE_GUEST_BINARY=${guest_binary_path}" "${machine_start_command_file}" >/dev/null
...
echo "verified: nimbus machine CLI proof helper captures deterministic isolated-root local-binary evidence"
