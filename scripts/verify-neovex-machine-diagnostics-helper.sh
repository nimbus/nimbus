...
tmp_base="${TMPDIR:-/tmp}"
tmp_base="${tmp_base%/}"
tmp_dir="$(mktemp -d "${tmp_base}/neovex-machine-diag-verify.XXXXXX")"
...
mkdir -p \
  "${home_dir}/.config/neovex/machine/default" \
  "${home_dir}/.local/state/neovex/machine/default" \
  "${runtime_root}" \
  "${bin_dir}" \
  "${output_dir}"

cat > "${home_dir}/.config/neovex/machine/default/config.json" <<'EOF'
{"name":"default","provider":"krunkit"}
EOF

cat > "${home_dir}/.local/state/neovex/machine/default/status.json" <<'EOF'
...
789 1 /Users/jack/src/github.com/agentstation/neovex/target/debug/neovex machine start
OUT
EOF

chmod +x "${bin_dir}/neovex" "${bin_dir}/ps"

bash "${repo_root}/scripts/collect-neovex-machine-diagnostics.sh" \
  --home "${home_dir}" \
...
grep -F "artifact.machine_config            present source=${home_dir}/.config/neovex/machine/default/config.json copy=${output_dir}/machine-config.json" "${summary_file}" >/dev/null
grep -F "artifact.machine_state             present source=${home_dir}/.local/state/neovex/machine/default/status.json copy=${output_dir}/machine-state.json" "${summary_file}" >/dev/null
...
grep -F "capture.neovex_machine_status      ok path=${output_dir}/neovex-machine-status.txt" "${summary_file}" >/dev/null
...
grep -F "manager: ready" "${output_dir}/neovex-machine-status.txt" >/dev/null

echo "verified: neovex machine diagnostics helper captured deterministic artifacts"
