...
tmp_base="${TMPDIR:-/tmp}"
tmp_base="${tmp_base%/}"
tmp_dir="$(mktemp -d "${tmp_base}/neovex-machine-guest-build-verify.XXXXXX")"
tmp_dir="$(cd "${tmp_dir}" && pwd)"
trap 'rm -rf "${tmp_dir}"' EXIT

helper="${repo_root}/scripts/build-neovex-machine-guest-binary.sh"
...
ENV
    ...
    cat > "${FAKE_REPO_ROOT}/target/${target}/${profile}/neovex" <<'INNER'
#!/usr/bin/env bash
exit 0
INNER
    chmod +x "${FAKE_REPO_ROOT}/target/${target}/${profile}/neovex"
    ;;
  *)
...
EOF
  ...
  NEOVEX_MACHINE_GUEST_BUILD_REPO_ROOT="${fake_repo_root}" \
  PATH="${bin_dir}:${PATH}" \
  TMPDIR="${scenario_dir}/tmp" \
  bash "${helper}" --cache-root "${cache_root}" > "${stdout_path}"

  grep -F "${fake_repo_root}/target/${expected_target}/release/neovex" "${stdout_path}" >/dev/null
  grep -F "target add ${expected_target}" "${logs_dir}/rustup.log" >/dev/null
  grep -F "${expected_subcommand} --target ${expected_target} -p neovex-bin --release" "${logs_dir}/cargo.log" >/dev/null
