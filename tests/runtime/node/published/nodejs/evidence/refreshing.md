...
```bash
make node-compat-refresh LANE=node24 TAG=v24.21.0 COMPARE_UPSTREAM=1 SOURCE_ROOT=~/src/github.com/nodejs/node
```

Apply a deliberate lane tag and fixture refresh:

```bash
make node-compat-refresh LANE=node24 TAG=v24.21.0 APPLY=1 SOURCE_ROOT=~/src/github.com/nodejs/node
```
