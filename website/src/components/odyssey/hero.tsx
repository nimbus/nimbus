...
import { type CSSProperties, type KeyboardEvent as ReactKeyboardEvent, type PointerEvent as ReactPointerEvent, type RefObject, useCallback, useEffect, useRef, useState } from 'react';
...
function Install() {
  ...
  return (
    <div className="hero-install">
      ...
      <pre role="tabpanel" id={`install-panel-${path.id}`} aria-labelledby={`install-tab-${path.id}`} aria-label={`${path.label} install commands`}>
        <code>
          {path.lines.map((line) => (
