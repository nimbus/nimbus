...
```
                            ...
                            │                                                                         │
                            │ ┌ Adapters ──┐  ┌ Network ───┐  ┌ Compute ───┐  ┌ Storage ────────┐     │
┌──────────────┐            │ │ Native     │  │ HTTP / WS  │  │ V8 Runtime │  │ SQLite          │     │  ┌──────────────────────┐
│              │            │ │ Convex     │  │ Real-time  │  │ Scheduling │  │ libSQL          │     │  │                      │
│ Apps & Agents├───────────▶│ │ Firebase   ├─▶│ Auth (JWT) ├─▶│ Cron       ├─▶│ redb            ├─────┼─▶│  Postgres  ·  MySQL  │
│              │            │ │ Functions  │  └────────────┘  └──────┬─────┘  └─────────────────┘     │  │  (external storage)  │
└──────────────┘            │ │ MongoDB    │                         │                                │  └──────────────────────┘
                            │ └────────────┘                  ┌──────┴──────┐                         │
                            │                                 │   Sandbox   │                         │
                            │                                 │ ┌─────────┐ │                         │
