...
const state: {
  http: NeovexRestClient | null;
  subscriptionClient: NeovexSubscriptionClient | null;
...
function refreshClients() {
  ...
  state.http = new NeovexRestClient(baseUrl);
...
function resetDefaults() {
  ...
  elements.insertJson.value = JSON.stringify(
    { title: "Ship convex-style Neovex demo", status: "open", priority: 1 },
    null,
    2,
  );
