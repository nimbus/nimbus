...
# A supplied binary already contains these generated assets. Without one, the
# nested target walks the same fresh-checkout dependency graph as `make build`.
ifeq ($(strip $(NIMBUS_EXAMPLES_VERIFY_BIN)),)
examples-verify-run: $(UI_DIST_INDEX) $(EMBEDDED_PKG_MANIFEST)
endif

examples-verify-run:
	bash scripts/examples-verify.sh
...
verify-network-sovereignty-tripwire:
	@test -n "$(RUNNER_ID)" || (echo "set RUNNER_ID=<stable-proof-runner-id>" && exit 1)
	@test -n "$(EXPECTED_HOSTNAME)" || (echo "set EXPECTED_HOSTNAME=<exact-hostname>" && exit 1)
	@test -n "$(HOST_CLASS)" || (echo "set HOST_CLASS=kvm|minicloud" && exit 1)
	@test -n "$(PROVIDER_KIND)" || (echo "set PROVIDER_KIND=kvm|linuxkit" && exit 1)
	@test -n "$(OUTPUT_DIR)" || (echo "set OUTPUT_DIR=/absolute/evidence/path" && exit 1)
	/bin/bash -p scripts/nimbus-network-sovereignty-tripwire.sh \
		--runner-id "$(RUNNER_ID)" \
...
verify-network-sovereignty-tripwire-helper:
	python3 scripts/verify-nimbus-network-sovereignty-tripwire.py
	bash scripts/nimbus-network-control-plane/sovereignty-tripwire-self-tests.sh
...
# Guard the recorded corpus baseline that the Rust compatibility lane reads.
# Runs in the PR lane, so a hand-edited or stale entry fails before merge.
node-compat-baseline-verify:
	python3 scripts/runtime/node/corpus_baseline.py verify
...
# covered; without it every lane must appear in the observed results.
node-compat-baseline-refresh:
	@test -n "$(OBSERVED_RESULTS)" || (echo "set OBSERVED_RESULTS=<aggregated json>" && exit 1)
	python3 scripts/runtime/node/corpus_baseline.py refresh --observed "$(OBSERVED_RESULTS)" $(foreach lane,$(LANES),--lane $(lane))
...
# Draft release notes from conventional commits, for a human to edit down
# into CHANGELOG.md. This writes CHANGELOG.draft.md and never touches
# CHANGELOG.md: that file is curated prose, reviewed in the release
# preparation pull request and published verbatim as the release notes, so
# regenerating it in place would replace it with raw commit subjects.
changelog-draft:
	git-cliff --output CHANGELOG.draft.md
	@printf 'wrote CHANGELOG.draft.md; edit its entries into CHANGELOG.md by hand\n'

# Print the release notes the workflow would publish, e.g.
# `make changelog-notes VERSION=v0.1.48`.
changelog-notes:
	@bash scripts/extract-release-notes.sh $(VERSION)
