...
# LR12: live node-run reconciler test against session systemd (Linux CI
# lane node-dbus-integration). Builds nimbus-bin, so it rides the same
# UI + embedded-package prerequisite graph as the workspace lanes.
test-node-run-live: $(UI_DIST_INDEX) $(EMBEDDED_PKG_MANIFEST)
	cargo test -p nimbus-bin --features node-run-integration-tests node_run_converges_transient_unit -- --nocapture
...
test-external-providers:
	$(SINGLE_FLIGHT) --key cargo-test-external-providers -- bash scripts/test-external-providers.sh
