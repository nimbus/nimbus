...
| Cloudflare API | Available with caveats | Served by default on the main listener (`--no-cloudflare` switches it off). Only the Workers KV REST data plane is served today; Durable Objects, D1, and R2 are not. See the [compatibility reference](/reference/cloudflare/compatibility/). |
