...
Produce or refine a plan for Neovex-native transport evolution that:

- keeps `neovex-core` and `neovex-engine` transport-agnostic,
...
2. `docs/plans/native-transport-evolution-plan.md` must only own:
   - Neovex-native transport and codec evolution after that groundwork
...
- `crates/neovex-server/src/ws/mod.rs`
- `crates/neovex-server/src/ws/socket/transport.rs`
- `crates/neovex-server/src/ws/socket/session.rs`
...
- `docs/plans/archive/firebase-adapter-plan.md`
- `crates/neovex-server/src/adapters/firebase/grpc/`
- `packages/firebase/`
...
4. What realistic performance or ergonomics problem would optional MessagePack
   solve for Neovex-native clients?
5. What realistic performance or reliability problem would optional
   WebTransport solve for Neovex-native clients?
...
- Do not propose protobuf for the native Neovex protocol.
- Do not propose replacing tonic for Firebase transport.
- Do not propose forcing one public wire protocol across Neovex, Convex, and
  Firebase.
