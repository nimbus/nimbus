...
| `operators/hardening.md` | In-server TLS via `--tls-cert`/`--tls-key` (startup-validated) or reverse-proxy posture; adapter listeners proxy-fronted | `crates/nimbus-server/src/tls.rs`, `crates/nimbus-bin/src/start/mod.rs`, `crates/nimbus-server/src/construction.rs` |
| `operators/troubleshooting.md` | Bind/auth/schema/storage/encryption/license/codegen/bundle error strings (each entry quotes source text) | `crates/nimbus-bin/src/start/network_bind.rs`, `crates/nimbus-operator/src/access_policy.rs`, `crates/nimbus-core/src/error.rs`, `crates/nimbus-crypto/src/runtime.rs`, `crates/nimbus-storage/src/sqlite/backend.rs`, `crates/nimbus-license/src/loading.rs`, `crates/nimbus-bin/src/codegen.rs`, `crates/nimbus-runtime/src/error.rs` |
...
| Doc page | Claim / surface | Source |
| --- | --- | --- |
| `reference/cli.md` | Root command list and command map (17 visible commands incl. `run`, `sandbox`, and `backup`) | `crates/nimbus-bin/src/main.rs`, `crates/nimbus-bin/src/run.rs`, `crates/nimbus-bin/src/sandbox.rs`, `crates/nimbus-bin/src/backup.rs` |
...
| `reference/cli.md` | `node` subcommands; exactly-one `--systemd`/`--container`; system scope default; node namespace limited to service-manager artifact management and diagnostics | `crates/nimbus-bin/src/node_service.rs` |
...
| `concepts/resource-model.md` | Backend kinds (sandbox/built-in/external); built-in provider allowlist; only sandbox-backed services launch | `crates/nimbus-services/src/catalog.rs`, `crates/nimbus-services/src/manager/definitions.rs`, `crates/nimbus-services/src/manager/service_start.rs` |
...
| `concepts/resource-model.md` | Deny-by-default egress; API redacts launch inputs | `crates/nimbus-egress/src/lib.rs`, `crates/nimbus-server/src/http/sandbox_spec.rs` |
...
| `concepts/architecture/node-lifecycle.md` | Reconciler desired-state derivation (active→running, deleting→stopped); inspect-first outcomes; status-evidence writer seam; node-agent multi-assignment disposition model | `crates/nimbus-node/src/reconciler.rs` |
| `concepts/architecture/node-lifecycle.md` | Hidden node workload executor constructs `SystemdTransientUnitBackend` + `NodeWorkloadReconciler` and runs a per-assigned-workload reconcile loop, while public workload intent belongs above the node namespace; multi-node scheduling/assignment is out of scope for a single process | `crates/nimbus-bin/src/node_workload_executor.rs`, `crates/nimbus-node/src/reconciler.rs`, `crates/nimbus-node/src/systemd_transient.rs` |
...
| `concepts/architecture/cli-codegen.md` | clap command tree (start/dev/deploy/run/sandbox/codegen/init/token/auth/ui/machine/node/compose/policy/encryption/packages + hidden node-workload-executor and sandbox-supervisor) | `crates/nimbus-bin/src/main.rs`, `crates/nimbus-bin/src/run.rs`, `crates/nimbus-bin/src/sandbox.rs`, `crates/nimbus-bin/src/node_workload_executor.rs` |
