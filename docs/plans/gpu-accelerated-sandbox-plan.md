# Plan: GPU-Accelerated Sandbox (Tier 3)
...
This plan covers Tier 3 of the three-tier sandbox roadmap. Tier 3 is one
long-lived tenant sandbox per workload (or warm-pooled per tenant) with
a host GPU exposed inside the guest. The sandbox backend is the shared
`libkrun_session` backend; the GPU policy is the only thing that
distinguishes Tier 3 from Tier 2 (computer use).

| Shape | Path | This plan? |
| --- | --- | --- |
| Lambda invocations (no GPU) | FSI plan, Firecracker | No |
| Computer-use session (display, optional GPU for rendering) | CUS plan, `libkrun_session` | No |
| GPU AI workload (GPU for compute, display optional) | this plan, `libkrun_session` | Yes |
...
- The Tier-2 display surface (CUS plan).
...
- macOS Tier-3 production support beyond Venus (CUDA stays
  Linux-fleet-only).
...
- macOS Tier-3 path is Venus-only and gated on real-host stability
