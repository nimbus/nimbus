...
NDS3 also has an upstream-alignment pause gate. After the `v2.8.0-nimbus.15`
checkpoint, do not start another serious fixture-promotion wave on the current
Deno-family fork stack. First execute
`docs/plans/deno-rusty-v8-upstream-alignment-plan.md` in a dedicated
worktree/PR, classify the accumulated Deno and `rusty_v8` fork delta against
upstream `denoland/deno@v2.8.1` and `denoland/rusty_v8@v149.2.0`, repin
Nimbus to immutable upstream-aligned fork tags, and then resume NDS3 from that
baseline. Focused NDS cleanup may finish the current checkpoint, but deeper
compatibility claims must not be built on stale fork overlap.
...
| Current row | `NDS3` |
| Latest verifier output | `15 passed, 19 failed (NDS3 in progress after Node24 networking plus loader/context checkpoint; Node24 is 892 / 5198 and current-row/future-row gates fail as expected)` |
