...
| Shared SQL-family scan planner | `crates/nimbus-storage/src/index/history_scan.rs` centralizes query-shape construction, encoded tuple bounds, cursor validation, and page finalization for SQLite, Postgres, and MySQL. Backends now own only physical row fetching and document hydration, reducing cross-backend semantic drift risk. |
