...
| MongoDB adapter surface | `docs/adapters/mongodb/README.md`, `docs/adapters/mongodb/operations.md` | CRUD, cursor, transaction, index, and change-stream commands inherit engine/storage correctness through the same mutation/query path. | Historical reads and PITR are not MongoDB-wire claims unless a documented extension exists; unsupported commands must fail clearly. |
