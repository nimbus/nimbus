# Plan: Computer Use Sandbox (Tier 2)
...
This plan covers Tier 2 of the three-tier sandbox roadmap. Tier 2 is one
session per tenant, long-lived (minutes to hours), with a desktop and an
agent loop running inside the guest. The session sandbox is built on the
shared `libkrun_session` backend; Tier 3 (GPU AI workloads) is a sibling
consumer of the same backend with different spec options.

| Shape | Canonical Nimbus path | This plan? |
| --- | --- | --- |
| Lambda-style invocations | Firecracker snapshot/restore (FSI plan) | No |
| Long-lived OCI services | krun service-microVM via conmon/crun | No |
| Computer-use session | `libkrun_session` backend, this plan | Yes |
| GPU AI workload | `libkrun_session` backend, GAW plan | No |

Do not build a separate `computer_use` backend. Tier 2 is the
`libkrun_session` backend with `display: Some(...)`, `input: Some(...)`,
and a long-idle lifetime policy.

## Why a Separate Plan

The Firecracker plan answers: how does Nimbus run one full-Linux
invocation quickly inside a fresh isolated microVM?

The libkrun-session research doc answers: what is the shared backend
shape for Tier 2 and Tier 3?
...
| CUS11 | `todo` | Docs, security model, residual risk, selectability posture. | Docs clearly state Tier-2 is opt-in, threat model is documented, and product selectability is gated on plan completion. |
