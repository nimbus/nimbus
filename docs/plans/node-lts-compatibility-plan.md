...
| Item | Status | Hard deps | Completion gate |
|------|--------|-----------|-----------------|
| NLC3 Core semantics family | `in_progress` | NLC2 | `assert`, `events`, `buffer`, `path`, `url`, `console`, `querystring`, `punycode`, and `string_decoder` satisfy the family closeout contract, with Node22 pass rate >=95%, Node20 divergence documented, and no unexplained failures. |
