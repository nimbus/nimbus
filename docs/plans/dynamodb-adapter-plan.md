...
- **Active item:** D8.3 (parity-test runner foundation). **All of D0–D7 and D8.1–D8.2 are `done`** — full T0–T6 operation surface implemented + SDK-proven, strict SigV4 enforced when enabled, runtime access-key store, and the `@nimbus/dynamodb` npm package (connection helpers + selftest) ships. Remaining: D8.3–D8.9 (parity runner / corpus / ExtendDB / harness / external suites / canaries — several gated on Docker/network availability) + D9 (failure-injection/soak/perf/enterprise-readiness evidence + closeout).
