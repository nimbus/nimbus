...
- **Active item:** D8.1 (`@nimbus/dynamodb` npm package). **All of D0–D7 are `done`** — the full T0–T6 operation surface is implemented and SDK-proven, strict SigV4 is enforced when enabled, and access keys can be configured/rotated at runtime via the persisted store. Remaining: D8 (npm package) + D9 (failure-injection/soak/perf/enterprise-readiness evidence + closeout).
