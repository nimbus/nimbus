...
| Bun/JSC optional runtime lane | JavaScriptCore VM memory, Bun global/API permission surface, package loading, VM lifecycle/reuse, adapter/pool capacity | manifest-selectable Bun/JSC policy metadata lane; executor stays lazy and execution fails closed while the Bun/JSC adapter is not linked; diagnostics expose `outer_quota_required` memory semantics | metadata may be admitted for inspection and planning, but runtime execution rejects until an adapter/pool exists; once linked, overload must be controlled by explicit executor/pool/outer-memory quotas | BEP permission, memory/reuse, package-loading, artifact, and Linux verification reports plus per-lane `/debug/runtime/metrics` executor/pool state | no separate admission gate until the linked adapter proves a concrete bottleneck; future promotion must name Bun/JSC executor slots, pool slots, and outer quota behavior separately from V8 isolate heap limits |
