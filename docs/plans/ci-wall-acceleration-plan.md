...
| CW4 | Warm-sccache optimization (research lane). Two prongs were considered: (a) drop `--tests` from the warm pass and let consumers pay test-binary compile inline; (b) prototype a per-target Swatinem cache layer for the warm-sccache job specifically. Lane (a) landed: `warm-sccache` in `ci.yml` now runs `cargo check --workspace` (no `--tests`). The lib/bin dep-graph compile that drives downstream cross-job sccache reuse is unchanged; test-binary rustc invocations had marginal cross-job sccache reuse (each integration test binary is its own rustc call with its own dev-dep mix) so paying their compile inline in the downstream test jobs (sharded by CW1 / CA3 / CW2 across runners) is a wall win. Lane (b) is deferred: `Swatinem/rust-cache@v2` already caches `target/` with built-in filtering, and the composite at `.github/actions/setup-rust-cached/` wires it onto warm-sccache via `ci-ubuntu-stable-warm-sccache-no-bin-v2`, so a bespoke additional layer requires CI-run measurement to justify. Verifier asserts: warm-sccache shape documented in `docs/operating/ci-modernization.md` ("PR critical-path acceleration" section) and either (a) `--tests` dropped or (b) target-cache restored on the warm-sccache job. | done |
