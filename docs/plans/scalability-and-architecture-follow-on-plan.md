...
| --- | --- | --- | --- |
| SA0 | todo | Introduce scoped task primitives for background workers and session-owned tasks | none |
| SA1 | todo | Move subscription re-evaluation off the synchronous write return path and pin delivery semantics | SA0 |
| SA2 | todo | Coalesce invalidation and fanout across durable-apply batches | SA1 |
| SA3 | todo | Retrofit structured concurrency across remaining socket, subscription, and runtime flows | SA0 |
| SA4 | todo | Add composite indexes and planner support for multi-field query shapes behind a design-note gate | none |
