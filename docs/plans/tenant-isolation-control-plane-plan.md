...
- Firebase/Firestore listen/WebSocket auth needs scoped-tenant proof.
- Cloud Functions public unauthenticated HTTP exposure needs an explicit note or
  policy decision separate from callable auth, because it does not carry bearer
  principal authority today.
- Native HTTP/local operator routes need the scoped session and tenant
...
### 2026-05-22 TIC5 Firebase gRPC Swapped-Tenant Proof

Added the Firestore gRPC counterpart to the REST `batchGet` proof.
...
- Added a Firebase auth/availability test that configures the real application
  custom-JWT verifier, creates `tenant-a` and `tenant-b`, and issues a signed
  token whose verified custom claim is `tenant_id=tenant-b`.
- Seeded `tenant-b` and proved gRPC `GetDocument` succeeds against
  `projects/tenant-b/databases/(default)/documents/cities/SF`.
- Proved the same bearer token is rejected with gRPC `PermissionDenied` when it
  targets `projects/tenant-a/databases/(default)/documents/cities/SF`, before
  the provider can turn the request into a cross-tenant read.
- Asserted the gRPC status message names both the authorized claim tenant and
  the rejected Firestore project tenant.

Verification evidence:

- `cargo test -p nimbus-server firebase_grpc_get_document_rejects_application_bearer_for_different_tenant -- --nocapture`
  - result: pass; 1 passed, 0 failed, 725 filtered out in `src/lib.rs`;
    MongoDB spec and reactive-loop integration targets had 0 matching tests.
- `cargo test -p nimbus-server firebase_auth_and_availability -- --nocapture`
  - result: pass; 11 passed, 0 failed, 715 filtered out in `src/lib.rs`;
    MongoDB spec and reactive-loop integration targets had 0 matching tests.
- `cargo fmt --all --check`
  - result: pass
...
### 2026-05-22 TIC5 Firebase Listen/WebSocket Swapped-Tenant Proof

Added the browser/WebSocket Listen counterpart to the Firestore REST and gRPC
swapped-tenant proofs.
...
- Added a Firebase auth/availability test that configures the real application
  custom-JWT verifier, creates `tenant-a` and `tenant-b`, and issues a signed
  token whose verified custom claim is `tenant_id=tenant-b`.
- Proved a Firestore Listen websocket authenticated through the
  `nimbus.firebase.auth.*` subprotocol bootstraps tenant B's
  `listenTenantProof` collection when the Listen request database and target
  parent both name `tenant-b`.
- Proved the same websocket auth offer is rejected with a policy close when the
  Listen request database and target parent name `tenant-a`, before the request
  can subscribe to tenant A's storage provider state.
- Hardened Firestore Listen websocket close frames so long gRPC status messages
  are capped to the WebSocket close-reason byte budget instead of producing an
  invalid control frame.

Verification evidence:

- `cargo test -p nimbus-server firebase_listen_websocket_rejects_application_bearer_for_different_tenant -- --nocapture`
  - result: pass; 1 passed, 0 failed, 728 filtered out in `src/lib.rs`;
    MongoDB spec and reactive-loop integration targets had 0 matching tests.
- `cargo test -p nimbus-server websocket_close_reason -- --nocapture`
  - result: pass; 2 passed, 0 failed, 727 filtered out in `src/lib.rs`;
    MongoDB spec and reactive-loop integration targets had 0 matching tests.
- `cargo test -p nimbus-server firebase_auth_and_availability -- --nocapture`
  - result: pass; 12 passed, 0 failed, 717 filtered out in `src/lib.rs`;
    MongoDB spec and reactive-loop integration targets had 0 matching tests.
...
### 2026-05-22 TIC5 Runtime HostBridge Tenant-Payload Rejection

Hardened the in-process Convex runtime HostBridge ABI so JavaScript payloads
cannot smuggle tenant identity into DB, scheduler, query, function, service,
or HTTP-route host calls.

Completed in this checkpoint:

- Added `serde(deny_unknown_fields)` to the Convex host-call request envelope
  and runtime payload structs for DB operations, scheduler operations,
  function calls, query builders, service lookup, and HTTP route invocation.
- Added direct HostBridge authorization tests for `ctx.db.insert` and
  `ctx.scheduler.runAfter` payloads that include a forged top-level
  `tenant_id=tenant-b` while the bridge isolation context belongs to the
  original tenant.
- Proved both forged payloads fail during payload decoding with an
  `unknown field \`tenant_id\`` runtime JSON error before either the original
  tenant or tenant B receives a document write or scheduled-job side effect.

Verification evidence:

- `cargo test -p nimbus-server runtime_host_bridge_rejects_payload_tenant -- --nocapture`
  - result: pass; 2 passed, 0 failed, 729 filtered out in `src/lib.rs`;
    MongoDB spec and reactive-loop integration targets had 0 matching tests.
- `cargo test -p nimbus-server adapters::convex::tests::authorization -- --nocapture`
  - result: pass; 8 passed, 0 failed, 723 filtered out in `src/lib.rs`;
    MongoDB spec and reactive-loop integration targets had 0 matching tests.
- `cargo test -p nimbus-server convex_functions::runtime_writes -- --nocapture`
  - result: pass; 7 passed, 0 failed, 724 filtered out in `src/lib.rs`;
    MongoDB spec and reactive-loop integration targets had 0 matching tests.
- `cargo test -p nimbus-server convex_functions::runtime_queries -- --nocapture`
  - result: pass; 13 passed, 0 failed, 1 ignored, 717 filtered out in
    `src/lib.rs`; MongoDB spec and reactive-loop integration targets had 0
    matching tests.
- `cargo test -p nimbus-server convex_runtime::http_routes -- --nocapture`
  - result: pass; 9 passed, 0 failed, 4 ignored, 718 filtered out in
    `src/lib.rs`; MongoDB spec and reactive-loop integration targets had 0
    matching tests.
...
### 2026-05-22 TIC5 Cloud Functions Public HTTP Policy Closure

Closed the remaining Cloud Functions public HTTP question as an explicit
current-phase policy rather than treating it like a bearer-auth swapped-tenant
surface.

Completed in this checkpoint:

- Confirmed the public HTTP exposure path does not derive tenant identity from
  tenant-controlled path, query, header, or bearer claims.
- Recorded the current policy: public HTTP targets are allowed only when the
  service has exactly one tenant; if more than one tenant exists, Nimbus
  rejects the request with `409 Conflict` and defers explicit multi-tenant HTTP
  binding to a later Cloud Functions phase.
- Kept the bearer-principal tenant-claim proof on the callable exposure, where
  Firebase callable auth actually supplies application bearer authority.

Verification evidence:

- `cargo test -p nimbus-server cloud_functions_http_handler_rejects_ambiguous_multi_tenant_binding -- --nocapture`
  - result: pass; 1 passed, 0 failed, 731 filtered out in `src/lib.rs`;
    MongoDB spec and reactive-loop integration targets had 0 matching tests.
- `cargo test -p nimbus-server cloud_functions_http_handler -- --nocapture`
  - result: pass; 4 passed, 0 failed, 728 filtered out in `src/lib.rs`;
    MongoDB spec and reactive-loop integration targets had 0 matching tests.

Remaining before TIC5 is done:

- Native HTTP/local operator routes are closed for the current product shape:
  local-admin auth/session is global operator authority, and per-tenant app
  membership for native HTTP is a future product surface, not the current API.

### 2026-05-22 TIC5 Native Local-Operator Policy Closure

Closed the native HTTP/local route item as a local-operator authorization
contract.

Completed in this checkpoint:

- Audited native HTTP tenant parsing: all tenant-facing native document, query,
  schema, scheduling, tenant, metadata, and service routes call
  `parse_operator_tenant_context(...)`, which rejects reserved user-tenant IDs
  and produces `TenantIsolationContext::operator(...)` before service/provider
  calls.
- Confirmed local server security remains the authority seam for native API and
  debug routes when configured: unauthenticated requests are rejected and the
  local-admin bearer authorizes global operator actions.
- Confirmed native tenant APIs reject and hide reserved `_nimbus` tenants, so
  local operator user-tenant paths cannot address system-control storage as a
  regular tenant.
- Recorded that per-tenant application membership for native HTTP would be a
  future product auth surface; the current native API is an operator plane, not
  an application tenant plane.

Verification evidence:

- `cargo test -p nimbus-server native_api_and_debug_routes_require_local_admin_auth -- --nocapture`
  - result: pass; 1 passed, 0 failed, 731 filtered out in `src/lib.rs`;
    MongoDB spec and reactive-loop integration targets had 0 matching tests.
- `cargo test -p nimbus-server local_admin_tenant_api_rejects_and_hides_reserved_system_tenants -- --nocapture`
  - result: pass; 1 passed, 0 failed, 731 filtered out in `src/lib.rs`;
    MongoDB spec and reactive-loop integration targets had 0 matching tests.

TIC5 status after this checkpoint: closed for the current native HTTP,
adapter, runtime HostBridge, scheduler, system-control, and Cloud
Functions/Firebase/Convex surfaces.

Remaining after TIC5:

- TIC4 is now closed for tenant-isolation enforcement.
...
- Lower `route via microvm_service` or `route via in_process_trusted_only`
  admission decisions through an execution path instead of returning the
  admission route as an error.

### 2026-05-22 TIC4 Runtime Fallback Decision And Native Package Proof

Lifted runtime fallback routing out of string-only errors and proved the
product-shaped unsafe cases.

Completed in this checkpoint:

- Added `RuntimePolicyAdmission` and `RuntimeIsolationRoute` so production
  `in_process_untrusted` admission can return a structured route decision with
  the reason and recommended tier.
- Initially kept the route as the existing admission error; the following
  checkpoint supersedes that with a call-boundary `RuntimeExecutionAdmission`
  seam.
- Added package-manager subprocess proof: an application Node policy with an
  `npm` run grant routes to `microvm_service`.
- Added native-addon/package-loading proof: an application policy with broad
  `$app_root`/`$cache_root` package-loading reads routes to
  `microvm_service`.

Verification evidence:

- `cargo test -p nimbus-server production_untrusted_runtime_admission_routes -- --nocapture`
  - result: pass; 3 passed, 0 failed, 731 filtered out in `src/lib.rs`;
    MongoDB spec and reactive-loop integration targets had 0 matching tests.
- `cargo fmt --all --check`
  - result: pass.
- `git diff --check`
...
Follow-up before TIC4 closure:

- Superseded by the following checkpoint, which wires
  `RuntimePolicyAdmission::Route(...)` into `RuntimeExecutionAdmission` and
  fails closed at the runtime execution boundary when no fallback executor is
  configured.

### 2026-05-22 TIC4 Runtime Execution Admission Handoff

Closed TIC4 for tenant-isolation enforcement by moving fallback handling to the
runtime execution boundary.

Completed in this checkpoint:

- Added `execution::runtime_admission::RuntimeExecutionAdmission` as the shared
  execution-boundary seam. It maps `RuntimePolicyAdmission::AdmitInProcess` to
  in-process execution and `RuntimePolicyAdmission::Route(...)` to an explicit
  `FallbackUnavailable` decision.
- Switched Convex runtime-backed invocation, Cloud Functions HTTP invocation,
  and Cloud Functions trigger invocation to use `RuntimeExecutionAdmission`
  before building a host bridge or invoking a runtime bundle.
- Removed the old string-only `ensure_runtime_policy_admitted(...)` guard from
  production code. Tenant isolation now has a structured admission decision at
  policy time and a structured execution decision at call time.
- Kept current production behavior fail-closed for routed fallbacks because
  Nimbus does not yet have a microVM JavaScript/function executor or a
  trusted-only runtime executor configured. This is the safe tenant-isolation
  behavior: unsafe policies cannot silently fall through to in-process
  execution.

Verification evidence:

- `cargo test -p nimbus-server runtime_execution_admission -- --nocapture`
  - result: pass; 2 passed, 0 failed, 734 filtered out in `src/lib.rs`;
    MongoDB spec and reactive-loop integration targets had 0 matching tests.
- `cargo test -p nimbus-server production_untrusted_runtime_admission_routes -- --nocapture`
  - result: pass; 3 passed, 0 failed, 733 filtered out in `src/lib.rs`;
    MongoDB spec and reactive-loop integration targets had 0 matching tests.
- `cargo test -p nimbus-server convex_runtime_only_query_can_run_runtime_only_query -- --nocapture`
  - result: pass; 1 passed, 0 failed, 735 filtered out in `src/lib.rs`;
    MongoDB spec and reactive-loop integration targets had 0 matching tests.
- `cargo test -p nimbus-server cloud_functions_http_handler -- --nocapture`
  - result: pass; 4 passed, 0 failed, 732 filtered out in `src/lib.rs`;
    MongoDB spec and reactive-loop integration targets had 0 matching tests.
- `cargo test -p nimbus-server cloud_functions_trigger_executor_reads_and_writes_via_runtime_bundle -- --nocapture`
  - result: pass; 1 passed, 0 failed, 735 filtered out in `src/lib.rs`;
    MongoDB spec and reactive-loop integration targets had 0 matching tests.
- `cargo fmt --all --check`
  - result: pass.
...
TIC4 status after this checkpoint: closed. Future work that adds an actual
...
### 2026-05-22 TIC7 MicroVM Resource Quotas And Handle Ownership

Closed TIC7 at the sandbox launch admission seam and the runtime service
handle projection seam.

Completed in this checkpoint:

- Added `SandboxResourceQuotaPolicy` and `SandboxResourceCharge` so active
  sandbox count, vCPU, memory, disk-reservation, and log-reservation budgets
  are first-class tenant quota inputs.
- Added the shared OCI `ResourceQuotaManager` and wired both krun and
  container backends through it before launch planning/materialization. Quota
  accounting reads only the target tenant's active manifests, while stopped or
  other-tenant manifests do not consume the requested tenant's budget.
- Added `disk_limit_bytes` and `log_limit_bytes` to sandbox resources, exposed
  them through Compose `x-nimbus.disk_limit` and `x-nimbus.log_limit`, and
  rejected zero-valued disk/log limits before bundle materialization.
- Passed per-sandbox log limits to conmon as `--log-size-max`, making service
  log growth bounded by the admitted sandbox resource contract.
- Added `tenant_id` to `SandboxHandle` and changed service-manager/runtime
  registry projections to reject or hide handles whose tenant differs from the
  requested tenant, even if a backend/catalog incorrectly returns one.
- Preserved the existing TIC3 published-port quota, TIC4 runtime
  active/in-flight/queued/nested-call budgets, and runtime fallback routing as
  part of the broader per-tenant resource contract.

Verification evidence:

- `cargo check -p nimbus-sandbox`
  - result: pass; `Finished dev profile` in 1.75s.
- `cargo check -p nimbus-bin`
...
- `cargo test -p nimbus-bin --no-run`
  - result: pass; `Finished test profile` in 15.56s.
- `cargo test -p nimbus-sandbox resource_quota -- --nocapture`
  - result: pass; 3 passed, 0 failed, 110 filtered out in `src/lib.rs`;
    guest-user-switch and Linux smoke targets had 0 matching tests.
- `cargo test -p nimbus-sandbox active_sandbox_quota -- --nocapture`
...
- `cargo test -p nimbus-sandbox conmon_launch_plan_injects_mount_prelude_for_image_backed_sandboxes -- --nocapture`
  - result: pass; 1 passed, 0 failed, 112 filtered out in `src/lib.rs`;
    guest-user-switch and Linux smoke targets had 0 matching tests.
- `cargo test -p nimbus-server service_binding -- --nocapture`
  - result: pass; 10 passed, 0 failed, 729 filtered out in `src/lib.rs`;
    MongoDB/reactive-loop targets had 0 matching tests.
- `cargo test -p nimbus-bin x_nimbus_disk -- --nocapture`
  - result: pass; 1 passed, 0 failed, 526 filtered out in `src/main.rs`;
    `server_discovery_serde` target had 0 matching tests.
- `cargo fmt --all --check`
  - result: pass.
- `git diff --check`
...
TIC7 status after this checkpoint: closed for current service placement,
launch quota admission, conmon log caps, and tenant-owned handle projection.
Disk limits are enforced as Nimbus launch reservations against per-tenant
