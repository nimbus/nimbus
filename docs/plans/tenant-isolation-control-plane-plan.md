...
- Add a canonical routing outcome for workloads rejected from
  `in_process_untrusted` so broad OS capability needs move to
  `microvm_service` or a trusted-only tier instead of only returning an
  admission error.
