...
| CA1 | Install `mold` in the `setup-rust-cached` composite action on Linux runners and configure `CARGO_TARGET_X86_64_UNKNOWN_LINUX_GNU_LINKER=mold` workflow-side. This benefits every Rust job that goes through the composite (12 sites today) but unblocks the Coverage `-j > 1` retest in particular. Verifier asserts: composite action references `mold` install + linker env; at least one workflow exports the linker env. macOS / Windows branches of the composite remain on the system linker. | done |
