...
(virtual filesystem, sandboxed process execution, HTTP client) to Nimbus via
WASI Component Model interfaces backed by swappable providers.

This document owns the durable forward-looking context for the `nimbus:agent`
...
Its WIT interfaces cover the existing `HostBridge` surface (`nimbus:host` —
database, scheduler, runtime, context).

Agent workloads need OS-level primitives that do not exist in the current
`HostBridge` contract:

| Capability | Why agents need it | Current Nimbus surface |
|---|---|---|
...
These are additive capabilities — standard `nimbus-function` components never
see them. The WASI Component Model enforces this: a component that does not
import `nimbus:agent/filesystem` cannot call filesystem operations, period. This
...
- The `nimbus:agent` WIT package (filesystem, process, http-client interfaces)
- The `ComponentWorld::NimbusAgent` variant and its admission gate
...
- The `nimbus:host` WIT interfaces (database, scheduler, runtime, context) —
...
| agent-os component | What it provides | How Nimbus uses it |
|---|---|---|
...
| `crates/execution` | Node.js process spawning | **Not used** — Nimbus's V8/wasmtime replaces this entirely |
...
|---|---|---|
| `wasi:filesystem` | Standard virtual filesystem | Evaluate as a base; extend with Nimbus-specific scoping if needed |
...
Prefer standard WASI interfaces where they fit. Define `nimbus:agent`-specific
interfaces only where the standard WASI interfaces are insufficient or where
Nimbus needs per-tenant scoping that WASI does not provide.
...
### nimbus:agent/filesystem

```wit
package nimbus:agent@0.1.0;
...
```wit
world nimbus-agent {
    // All standard Nimbus host capabilities
    import nimbus:host/database;
    import nimbus:host/scheduler;
    import nimbus:host/runtime;
    import nimbus:host/context;
...
A `nimbus-agent` world component has strictly more capabilities than a
`nimbus-function` component. The type system enforces this: a component compiled
against `nimbus-function` will not import agent interfaces and therefore cannot
call them.
...
InvocationHostState (extended)
  - bridge: Arc<dyn HostBridge>           (existing — nimbus:host)
  - agent_provider: Option<Arc<dyn AgentOsProvider>>  (new — nimbus:agent)
  - context: RuntimeInvocationContext
  - cancellation: Option<HostCallCancellation>
  - limiter: StoreLimiter

Linker bindings:
  nimbus:host/* → HostBridge::call / call_async     (from wasmtime plan)
  nimbus:agent/filesystem/* → agent_provider.fs_*   (new)
  nimbus:agent/process/* → agent_provider.process_* (new)
  nimbus:agent/http-client/* → agent_provider.http_*(new)
```

If `agent_provider` is `None` (tenant lacks agent capabilities), the linker
still contains the bindings but instantiation of a `nimbus-agent` world
...
ComponentWorld::NimbusFunction
  → always allowed
  → linker binds nimbus:host only

ComponentWorld::NimbusAgent
  → requires tenant-level agent capability flag
  → linker binds nimbus:host + nimbus:agent
...
path. Once deployed, a `NimbusAgent` component runs through the same scheduling
...
1. WASM component calls `nimbus:agent/process/exec`
...
Nimbus process
  ...
  │     └── Store → nimbus:agent/filesystem/read("data.json")
...
If agent-os abstracts its kernel over a capability-based interface (or if Nimbus
...
### What Nimbus replaces in agent-os

| agent-os component | Nimbus replacement |
|---|---|
| `crates/execution` (Node.js process spawning) | Nimbus V8 or wasmtime execution |
| `crates/sidecar` service loop | Nimbus `RuntimeExecutor` scheduling |
| agent-os TypeScript SDK | Nimbus WASM component + WIT imports |
| Node.js `--permission` sandboxing | WASI Component Model capability scoping |

What Nimbus keeps from agent-os: the virtual kernel patterns (VFS, process
table, PTY, permission model) — either as a sidecar dependency or as extracted
design patterns.

## Required Invariants

- Agent capabilities must be strictly additive. `nimbus-function` components
...
1. `docs/plans/wasmtime-backend-plan.md` Phase W3 (run-to-completion wasmtime
   backend) has reached `done` status, including the `nimbus:host` WIT linker.
...
|-------|--------|---------|-------------------|-----------|
| A1 | `todo` | `nimbus:agent` WIT interface definitions | Wasmtime plan W3 `done` | define `filesystem`, `process`, `http-client` interfaces and `nimbus-agent` world; validate WIT compilation and component targeting |
| A2 | `todo` | `AgentOsProvider` trait and `InMemoryVfsProvider` | A1 | define the provider trait; implement an in-memory virtual filesystem provider for development and testing; wire into wasmtime linker as `nimbus:agent/*` bindings |
| A3 | `todo` | `ComponentWorld::NimbusAgent` admission gate | A1, A2 | extend `BundleContent::WasmComponent` with `target_world`; enforce capability check at deploy time; reject `NimbusAgent` bundles for tenants without agent capability |
...
| A6 | `todo` | End-to-end agent workflow validation | A2, A3, and at least one of A4 or A5 | full agent scenario: WASM component imports agent capabilities, executes filesystem and process operations, calls LLM via HTTP, returns result through Nimbus scheduling and admission |

## Phase Order and Dependencies

```text
Wasmtime plan W3 (done)
  └── A1 nimbus:agent WIT definitions
...
|------|-------|---------|---------|--------------|-----------|
| 2026-04-05 | meta | documented | Initial plan authored. Covers `nimbus:agent` WIT interfaces (filesystem, process, http-client), `AgentOsProvider` trait with three concrete providers (in-memory, agent-os sidecar, sandboxed OS), `ComponentWorld` admission gate, and integration path with rivet-dev/agent-os. Includes agent-os architecture review finding: agent-os value is the virtual kernel, not the compute layer. | document review against `ARCHITECTURE.md`, `wasmtime-backend-plan.md`, and agent-os source analysis | keep deferred until wasmtime plan W3 reaches `done` |
...
- `AgentOsSidecarProvider` integration tests (IPC round-trip, sidecar lifecycle,
  tenant scoping)
- `ComponentWorld::NimbusAgent` admission gate tests (deploy accepted with
...
- `nimbus-function` world isolation test (verify agent imports are unreachable)
...
- **`docs/plans/service-identity-provider-auth-plan.md`**: soft dependency for
  ...
  identity-bearing credential is minted from the admitted `WorkloadIdentity`
  subject and then consumed by the secret-management or provider adapter layer.
