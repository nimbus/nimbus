...
- No patched `neovex-crun` binary has been built or installed yet.
- No Debian/Fedora system-integration drill has been completed yet.
- A checked-in conmon lifecycle drill preparer now exists, but no supported
  Linux host has executed the generated `run-conmon.sh` path yet.
- A checked-in direct private-runtime drill preparer now exists, but no
  supported Linux host or Linux machine-VM guest has executed the generated
  `start-runtime.sh` path yet.
...
| --- | --- | --- | --- |
| `LH1` Host preflight | `todo` | Run `bash scripts/check-vmm-host.sh` and `bash scripts/collect-vmm-package-versions.sh`; confirm distro, `/dev/kvm`, package manager, runtime/build dependencies, and package versions | full command output, host OS/version, package/version inventory, failing checks if any |
| `LH2` Patch fidelity on the real Linux host | `todo` | Run `bash scripts/verify-crun-patch.sh ~/src/github.com/containers/crun`; confirm the pinned upstream checkout still matches expectations on that machine | source checkout path, tag/commit if checked, patch-apply output |
| `LH3` Build and install the private runtime | `todo` | Run `bash scripts/build-neovex-crun.sh --source ~/src/github.com/containers/crun --output <stage>` and then install to `/usr/libexec/neovex/crun` | staged binary path, install path, exact commands, `--version` output |
| `LH4` Prove system runtime separation | `todo` | Run `bash scripts/verify-runtime-separation.sh --system-runtime /usr/bin/crun --private-runtime /usr/libexec/neovex/crun`; confirm distro `crun` and Podman remain pointed at the system runtime while neovex uses `/usr/libexec/neovex/crun` | full helper output including system/private runtime paths, versions, Podman runtime path, realpath comparison, and final `result` |
| `LH5` Generate and boot the first real krun bundle | `todo` | Use `scripts/prepare-krun-bundle.sh` with a real mounted rootfs, then run the private runtime directly and prove host-to-guest connectivity over the mapped port | bundle dir/config path, rootfs path, `krun.port_map`, process args, exact run command, connectivity probe command and observed result |
| `LH6` Conmon lifecycle drill | `todo` | Run the first real `conmon -> /usr/libexec/neovex/crun -> guest` flow and prove logs, exit file, pid file, attach socket, process tree, and graceful stop behavior | exact conmon command, process-tree output, absolute log/exit/pid/socket paths, graceful-stop and exit-status output |
...
|-------|--------|-----------|-------|
| V1: Patch crun | `in_progress` | none | Patch artifact, Linux host probe, Linux command-bundle helper, build helper, bundle helper, direct-runtime drill helper, macOS diagnostics helper, macOS socket-path helper, macOS readiness helper, macOS recreate helper, focused verifiers, runbook, explicit macOS architecture/CLI naming guidance, and reduced-volume plus short-TMPDIR macOS evidence are checked in or recorded. The once-stale `users-only` machine has now been rebuilt successfully with the checked-in short-root recreate flow on this host. Linux build/install and runtime proof still pending |
| V2: System integration | `todo` | V1, system packages installed | Repo-owned conmon drill helper and focused verifier are checked in; supported-host execution and evidence are still pending |
