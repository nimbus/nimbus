...
| CO1 | todo | Split `crates/neovex-runtime/src/runtime/bootstrap.rs` into concept-owned runtime bootstrap modules | CO0 |
| CO2 | todo | Decompose `crates/neovex-storage/src/store.rs` around durable write, journal, recovery, and read-snapshot ownership | CO0 |
| CO3 | todo | Split `crates/neovex-engine/src/service/scheduler.rs` by grouped scheduler concepts and normalize wrapper patterns | CO0, CO2 |
| CO4 | todo | Decompose `crates/neovex-engine/src/tenant/subscription_delivery.rs` around queue state, worker lifecycle, stats, and testing seams | CO0 |
| CO5 | todo | Normalize the Convex host-bridge direct ctx-op surface to remove repeated sync, async, and cancellable wrapper logic | CO0, CO1, CO3 |
| CO6 | todo | Perform the concept-owned test-surface and idiomatic Rust cleanup sweep after the main production ownership boundaries stabilize | CO1, CO2, CO3, CO4, CO5 |
| CO7 | todo | Update docs and run the full verification sweep | CO1, CO2, CO3, CO4, CO5, CO6 |
...
| CO1 | not started | extract the runtime bootstrap ownership map and lock down targeted runtime coverage before moving high-risk host-call or snapshot code |
| CO2 | not started | split `store.rs` around `TenantWriteTransaction`, journal and recovery paths, read-snapshot and scan behavior, and low-level probing helpers without changing storage atomicity |
| CO3 | not started | separate scheduler service ownership for scheduled-job CRUD, cron CRUD, tenant-loading coordination, and async or cancellable wrappers once the storage seams settle |
| CO4 | not started | split subscription delivery ownership into queue state, worker thread lifecycle, stats accounting, and test pause support without changing shutdown or delivery ordering |
| CO5 | not started | normalize the server Convex direct ctx-op surface after runtime bootstrap and scheduler ownership stabilize, keeping runtime result encoding and auth behavior unchanged |
| CO6 | not started | move the highest-value clumped tests toward concept-owned module surfaces and perform the final naming, visibility, and helper-placement cleanup once the structural items land |
| CO7 | not started | confirm docs and plan indexes reflect the active workstream, then run the repo-wide verification sweep and record the closure state |
