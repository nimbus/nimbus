...
- `nimbus start &` then `nimbus ui` exits 0; reads
  `$XDG_RUNTIME_DIR/nimbus/server.json` and hands the URL to `open::that`
...
- After spawn, `cat $XDG_RUNTIME_DIR/nimbus/server.json | jq` shows valid
  `{port, ticket, pid}` shape
