...
| Complete `BJA5` HostBridge integration | `in_progress` | Bun/JSC code can perform explicitly granted HostBridge operations through the same service/engine path as V8. Tests cover allowed operation, denied operation, forged tenant/context rejection, and no raw host token exposure inside guest code. |
