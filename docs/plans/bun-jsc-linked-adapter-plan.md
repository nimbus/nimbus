...
| BJA4L | `in_progress` | Prove Linux in-process symbol isolation for Bun/JSC beside Deno/V8. | Debian 13 `minicloud` can run the linked pure invocation gate without `--allow-multiple-definition`, without duplicate `simdutf` linker errors, and without runtime crashes. Gate 41 selected an in-process dynamic-library adapter with hidden/local native symbols and an explicit Nimbus C ABI as the next feasibility and implementation lane; because current Bun/WebKit release objects are non-PIC, BJA4L2 must first prove a PIC-capable dynamic artifact. A Nimbus-owned Bun/WebKit namespace fork remains the fallback if dynamic PIC cost or final packaging breaks the product contract. Evidence must prove both the existing V8 lane and the Bun/JSC lane can coexist in the same product process shape. |
...
| BJA4L1 | `done` | Choose the least risky in-process isolation strategy. | Gate 41 compares namespacing or hiding `simdutf` in a Nimbus-owned Bun/WebKit fork, namespacing or hiding it in `nimbus/rusty_v8`, and an in-process dynamic-library Bun adapter with hidden/local native symbols plus an explicit Nimbus C ABI. The selected BJA4L2 proof path is the dynamic adapter if a PIC-capable Bun/WebKit artifact can be produced cleanly; the rollback trigger is unacceptable PIC build/source cost or failure to satisfy the final product packaging contract, at which point the fallback is a Nimbus-owned Bun/WebKit namespace fork. |
| BJA4L2 | `pending` | Implement the smallest source-owned isolation change. | First prove whether the Bun embedder can produce a PIC-capable dynamic artifact despite current non-PIC release objects. If dynamic remains viable, the adapter build exposes only the expected Nimbus Bun C ABI and hides WebKit/Bun native symbols. If not, implement the Bun/WebKit namespace fork fallback. The implementation is reproducible from a committed source revision or tag, not local edits. |
| BJA4L3 | `pending` | Harden the linked verification gate against unsafe symbol fixes. | `scripts/verify-bun-jsc-linked-adapter.sh` or its `make` wrapper rejects `--allow-multiple-definition`, records the selected source revision, audits exported symbols, and fails if unsafe duplicate symbol families are present. |
