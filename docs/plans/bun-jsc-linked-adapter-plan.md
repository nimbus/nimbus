...
| Run disk/cache verification preflight | `pending` | Before broad BJA8 gates, record local and `minicloud` free-space state, identify heavy Bun/Nimbus caches that are safe to reuse or prune, and ensure final verification uses canonical worktrees plus home-backed `minicloud` paths instead of new throwaway checkouts or `/tmp` growth. If the local Mac cannot run the source-backed linked gate because WebKit artifacts are intentionally absent, record that environment gate and keep the Debian source-backed linked proof as the required loaded-adapter evidence. |
