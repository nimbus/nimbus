...
| BJA4L2 | `pending` | Implement the smallest source-owned isolation change. | Implement source-owned Bun/WebKit-side namespace isolation for the colliding `simdutf::` family and Bun's `simdutf__` wrappers while preserving the static same-binary link. The implementation is reproducible from a committed source revision or tag, not local edits or binary-only post-processing. |
