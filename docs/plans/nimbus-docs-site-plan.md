...
- **DOC3 — Restructure `docs/` + IA shell + landing + migration map.** Make the
  ...
  that DOC4..DOC7 execute against. *Gate:* `docs/` top level is exactly the
...
3. Starlight content is loaded only from the five public groups
   `docs/{get-started,developers,operators,concepts,reference}/**` (explicit
   allow-list globs), with nothing authored under `website/src/content/docs/`
   except an index.
