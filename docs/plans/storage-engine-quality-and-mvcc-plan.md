...
- Historical reads must define authorization semantics. Current query
  ...
  current engine path; MVCC must explicitly choose and test policy-at-read,
...
- Historical index query-shape construction is centralized in
  `crates/nimbus-storage/src/index/history_scan.rs`. Backends own only physical
  row fetching/hydration, so equality, prefix, range, composite-range, cursor,
  and page-finalization semantics cannot drift separately across SQLite,
  Postgres, and MySQL.
- SQL-family production storage roots stay below the repo's 1,500-line review
  threshold. MySQL table-id catalog logic lives in
  `crates/nimbus-storage/src/mysql/table_catalog.rs`, SQL-family document/index
  filtering helpers live in `query_helpers.rs` modules, and Postgres schema
  event/cache shaping lives in
  `crates/nimbus-storage/src/postgres/write_schema_events.rs`.
