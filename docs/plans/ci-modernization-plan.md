...
| CM3 | Replace `runs-on: ubuntu-latest` with `runs-on: ubuntu-24.04` across all 22 sites that use it. ARM jobs (`ubuntu-24.04-arm`) already pinned. Verifier asserts zero `ubuntu-latest` references remain. | done |
| CM4 | Bump `actions/create-github-app-token@v3.2.0` to `@v3` so patch updates flow naturally. One-site change. Verifier asserts no patch-version pinning of `actions/*` actions. | done |
