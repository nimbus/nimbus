...
at rest across Nimbus-owned local persistence.
...
- `crates/nimbus-storage/src/async_storage/control.rs`
- `crates/nimbus-storage/src/store/write/store_entry.rs`
...
auditable, and aligned with real storage ownership boundaries. Nimbus now has
those boundaries:
...
- libsql replica mode materializes local SQLite cache files that Nimbus owns
- Postgres and MySQL tenant data live in external databases that Nimbus does
...
- when enabled, Nimbus encrypts the local files it owns
...
- `EmbeddedRedbControlPlaneProvider` still opens the cross-tenant
  `nimbus-control.db` usage store under `control_data_dir`.
...
- libsql replica config already has separate host-side routing state:
  `metadata_namespace` defaults to `nimbus_provider`,
...
- Postgres and MySQL providers keep tenant data outside Nimbus-owned local
  files.
- `crates/nimbus-storage/src/libsql.rs` is already near the repo's 1,500-line
  signal threshold, so `EAR5` cannot land as additional inline mixed ownership
  in that root.
- `crates/nimbus-engine/src/service/mod.rs`,
  `crates/nimbus-engine/src/persistence_config.rs`, and
  `crates/nimbus-bin/src/main.rs` are currently cohesive and should remain thin
...
| --- | --- | --- |
| Embedded SQLite tenant provider | Nimbus-owned `.sqlite3` tenant files | Nimbus encrypts |
| Embedded redb tenant provider | Nimbus-owned `.redb` tenant files | Nimbus encrypts |
| Control plane | Nimbus-owned `nimbus-control.db` redb file | Nimbus encrypts |
| libsql replica provider | Nimbus-owned local replica cache files | Nimbus encrypts local cache files; remote primary remains provider-managed |
| Postgres provider | No Nimbus-owned tenant data files | External provider responsibility |
| MySQL provider | No Nimbus-owned tenant data files | External provider responsibility |

This table is the core design rule for the entire plan: Nimbus encrypts the
...
- encrypted-by-default Nimbus-managed on-disk working artifacts that already
...
- Nimbus-managed at-rest encryption of external Postgres or MySQL data files
...
- in-memory snapshot/bootstrap structs or HTTP responses that are not persisted
  to disk by a Nimbus-owned workflow
- blanket claims of HIPAA, SOC2, ISO, or FIPS compliance

For external providers, Nimbus must document operator responsibilities and
...
5. Postgres and MySQL are explicitly treated as external-provider-managed for
   tenant data encryption, while Nimbus still encrypts any local control-plane
   files it owns.
6. One typed config surface governs local encryption across CLI, env, config
   file, and programmatic API use.
7. Every protected local database and every covered Nimbus-owned persisted
...
12. Current Nimbus-managed on-disk migration/rebuild/cutover artifacts emitted
...
13. Successful migration and rotation retire predecessor plaintext artifacts
    from active Nimbus-managed paths and surface `retirement_pending` until
    residue is cleared.
...
- Treat the concrete Nimbus-managed on-disk migration, rebuild, cutover, and
...
- Do not silently leave predecessor plaintext artifacts in active Nimbus-
...
- Do not treat in-memory structs or HTTP/JSON responses as at-rest artifacts
  unless a Nimbus-owned workflow persists them to disk.
...
- Keep existing composition roots thin. In particular,
  ...
  `crates/nimbus-storage/src/sqlite.rs`, and
  `crates/nimbus-storage/src/libsql.rs` should remain entry or provider roots,
...
- `EAR5` must not add another large mixed-owner layer inline to
  `crates/nimbus-storage/src/libsql.rs`; extract a `crates/nimbus-storage/src/libsql/`
...
- `cargo test -p nimbus-engine -p nimbus-server`
...
correct for Nimbus. The live architecture uses:
...
- how Nimbus encrypts retained redb files it owns
- how Nimbus encrypts embedded SQLite files it owns
- how Nimbus encrypts local libsql replica cache files it owns
- how Nimbus reports external-provider responsibility for tenant data it does
  not own

### Sane operator default

Nimbus needs two defaults, not one:
...
cipher choice. `master-key-file`, `key-dir`, and `aws-kms` decide how Nimbus
...
| Snapshot and bootstrap artifact encryption | Nimbus-owned on-disk exports, bootstraps, rebuilds, and recovery artifacts from encrypted local stores | **Encrypted by default** with per-artifact DEKs and the same envelope family | Prevents encrypted deployments from recreating plaintext secondary artifacts through backup and recovery flows |
| External tenant stores | Postgres, MySQL, and remote libsql/Turso primary storage | **Provider-managed at-rest encryption** | Operator/provider responsibility | Nimbus does not own those underlying data files, so a Nimbus-side file cipher would be fake coverage |
...
The wrapped-DEK envelope is not the place where Nimbus should express
...
consistent envelope shape unless a provider gives Nimbus a materially better
...
That split gives Nimbus a simpler and more trustworthy story:
...
- `crates/nimbus-storage/src/encryption/` should become the concept-owned home
...
clearest envelope for a Nimbus-owned design that binds key metadata and will
be reused across multiple storage families?"

AES-256-GCM-SIV was chosen because it balances the properties we care about:

- misuse-resistant AEAD semantics are valuable in a Nimbus-owned envelope
  format
...
| **AES-KW / AES-KWP** | NIST-approved key-wrapping modes explicitly designed for protecting keys | Great for pure key wrap, but not as natural when Nimbus also wants authenticated metadata binding; would likely push us toward extra envelope machinery around AAD |
...
If Nimbus later needs a stricter enterprise profile, the most credible upgrade
paths are:
...
- For **redb**, Nimbus owns the page layer, so misuse resistance is worth
  prioritizing.
- For **SQLite**, correctness and maturity of the pager integration matter
  more than chasing the newest primitive.
- For **libsql**, Nimbus should not fight the provider surface for a
...
incorrect offset math. Nimbus should use one adjacent authenticated sidecar
manifest per local database or persisted encrypted artifact instead:

`<protected-path>.nimbus-enc`
...
This gives Nimbus one consistent key-metadata story across redb, SQLCipher,
...
- Nimbus is writing a custom page-encryption layer here, so nonce-misuse
...
Nimbus should not market this as a FIPS feature. If a future deployment needs
...
Nimbus should not build its own SQLite pager encryption. The correct v1 choice
...
| --- | --- |
| SQLite SEE | proprietary license and distribution friction for public Nimbus binaries |
...
| Filesystem-only encryption | useful defense in depth, but not sufficient as Nimbus's product-level enterprise story |

For embedded SQLite, Nimbus will:
...
Before libsql cache encryption can land, Nimbus needs one explicit local-SQLite
...
For local libsql replica caches, Nimbus will:
...
because the replica cache files are Nimbus-owned local artifacts.
...
Nimbus does not own Postgres or MySQL data files. Therefore:

- Nimbus does not implement its own at-rest encryption layer for tenant data
  inside those providers
- operator and provider controls remain responsible for remote at-rest
  encryption
- local Nimbus-owned control-plane files remain in scope
- diagnostics must say `external-provider-managed` for tenant data encryption,
  not `disabled`

The same rule applies to the remote primary in libsql/Turso. Nimbus owns the
...
7. Extend the same envelope model to the concrete Nimbus-managed on-disk
...
- `crates/nimbus-storage/src/` (new key-management module tree)
- `crates/nimbus-storage/src/encryption/` (new concept-owned module tree for
...
- `crates/nimbus-storage/src/encrypted_redb.rs` (new)
...
- `nimbus-control.db` can run encrypted
...
2. Keep `crates/nimbus-storage/src/sqlite.rs` as a thin provider root by
...
3. Switch the embedded SQLite build to a SQLCipher-capable configuration once
   verified for Nimbus's release targets.
...
8. Prove the SQLCipher-capable build on every supported Nimbus release target
...
- `crates/nimbus-storage/src/sqlite.rs`
- `crates/nimbus-storage/src/sqlite/` (expanded concept-owned module tree for
...
- the SQLCipher-capable build is proven on Nimbus's supported release targets
...
**Priority:** highest after EAR1 and EAR2
**Expected impact:** closes the last Nimbus-owned SQLite file gap for the
...
2. Extract a dedicated `crates/nimbus-storage/src/libsql/` module tree or an
...
- `crates/nimbus-storage/Cargo.toml`
- `crates/nimbus-storage/src/libsql.rs`
- `crates/nimbus-storage/src/libsql/` (new concept-owned cache encryption,
...
2. Keep `crates/nimbus-engine/src/service/mod.rs` as a composition root by
...
- `crates/nimbus-engine/src/service/mod.rs`
- `crates/nimbus-engine/src/service/encryption/` (new startup validation and
...
- `crates/nimbus-server/src/http/encryption.rs` (new or equivalent focused
  status surface)
- `crates/nimbus/src/lib.rs`
...
1. Add an explicit admin CLI namespace for encryption operations. The exact
   command names should follow Nimbus's CLI taxonomy, but the required
   capabilities are:
   - inspect coverage
...
2. Keep `crates/nimbus-bin/src/main.rs` as an entrypoint by landing the admin
...
6. Artifact rules:
   - current Nimbus-managed on-disk migration/rebuild/cutover artifacts
...
7. Cutover and retirement rules:
   ...
   - predecessor plaintext databases, WAL/SHM sidecars, rollback journals,
     ...
     removed from active Nimbus-managed paths after cutover
...
- `crates/nimbus-bin/src/main.rs`
- `crates/nimbus-bin/src/encryption/` (new CLI module tree for inspect,
...
- `crates/nimbus-storage/src/` (new AWS KMS provider module)
- `crates/nimbus-engine/src/persistence_config.rs`
- `crates/nimbus-bin/src/main.rs`
- `docs/operating/cli.md`
...
| 2026-04-20 | EAR1 | done | Added typed local-encryption config surface. `LocalEncryptionConfig` with `Disabled` default and `Enabled(LocalKeyProviderConfig)` variants. `LocalKeyProviderConfig` supports `MasterKeyFile`, `KeyDirectory`, and `AwsKms`. Added CLI (`--encryption-key-provider`, `--encryption-master-key-file`, `--encryption-key-dir`, `--encryption-aws-kms-key-id`, `--encryption-aws-region`, `--encryption-aws-endpoint-url`), env (`NIMBUS_ENCRYPTION_*`), and config-file inputs with `CLI > env > file` precedence. Added provider-aware validation, diagnostics-safe descriptors (`KeyProviderDescriptor`, `EncryptionConfigDescriptor`), and `LocalPersistenceFamily` for coverage reporting. Added 8 focused tests for config parsing and validation. Verification: `cargo fmt --all --check`, `cargo clippy -p nimbus-engine -p nimbus-bin -p nimbus --all-targets -- -D warnings`, `cargo test -p nimbus-storage`, `cargo test -p nimbus-engine -p nimbus-server`, all passed. Next: EAR2. |
| 2026-04-20 | EAR2 | done | Added authenticated sidecar key manifests and local key providers. Created `crates/nimbus-storage/src/encryption/` module tree with: `LocalKeySubject` (subject model with derivation context and descriptors), `LocalKeySubjectKind` (`Database`/`Artifact`), `LocalDatabaseRole`, `LocalArtifactRole`, `WrappedDatabaseKey` (cipher + ciphertext), `GeneratedDatabaseKey` (plaintext + wrapped, zeroes on drop), `WrappingCipher::Aes256GcmSiv`, `LocalKeyProvider` trait, `LocalKeyProviderError`, `KeyProviderKind`, `MasterKeyFileProvider` (HKDF-SHA256 derivation + AES-256-GCM-SIV wrapping), `KeyDirectoryProvider` (per-subject key files), `KeyManifest`, `KeyManifestHeader` (with `to_aad()` for authenticated binding), `ManifestCipher`, manifest binary serialization with atomic write-to-temp-then-rename. Exported from `lib.rs`. Module-level integration tests verify manifest round-trip, provider integration, unique DEK generation, per-subject key derivation, cross-subject unwrap rejection, control plane subjects, artifact subjects, libsql cache subjects, and manifest tampering detection. Verification: `cargo fmt --all --check`, `cargo clippy -p nimbus-storage --all-targets -- -D warnings`, `cargo test -p nimbus-storage` (152 passed), `cargo test -p nimbus-engine -p nimbus-server` (177 passed). Next: EAR3 or EAR4. |
| 2026-04-20 | EAR3 | done | Implemented retained redb encrypted backend. Created `crates/nimbus-storage/src/encrypted_redb.rs` with: `EncryptedFileBackend` (file-backed encrypted storage), `EncryptedMemoryBackend` (in-memory encrypted storage for testing), per-page AES-256-GCM-SIV encryption with 12-byte random nonces and 16-byte tags, AAD binding (format version + page index + logical page size) to prevent page-swap attacks, logical-to-physical offset translation (4096-byte logical pages → 4124-byte physical pages), transparent read/write/set_len operations. Added `open_encrypted` and `create_in_memory_encrypted` methods to `TenantStore` and `UsageStore`. Tests verify: basic operations, cross-page reads/writes, wrong-key rejection, redb integration (create, write, reopen, verify), page-swap attack detection. Exported `EncryptedFileBackend`, `EncryptedMemoryBackend`, `ENCRYPTED_FORMAT_VERSION`, `LOGICAL_PAGE_SIZE`, `PHYSICAL_PAGE_SIZE` from `lib.rs`. Verification: `cargo fmt --all --check`, `cargo clippy -p nimbus-storage --all-targets -- -D warnings`, `cargo test -p nimbus-storage` (159 passed), `cargo test -p nimbus-engine -p nimbus-server` (177 passed). Next: EAR4. |
| 2026-04-20 | EAR4 | in_progress | Implemented the shared local-SQLite seam and SQLCipher runtime helpers. `rusqlite` now builds with `bundled-sqlcipher`, `crates/nimbus-storage/src/sqlite/encryption.rs` owns encrypted open/export/rekey helpers, and the embedded SQLite open path applies raw per-database DEKs from the manifest flow. Added `scripts/verify-sqlcipher-proof.sh` plus release-workflow and Linux-packaging hooks so the supported target matrix and tarball-consumer path exercise the SQLCipher helper lane, tenant-store lane, and packaged CLI migration/export flow. The helper now emits durable proof bundles, the hosted workflows upload them as `sqlcipher-proof-<target>` and `sqlcipher-package-proof-<arch>` artifacts, and `scripts/collect-sqlcipher-proof-bundles.sh` / `make collect-sqlcipher-proof-bundles RUN_ID=...` can download those artifacts into a closeout bundle. The remaining step is still the first green hosted multi-target run plus Linux packaging consumers, then linking those artifact names or collected bundle paths here before `EAR4` can close. |
