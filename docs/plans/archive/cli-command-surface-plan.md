...
Canonical execution plan for the next Neovex CLI surface wave: add a first-class
...
`neovex compose` using Docker-native command semantics.

This plan intentionally uses `neovex start` for the foreground server process
instead of `neovex serve`, `neovex run`, or `neovex agent`. `dev` owns the
...
Neovex gains true agent semantics.
...
- **Closeout:** all P1-P6 phases implemented; final verification completed
  ...
  `crates/neovex-bin/src/start/`. Keep this plan as the live CLI
...
Neovex now has a coherent first-party `codegen` command and a much cleaner
`--app-dir` contract. Before P1, the overall developer journey was still split
awkwardly between:

```bash
neovex codegen
neovex start --app-dir ./my-app
neovex service up
```
...
2. **There was no explicit push/update path for a running instance.**
   ...
   P5 adds the public `neovex deploy` CLI over it.
...
is to make Neovex feel unsurprising:

- **Convex migrants** should discover `neovex dev`, `neovex deploy`, and
  `neovex codegen` as the obvious path.
- **Docker migrants** should discover `neovex compose ...` with familiar
  subcommand names where semantics actually match.
- **Operators** should use `neovex start` for the long-running foreground
...
- `neovex codegen` already ships and is the first-party artifact-generation
  command.
- `neovex start --app-dir` already accepts both `neovex/` and `convex/`
  application roots and already performs a one-shot codegen preflight unless
  `--skip-codegen` is set.
- `neovex start` still owns the only server-start path. It is an explicit
  subcommand, not an implicit root action.
- P1 is implemented in the current worktree: the Compose-backed command surface
  is now `neovex compose config`, `up`, `down`, `ps`, `inspect`, `logs`, and
  `top`.
- P1 intentionally removed the `neovex service ...` command surface instead of
...
- P4 is implemented in the current worktree: the server exposes an
  ...
  user-facing `neovex deploy` CLI over that contract.
...
1. **Use `start` for the foreground server process.**
   ...
   reserve `agent` for a future clustered node role if Neovex actually gains
   one.
...
6. **Do not advertise routes or dashboards that do not exist.**
   ...
   promise a dashboard URL until Neovex actually ships one.
...
```text
neovex dev              # Local development loop
neovex deploy           # Push app artifacts to a running Neovex instance
neovex start            # Long-running server
neovex codegen          # One-shot artifact generation
neovex compose ...      # Compose-backed local service lifecycle
neovex machine ...      # Existing machine lifecycle namespace
neovex encryption ...   # Existing encryption admin namespace
```

### Explicit non-goals for this wave

- no `neovex agent` rename
- no implicit server start from the root command
- no fake Docker parity where Neovex semantics differ materially
...
```text
neovex agent ...        # If Neovex gains real node/cluster agent semantics
neovex service ...      # If Neovex gains cluster-scoped service management
```
...
**Audience:** developer iterating locally on a Neovex or Convex-compatible app.
...
- Neovex is self-hosted and local-first, so the command must also print the
  local URL clearly.

**Behavior**

- detects `neovex/` or `convex/` under the selected app directory
- performs one initial codegen pass unless `--skip-codegen` is set
- starts a local Neovex server with development defaults
...
| data root | `./.neovex/dev/` (shared v1 root for both tenant data and control state) |
| codegen | enabled |
...
| `--data-dir` | `./.neovex/dev/` | explicit override for the shared v1 dev persistence root |

**Terminal output contract**

- sync/progress/errors go to `stderr`
- function/runtime logs go to `stdout`
- the startup banner prints only values Neovex actually knows today

Example:

```text
  NEOVEX v0.1.0  ready in 342 ms

  Local:    http://localhost:3210/
  App dir:  /path/to/my-app

  Functions ready. Watching for changes in neovex/ ...
```

On change:

```text
  Preparing Neovex functions...
  10:42:15 Neovex functions ready (0.3s)
```

On error:

```text
  Failed to update Neovex functions
  Error: neovex/messages.ts(12): Type 'string' is not assignable to type 'number'
```
...
`dev` is the command that should feel familiar to Convex users. If Neovex later
...
**DX model:** Convex `deploy` for the user-facing workflow, but with Neovex's
...
```bash
neovex deploy --url http://my-neovex.example.com --app-dir ./my-app
```

or in CI:

```bash
NEOVEX_DEPLOY_URL=http://my-neovex.example.com \
neovex deploy --app-dir ./my-app
```
...
8. **Rollback**
   ...
   - an explicit rollback command stays out of scope until Neovex defines
...
| Flag | Default | Notes |
| --- | --- | --- |
| `--url` | `NEOVEX_DEPLOY_URL` or required | explicit target in v1 |
...
```text
  Preparing Neovex functions...
  Uploading app artifacts to http://my-neovex.example.com ...
  Validating schema and indexes...
  Deployed Neovex app to http://my-neovex.example.com
...
**Audience:** operator running a long-lived Neovex server in development,
staging, or production.

**DX model:** explicit foreground process startup, with the developer happy path
handled by `neovex dev`.
...
- `run` is better reserved for a future `neovex run <function>` command that
  mirrors Convex's function-invocation vocabulary
- `agent` would create more confusion than clarity until Neovex has real
...
- no legacy `neovex serve` alias is retained
...
dependencies for one Neovex project.

**DX model:** Docker Compose command naming, combined with the already-landed
Neovex output rules for action, summary, and inspect surfaces.
...
#### Rename: `neovex service` -> `neovex compose`
...
| --- | --- | --- |
| `neovex service config` | `neovex compose config` | `docker compose config` |
| `neovex service up` | `neovex compose up` | `docker compose up` |
| `neovex service down` | `neovex compose down` | `docker compose down` |
| `neovex service list` | `neovex compose ps` | `docker compose ps` |
| `neovex service logs` | `neovex compose logs` | `docker compose logs` |
| `neovex service ps` | `neovex compose top` | `docker compose top` |
| `neovex service inspect` | `neovex compose inspect` | explicit Neovex extension |

#### Why there is no `compose ls` in this wave

Docker's `compose ls` lists Compose projects, not services within one project.
Neovex does not yet expose a project-scoped multi-project listing surface for
...
That is already a better fit for current Neovex architecture than pretending we
...
only after Neovex has a real multi-stream log mux instead of a single-service
poll loop.
...
it should be treated as a follow-on slice only if Neovex has a real event stream
...
```bash
neovex dev
neovex deploy --url http://my-neovex.example.com
```
...
That is much easier to learn than teaching Docker users that Neovex `compose ls`
means status and Neovex `compose ps` means process snapshots.
...
- keep `inspect` as an explicit Neovex extension
...
- focused `cargo test -p neovex-bin service_ -- --nocapture` updated for new
  command names
- direct help proof for:
  - `neovex compose --help`
  - `neovex compose ps --help`
  - `neovex compose top --help`
  - `neovex compose logs --help`

**Verification completed in this worktree**

- `cargo fmt --all --check`
- `cargo check -p neovex-bin`
- `cargo test -p neovex-bin compose::tests::parse_help -- --nocapture`
- `cargo test -p neovex-bin compose::tests -- --nocapture`
- `cargo test -p neovex-bin`

**Additional closeout notes**

- Current reference docs now use `neovex compose ...` instead of the retired
  `neovex service ...` public command surface:
  `docs/README.md`, `docs/reference/microvm-service-baseline.md`, and
  `docs/reference/macos-machine-flow.md`.
- The CLI implementation module was also renamed from
  `crates/neovex-bin/src/service/` to `crates/neovex-bin/src/compose/` so the
  code layout matches the public command surface. The Compose-file parser and
  lowerer live under `crates/neovex-bin/src/compose/file/`, matching Docker's
  "Compose file" terminology while avoiding confusion with the
  `neovex compose config` command. Domain references to individual Compose
...
- `cargo test -p neovex-bin compose::tests -- --nocapture`
- `cargo test -p neovex-bin`
- `cargo run -p neovex-bin -- service --help` rejects the legacy namespace
- `cargo run -p neovex-bin -- compose --help` renders the Compose namespace

### P2: `neovex dev` bootstrap slice
...
- `neovex dev` auto-detects an app root by walking upward from the current
  directory for `neovex/`, `convex/`, or generated `.neovex/convex/`
...
- The default dev persistence root is the resolved app directory's
  `.neovex/dev/`, passed as both `data_dir` and `control_data_dir` for the
  shared v1 root. `--data-dir` explicitly overrides that shared root.
- `neovex dev` forces embedded SQLite tenant persistence for the local dev
...
- `cargo fmt --all --check`
- `cargo check -p neovex-bin`
- `cargo test -p neovex-bin dev::tests -- --nocapture`
- `cargo test -p neovex-bin`
- `cargo test -p neovex-server`
- `cargo test -p neovex-engine`
- `cargo run -p neovex-bin -- dev --help`

### P3: `neovex dev` watch loop

**Status:** implemented in the current worktree.

**Scope**

- file watching for `neovex/` or `convex/`
...
- Default `neovex dev` now detects the selected `neovex/` or `convex/` source
  ...
  as `_generated`, `node_modules`, `.neovex`, `.next`, `dist`, and `build` so
...
- After P4 introduced the generation-swap seam, watched codegen now packages
  ...
  API that `neovex deploy` will use. `neovex dev` injects an internal
...
- `cargo check -p neovex-bin`
- `cargo test -p neovex-bin dev::tests -- --nocapture`
- `cargo test -p neovex-bin deploy::tests -- --nocapture`
- `cargo test -p neovex-bin`
- `cargo test -p neovex-server`
- `cargo test -p neovex-engine`
- `cargo run -p neovex-bin -- dev --help`
...
- Added `POST /api/admin/deploy`, disabled unless the server starts with
  `NEOVEX_DEPLOY_TOKEN` or a local dev workflow supplies an internal token.
...
- `cargo test -p neovex-server deploy -- --nocapture`
- `cargo test -p neovex-server`
- `cargo test -p neovex-engine`
- `cargo test -p neovex-runtime`
- Exact reruns for transient parallel runtime failures:
  `cargo test -p neovex-runtime runtime::tests::cooperative::runtime_cooperative_locker_slot_completes_immediate_async_host_work_without_parking_subprocess -- --ignored --exact --nocapture`
  and
  `cargo test -p neovex-runtime runtime::tests::cooperative::warm_pool_cooperative_async_host_two_cycles_subprocess -- --ignored --exact --nocapture`
- `npm run docs:validate-refs:strict` was attempted, but this checkout has no
  such npm script.

### P5: `neovex deploy` CLI
...
- `neovex deploy` requires an explicit target URL through `--url` or
  `NEOVEX_DEPLOY_URL`, and requires deploy credentials through `--token` or
  `NEOVEX_DEPLOY_TOKEN`.
- The command auto-detects the app directory with the same `neovex/`,
  `convex/`, or generated `.neovex/convex/functions.json` ancestor rules used
  by `neovex dev`, with current-directory fallback.
- Deploy runs codegen first unless `--skip-codegen` is set, packages generated
  artifacts from `.neovex/convex/`, and calls `POST /api/admin/deploy`.
...
- `cargo test -p neovex-bin deploy::tests -- --nocapture`
- `cargo test -p neovex-bin`
- `cargo run -p neovex-bin -- deploy --help`
...
- Post-review naming cleanup replaced the public `neovex serve` command with
  `neovex start` without retaining a legacy alias.
- The CLI implementation module moved from `crates/neovex-bin/src/serve/` to
  `crates/neovex-bin/src/start/`, with `StartCommand` and
  `run_start_command` naming so the code layout matches the public surface.
- `neovex start` now emits a concise startup summary after the listener binds:
...
- `cargo fmt --all --check`
- `cargo check -p neovex-bin`
- `cargo test -p neovex-bin start::tests::start_startup_summary_mentions_url_app_codegen_and_deploy_api -- --nocapture`
- `cargo run -p neovex-bin -- serve --help` rejects the retired command
- `cargo run -p neovex-bin -- start --help`
...
- `neovex compose` is the public Compose-backed namespace; the legacy
  `neovex service` public surface is not retained.
- `neovex dev` now owns the Convex-migration local loop: app detection,
  project-local `.neovex/dev/` persistence, codegen, local server startup,
...
- `neovex deploy` packages generated artifacts, requires an explicit target and
  deploy token, supports `--dry-run`, and renders a clear human diff.
- `neovex start` keeps its existing contract while printing a concise startup
  summary with URL, app/codegen state, optional Compose file, and deploy-admin
  API status.
- The retired public `neovex serve` command is not retained as an alias.
...
- Final implementation review aligned helper names with the public CLI surface:
  ...
  service domain type.
- Root help now describes Neovex as a Convex-compatible reactive backend and
...
- `cargo fmt --all --check`
- `cargo check -p neovex-bin`
- `cargo test -p neovex-bin start::tests -- --nocapture`
- `cargo test -p neovex-bin dev::tests -- --nocapture`
- `cargo test -p neovex-bin`
- `cargo run -p neovex-bin -- start --help`
- `cargo run -p neovex-bin -- serve --help` rejects the retired command
- `cargo run -p neovex-bin -- --help`
...
- `cargo run -p neovex-bin -- --help`
- `cargo run -p neovex-bin -- start --help`
- `cargo run -p neovex-bin -- compose --help`
- `cargo run -p neovex-bin -- serve --help` rejects the retired command
- Archive closeout reran `cargo fmt --all --check`, `cargo check -p
  neovex-bin`, and `cargo test -p neovex-bin` after moving the plan and
  updating `AGENTS.md` / `docs/plans/README.md`.
- Current reference docs and active/future plans no longer present `neovex
  serve` or `neovex service` as current public commands; remaining hits are
  retired-command notes or labeled historical proof rows.
- `crates/neovex-bin/src/start/` no longer exposes stale
  `service_persistence_config_*` helper names.
- `crates/neovex-bin/src/compose/` command dispatch now uses
...
- The post-review `serve` -> `start` rename is covered by focused CLI gates and
  the full `neovex-bin` test crate. Workspace-wide `make ci` was not rerun
...
5. **`deploy` is explicit-target in v1.**
   Self-hosted Neovex should not pretend there is a cloud deployment identity
...
7. **Formatting is part of the deliverable, but semantics come first.**
   ...
   frame-for-frame.
8. **`dev` uses a project-local `./.neovex/dev/` root in v1.**
...
1. **`dev` defaults to a project-local `./.neovex/dev/` root.**
...
2. **The first `dev` milestone did not include live artifact activation.**
   ...
   `.neovex/convex/` at boot and did not expose a generation-swap seam, so P2
