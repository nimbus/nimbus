...
| 2026-05-31 | H7 | `done` | **Evidence rigor + doc accuracy.** Wired the exported-but-dead `validate_attribute_names` into the write-path `validate_item`, so a reserved attribute name (`_pk`/`_sk`/`_nimbus_*`) is rejected on PutItem/store_item (F12). Bench now asserts the **expected** status (`assert_eq!(status, 200, …)`) instead of `< 500`, which a modeled 4xx would silently pass (F15). Soak gained a read-back correctness invariant — a GetItem after each UpdateItem asserts the written value is read back, proving the workload mutates+persists state, not just returns 2xx (F15). Added an executable **ground-truth corpus** (`crates/nimbus-dynamodb/tests/ground_truth.rs`): canonical CreateTable/DescribeLimits/PutItem/GetItem/UpdateItem/Query/DeleteItem + an error-envelope golden, each diffed field-by-field against the DynamoDB Local / AWS API response contract through `dispatch` (no Docker), with the proof doc at `docs/plans/proof/dynamodb-adapter-hardening/ground-truth-corpus.md` (F14). Corrected DDB-DIV-002 ("planned" → the sortable-key encoding landed, points to the two existing range tests; F16) and the divergences "classifies any unrecorded difference" overclaim (now references the parity suite + ground-truth corpus as the executable checks; F14). | `cargo test -p nimbus-dynamodb` (255 pass: 243 unit + 7 + ground_truth 1 + soak 1 + 4 isolation); `dynamodb_spec` 27/27; fmt + clippy (all targets) clean; verifier `11 passed, 0 failed`. New tests: `validate_item_rejects_reserved_attribute_names`, `ground_truth_corpus_matches_the_dynamodb_contract`, soak read-back assertion. |
| 2026-05-31 | H6 | `done` | **Query sparse-index robustness for non-scalar keys.** The Query partition loop now encodes the *client* PK value once (a non-scalar query value is still a propagated `ValidationException`) and **skips** any item whose partition-key attribute is non-scalar (M/L/BOOL/NULL) or absent — `let Ok(item_key) = sortable_key(item_pk) else { continue }` — instead of aborting the whole request on the un-encodable value; `compare` (sort-key path) likewise skips an item whose stored sort key is non-scalar while still propagating a non-scalar *client bound*. Removed the now-unused `sortable_eq` (its `?` was the F7 abort). This matches the existing absent-attribute sparse handling and `sort_cmp`'s swallow, so the three paths are now consistent. | `cargo test -p nimbus-dynamodb` (254 pass: 242 unit + 7 + 1 + 4); `dynamodb_spec` 27/27; fmt + clippy clean; verifier HC8 green (retargeted to the non-scalar marker). New test: `query_gsi_skips_non_scalar_and_absent_index_keys` (GSI Query over items with the index key as Map/List/absent returns only the scalar match). |
