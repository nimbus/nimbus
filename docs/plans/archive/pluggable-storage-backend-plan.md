...
| SB14 | done; reran the final repo-wide verification sweep from the settled provider-seam state, then archived this completed migration plan and updated the repo entry docs so future work starts from the right follow-on plans instead of this finished control plane | archived complete; future provider-topology work belongs to `docs/plans/external-sql-storage-backends-plan.md`, and retained redb encryption work remains in `docs/plans/encryption-at-rest-plan.md` |
...
| 2026-04-08 | SB9 | in_progress | Started the benchmark-gate slice by adding a checked-in redb-vs-SQLite harness in `crates/neovex-engine/examples/storage_backend_benchmarks.rs` and a matching `make bench-storage-backends` entrypoint. The harness compiles after a focused `cargo check`, and it is designed to write the checked-in markdown report directly, but the first release-mode benchmark run was interrupted cleanly when the user asked to pause before moving on to `SB10`, so no benchmark numbers are claimed yet. | `cargo check -p neovex-engine --example storage_backend_benchmarks`; `cargo fmt --all` | rerun `make bench-storage-backends REPORT=docs/research/sqlite-storage-benchmark-report.md`, inspect the generated results, and only then record the benchmark gate decision |
...
| 2026-04-09 | SB14 | done | Completed the final closure sweep from the settled provider-seam state. `make ci` passed after rerunning it with elevated filesystem access so `cargo deny` could take its advisory-db lock outside the workspace, and the repo entry docs now point future work at the right active/deferred plans instead of this completed migration control plane. | `make ci` (rerun with elevated filesystem access for the `cargo deny` advisory-db lock) | archived complete; use `docs/plans/external-sql-storage-backends-plan.md` for future provider-topology work and `docs/plans/encryption-at-rest-plan.md` for retained redb encryption work |
