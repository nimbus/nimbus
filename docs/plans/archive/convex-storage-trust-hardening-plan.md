...
| 2026-05-29 | CST6 | `done` | Corroboration (deep research, no code change). Confirmed Convex's **production** backend uses the same single shared, multi-versioned `documents` log keyed by `(ts, table_id, id)` — `table_id` is a column, never a per-user-table DDL — verified three ways: OSS source (`/Users/jack/src/github.com/get-convex/convex-backend` `crates/{sqlite,postgres,mysql}/src/...`, one `documents` + one `indexes` table behind a pluggable `Persistence` trait), Convex's own design writeup, and an adversarial refutation pass that found no per-table tables and no proprietary divergent store. The OSS backend is "exactly the same fully up-to-date code used by the cloud service"; production migrated AWS Aurora MySQL → PlanetScale for Postgres on 2025-07-01 with the single-table schema unchanged. This validates the CST6 `intentionally_latest_row` decision as a deliberate divergence on **versioning** (Convex = full MVCC log; Nimbus = latest-row + durable commit log), not on the shared-`documents(table_id, id)` **layout**, which Nimbus matches. Sources: `https://stack.convex.dev/how-convex-works`; `https://news.convex.dev/powered-by-planetscale-for-postgres/` (2025-07-01); `https://news.convex.dev/convex-goes-open-source/` (2024-03-12). |
