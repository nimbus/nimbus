...
- `crates/neovex-storage/src/query_read.rs`
...
- `crates/neovex-storage/src/query_read.rs` is still dominated by repetitive
...
| --- | --- | --- |
| SH1 | Complete. `crates/neovex-storage/src/query_read.rs` now uses a single macro-backed implementation instead of eight hand-written forwarding impls, with the trait surface and call graph unchanged. | move to `SH2` and make the embedded SQLite read-pool budget explicit |
...
- Replace the eight hand-written forwarding impls in
  `crates/neovex-storage/src/query_read.rs` with a macro-backed implementation.
