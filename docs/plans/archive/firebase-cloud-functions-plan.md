...
Canonical execution plan for Cloud Functions-compatible compute on Nimbus:
...
Nimbus-hosted HTTP dispatch for the covered handler shapes.
...
shared behavior into a protocol-neutral Nimbus primitive before adding
...
handler source to Nimbus without rewriting imports or handler bodies. A
...
Cloud Functions authoring compatibility on Nimbus; it does not claim first-
...
Cloud Functions compatibility, Nimbus asks migrating developers to "figure out
...
- **V8 runtime** (`nimbus-runtime`): executes JavaScript with host access via
...
- **Deploy admin API and generation activation**: Nimbus already stages,
...
|-----------|----------|-----------|
| `CommitEntry` plus per-write previous/current documents | `nimbus-core`, engine mutation path | Authoritative committed document change source |
| Durable mutation journal | `nimbus-engine`, `nimbus-storage` | Canonical replay/recovery substrate for at-least-once trigger delivery |
| Resource-path metadata | Firebase adapter F0.2/F3.2 work | Resolves Firestore document resources from committed writes |
| V8 runtime + `HostBridge` | `nimbus-runtime` | Executes JS with host database access |
| `ConvexHostBridge` | `nimbus-server/src/adapters/convex/host_bridge/` | Per-invocation V8 execution with mutation unit |
| Atomic write batch | `nimbus-engine` (F0.3) | Triggered functions commit atomically |
...
| Subscription diff helper | `nimbus-core/src/subscription.rs` | Useful secondary helper for tests/comparisons, but not the authoritative trigger event source |
...
- A Nimbus-owned internal artifact layout for Firebase apps, parallel to the
  Convex `.nimbus/convex/` pattern, so generated outputs stay out of the user
...
- **App-discovery and project-layout compatibility:** Nimbus should follow
  ...
  an explicit override for unusual repos. For Firebase projects, Nimbus must
  ...
  instead of requiring a Nimbus-specific source-root rename.
- **Exact source compatibility:** for covered surfaces, existing handler
  modules keep the same import specifiers and handler bodies. Nimbus must
  ...
  migration.
- **Deploy compatibility:** Nimbus provides its own deploy and binding
...
- **Runtime compatibility:** Nimbus hosts execution inside its own runtime and
  HTTP server. First release covers execution of covered handlers on Nimbus,
...
- **The first slice matches Firebase document trigger scope.** Trigger patterns
  ...
  matching or other non-Firebase trigger shapes stay deferred unless Nimbus
  intentionally adds them as a documented extension.
- **CloudEvent types and payload shapes match the GCP standard.** Nimbus
...
- **Functions Framework compatibility is authoring/deploy compatibility first,
  not full standalone server parity.** The first release targets Nimbus-hosted
...
- **HTTP and callable option parity is explicit, not implied.** `onRequest`
  ...
  `CallableOptions` fields must fail validation; Nimbus must not silently
...
- **Existing app structure is a compatibility input.** The first slice must
  ...
  code into a Nimbus-specific source tree.
- **Most migrations should not need `--app-dir`.** If a modern Firebase app
  ...
  expected config and source layout, running Nimbus from that repo or package
...
- **Covered `firebase-admin` imports are first-class.** Modern Firebase
  ...
  `firebase-admin/*`. If those imports still require body rewrites, Nimbus
...
### Nimbus Core / Engine Owns (Shared Primitive)

The trigger registry and event dispatch are **not Firebase-adapter-local**. They
are shared Nimbus primitives that both adapters could use:
...
- **Internal artifact layout:** a Nimbus-owned generated output root for
  Firebase apps, analogous to `.nimbus/convex/`, so codegen and deploy
...
- **Import-resolution contract:** exact source compatibility for
  ...
  Nimbus-owned build or deploy alias layer that preserves source imports
  without rewrites.
...
- **Runtime compatibility boundary:** first release covers Nimbus-hosted
...
|---------|-----------------|-------|
| Auto-detected Firebase project root | `firebase.json` | Running Nimbus from a Firebase repo or a child path should discover the nearest compatible Firebase app root without requiring `--app-dir` |
| Auto-detected standalone Cloud Functions package root | `package.json` + framework dependency | Running Nimbus from a conventional Functions Framework package should discover that package root without requiring `--app-dir` |
...
| Nimbus-generated artifact root | `.nimbus/firebase/` (planned) | Generated manifests and bundles stay in a Nimbus-owned internal directory, parallel to `.nimbus/convex/` |
...
2. **Deploy-time binding compatibility:** Nimbus bind manifests/config attach
...
| `setGlobalOptions()` root defaults | `firebase-functions/v2` | In scope only for published `GlobalOptions` fields that the covered surfaces inherit under Nimbus |
...
| API | Source package | Notes |
|-----|---------------|-------|
| `functions.http(name, handler)` | `@google-cloud/functions-framework` | Exact-source-compatible named HTTP target; served through Nimbus HTTP routing, not the standalone framework dev server |
...
- **Storage triggers** (`onObjectFinalized`, etc.): requires storage event
  pipeline.
- **Pub/Sub triggers:** requires event source outside Nimbus's current scope.
- **Firebase Extensions:** out of scope.
- **Firebase Emulator Suite compatibility:** out of scope for first release.
- **`gcloud functions deploy` CLI compatibility:** Nimbus provides its own
...
Location: `nimbus-core/src/trigger.rs`, `nimbus-engine/src/triggers/`,
...
- Decide the generalized runtime artifact, import-resolution, and deploy
  ...
  includes whether Nimbus generalizes the existing deploy/admin/runtime
  ...
  covered `firebase-admin` surfaces resolve under Nimbus.
- Define the shared app-root and internal-artifact contract: how an explicit
  ...
  ambiguity resolution, and where Nimbus writes generated manifests or bundles
  (planned `.nimbus/firebase/`) without requiring user source-tree renames.
...
- Preserve unchanged source imports through a **Nimbus-owned deploy/build alias
...
- HTTP bindings must use `http` signature type, request-scoped execution, and
  explicit Nimbus-hosted path exposure.
...
Location: `nimbus-engine/src/triggers/dispatch.rs`,
`nimbus-engine/src/triggers/execution.rs`.
...
Location: `crates/nimbus-server/src/adapters/firebase/triggers/`,
`crates/nimbus-server/src/adapters/cloud_functions/`, the chosen generalized
...
- Add exact-source-compatible authoring surfaces for both packages. For
  ...
  Nimbus's chosen package or alias strategy. If a surface cannot preserve
...
- Add shared app-root discovery and codebase handling: auto-detect the nearest
  ...
  artifacts under the Nimbus-owned Firebase artifact root. `--app-dir`
...
- Add HTTP handler dispatch for `functions.http(name, handler)` and
  ...
  Express-style `(req, res)` semantics. This is Nimbus-hosted handler
...
**R9: App-shape drift versus Convex-style ergonomics (Medium, T0-T2).** Nimbus
...
`codebase` handling, and `.nimbus/firebase/` internal artifact ownership as
...
- `firebase deploy` / `gcloud functions deploy` CLI compatibility — Nimbus
...
| T2: Authoring surfaces and deploy integration | `done` | 8-11 context windows | T1 is `done`; relevant package/deploy scaffolding is chosen | Both `onDocumentCreated` (Firebase) and `functions.cloudEvent()` (framework) work against local Nimbus through the shared trigger/deploy path with exact import compatibility for covered surfaces, shared auto-discovery plus `--app-dir` override behavior for common app layouts, preserved Firebase project structure, and the covered `firebase-admin` subset |
| T3: HTTP handler support | `done` | 4-5 context windows | T2 deploy contract is `done` | `functions.http()`, `onRequest()`, and `onCall()` work against local Nimbus through the shared runtime/deploy path, and supported option fields are documented explicitly |
...
| T0.8 App-root discovery, project-layout, and server-SDK contract | `done` | T0.4, T0.6 | The plan explicitly decides the shared app-root resolver contract: explicit `--app-dir` override, cwd/parent auto-discovery for existing Firebase project roots and common standalone Functions Framework package roots, `firebase.json` `functions.source` / `codebase` layouts, the `.nimbus/firebase/` artifact root, and which `firebase-admin/app` and `firebase-admin/firestore` imports or methods are covered in the first slice. | Design proof plus validation tests for Firebase and standalone project discovery, ambiguity handling, codebase mapping, internal artifact layout, covered admin-import resolution, and unsupported-admin-method rejection. |
...
| Item | Status | Hard deps | Completion gate | Verification evidence |
|------|--------|-----------|-----------------|-----------------------|
| T3.1 HTTP handler dispatch | `done` | T2.1, T2.5 | `functions.http(name, handler)` routes incoming HTTP requests to the registered handler through V8 with Express-style `(req, res)` semantics under Nimbus-hosted routing. Full standalone Functions Framework local server parity remains explicitly deferred. | Handler tests for request routing, response writing, error handling, concurrent requests, unsupported runtime-parity features failing clearly, and generated bundle selftests for req/res materialization. |
...
| 2026-04-25 | T0.4 Generalized runtime artifact, import-resolution, and deploy contract | `done` | Chose a sibling `.nimbus/firebase/` artifact family instead of forcing the current Convex deploy manifest into a fake generic schema. The first-slice Cloud Functions contract now fixes `artifact.json`, `bundle.mjs`, `bundle.sha256`, and `targets.json` under that internal root, and records `deploy_alias_layer` as the exact import-resolution strategy for covered `firebase-functions/v2`, `@google-cloud/functions-framework`, and `firebase-admin` specifiers. | `cargo test -p nimbus-server cloud_functions::tests --lib`; `cargo fmt --all`; `cargo fmt --all --check`; `npm run docs:validate-refs:strict` (unavailable in this checkout: missing script). |
| 2026-04-25 | T0.4 Generalized runtime artifact, import-resolution, and deploy contract | `in_progress` | Started the deploy/runtime contract pass. Source review confirmed the current staging and activation path is still deeply Convex-shaped: deploy uploads stage only `.nimbus/convex/*`, the loader is `ConvexRegistry::from_app_dir(...)`, and the manifest schema is specific to Convex function plans and HTTP routes. The working direction for this item is to keep the deploy/admin guarantees shared (staging, integrity validation, dry-run diffing, atomic generation activation) but add a sibling Cloud Functions artifact family under `.nimbus/firebase/` rather than force the current `ConvexRegistry` schema into a fake generic shape. Next action: codify that sibling artifact contract, the exact covered import-resolution strategy, and the future target-manifest hook in code and docs. | Source review of `crates/nimbus-server/src/http/deploy.rs`, `crates/nimbus-server/src/adapters/convex/{mod.rs,manifest.rs,registry/{mod.rs,loading.rs,deploy_summary.rs}}`, `packages/codegen/src/main.mjs`, and `docs/reference/deploy-admin-api.md`. |
| 2026-04-25 | T0.3 Durable cursor / invocation ledger model | `done` | Chose a hybrid durable-delivery contract that treats the invocation ledger as authoritative and the journal sequence cursor as materialization progress only. The shared trigger primitive now models a monotonic per-tenant `TriggerDeliveryCursor` plus durable `TriggerInvocationRecord` entries keyed by `(registration_id, cloud_event.id)` with explicit `Pending`, `Running`, `RetryPending`, `Completed`, and `TerminalFailure` states. This lets future dispatch work advance the journal cursor once matched invocations are durably recorded, while retries and completion survive restart independently per handler invocation. I stopped before `T0.4` because the next item is a broader cross-surface design boundary covering deploy artifacts, import resolution, and generalized runtime activation. Next action: inspect the current Convex-shaped deploy/admin/runtime registry path, package alias surfaces, and handler discovery contract to decide whether Nimbus should generalize that path or add a sibling Cloud Functions artifact pipeline. | `cargo test -p nimbus-core trigger --lib`; `cargo fmt --all`; `cargo fmt --all --check`; `make clippy` (still fails on pre-existing Firebase gRPC lint debt in `crates/nimbus-server/src/adapters/firebase/grpc/{listen_stream,unary,write_stream,mod,listen_websocket}.rs`, not on the new trigger primitives). |
| 2026-04-25 | T0.3 Durable cursor / invocation ledger model | `in_progress` | Started the durable-delivery design pass. Initial engine/storage review shows the existing durable journal already provides the authoritative per-tenant sequence cursor, but cursor-only replay would not capture independent retry/completion state once a single commit fans out to multiple trigger matches. The working direction for this item is therefore an invocation-ledger model with a journal-backed materialization cursor: persist one durable record per matched handler invocation, keep retry/completion state on those records, and use the cursor only to track how far committed writes have been expanded into the ledger. Next action: land the shared ledger state machine and persistence-roundtrip tests in `nimbus-core`, then record the choice in this plan before touching dispatch/storage code. | Startup refresh of `AGENTS.md`, `README.md`, `ARCHITECTURE.md`, `docs/README.md`, `docs/plans/README.md`, and `docs/plans/firebase-cloud-functions-plan.md`; `git status --short`; source review of `crates/nimbus-engine/src/persistence/tenant/{provider_state,journal}.rs`, `crates/nimbus-engine/src/service/mutations/journal.rs`, and `crates/nimbus-storage/src/store/{read,journal,journal_snapshot}.rs`. |
| 2026-04-25 | T0.2 CloudEvent envelope, Firestore event types, and service-principal contract | `done` | Added a shared trigger-event model in `nimbus-core` instead of letting Firebase or Functions Framework adapters invent their own event payloads first. The new `trigger.rs` module now defines standard Firestore CloudEvent type strings, fixed `specversion: "1.0"`, protocol-neutral `DocumentEventData` / update-mask shapes, Firestore metadata with captured params, commit metadata, and an explicit service-principal execution contract. I stopped before `T0.3` because the durable cursor versus invocation-ledger choice is the next broader engine/storage design boundary. Next action: inspect the existing durable journal progress and invocation-state seams in `nimbus-engine` / `nimbus-storage`, then choose and document the persisted trigger cursor or invocation-ledger model before touching dispatch code. | `cargo test -p nimbus-core trigger --lib`; `cargo fmt --all`; `cargo fmt --all --check`; `make clippy` (failed on pre-existing Firebase gRPC lint debt in `crates/nimbus-server/src/adapters/firebase/grpc/{listen_stream,unary,write_stream,mod}.rs`, not in the new core trigger primitives). |
| 2026-04-25 | T0.2 CloudEvent envelope, Firestore event types, and service-principal contract | `in_progress` | Closed `T0.1` after landing a shared `DocumentTriggerPattern` in `nimbus-core` on top of the existing Firestore `DocumentPath` model. The new primitive matches only document-terminal paths, supports wildcard captures on alternating collection/document segments, rejects collection-terminal shapes, and produces deterministic wildcard params for later trigger dispatch. The next slice is the shared CloudEvent/event-payload model that both Firebase and Functions Framework adapters will consume. | `cargo test -p nimbus-core resource_path --lib`; `cargo fmt --all`; `cargo fmt --all --check`. |
| 2026-04-25 | T0.1 Document-path trigger semantics and pattern model | `done` | Added a protocol-neutral document trigger path primitive to `nimbus-core` by extending the path identity module rather than building adapter-local matching. `DocumentTriggerPattern` now parses Firestore-style document-terminal trigger patterns, supports wildcard captures on both collection and document segments, rejects collection-terminal shapes and duplicate wildcard names, and matches against the shared `DocumentPath` resource model with deterministic captured params. | `cargo test -p nimbus-core resource_path --lib`; `cargo fmt --all`; `cargo fmt --all --check`. |
| 2026-04-25 | T0.1 Document-path trigger semantics and pattern model | `in_progress` | Activated the Cloud Functions follow-on plan immediately after closing the Firebase adapter plan. The first slice is the shared document-trigger path primitive in `nimbus-core`: define Firestore-style document-terminal trigger patterns, support wildcard captures on alternating collection/document segments, reject collection-terminal shapes, and expose deterministic path-param extraction without pushing any protocol concerns into the type. | Startup refresh of `AGENTS.md`, `README.md`, `ARCHITECTURE.md`, `docs/README.md`, `docs/plans/README.md`, and `docs/plans/firebase-cloud-functions-plan.md`; `git status --short`; source review of `crates/nimbus-core/src/resource_path.rs` and the Cloud Functions roadmap/status ledger. |
