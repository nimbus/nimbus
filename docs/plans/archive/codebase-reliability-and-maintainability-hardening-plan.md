...
- `crates/neovex-runtime/src/runtime/tests/support.rs`
- `crates/neovex-runtime/src/runtime/tests/cooperative.rs`
- `crates/neovex-engine/src/tests/provider_fixtures.rs`
- `crates/neovex-engine/src/tests/mutation_journal/queued.rs`
...
- `crates/neovex-storage/src/tests/sqlite_foundation.rs`
...
- The largest active Rust code files now sit below the hard threshold:
  - `crates/neovex-storage/src/libsql.rs` at 1,471 lines
  - `crates/neovex-storage/src/mysql.rs` at 1,445 lines
  - `crates/neovex-engine/src/tests/materialized_serving.rs` at 1,389 lines
  - `crates/neovex-engine/src/tests/postgres_provider.rs` at 1,358 lines
  - `crates/neovex-sandbox/src/backends/container/runtime.rs` at 1,328 lines
  - `crates/neovex-storage/src/tests/sqlite_foundation.rs` at 1,320 lines
  - `crates/neovex-storage/src/postgres.rs` at 1,287 lines
  - `crates/neovex-engine/src/tests/subscriptions.rs` at 1,202 lines
...
- `crates/neovex-storage/src/libsql.rs` at 1,471 lines,
  ...
  `crates/neovex-storage/src/postgres.rs` at 1,287 lines are notable but
...
- `cargo test -p neovex-engine postgres_provider`
- `cargo test -p neovex-engine subscriptions`
- `cargo test -p neovex-engine materialized_serving`
- `cargo test -p neovex-storage sqlite_foundation`
...
| RMH3 | `done` | split `crates/neovex-engine/src/tests/postgres_provider.rs` into scenario-owned provider regression modules with a thin root and local fixture support seam | RMH1, RMH2 | completed on 2026-04-19 after regrouping the provider proofs into lifecycle, CRUD, journal, and scheduler modules while preserving external-provider semantics |
| RMH4 | `done` | split `crates/neovex-engine/src/tests/subscriptions.rs` and `materialized_serving.rs` into scenario-owned engine proof modules | RMH1, RMH2 | completed on 2026-04-19 after extracting behavior-owned module trees for cache, filtering, lifecycle, journal, retention, concurrency, reuse, and eviction semantics |
| RMH5 | `done` | split `crates/neovex-storage/src/tests/sqlite_foundation.rs` into scenario-owned storage proof modules with a small local support seam | RMH1, RMH2 | completed on 2026-04-19 after regrouping WAL, cancellation, schema, journal, scheduler, and snapshot proofs without changing SQLite foundation semantics |
| RMH6 | `done` | extracted the inline tests from `crates/neovex-sandbox/src/backends/container/runtime.rs` into a sibling proof surface with planning, lifecycle, and support ownership | RMH1 | completed on 2026-04-19 after moving the container runtime proof surface beside the root without changing backend behavior |
...
| --- | --- | --- |
| RMH0 | done | start `RMH1` by inventorying the existing wait-budget, eventual-assertion, fault-gate, and CI-duration helpers across runtime, engine, storage, sandbox, and `neovex-testing`, then choose the smallest canonical shared seam that improves consistency without forcing artificial coupling |
| RMH1 | done | extended `neovex-testing` with `ci_or_local_duration`, `duration_ms_env_or`, and `usize_env_or`, improved eventual-assertion timeout diagnostics, aligned runtime-local timing helper names with the shared contract, replaced the engine’s bespoke external-provider timeout helper with the canonical shared helper, and updated `docs/reference/verification-architecture.md` so the landed ownership map matches the code. |
...
| RMH3 | done | replaced the monolithic `crates/neovex-engine/src/tests/postgres_provider.rs` proof surface with a thin root plus local `lifecycle`, `crud`, `journal`, `scheduler`, and `support` modules, preserving the existing scenarios while making provider fixture ownership and scenario boundaries explicit. |
...
| RMH5 | done | replaced the monolithic `crates/neovex-storage/src/tests/sqlite_foundation.rs` proof surface with a thin root plus `foundation`, `cancellation`, `schema`, `journal`, `scheduler`, `snapshot`, and `support` modules so each storage invariant lives beside the scenarios that prove it. |
...
1. Inventory the existing helper seams for waits, eventual assertions,
   ...
   across runtime, engine, storage, sandbox, and `neovex-testing`.
...
2. Replace the flat
   `crates/neovex-engine/src/tests/postgres_provider.rs` proof surface with a
...
1. Keep `crates/neovex-sandbox/src/backends/container/runtime.rs` as the
