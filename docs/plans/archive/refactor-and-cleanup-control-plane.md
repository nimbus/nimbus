...
refactor and cleanup pass across the current Nimbus engine, server, and runtime
hot spots.
...
- `crates/nimbus-server/src/owned_tasks.rs`
...
Nimbus's crate-level architecture is still sound, but the current codebase has
accumulated several large multi-responsibility files and cross-cutting seams
that now slow down safe iteration:

- `crates/nimbus-engine/src/service/queries.rs` is 3073 lines
- `crates/nimbus-engine/src/tenant.rs` is 2982 lines
- `crates/nimbus-engine/src/service/mutations.rs` is 1040 lines
- `crates/nimbus-engine/src/subscriptions.rs` is 632 lines
...
| --- | --- | --- |
| RC0 | done; mapped existing coverage across engine, server, runtime, and storage hot paths and captured reusable targeted verification hooks for the upcoming refactor items; confirmed native `/ws` disconnect and bootstrap cleanup were already covered in `crates/nimbus-server/tests/reactive_loop/socket/subscriptions.rs` | start `RC1` by extracting tenant-local subsystems behind the current `TenantRuntime` facade without changing diagnostics or lifecycle behavior |
...
- `RC1` tenant subsystem extraction:
  `cargo test -p nimbus-engine service_reload_recovers_durable_journal_before_serving_async_reads`;
  `cargo test -p nimbus-engine shadow_materializer_queries_match_live_service_path`;
  `cargo test -p nimbus-server tenant_engine_metrics_route_surfaces_worker_and_serving_health_after_mixed_traffic`
- `RC2` query-module split:
  `cargo test -p nimbus-engine query_uses_three_field_composite_range_index_through_planner`;
  `cargo test -p nimbus-engine query_planning_stats_distinguish_composite_single_field_and_fallback_paths`;
  `cargo test -p nimbus-engine service_read_policy_filters_full_scans_pagination_and_subscription_results`;
  `cargo test -p nimbus-engine async_subscription_bootstrap_catches_up_writes_committed_before_activation`;
  `cargo test -p nimbus-engine sync_subscription_bootstrap_does_not_miss_lagged_applied_commit`
- `RC3` mutation-module split:
  `cargo test -p nimbus-engine mutation_async_cancellable_before_commit_rolls_back_document_index_and_durable_journal`;
  `cargo test -p nimbus-engine mutation_async_cancellable_after_commit_returns_committed_result`;
  `cargo test -p nimbus-engine mutation_journal_returns_only_after_apply_visibility`;
  `cargo test -p nimbus-engine mutation_admission_gate_buffers_while_journal_is_paused_without_losing_in_flight_response`;
  `cargo test -p nimbus-engine subscription_updates_publish_only_after_journal_apply`
- `RC4` subscription bootstrap and task ownership:
  `cargo test -p nimbus-server websocket_disconnect_drops_subscription_without_explicit_unsubscribe`;
  `cargo test -p nimbus-server websocket_disconnect_before_bootstrap_activation_cancels_pending_subscription_and_reconnects_cleanly`;
  `cargo test -p nimbus-server websocket_unsubscribe_during_bootstrap_activation_keeps_subscription_gone`;
  `cargo test -p nimbus-server convex_websocket_disconnect_releases_runtime_subscription_children`;
  `cargo test -p nimbus-server convex_websocket_auth_change_drops_active_subscriptions_until_resubscribed`
- `RC5` runtime host-dispatch cleanup:
  `cargo test -p nimbus-server runtime_host_request_rejects_unknown_operation_names_during_deserialization`;
  `cargo test -p nimbus-server runtime_cancellable_query_builder_start_short_circuits_before_dispatch`;
  `cargo test -p nimbus-server runtime_async_db_get_precancel_records_canceled_host_op_metric`;
  `cargo test -p nimbus-server convex_named_query_can_use_ctx_query_host_binding`;
  `cargo test -p nimbus-runtime`
