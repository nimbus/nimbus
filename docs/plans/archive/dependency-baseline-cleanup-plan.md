...
`make ci` baseline without re-expanding Nimbus's `libsql` dependency surface.
...
to restore a clean `cargo deny` / `make ci` result without weakening Nimbus's
...
`CDLA-Permissive-2.0`-licensed `webpki-roots` crate even though Nimbus's remote
...
- Nimbus uses `libsql` only for remote Hrana access. The repo no longer uses
  `libsql` embedded replication or local libsql-backed SQLite access.
- Nimbus already centralizes remote `libsql` database construction in
  `crates/nimbus-storage/src/libsql.rs::open_remote_database`.
...
- Remaining `cargo deny` duplicate warnings are tolerated upstream major-line
  splits rather than unresolved Nimbus-owned version drift.
...
- `libsql 0.9.30` enables `hyper-rustls` with the `webpki-roots` feature in its
  ...
  `.with_native_roots()`.
- Because Nimbus only uses remote `libsql`, the canonical seam is to let
  Nimbus own connector construction and to disable `libsql`'s builtin TLS
...
- The advisory text for `RUSTSEC-2026-0097` requires `rand`'s `log` and
  ...
  enable `log`, and repo code inspection shows Nimbus uses
...
- `cargo tree -d --workspace` now reports a broader duplicate-version warning
  ...
  inconsistent Nimbus workspace pins.
- The clearest directly-owned duplicate at plan start was
  `tokio-tungstenite`: Nimbus pinned `0.27` for test fixtures while the server
...
- Preserve current `libsql` provider semantics, including remote-primary writes,
  ...
  management.
- Keep the provider-owned transport seam explicit: Nimbus owns connector/config
...
### `libsql` is remote-only in Nimbus

Nimbus should depend on `libsql` with `default-features = false` and only the
...
### Nimbus owns the connector seam

Nimbus should provide the connector passed into `Builder::connector(...)` for
both plain HTTP and HTTPS paths. That keeps dependency shape, trust-root policy,
and transport behavior explicit at the Nimbus boundary instead of inheriting
...
- Focused provider verification:
  ...
  - `cargo test -p nimbus-engine sqlite_replica_provider -- --nocapture`
...
| Item | Status | Notes |
| --- | --- | --- |
| DB1 | done | `libsql` now uses a remote-only feature set and a Nimbus-owned connector seam |
...
- [x] Land the Nimbus-owned HTTPS connector path.
...
- [x] Confirm the remaining `rand 0.8.5` lane does not enable the advisory's
  `log` feature precondition and that Nimbus does not install a custom
...
Reduce the workspace `libsql` dependency to only the features Nimbus uses
today, remove reliance on `libsql`'s builtin TLS dependency chain, and make the
connector path explicit in Nimbus storage code.
...
| --- | --- | --- | --- | --- | --- |
| 2026-04-11 | DB1 | in_progress | Confirmed the live deny failures are rooted in `libsql`'s builtin TLS dependency shape; promoted this cleanup into its own active control plan. | `cargo tree -i rustls-webpki@0.102.8 --workspace`; `cargo tree -i webpki-roots@0.26.11 --workspace`; manifest inspection of `libsql 0.9.30` | Narrow `libsql` to remote-only features and land the Nimbus-owned connector path |
| 2026-04-11 | DB1 | done | Reduced the workspace `libsql` dependency to `default-features = false, features = ["remote"]`, added a Nimbus-owned native-TLS-backed connector, and updated direct `libsql` test helpers to use that seam. | `cargo check -p nimbus-storage -p nimbus-engine`; `cargo tree -i rustls-webpki --workspace`; `cargo tree -i webpki-roots --workspace` | Run focused provider tests and workspace verification |
| 2026-04-11 | DB2 | done | Verified the replica-provider lanes after the transport change; the vulnerable `rustls-webpki 0.102.8` lane and all `webpki-roots` nodes dropped out of the workspace graph. | `cargo test -p nimbus-storage libsql_provider -- --nocapture`; `cargo test -p nimbus-engine sqlite_replica_provider -- --nocapture`; `cargo tree -i rustls-webpki --workspace`; `cargo tree -i webpki-roots --workspace` | Run workspace verification and trim stale deny allow-list entries |
