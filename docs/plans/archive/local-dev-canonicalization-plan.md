...
| LD7 | Plan closeout. Flipped every ledger row to `done` (LD0-LD7); appended the Execution Log below with the actual commit SHAs that landed each LD. Moved this file to `docs/plans/archive/local-dev-canonicalization-plan.md`. Updated `docs/plans/README.md`: removed the active entry, added a paragraph under "Current Reference Baselines" naming the LD scope, the seven inlined `npm` steps deleted, the new actionable `build.rs` error contract, the `docs/operating/local-dev.md` build contract, the `/goal` verifier script, and closeout date 2026-05-21. Updated the routing entry in `AGENTS.md` (which `CLAUDE.md` symlinks to) to point at the archived path and add the "promote a new active plan before another local-dev / build-graph wave" boilerplate. Final commit pushed to main; awaiting `gh run list --branch main --workflow=CI --limit 1` to report `success` (condition 10 in the verifier). | done |
...
Landed on `main` between 2026-05-19 and 2026-05-21 (autonomous mode under
the standing local-dev / desktop-* authorization; pre-launch, no PRs).

| LD  | Commit(s) | Subject |
|-----|-----------|---------|
| LD0 | `f00fc09c`, `c39d3aad` | Plan + plans/README index; drift fixes (step count, grep scope) |
| LD1 | `f8a7a221` | Makefile dependency graph for nimbus-ui artifacts |
| LD2 | `0ff1e182` | Delete build.rs stub fallback, error actionably |
| LD3 | `8c8b95f7` | ci.yml — delete inlined npm orchestration, route through make |
| LD4 | `a4c52f5e` | Build-contract docs + CLAUDE.md routing entry |
| LD5 | `4e513b79` | Fresh-clone proof — make ci-required green in worktree |
| LD6 | `0c7181b9` | Add /goal control-plane verifier script |
| LD7 | (this commit) | Plan closeout — archive + README + routing entry update |

The /goal stop condition `bash scripts/verify-local-dev-canonicalization.sh
exits 0` clears once this commit lands on `main` and the resulting CI run
reports `success` (verifier condition 10 polls
