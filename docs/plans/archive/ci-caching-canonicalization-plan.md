...
| CC9 | Post-closeout correctness wave. CC8's first push (`a29f14df`) failed CI with HTTP 400 on every sccache startup because `mozilla-actions/sccache-action@v0.0.6` (Sep 2024) predates the actions cache v1 → v2 migration that v0.0.8 (Mar 2025, "Please update quickly!") and v0.0.9 (Jun 2025, "force github action 2") fixed. Bumped the pin floor to `v0.0.10` across all 12 sites in `ci.yml`, `desktop-ui.yml`, and `node-compat-nightly.yml`. Retracted CC3's wrong-headed `save-always: true` Swatinem input (not a valid v2 input; produced warnings on every job; runtime effect was nil). Audited every other action pin and bumped three more stale floors: `actions/cache@v4 → v5` (1 site), `actions/upload-artifact@v4 → v7` (2 sites), `actions/download-artifact@v7 → v8` (3 sites). Rewrote verifier condition 4 from "save-always count matches Swatinem count" to "sccache-action pinned ≥ v0.0.10 and save-always retracted" so a future careless downgrade re-trips the gate. Updated `docs/operating/ci-caching.md` to reflect actual Swatinem v2 rerun semantics (no save-always knob) and added a triage entry for the HTTP-400 symptom. Proof at `docs/plans/proof/ci-caching-canonicalization/cc9-pin-and-save-always-retraction.md`. | done |
...
| CC8 | `a29f14df` | CC8: closeout — archive plan, update routing, mark gate complete |
| CC9 | (this commit) | CC9: bump sccache-action v0.0.6→v0.0.10, retract save-always, audit stale pins |
