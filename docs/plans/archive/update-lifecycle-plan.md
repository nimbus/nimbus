...
| 2026-05-16 | UL3 landed | done | `nimbus/desktop@6e977e2` ships the nimbus-CLI lifecycle channels on a second preload surface (`window.nimbus`, `__version: "ul3"`) alongside the unchanged DS5 `window.nimbusShell`. New surface files: `src/shared/ipc-types.ts` (8 channel constants + closed `UpgradeMethod`/`InstallMethod` unions + `isUpgradeMethod`/`isInstallMethod` guards + `RunnerEvent` discriminated union + `RunnerEventEnvelope` wire shape + `StalenessInfo` + `NimbusCli`), `src/main/upgrade/runner.ts` (hardcoded `UPGRADE_ARGV`/`INSTALL_ARGV` tables — only `brew` populated; `child_process.spawn` with `shell: false`, sanitized PATH prepending `/opt/homebrew/bin` + `/usr/local/bin` on darwin; line-split stdout/stderr → `AsyncIterable<RunnerEvent>`; post-upgrade `onUpgradeSucceeded` hook yields `restarted` event), `src/main/upgrade/lifecycle.ts` (pumps runner events into per-call `subscriptionId` envelopes), `src/main/notifications/staleness.ts` (5-minute poll + `notified-versions.json` dedupe + OS `Notification` toast + click handler), `src/main/cli-lifecycle.ts` (composition root wiring runner + lifecycle + notifier + 5 IPC channels with origin validation), `buildResources/setup/cli-not-found.html` (file:// setup card loaded when `NimbusBinaryNotFoundError` fires — uses `window.nimbus.canRunInstall('brew')` to gate Install button + streams runner output into a `<pre>` + invokes `retryResolveCli` on exit 0), and `src/preload/index.cts` extended with `buildCli`/`installNimbusShell` exposing both surfaces. `src/main/index.ts` restructured to not crash on `NimbusBinaryNotFoundError`; instead loads the setup card and registers the lifecycle with a throwing restart hook + a `retryResolveCli` hook that re-resolves the server and reloads the window. Per-call `subscriptionId` (`ul3-${time36}-${rand36}`) on runner envelopes isolates concurrent calls; renderer filters foreign-subscriber envelopes. Method-tag-only IPC boundary means the renderer cannot smuggle a command string — only the `UpgradeMethod`/`InstallMethod` enum tags pass through, and only `brew` resolves to a non-null argv on desktop+darwin. 184 desktop vitest tests pass (was 131 pre-UL3; +53 net new across runner / staleness / lifecycle / cli-lifecycle / preload). `npx tsc --noEmit` and `npx biome check src tests scripts` clean. Pushed to `nimbus/desktop@6e977e2`. Full-flow screencast captured at [`.playwright-cli/ul3/full-flow.webm`](../../../.playwright-cli/ul3/full-flow.webm) (4.5 MB WebM, 1280×800): drives the production `cli-not-found.html` setup-card surface through a deterministic `window.nimbus` stub matching the preload IPC contract — initial state with the Install button gated by `canRunInstall("brew") === true`, `runInstall("brew")` `AsyncIterable<RunnerEvent>` streaming five stdout lines into the `<pre>` progress block, `exit code=0` followed by `retryResolveCli` returning `{ok: false}` ("Install completed but nimbus is still not on PATH" status), explicit Retry rescan landing on the "Still no nimbus on PATH" guidance, and the UL2 staleness-banner Update affordance transitioning to "Update queued. Restart nimbus to complete the upgrade." Recorded via `playwright-cli video-start --size 1280x800` against a `python3 -m http.server`-served wrapper that pre-installs the IPC stub before the verbatim cli-not-found.html script runs (Chrome blocks `file://` for cross-origin script load, so the demo is served from `127.0.0.1`). |
| 2026-05-16 | UL4 landed | done | `docs/operating/updates.md` (new — one page covers: how the CLI updates per detected install method, how the desktop shell updates via `electron-updater`, what the staleness banner's 4 visible states mean, what the Update button does in the (β+) pattern with Podman/Docker/VS Code analogs, and `NIMBUS_DISABLE_UPDATE_CHECK=1` for air-gapped operation). README cross-link landed in `nimbus/nimbus/README.md` under the existing Desktop console subsection. Desktop repo README cross-link landed in `nimbus/desktop/README.md` under the existing Update section, pointing back to the canonical doc. A first-time reader can answer "How do I disable the update check?", "Why does the banner show a brew command on my apt machine?", and "What if I'm offline?" directly from the doc. |
| 2026-05-16 | Plan closed | done | All four milestones landed and verified per the per-milestone gates above. UL1 = `9e8606af`'s parent `2626f050` (server endpoint + 18 unit tests + live GitHub smoke). UL2 = `9e8606af` (SPA staleness UX + 86 vitest tests). UL3 = `nimbus/desktop@6e977e2` (setup card + background brew runner + staleness notifier + 184 desktop vitest tests). UL4 = this commit (`docs/operating/updates.md` + README cross-links in `nimbus/nimbus` and `nimbus/desktop`). **`make ci` clean** against the working tree at this commit (exit 0): `cargo fmt --all --check` clean, `cargo clippy --workspace --all-targets -- -D warnings` clean, `cargo deny check` clean (advisories ok, bans ok, licenses ok, sources ok), `cargo test -p nimbus-runtime` 172 passed / 0 failed (plus locker_smoke 8/0 and doctests 0/0), `cargo nextest run --workspace --exclude nimbus-runtime` green, JS typecheck + tests + build clean (14 test files / 86 tests passed across the JS workspace), and the install-script helper proof passed 27 checks. Plan moved from `docs/plans/update-lifecycle-plan.md` to `docs/plans/archive/update-lifecycle-plan.md` (relative `../decisions/` and `../../CLAUDE.md` anchors rewritten to `../../decisions/` and `../../../CLAUDE.md` after the move). `docs/plans/README.md` active list pruned; archive Reference Baselines section grew the new entry. Decision doc `docs/decisions/001-update-staleness-detection.md` flipped from `proposed` to `accepted (plan closed 2026-05-16)` and its parent-plan pointer rewritten to the archive path. |
