> Archived on 2026-04-08 after `TH1` through `TH7` completed. Historical
> record only; do not resume as the active control plane.
...
| TH1 | done | start `TH3` by replacing the highest-value sleep-based runtime, websocket, demo-flow, and scheduler waits with deterministic waiters or explicit eventual assertions |
| TH2 | done | move to `TH3` deterministic waiters now that the runtime isolation category is real and the CI runtime lane no longer depends on single-thread containment |
| TH3 | done | start `TH4` by promoting the new eventual helpers, explicit runtime profiles, and transport/runtime repro utilities into clearer shared harness ownership surfaces |
| TH4 | done | use the hardened harness surfaces to add reproducible liveness and safety campaigns |
| TH5 | done | once campaigns and harness categories are real, align CI and coverage topology to them |
| TH6 | done | close out the workstream with docs, full verification, and archive handoff |
| TH7 | done | archived the completed control plane and removed live entrypoints to it after the full verification sweep passed |
