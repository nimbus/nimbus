...
| CA3 | `3996dd9a` + five hotfixes in CA5 | shard Coverage across 3 parallel lanes with cargo llvm-cov reducer. Hotfix 1 (`0d7b868e`): profraw path + engine shard libsql gating. Hotfix 2 (`e86e75c2`): rest shard libsql tests in nimbus-storage; retire `needs-providers` flag. Hotfix 3 (`ccae3a3d`): reducer's deprecated `cargo llvm-cov --no-run` → `cargo llvm-cov show-env + cargo test --no-run`. Hotfix 4 (`0361afc1`): standardize on show-env convention across shards + reducer (shards stop using `--no-report`; profraws live in `target/nimbus-*.profraw`; report step also sources show-env). Hotfix 5 (this commit): raise the postgres CRUD test's CI budget from 180s to 360s to absorb cargo-llvm-cov instrumentation overhead (local-dev stays at 60s). |
| CA4 | `d66f85fb` | migrate release.yml to setup-rust-cached composite |
| CA5 | `598dd74e` + `0d7b868e` + `e86e75c2` + `ccae3a3d` + `0361afc1` + hotfix 5 (this commit) | closeout — promote contract, archive plan, update routing; all five CA3 hotfixes bundled in CA5 |
