...
Canonical plan for the VMM infrastructure that enables nimbus to run OCI/Docker
images in hardware-isolated microVMs. Follows the Podman distribution model:
nimbus is a single binary with system package dependencies.

**Platform scope: Linux.** The entire stack (conmon, crun, buildah, libkrun/KVM)
requires Linux. On macOS, nimbus runs inside a Linux machine VM
...
- **Related plans:**
  - `docs/plans/archive/runtime-sandbox-architecture-plan.md` — completed
    baseline that owns the canonical `nimbus-sandbox` crate naming and the
  ...
  - `distribution-plan.md` — packages nimbus + dependencies for each channel
  - `distribution-plan.md` Channel 4 — macOS support via krunkit machine VM
    (runs nimbus inside a Linux guest VM, with standard containers on macOS
...
- The naming and crate-boundary prerequisite is complete in
  `docs/plans/archive/runtime-sandbox-architecture-plan.md`: `nimbus-sandbox`
  is the canonical sandbox seam and `nimbus-server` owns the first
...
- The repo now owns the first concrete V1 artifacts:
  ...
  `scripts/build-nimbus-crun.sh`, `scripts/prepare-krun-bundle.sh`,
  ...
  `make check-vmm-host`, `make build-nimbus-crun`,
...
- The repo now also owns the first V3 code slice under
  `crates/nimbus-sandbox/src/backends/krun/`: `bundle.rs` for OCI config
  generation, `buildah.rs` for backend-local buildah command assembly,
  `conmon.rs` for `conmon -> /usr/libexec/nimbus/crun` launch planning,
...
- Homebrew Podman `5.8.1` is installed on the current Mac. The original
  `applehv`/`vfkit` machine (`nimbus-vmm-validation`) proved unusable: its
  serial log at
  `/var/folders/kw/d608x5pn4cq73rz78ztl92cw0000gn/T/podman/nimbus-vmm-validation.log`
...
- A fresh libkrun-backed Podman machine,
  `nimbus-libkrun-validation`, can now be created on this host with
...
- GUI menu-bar listings on this host are not a canonical machine-health source.
  ...
  `nimbus-libkrun-users-only` and `nimbus-libkrun-validation`. Treat the
...
- The current macOS guest lane is still blocked on guest readiness rather than
  ...
  nimbus-libkrun-validation` reaches `State: running`, but both
  ...
  `/var/folders/kw/d608x5pn4cq73rz78ztl92cw0000gn/T/podman/nimbus-libkrun-validation.log`
...
- The latest unrestricted macOS diagnostics capture at
  `/tmp/nimbus-libkrun-diagnostics` now preserves the current libkrun-machine
  state for this host. It shows `podman machine list` and
  `podman machine inspect` succeeding for `nimbus-libkrun-validation`,
...
- A second unrestricted diagnostics capture at
  `/tmp/nimbus-libkrun-users-only-diagnostics` now preserves the reduced-volume
  follow-up machine, `nimbus-libkrun-users-only`. Its copied machine config
...
- Host-side VMM control is healthier than guest readiness, but still not
  ...
  nimbus-libkrun-users-only` hangs as well.
- The reduced-volume machine also still lacks the expected host-side networking
  helpers. There is no live `gvproxy` process, no
  `nimbus-libkrun-users-only-gvproxy.sock`, and no
  `nimbus-libkrun-users-only-api.sock` under the Podman tmp root even while
...
- A clean repro on 2026-04-12 moved the macOS diagnosis from "generic guest
  ...
  rm -f nimbus-libkrun-users-only` succeeded, a fresh `podman machine init`
  ...
  (`.../nimbus-libkrun-users-only-gvproxy.sock` length 94,
  `...-api.sock` length 90, `.../nimbus-libkrun-users-only.sock` length 86),
...
- A short-runtime-dir rerun now validates that path-budget diagnosis enough to
  ...
  `/tmp/nimbus-libkrun-users-only-readiness` still shows
  `podman --connection nimbus-libkrun-users-only info --debug` failing with
...
- A fresh short-root machine now proves that second-stage failure is not a
  ...
  `nimbus-libkrun-sr-fresh`, created from scratch with the same short tmp root
  ...
  `/tmp/nimbus-libkrun-sr-fresh-readiness` reports both connection-targeted
...
- The repo now owns a durable repair path for that stale-state case, and it is
  ...
  readiness bundle. A live run at `/tmp/nimbus-libkrun-users-only-recreate`
  shows the pre-recreate failure signature preserved under
  `/tmp/nimbus-libkrun-users-only-recreate/pre-diagnostics/summary.txt`
  ...
  `/tmp/nimbus-libkrun-users-only-recreate/readiness/summary.txt` reports
...
- Podman's published macOS contract is now explicit in this workstream:
  ...
  the architectural target for nimbus on macOS: a thin host-side control shim
...
- The intended CLI taxonomy for that future macOS path is now explicit too:
  keep `nimbus serve` as the server-start verb, use `nimbus machine ...` for
  ...
  `nimbus services ...` namespace for listing or inspecting managed workloads
...
- A supplementary macOS userspace proof lane now exists:
  `scripts/verify-nimbus-crun-fedora-userspace.sh` succeeds through Docker
  ...
  `/tmp/nimbus-crun-fedora-userspace-output/crun`. This is preflight evidence
...
- A patched `nimbus-crun` binary (crun 1.27 with `+LIBKRUN`) is now built and
  installed at `/usr/libexec/nimbus/crun` on the Debian 13 Linux host.
...
- `crates/nimbus-sandbox/` no longer stops at an empty seam. The first backend-
...
- The repo now also owns the first Rust-backend Linux smoke path definition:
  `crates/nimbus-sandbox/tests/krun_linux_smoke.rs` and
...
- The remaining `V3` gap is host-level integration rather than crate shape:
  ...
  confirms the VM survives a nimbus-process restart while conmon remains alive.
...
- That stale-state theory now has a working repo-owned mitigation. The
  pre-recreate bundle at `/tmp/nimbus-libkrun-users-only-recreate/pre-diagnostics`
  still shows the old missing API/gvproxy socket pair and `podman info --debug`
  failure, while the post-recreate bundle at
  `/tmp/nimbus-libkrun-users-only-recreate/readiness` reports both
...
- Use Podman core, not Podman Desktop, as the canonical implementation
  reference for nimbus's macOS machine layer. Podman Desktop is still useful
...
- Podman's machine-os package set is now a positive reference point for nimbus.
...
- Keep packaging facts and provider behavior separate. Homebrew `podman`
  ...
  Homebrew dependency contract for `krunkit`. If nimbus standardizes on
  `krunkit` for macOS, that dependency must be owned directly by nimbus. This
...
- Podman's upstream macOS `.pkg` source is broader than the Homebrew contract:
  ...
  but keep the Homebrew and `.pkg` packaging stories distinct in nimbus docs.
...
- The first VMM slice should stay focused on patch fidelity and host-level
  verification. Do not jump to `nimbus-sandbox` backend implementation before
...
- **Conmon attach lifecycle:** conmon with `--full-attach` does not call
  `crun start` automatically. The `nimbus-sandbox` backend must either connect
...
- **Rootless krun requires a user namespace:** the krun handler writes
  ...
  `nimbus-sandbox` backend must either run crun/conmon inside a user namespace
...
| --- | --- | --- |
| Public crate boundaries | `nimbus-runtime` remains execution-only and `nimbus-sandbox` remains the isolation seam | do not re-couple execution and sandbox concerns |
...
| System Podman/crun | system Podman must keep using distro `crun` untouched | nimbus uses `/usr/libexec/nimbus/crun` only |
...
| Upstream drop path | if upstream gains equivalent support, nimbus can delete the patch and stop shipping `nimbus-crun` | keep the delta small and easy to remove |
...
- If a host-local artifact cannot live in git (for example
  `/usr/libexec/nimbus/crun`, a local OCI bundle directory, or generated log
...
nimbus follows the same process model and dependency pattern as Podman.
nimbus is a single binary. The VMM stack is system packages.
...
nimbus serve
  │
  └── conmon -r /usr/libexec/nimbus/crun (per VM, long-lived, survives nimbus restart)
        │
        ├── stdout/stderr → log files (persistent)
        ├── exit status → exit file
        ├── attach socket for interactive access
        │
        └── /usr/libexec/nimbus/crun run --bundle path id
              │
...
conmon's `-r` flag accepts an arbitrary path to any OCI runtime binary. nimbus
passes `-r /usr/libexec/nimbus/crun` so the forked crun is used without
...
Podman:                             nimbus:
  conmon               ✓             conmon
  crun | runc          ✓             nimbus-crun (patched crun, small one-file delta, at /usr/libexec/nimbus/crun)
...
nimbus drops only netavark (TSI replaces container networking) and adds
...
4. **Security proxying doesn't need vsock.** A TCP proxy in nimbus (v2)
...
If added, nimbus-init and vsock support in crun would be revisited then.
...
v2 (future, microvm-runtime-plan Phase M4+):
  V8 → nimbus proxy (policy, audit, rate limit) → TCP → TSI → guest
  Same transport, nimbus is now in the data path for observability
```
...
If nimbus evolves toward high-density ephemeral VMs, the migration path is:
...
the canonical crate target is `nimbus-sandbox`, not `nimbus-vmm`.
...
module such as `crates/nimbus-sandbox/src/backends/krun/`, which may own the
...
A full GitHub fork is overkill for a small one-file C patch. nimbus uses the standard
...
via `git format-patch`. It is checked into the nimbus repo alongside the
Rust source code.

**Step 2: CI builds the patched crun binary**

A GitHub Actions workflow (defined in `distribution-plan.md` Phase D1) runs
on each nimbus release tag:

```bash
# .github/workflows/build-nimbus-crun.yml (conceptual)
CRUN_VERSION=1.22
...
|---------|-----------------|---------------------|
| Binary tarball | `nimbus-linux-amd64.tar.gz` containing `nimbus` + `crun` | CI applies patch, ships binary |
| Debian (deb) | `nimbus-crun_1.22+nimbus1_amd64.deb` | `debian/patches/series` + quilt (standard) |
| Fedora (rpm) | `nimbus-crun-1.22-1.nimbus1.x86_64.rpm` | `Patch0:` in spec, `%autosetup -p1` |
| Homebrew | Formula with `patch :DATA` block | Homebrew applies patch before `make` |
| Container image | `ghcr.io/nimbus/nimbus:latest` | `RUN patch -p1 < ...` in Dockerfile |

Each format has a native mechanism for applying patches — the patch file is
the same, only the build harness differs.

**Step 4: User installs nimbus, gets patched crun automatically**

```bash
# Debian/Ubuntu
curl -fsSL https://github.com/nimbus/nimbus/releases/latest/download/install.sh | sh
# install.sh adds apt repo, then:
# apt install nimbus  (depends on nimbus-crun, conmon, buildah, ...)
# nimbus-crun installs to /usr/libexec/nimbus/crun
...
# Manual tarball
tar xzf nimbus-linux-amd64.tar.gz
sudo mv nimbus /usr/local/bin/
sudo mkdir -p /usr/libexec/nimbus
sudo mv crun /usr/libexec/nimbus/crun
```

The user never interacts with the patch. They install `nimbus`, which depends
on `nimbus-crun`, which is a pre-built binary at `/usr/libexec/nimbus/crun`.
nimbus invokes it via `conmon -r /usr/libexec/nimbus/crun`.

**If upstream independently adds port mapping support:** Delete the patch
file, stop building `nimbus-crun`, change the `nimbus` package to depend on
system `crun` (>= the version with port mapping). The
`/usr/libexec/nimbus/` path is no longer needed — nimbus uses system crun
directly.
...
- **Binary tarball:** Include a `SOURCE.md` pointing to the upstream release
  URL and the patch file location in the nimbus repo.

### Package naming

The patched crun is packaged as `nimbus-crun`:

- **Binary name:** `crun` (it IS crun, just patched)
- **Install path:** `/usr/libexec/nimbus/crun` (private to nimbus)
- **Package name:** `nimbus-crun` (deb/rpm — makes the relationship clear)
...
The `nimbus-crun` name follows the convention of scoping a patched build to
...
|---------|------|----------------------|------------------------|
| `nimbus-crun` | Patched crun with TSI port mapping | We package it | We package it |
...
| `passt` | Rootless networking | Non-root nimbus operation |
| `uidmap` | User namespace mapping | Non-root nimbus operation |
...
## How nimbus uses buildah (instead of custom OCI code)
...
```bash
# nimbus shells out to buildah to pull and mount an image:
buildah from --name nimbus-postgres docker://postgres:16
ROOTFS=$(buildah mount nimbus-postgres)
...
```bash
buildah bud -t nimbus-myapp -f ./Dockerfile .
buildah from --name nimbus-myapp localhost/nimbus-myapp
ROOTFS=$(buildah mount nimbus-myapp)
```

### Cleanup

```bash
buildah umount nimbus-postgres
buildah rm nimbus-postgres
```

### What this eliminates from nimbus's Rust code
...
| `.krun_config.json` generation | Still in nimbus (reads OCI image config via `buildah inspect`) |
| OCI bundle `config.json` generation | Still in nimbus |
...
| --- | --- | --- | --- |
| `LH1` Host preflight | `done` | Debian 13 x86_64, kernel 6.12.74, `/dev/kvm` present and accessible, build tools present, conmon 2.1.12, buildah 1.39.3, system crun 1.21, podman 5.4.2, catatonit 0.2.1. libkrun/libkrunfw not in Debian repos — built from source (libkrun 1.17.4, libkrunfw 5.3.0) | `/tmp/nimbus-linux-vmm-validation/artifacts/lh1/check-vmm-host.txt`, `/tmp/nimbus-linux-vmm-validation/artifacts/lh1/collect-vmm-package-versions.txt` |
| `LH2` Patch fidelity on the real Linux host | `done` | Patch applies cleanly (with minor offsets) against upstream crun `1.27` (tag `a718a92c`) at `~/src/github.com/containers/crun` | `/tmp/nimbus-linux-vmm-validation/artifacts/lh2/verify-crun-patch.txt` |
| `LH3` Build and install the private runtime | `done` | Built patched crun 1.27 with `+LIBKRUN`, staged at `/tmp/nimbus-linux-vmm-validation/stage/crun`, installed at `/usr/libexec/nimbus/crun`. libkrun 1.17.4 built from source (`~/src/github.com/containers/libkrun` tag `v1.17.4`) and installed at `/usr/local/lib64/libkrun.so.1.17.4`. libkrunfw 5.3.0 built from source (`~/src/github.com/containers/libkrunfw` tag `v5.3.0`) and installed at `/usr/local/lib64/libkrunfw.so.5.3.0` | `/tmp/nimbus-linux-vmm-validation/artifacts/lh3/build-stage-runtime.txt`, `/tmp/nimbus-linux-vmm-validation/artifacts/lh3/stage-runtime-version.txt`, `/tmp/nimbus-linux-vmm-validation/artifacts/lh3/install-private-runtime.txt`, `/tmp/nimbus-linux-vmm-validation/artifacts/lh3/install-runtime-version.txt` |
| `LH4` Prove system runtime separation | `done` | System crun 1.21 at `/usr/bin/crun`, private nimbus crun 1.27+LIBKRUN at `/usr/libexec/nimbus/crun`, Podman runtime pointed at system `/usr/bin/crun`. Realpaths distinct, separation confirmed | `/tmp/nimbus-linux-vmm-validation/artifacts/lh4/verify-runtime-separation.txt` |
| `LH5` Generate and boot the first real krun bundle | `done` | krun VM booted via `crun run` inside `buildah unshare` with busybox rootfs overlay mount. TSI port mapping `18080:8080` verified — `libkrun VM` bound port 18080 on the host. BusyBox httpd responded via TSI (HTTP/1.1 404 — expected, empty docroot proves connectivity). OCI config required removing `network` namespace and setting `terminal: false` for krun bundles — fix checked into `scripts/prepare-krun-bundle.sh`. SIGTERM does not cleanly stop the krun VMM (SIGKILL required) | `/tmp/nimbus-linux-vmm-validation/artifacts/lh5/direct-drill-full.txt`, `/tmp/nimbus-linux-vmm-validation/artifacts/lh5/direct-probe-http.txt` |
| `LH6` Conmon lifecycle drill | `done` | First real `conmon -> /usr/libexec/nimbus/crun -> krun VM` flow proven inside `buildah unshare`. Process tree: `conmon(90649) → libkrun VM(90651)` with 8 worker threads. TSI port 18080 bound by `libkrun VM`. HTTP connectivity confirmed. Exit file written at `/tmp/nimbus-linux-vmm-validation/conmon-drill/exits/nimbus-http` with exit code 137 (SIGKILL). Attach socket at `/tmp/nimbus-linux-vmm-validation/bundle/attach`. Key finding: conmon with `--full-attach` waits for attach connection before calling `crun start`; `crun start` must be called explicitly or via attach to transition the krun container from `created` to `running` | `/tmp/nimbus-linux-vmm-validation/artifacts/lh6/conmon-drill-full.txt`, `/tmp/nimbus-linux-vmm-validation/artifacts/lh6/conmon-probe-http.txt`, `/tmp/nimbus-linux-vmm-validation/artifacts/lh6/process-tree.txt`, `/tmp/nimbus-linux-vmm-validation/artifacts/lh6/conmon-exit-status.txt` |

Supplementary macOS-only preflight evidence that does **not** close `LH1`-`LH6`:
- `bash scripts/verify-nimbus-crun-fedora-userspace.sh --crun-source ~/src/github.com/containers/crun --output-dir /tmp/nimbus-crun-fedora-userspace-output --work-dir /tmp/nimbus-crun-fedora-userspace-build`
- `file /tmp/nimbus-crun-fedora-userspace-output/crun`
- `bash scripts/collect-podman-machine-diagnostics.sh --machine nimbus-libkrun-validation --provider libkrun --output-dir /tmp/nimbus-libkrun-diagnostics`
...
- `bash scripts/build-nimbus-crun.sh --source ~/src/github.com/containers/crun --output /tmp/nimbus-crun-stage/crun`
- `bash scripts/build-nimbus-crun.sh --source ~/src/github.com/containers/crun --output /tmp/nimbus-crun-stage/crun --install-path /usr/libexec/nimbus/crun --sudo-install`
- `bash scripts/verify-runtime-separation.sh --system-runtime /usr/bin/crun --private-runtime /usr/libexec/nimbus/crun`
...
- the first real `conmon -r /usr/libexec/nimbus/crun ...` lifecycle command
...
3. Build patched crun and install to `/usr/libexec/nimbus/crun`
...
1. **Repo-owned artifacts**
   ...
   - `scripts/verify-runtime-separation.sh`
   - `scripts/verify-runtime-separation-helper.sh`
   - `scripts/verify-nimbus-crun-fedora-userspace.sh`
   - `scripts/collect-podman-machine-diagnostics.sh`
   - `scripts/verify-podman-machine-diagnostics-helper.sh`
   - `scripts/prepare-linux-vmm-validation-bundle.sh`
   - `scripts/verify-linux-vmm-validation-bundle-helper.sh`
   ...
   - `make check-vmm-host`
   - `make build-nimbus-crun`
   - `make prepare-krun-bundle`
   - `make verify-krun-bundle-helper`
   - `make verify-nimbus-crun-fedora-userspace`
   - `make prepare-direct-krun-drill`
   - `make verify-direct-krun-drill-helper`
   - `make verify-runtime-separation`
   - `make verify-runtime-separation-helper`
...
- `bash -n scripts/build-nimbus-crun.sh`
...
- `bash -n scripts/verify-nimbus-crun-fedora-userspace.sh`
...
- `bash scripts/verify-nimbus-crun-fedora-userspace.sh --help`
...
- `bash scripts/check-podman-machine-socket-paths.sh --machine nimbus-libkrun-users-only --tmp-root /var/folders/kw/d608x5pn4cq73rz78ztl92cw0000gn/T/podman`
- `bash scripts/check-podman-machine-socket-paths.sh --machine nimbus-libkrun-users-only --tmp-root /tmp/podman`
...
**Acceptance criteria:**
- `/usr/libexec/nimbus/crun run` with a krun-configured OCI bundle boots a VM with TSI port mapping
...
**Goal:** Verify nimbus can spawn conmon → crun → VM using system packages.
...
3. **Operational proof outputs**
   - a captured process tree showing `nimbus/test shell -> conmon -> crun`
...
- Process tree matches the krun model (nimbus → conmon → crun, where the crun
...
### Phase V3: `nimbus-sandbox` krun backend

**Goal:** nimbus can spawn and manage VMs programmatically.
...
1. `docs/plans/archive/runtime-sandbox-architecture-plan.md` `RS4` is complete so the Rust wrapper
   ...
   lifecycle surface.
2. `crates/nimbus-sandbox/src/backends/krun/conmon.rs`: Spawn conmon with
   `-r /usr/libexec/nimbus/crun` as subprocess, read sync pipe, manage
   PID files, read exit files, connect to attach socket
3. `crates/nimbus-sandbox/src/backends/krun/bundle.rs`: Generate OCI bundle for crun
   (config.json with krun handler, `krun.port_map` annotation)
4. `crates/nimbus-sandbox/src/backends/krun/buildah.rs`: Shell out to buildah for image
   pull/build/mount/inspect
5. `crates/nimbus-sandbox/src/backends/krun/vm.rs`: backend-local VM handle
   wrapping conmon management
6. `crates/nimbus-sandbox/src/lib.rs`: expose the first generic
   `SandboxBackend` / `SandboxHandle` seam needed by the server integration

**Required verifiable outputs:**
1. **Repo-owned code outputs**
   - concrete backend code under `crates/nimbus-sandbox/src/backends/krun/`
...
2. **Repo-owned verification outputs**
   - recorded `cargo check` / `cargo test` commands for `nimbus-sandbox` and
...
3. **Host-level integration outputs**
   ...
   - recorded proof that logs persist and the VM survives a nimbus-process
...
- run focused `cargo check` and targeted tests for `nimbus-sandbox` plus any
  touched server integration seams
- record the exact smoke path and observed lifecycle outcomes in
  `Execution Log`

**Acceptance criteria:**
- `nimbus` can programmatically boot a postgres:16 VM, connect via TSI,
  run a query, stop the VM, verify exit status
- VM survives `nimbus` process restart (conmon keeps it alive)
- Logs are persisted to disk via conmon
- System crun/Podman remain functional (nimbus uses private crun path)
...
- V1 patch-artifact work:
  ...
  - `bash -n scripts/verify-podman-machine-recreate-helper.sh`
  - `bash scripts/verify-crun-patch.sh ~/src/github.com/containers/crun`
  - `bash scripts/build-nimbus-crun.sh --help`
...
- V2 system-integration work:
  ...
  - capture process-tree, logs, exit-file, and port-mapping evidence in the
    execution log
- V3 `nimbus-sandbox` backend work:
...
|-------|--------|-----------|-------|
| V1: Patch crun | `done` | none | All repo-owned artifacts checked in. Patch updated to target upstream crun 1.27 (was 1.22). Linux host validation complete on Debian 13 x86_64: patched crun 1.27 with `+LIBKRUN` built and installed at `/usr/libexec/nimbus/crun`, libkrun 1.17.4 and libkrunfw 5.3.0 built from source, system/private runtime separation verified, krun VM booted via direct `crun run` with TSI port mapping `18080:8080` proven via HTTP connectivity. The `prepare-krun-bundle.sh` script now removes the network namespace and sets `terminal: false` for krun bundles |
...
4. Start `V3` `nimbus-sandbox` backend implementation slices
...
- `ICP2` Host-level validation:
  ...
  - future CLI naming is recorded explicitly: `nimbus serve` is the server
    verb, `nimbus machine ...` owns VM lifecycle, and `service` stays reserved
    away from daemon startup; if workload-management nouns arrive later,
    prefer a plural `nimbus services ...` namespace for list/inspect/logs style
    commands
  - the latest unrestricted macOS diagnostics artifact at
    `/tmp/nimbus-libkrun-diagnostics` shows the current libkrun machine still
  ...
  - the reduced-volume comparison artifact at
    `/tmp/nimbus-libkrun-users-only-diagnostics` proves a one-mount
  ...
  - the latest live short-root readiness bundle at
    `/tmp/nimbus-libkrun-users-only-readiness` still reports
    `podman --connection nimbus-libkrun-users-only info --debug`
  ...
  - a fresh disposable machine,
    `nimbus-libkrun-sr-fresh`, created with the same short tmp root and
    one-mount layout reaches full readiness on this host; the bundle at
    `/tmp/nimbus-libkrun-sr-fresh-readiness` records both
  ...
  - the checked-in recreate flow now closes that stale-state loop on this host:
    ...
    `/tmp/nimbus-libkrun-users-only-recreate/readiness/summary.txt` records
    `result ready info=ok ssh=ok` after `rm -f`, reinit, and restart under the
    same `/tmp/podman` short-root contract
  - patched `nimbus-crun` builds and installs to `/usr/libexec/nimbus/crun`
...
- `ICP3` Rust handoff:
  - `nimbus-sandbox` now owns `backends/krun/` code after `ICP1` and `ICP2`
  ...
  - the repo also now owns a Linux-only ignored integration test,
    `crates/nimbus-sandbox/tests/krun_linux_smoke.rs`, and an operator runbook
...
| crun krun.c churn causes patch conflicts | Medium | Low | krun.c has ~30 commits/year, but the nimbus delta stays intentionally small and localized. Verify `patch --dry-run` on each upstream release. Manual resolution should stay straightforward while the patch remains one focused handler fix. |
...
| 2026-04-11 | V2 prep | `documented` | Added a checked-in conmon lifecycle drill preparer and focused verifier so the Linux host no longer has to reconstruct the `conmon -> /usr/libexec/nimbus/crun -> guest` command from prose. `scripts/prepare-conmon-krun-drill.sh` now lays out deterministic pid/log/exit/persist paths, emits a runnable `run-conmon.sh`, and generates helper scripts for attach-socket discovery, process-tree capture, wait-for-exit, and graceful/forced stop. The runbook, Makefile, CI lane, and plan now all point at the same reproducible drill path, while the actual Linux-host execution evidence remains pending. | `bash -n scripts/prepare-conmon-krun-drill.sh`; `bash -n scripts/verify-conmon-krun-drill-helper.sh`; `bash scripts/prepare-conmon-krun-drill.sh --help`; `bash scripts/verify-conmon-krun-drill-helper.sh`; `cargo fmt --all --check` | Run `LH6` on a supported Linux host using the generated `run-conmon.sh` flow, then record the concrete attach-socket path, process tree, connectivity probe, graceful stop, and exit-status evidence |
...
| 2026-04-11 | V1 prep | `documented` | Added a checked-in direct private-runtime drill preparer and focused verifier so `LH5` no longer depends on a hand-written `/usr/libexec/nimbus/crun run --bundle ...` command. `scripts/prepare-direct-krun-drill.sh` now derives the first probe port from `krun.port_map`, emits runnable `run-runtime.sh` and `start-runtime.sh` scripts, and generates deterministic stdout, stderr, pid, launcher-pid, exit-status, and HTTP-probe helpers. The runbook, Makefile, CI lane, and plan now point at the same reproducible direct-runtime path, while the actual Linux-host execution evidence remains pending. | `bash -n scripts/prepare-direct-krun-drill.sh`; `bash -n scripts/verify-direct-krun-drill-helper.sh`; `bash scripts/prepare-direct-krun-drill.sh --help`; `bash scripts/verify-direct-krun-drill-helper.sh`; `cargo fmt --all --check` | Run `LH5` on a supported Linux host or Linux machine-VM guest using the generated `start-runtime.sh` flow, then record the direct-runtime command, concrete log/pid/exit paths, connectivity probe, graceful stop, and exit-status evidence |
| 2026-04-11 | V1 prep | `documented` | Added a supplementary macOS userspace validation lane and recorded the current Mac host evidence. `scripts/verify-nimbus-crun-fedora-userspace.sh` now succeeds through Docker Desktop against `fedora:43`, using `~/src/github.com/containers/crun` as the source checkout and staging a Linux `aarch64` binary at `/tmp/nimbus-crun-fedora-userspace-output/crun`. The first reruns exposed two real helper gaps that are now fixed in `scripts/build-nimbus-crun.sh`: explicit `libocispec` generation before `crun`, and explicit `git-version.h` generation before the final build. In parallel, the local Homebrew Podman machine remains unusable for Linux-guest validation: `podman machine list` reports `nimbus-vmm-validation` as `Currently starting`, `podman machine inspect nimbus-vmm-validation` reports `State: running` with `VM TYPE applehv`, and the serial log at `/var/folders/kw/d608x5pn4cq73rz78ztl92cw0000gn/T/podman/nimbus-vmm-validation.log` shows Ignition failure, `systemd-fsck-root.service` failure, and emergency mode. | `bash -n scripts/build-nimbus-crun.sh`; `bash -n scripts/verify-nimbus-crun-fedora-userspace.sh`; `bash scripts/verify-nimbus-crun-fedora-userspace.sh --help`; `bash scripts/verify-nimbus-crun-fedora-userspace.sh --crun-source ~/src/github.com/containers/crun --output-dir /tmp/nimbus-crun-fedora-userspace-output --work-dir /tmp/nimbus-crun-fedora-userspace-build`; `file /tmp/nimbus-crun-fedora-userspace-output/crun`; `podman machine list`; `podman machine inspect nimbus-vmm-validation`; `tail -n 60 /var/folders/kw/d608x5pn4cq73rz78ztl92cw0000gn/T/podman/nimbus-vmm-validation.log`; `cargo fmt --all --check` | Keep the Fedora userspace lane as supplementary preflight, but continue `LH1`-`LH6` on a usable Linux host or Linux guest before attempting V2/V3 closeout |
| 2026-04-11 | meta | `documented` | Tightened the VMM helper CI lane so GitHub Actions now syntax-checks all checked-in crun/VMM helper scripts and exercises the non-host help entrypoints before downloading the pinned upstream crun source. This keeps the repo-owned control-plane scripts honest even while the Linux `/dev/kvm` evidence remains a separate host lane. | `cargo fmt --all --check`; workflow review of `.github/workflows/verify-nimbus-crun-patch.yml` | Use the CI lane to catch script drift early, then keep advancing the remaining Linux-host queue items |
| 2026-04-11 | meta | `documented` | Strengthened the GitHub Actions VMM lane further so it now runs the full Docker-based Fedora userspace build helper against the pinned upstream `crun` checkout on `ubuntu-latest`, not just script syntax and `--help` entrypoints. This gives the repo a real automated Linux userspace build proof while `/dev/kvm` runtime proof remains delegated to `LH1`-`LH6`. | `cargo fmt --all --check`; workflow review of `.github/workflows/verify-nimbus-crun-patch.yml`; prior local confirmation via `bash scripts/verify-nimbus-crun-fedora-userspace.sh --crun-source ~/src/github.com/containers/crun --output-dir /tmp/nimbus-crun-fedora-userspace-output --work-dir /tmp/nimbus-crun-fedora-userspace-build` | Keep the CI userspace lane green, then continue the remaining Linux-host queue items for real runtime and conmon proof |
| 2026-04-11 | V1 prep | `documented` | Added a checked-in runtime-separation verifier so `LH4` no longer depends on manually reassembling `command -v crun`, `crun --version`, and `podman info` probes. `scripts/verify-runtime-separation.sh` now records the system-runtime path and version, the private nimbus runtime path and version, Podman runtime evidence, resolved realpaths, and a final separation verdict. `scripts/verify-runtime-separation-helper.sh`, the Makefile, the operator runbook, the GitHub Actions lane, and this plan now all point at the same reproducible `LH4` entrypoint while the actual supported-host proof remains pending. | `bash -n scripts/verify-runtime-separation.sh`; `bash -n scripts/verify-runtime-separation-helper.sh`; `bash scripts/verify-runtime-separation.sh --help`; `bash scripts/verify-runtime-separation-helper.sh`; `make verify-runtime-separation-helper`; `cargo fmt --all --check` | Run `LH4` on a supported Linux host after `LH3`, capture the full helper output, and confirm Podman still points at the distro runtime while nimbus uses `/usr/libexec/nimbus/crun` |
...
| 2026-04-12 | V1 prep | `documented` | Materialized the first real Linux-host execution bundle from the pinned upstream checkout on the current machine so the other laptop has an immediate handoff target, not just the generator source. Running `bash scripts/prepare-linux-vmm-validation-bundle.sh --crun-source ~/src/github.com/containers/crun --output-root /tmp/nimbus-linux-vmm-validation` produced a stable bundle rooted at `/tmp/nimbus-linux-vmm-validation`, including `session.env`, `README.md`, `commands/00-run-through-lh6.sh`, the numbered queue scripts under `commands/`, and `99-writeback-checklist.txt`. This host-local artifact is now the concrete Linux-laptop handoff path: the operator can copy or recreate that bundle and run the exact queue commands without reconstructing the workflow from the docs first. | `bash scripts/prepare-linux-vmm-validation-bundle.sh --crun-source ~/src/github.com/containers/crun --output-root /tmp/nimbus-linux-vmm-validation`; observed outputs `bundle.session_env=/tmp/nimbus-linux-vmm-validation/session.env`, `bundle.queue_runner=/tmp/nimbus-linux-vmm-validation/commands/00-run-through-lh6.sh`, `bundle.checklist=/tmp/nimbus-linux-vmm-validation/99-writeback-checklist.txt`; `cargo fmt --all --check` | Hand `/tmp/nimbus-linux-vmm-validation` to the Linux host or rerun the same generator there, then execute `commands/00-run-through-lh6.sh` and write each resulting artifact path back into this plan |
| 2026-04-12 | V1+V2 | `done` | Completed the full Linux host validation queue (`LH1`-`LH6`) on Debian 13 x86_64. Updated the checked-in patch to target upstream crun `1.27` (was `1.22`). Built libkrun `1.17.4` from source (`~/src/github.com/containers/libkrun` tag `v1.17.4`) and libkrunfw `5.3.0` from source (`~/src/github.com/containers/libkrunfw` tag `v5.3.0`); both installed to `/usr/local/lib64/` with ldconfig entry at `/etc/ld.so.conf.d/libkrun.conf`. Built patched crun `1.27-dirty` with `+LIBKRUN` and installed at `/usr/libexec/nimbus/crun`. Runtime separation verified: system crun `1.21` at `/usr/bin/crun`, Podman `5.4.2` using system runtime. Fixed `scripts/prepare-krun-bundle.sh` to remove the `network` namespace (TSI requires host network) and set `terminal: false` for non-interactive krun drills. Direct `crun run` inside `buildah unshare` proved: krun VM boot, TSI port binding (`18080:8080`), and HTTP connectivity to BusyBox httpd via TSI. Conmon lifecycle drill inside `buildah unshare` proved: `conmon → crun → libkrun VM` process tree with 8 worker threads, TSI port binding, exit file written (code `137` from SIGKILL), attach socket at bundle directory. Key findings: (1) krun containers require no network namespace in the OCI config because TSI handles networking via vsock; (2) conmon `--full-attach` holds `crun start` until an attach connection arrives — production Podman handles this, but the drill needed manual `crun start`; (3) the krun handler writes `.krun_config.json` to the rootfs during `crun create` via `openat2`, which requires the rootfs to be writable from the creating namespace — `buildah unshare` provides this; (4) libkrun and libkrunfw are not packaged for Debian 13 and must be built from source. | `bash scripts/prepare-linux-vmm-validation-bundle.sh --crun-source ~/src/github.com/containers/crun --output-root /tmp/nimbus-linux-vmm-validation`; `bash scripts/verify-crun-patch.sh ~/src/github.com/containers/crun` (against `1.27`); `PKG_CONFIG_PATH=/usr/local/lib64/pkgconfig bash scripts/build-nimbus-crun.sh --source ~/src/github.com/containers/crun --output /tmp/nimbus-linux-vmm-validation/stage/crun --install-path /usr/libexec/nimbus/crun --sudo-install`; `bash scripts/verify-runtime-separation.sh --system-runtime /usr/bin/crun --private-runtime /usr/libexec/nimbus/crun`; `buildah unshare -- crun run --bundle /tmp/nimbus-linux-vmm-validation/bundle nimbus-http` (TSI port `18080` bound, HTTP `404` from BusyBox); `buildah unshare -- conmon ... + crun start nimbus-http` (process tree `conmon→libkrun VM`, exit file `137`); `cargo fmt --all --check` | Close out the script gaps exposed during `LH1`-`LH6` execution, then start V3 |
| 2026-04-12 | V1+V2 closeout | `done` | Closed the five script gaps exposed during `LH1`-`LH6` execution. (1) `scripts/build-nimbus-crun.sh` now auto-detects `libkrun.pc` in `/usr/local/lib64/pkgconfig` or `/usr/local/lib/pkgconfig` and sets `PKG_CONFIG_PATH` before `./configure --with-libkrun`, so the build no longer requires the caller to export the variable manually. (2) `scripts/check-vmm-host.sh` now checks for `libkrun.so` and `libkrunfw.so` via `ldconfig -p` and common non-standard paths, and verifies `libkrun.pc` via `pkg-config`. The distro-package checks for `libkrun`/`libkrunfw` are now informational (`optional`), so source-built installs no longer cause false failures. (3) `scripts/prepare-direct-krun-drill.sh` now emits a `buildah unshare` auto-re-exec preamble in the generated `run-runtime.sh`, so the krun handler's `openat2 .krun_config.json` succeeds in rootless mode without manual wrapping. (4) `scripts/prepare-conmon-krun-drill.sh` emits the same `buildah unshare` preamble in `run-conmon.sh`, plus a new `start-container.sh` script that polls `crun state` until the container reaches `created`, then calls `crun start` to boot the krun VM. This closes the conmon `--full-attach` lifecycle gap where the VM never started because nothing connected to the attach socket or called `crun start`. (5) Added `docs/research/krun-ci-build-and-distribution.md` capturing the full dependency map, source-build steps for libkrun/libkrunfw, GitHub runner requirements, key lifecycle learnings, and the gap between Debian (build from source) and Fedora (packaged) for CI targeting. All changed scripts pass `bash -n`, the three helper verifiers pass, and `cargo fmt --all --check` is clean. | `bash -n scripts/build-nimbus-crun.sh`; `bash -n scripts/check-vmm-host.sh`; `bash -n scripts/prepare-krun-bundle.sh`; `bash -n scripts/prepare-direct-krun-drill.sh`; `bash -n scripts/prepare-conmon-krun-drill.sh`; `bash scripts/check-vmm-host.sh` (reports `supported` on this host with source-built libkrun); `bash scripts/verify-krun-bundle-helper.sh`; `bash scripts/verify-direct-krun-drill-helper.sh`; `bash scripts/verify-conmon-krun-drill-helper.sh`; `cargo fmt --all --check` | Start V3 `nimbus-sandbox` krun backend implementation now that V1, V2, and the supporting script fixes have concrete Linux host evidence |
| 2026-04-12 | V3 | `in_progress` | Landed the first backend-owned `nimbus-sandbox` krun implementation slice. `SandboxSpec` now carries generic filesystem, process, and published-port intent via `SandboxFilesystemSpec`, `SandboxProcessSpec`, and `SandboxPortBinding`. `crates/nimbus-sandbox/src/backends/krun/` now contains `bundle.rs` for OCI config generation with `run.oci.handler=krun` and `krun.port_map`, `buildah.rs` for backend-local buildah command assembly and `buildah unshare` wrapping, `conmon.rs` for deterministic `conmon -> /usr/libexec/nimbus/crun` launch planning, `command.rs` for reusable backend-local command specs, and `vm.rs` for manifest-backed `start` / `inspect` / `stop` lowering behind the generic `SandboxBackend` trait. The backend now also distinguishes deliberate shutdown from unexpected crash semantics, so a forced stop can still surface as `SandboxStatus::Stopped`. The repo-owned Linux smoke path is now checked in via `crates/nimbus-sandbox/tests/krun_linux_smoke.rs` plus `docs/reference/krun-sandbox-backend-smoke.md`; the missing closeout step is running that path on a supported Linux host and writing the resulting connectivity, log-persistence, and restart-survival evidence back into this plan. | `cargo fmt --all --check`; `cargo check -p nimbus-sandbox -p nimbus`; `cargo test -p nimbus-sandbox` | Continue V3 on a supported Linux host by running the smoke test and recording evidence |
| 2026-04-12 | V3 | `done` | Ran the real Linux-host Rust backend smoke path on Debian 13 x86_64 and closed three issues in the process. (1) `bundle.rs` was missing the `mounts` block from the generated OCI config — `crun` requires at least the standard Linux mount set (`/proc`, `/dev`, `/dev/pts`, `/dev/shm`, `/dev/mqueue`, `/sys`, `/sys/fs/cgroup`). Added `default_linux_mounts()`. (2) The smoke test had a compile error: `guest_port.to_string()` returned `String` inside a `[&str; 5]` array literal. Fixed with a `let` binding. (3) The smoke test's HTTP probe used `read_to_string` after `shutdown(Write)` — TSI drops the vsock connection on half-close, producing empty reads. Fixed to use `HTTP/1.0`, no write-shutdown, and `read` with a fixed buffer and read timeout. After these three fixes the smoke test passes reproducibly in ~6s. Observed evidence: sandbox id `http-smoke-01kp1w86xbcyw96ph6f932asx3`, rootfs `/tmp/nimbus-sandbox-smoke-rootfs` (extracted BusyBox via `buildah unshare`), workdir `/tmp/nimbus-sandbox-smoke`, bundle at `workdir/bundles/<id>/config.json` with 7 mounts and `krun.port_map: 18080:8080`, conmon pid written, TSI port `*:18080` bound by `libkrun VM`, HTTP probe returned BusyBox httpd `404 Not Found` response, fresh `KrunSandboxBackend` instance recovered the running sandbox from the persisted manifest, stop succeeded with exit code `137` (SIGKILL), `ctr.log` and `oci.log` present under `workdir/state/containers/<id>/`. Phase-close verification: `cargo fmt --all --check`, `cargo check -p nimbus-sandbox -p nimbus`, `cargo test -p nimbus-sandbox` (9 pass), and the ignored smoke test all pass. V3 is complete: the Rust backend boots a real VM through the generic `SandboxBackend` trait, proves host-to-guest connectivity over TSI, survives backend restart via manifest recovery, and records stop/exit outcomes. The next promoted slice is `M1` from `docs/plans/microvm-runtime-plan.md`. | `cargo test -p nimbus-sandbox krun_backend_smoke_boots_http_service_and_survives_backend_restart -- --ignored --nocapture` with `NIMBUS_KRUN_SMOKE_ROOTFS=/tmp/nimbus-sandbox-smoke-rootfs NIMBUS_KRUN_SMOKE_WORKDIR=/tmp/nimbus-sandbox-smoke NIMBUS_KRUN_SMOKE_RUNTIME=/usr/libexec/nimbus/crun NIMBUS_KRUN_SMOKE_CONMON=/usr/bin/conmon NIMBUS_KRUN_SMOKE_BUILDAH=/usr/bin/buildah NIMBUS_KRUN_SMOKE_HOST_PORT=18080 NIMBUS_KRUN_SMOKE_GUEST_PORT=8080`; `cargo fmt --all --check`; `cargo check -p nimbus-sandbox -p nimbus`; `cargo test -p nimbus-sandbox` | Promote `M1` from `docs/plans/microvm-runtime-plan.md` now that V1, V2, and V3 are complete |
