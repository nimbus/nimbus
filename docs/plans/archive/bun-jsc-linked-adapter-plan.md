...
`docs/plans/archive/bun-jsc-embedder-api-and-pool-plan.md`.
...
- **Status:** complete; `BJA8` final verification passed locally and on
  Debian 13 `minicloud`
...
- **Current Bun proof source:** `nimbus/bun` tag `nimbus-bun-jsc-proof-main-20260525`
  (`ad0e1d2bbc6690651e04f10eaf1dcdf8a6c0de57`)
- **Predecessor:** `docs/plans/archive/bun-jsc-embedder-api-and-pool-plan.md`
...
| BJA6 | `done` | Enforce cancellation, teardown, and memory policy for untrusted Bun/JSC. | Gate 52 proves before-entry cancellation, after-entry sync-loop cancellation from the Bun source probe, promise/microtask progress, in-flight HostBridge cancellation, normal completion, teardown, fresh/discard after each untrusted invocation, and outer memory quota behavior. Focused local runtime checks passed, and the full loaded shared-adapter gate passed on Debian 13 `minicloud`. |
| BJA7 | `done` | Promote Bun/JSC selection into product metadata only after linked execution is real. | Gate 53 proves function-level `"use bun";` codegen, separate `bun_program_bundle.js` plus SHA-256 sidecar, V8/Node `bundle.mjs` separation, no-link fail-closed selection, linked-state diagnostics, and the server linked-lane diagnostics proof on Debian 13 `minicloud`. |
| BJA8 | `done` | Close the plan with reproducible verification, CI, docs, and source ownership. | Gate 56 records the final closeout. The source-owned Bun fork tag `nimbus-bun-jsc-proof-main-20260525` passed local macOS and Debian 13 source-backed linked gates, including exact source checks, 11 exported Nimbus ABI symbols, 0 leaked native defined symbols, strict Linux simdutf namespace audit, same-process V8 plus Bun/JSC tests, HostBridge allow/deny/forged-context/cancellation tests, linked diagnostics, and whitespace checks. Gate 55 records the broad local baseline. |
...
`docs/plans/archive/bun-jsc-linked-adapter-plan.md` autonomously. Treat the plan file,
...
| Complete `BJA5` HostBridge integration | `done` | Gate 51 proves Bun/JSC code can perform explicitly granted HostBridge operations through the same `HostBridge` seam as V8. Tests cover allowed operation, denied operation, forged tenant/context rejection, and no raw host token exposure inside guest code. The source-owned Bun fork tag `bun-v1.4.0-nimbus.4` points at `7c6dd4312e437c67a6c4c8cbb252f0d7ae898db8`, and the final BJA8 source contract is superseded by `nimbus-bun-jsc-proof-main-20260525` at `ad0e1d2bbc6690651e04f10eaf1dcdf8a6c0de57`. |
| Complete `BJA6` lifecycle and memory policy | `done` | Gate 52 proves before-entry cancellation, after-entry sync-loop cancellation from the Bun source probe, promise or microtask progress, in-flight HostBridge cancellation, normal completion, teardown, fresh/discard for untrusted invocations, and `outer_quota_required` diagnostics. Focused local runtime checks passed, and the full loaded shared-adapter gate passed on Debian 13 `minicloud`. |
| Complete `BJA7` product metadata and diagnostics | `done` | Gate 53 proves codegen/server artifacts express function-level Bun/JSC selection through `"use bun";`, no-link builds reject selection clearly, default V8/Node bundles exclude Bun handlers, `/debug/runtime/metrics` and operator diagnostics distinguish V8, Node LTS lanes, and Bun/JSC `linked` or `not_linked` state, and the linked server diagnostics proof passes on Debian 13. |
| Run disk/cache verification preflight | `done` | Gate 54 records local free space, `minicloud` free space, heavy local and remote caches, the local missing-WebKit environment gate, and the decision to reuse canonical worktrees plus home-backed `minicloud` proof caches instead of new throwaway checkouts or `/tmp` growth. |
| Complete `BJA8` final baseline | `done` | Gate 55 records passing `make check`, `make clippy`, `npm run typecheck`, `npm run test`, `npm run build`, `cargo fmt --all --check`, and `git diff --check`. Docs reference validation is unavailable because the npm script does not exist. Gate 56 resolves the local WebKit prerequisite, commits and pushes the source-owned Bun `.5` checkpoint, and records passing local macOS plus Debian 13 `minicloud` linked-adapter gates. |
...
| 2 | Checkpoint the proven BJA4L6 baseline cleanly. | Commit only owned Nimbus plan/runtime/verifier/proof files after the BJA4L6 proof passes. The final Bun fork source change is committed, tagged, and pushed as `nimbus-bun-jsc-proof-main-20260525`. Unrelated generated files, screenshots, package-lock churn, and unrelated plans remain unstaged. |
| 3 | Preflight disk and cache hygiene. | Local and `minicloud` free-space state is recorded before expensive gates. Heavy caches are reused where they are canonical, pruned only when safe, and final verification avoids new throwaway checkouts. `minicloud` linked proof uses `$HOME/.cache/nimbus-bun-proof/*` and not `/tmp`. |
| 4 | Re-run the broader baseline gates. | `make check`, `make clippy`, `npm run typecheck`, `npm run test`, `npm run build`, docs reference validation when available, and `git diff --check` pass from the checkpoint baseline. Disk-heavy verification uses the repo shared target or home-backed `minicloud` caches rather than `/tmp` or `/private/tmp` sprawl. |
| 5 | Make runtime diagnostics a contract. | `/debug/runtime/metrics` has API documentation or golden tests that lock lane state, `execution_adapter_state`, `executor_started`, and memory enforcement semantics. The contract distinguishes default V8, Node 20/22/24, and Bun/JSC. |
| 6 | Update operator-visible diagnostics. | The operator UI shows the same lane states as the backend: default V8, Node LTS lanes, Bun/JSC `not_linked` or `linked`, and Bun/JSC `outer_quota_required` memory semantics. |
| 7 | Add CI regression gates. | CI or a checked verifier proves Bun/JSC does not eagerly start executors, remains fail-closed when not linked, cannot leak Bun backend axes into V8/Node lanes, and rejects memory-policy mismatches. |
| 8 | Complete BJA5-BJA6 execution hardening. | End-to-end tests cover pure invocation, HostBridge allow, HostBridge deny, forged tenant/context rejection, cancellation, teardown, fresh/discard lifecycle, and memory policy locally and on `minicloud`. |
| 9 | Close BJA8 with broad verification. | `make verify-bun-jsc-runtime-contract`, `make verify-bun-jsc-linked-adapter`, the Debian linked proof, `make check`, `make clippy`, `npm run typecheck`, `npm run test`, `npm run build`, docs reference validation when available, and `git diff --check` pass. The plan and proof docs record the source tag/revision, residual risks, operator behavior, product behavior, and disk/cache hygiene before marking complete. |
...
Complete `docs/plans/archive/bun-jsc-linked-adapter-plan.md` from `BJA0` through
...
- Bun source ownership is reproducible: upstream release/tag or Nimbus-owned
  ...
  source is `nimbus/bun` tag `nimbus-bun-jsc-proof-main-20260525` at
  `ad0e1d2bbc6690651e04f10eaf1dcdf8a6c0de57`.
...
- Disk/cache hygiene is verified before broad gates: record local and
  `minicloud` free-space state, reuse canonical build caches where safe, and
  record local WebKit prerequisites and resolution instead of silently
  skipping source-backed linked verification.
...
| 2026-05-24 | BJA6-cancellation-teardown-memory | `done` | Nimbus rejects pre-cancelled Bun/JSC invocations before guest entry, records cancellation returned by the linked adapter while still completing teardown, maps HostBridge cancellation into guest-visible `cancelled` errors without raw tokens, proves microtask progress, and proves untrusted fresh/discard state reset. Memory remains an explicit outer-quota-required contract because the Bun source probe records heap behavior but not a hard JSC heap limit. | `cargo fmt --all --check` passed. `cargo test -p nimbus-runtime --lib backends::bun_jsc` passed 9 tests. `cargo test -p nimbus-runtime --features bun-jsc-linked-adapter --lib backends::bun_jsc` passed 12 tests. `RUSTFLAGS=--cfg=nimbus_bun_jsc_shared_adapter cargo test -p nimbus-runtime --features bun-jsc-linked-adapter --test bun_jsc_linked_adapter --no-run` compiled the loaded shared-adapter integration tests. `git diff --check` passed. On Debian 13 `minicloud`, the warmed `bash scripts/verify-bun-jsc-linked-adapter.sh` gate passed after applying the BJA6 patch: default no-link contract, 11 Bun exports, shared adapter build/export/leak/simdutf audit, 12 linked unit tests, 7 loaded shared-adapter integration tests, and Nimbus/Bun whitespace diff checks. The local Mac full linked verifier is environment-gated by the missing local WebKit checkout, so the loaded-adapter proof for this gate is the Debian source-backed run. | Start BJA7 product metadata and diagnostics promotion. |
| 2026-05-24 | BJA7-product-metadata-and-linked-diagnostics | `done` | Codegen now recognizes `"use bun";`, emits Bun/JSC metadata, writes a separate `bun_program_bundle.js` plus SHA-256 sidecar, and excludes Bun handlers from the V8/Node `bundle.mjs`. The server registry loads the Bun program bundle per selected function, no-link builds reject before guest execution, and runtime/server diagnostics report the actual linked or not-linked adapter state. | Gate 53 records local checks: `npm run test --workspace @nimbus/codegen`, `npm run typecheck --workspace @nimbus/codegen`, `cargo test -p nimbus-server registry_and_license::registry --lib` passed 15 tests, no-link fail-closed test passed 1 test, `cargo check -p nimbus-bin --features bun-jsc-linked-adapter`, `cargo fmt --all --check`, `bash -n scripts/verify-bun-jsc-linked-adapter.sh`, `git diff --check`, and `make verify-bun-jsc-runtime-contract` passed outside the sandbox. On Debian 13 `minicloud`, the full linked verifier passed: default no-link contract, 11 Bun exports, shared adapter build/export/leak/simdutf audit, 12 linked unit tests, 7 loaded shared-adapter integration tests, linked server diagnostics proof, and Nimbus/Bun whitespace checks. | Start BJA8 with disk/cache verification preflight, then run final broad verification. |
| 2026-05-24 | BJA8-disk-cache-preflight | `done` | Gate 54 records local and Debian disk/cache state before broad gates. Local Mac has about 201Gi available, local `target` is 58G, and local Bun source-backed linked verification remains environment-gated by missing `vendor/WebKit` and unset `BUN_WEBKIT_PATH`. `minicloud` has about 157G available, `/tmp` is only a 3.9G tmpfs, `/home/nimbus/.cache/nimbus-bun-proof` is 37G, and the proof worktree target is 35G. | Read-only `df`, `du`, WebKit, shared-adapter artifact, and source-revision checks passed locally and on `minicloud`. The final strategy is to reuse canonical local/remote targets and home-backed `minicloud` proof caches, avoid `/tmp`, and keep the Debian linked proof as the source-backed loaded-adapter evidence. | Run final BJA8 broad verification gates. |
| 2026-05-24 | BJA8-final-verification-progress | `superseded` | Gate 55 records final local broad verification progress after the BJA7 and preflight commits. `make check`, `make clippy`, `npm run typecheck`, `npm run test`, `npm run build`, `cargo fmt --all --check`, and `git diff --check` passed. A later repo-wide docs gate added `npm run docs:validate-refs:strict`, which now passes against tracked current Markdown docs. | Local `make verify-bun-jsc-linked-adapter` passed the default no-link contract, linked no-shared-library unit contract, Bun source check, and Bun Rust format, then stopped at `[5/11] Bun native shared adapter build` because `/Users/jack/src/github.com/nimbus/bun/vendor/WebKit` was absent and `BUN_WEBKIT_PATH` was unset. Debian source-backed linked proof remained green from Gate 53. | Superseded by Gate 56, which installs the local WebKit source prerequisite, fixes the macOS shared adapter link/export issues in the Bun fork, and closes BJA8. |
| 2026-05-24 | BJA8-final-closeout | `done` | Gate 56 closes the plan. The final source-owned Bun checkpoint is `nimbus-bun-jsc-proof-main-20260525` at `ad0e1d2bbc6690651e04f10eaf1dcdf8a6c0de57`, pushed to `nimbus/bun`. The `.5` patch removes executable-only macOS shared-link flags and Bun's executable export list from the shared adapter while preserving Linux symbol isolation. | Local `make verify-bun-jsc-linked-adapter` passed after installing local WebKit source at `782504c968e2ae06a511c9e7a4d48318b2a23263`: default no-link contract, 12 linked unit tests, 11 Bun exports, native shared adapter build, 11 exported Nimbus ABI symbols, 0 leaked native defined symbols, 7 linked integration tests, linked server diagnostics, and whitespace checks. Debian 13 `minicloud` passed the same 11-step gate with strict Linux symbol audit: 11 exported Nimbus ABI symbols, 0 leaked native defined symbols, simdutf namespace counts intact, 12 linked unit tests, 7 linked integration tests, linked server diagnostics, and whitespace checks. | Commit the BJA8 closeout and mark the autonomous goal complete after final local `cargo fmt --all --check`, `npm run typecheck`, and `git diff --check` pass. |
