...
- Refined 2026-04-01 after review: replica-local query or pagination evaluation
  now needs to remain schema-aware too, not just document-aware. The follow-up
  fix synchronizes schema state on catch-up even when there are no new durable
  mutation records and routes local evaluation through the same
  schema-and-principal-aware query helpers as the live service.
