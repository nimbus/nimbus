...
End-state honesty: even after NDB7 the live client is *default-constructed
on Linux and exercised by CI*, but **no production daemon calls it yet** —
`NodeWorkloadReconciler` still has no caller (TSB14 deferred; see "Why this
plan exists"). The trust delivered is "the binding provably works and is
guarded against regression," not "the binding is on a live request path."
Wiring a caller is the next plan's job.
...
| NDB1 | 1aaac8d6 | wire zbus_systemd workspace dep behind systemd-dbus feature |
| NDB2 | 4f50e98a | ZbusSystemdClient skeleton with bus selection + capability probe |
| NDB3 | 8fa707d1 | signal-correlated job completion via Manager.Subscribe + JobRemoved |
| NDB4 | 7a9986db | zbus error taxonomy → nimbus_core::Error mapping |
| NDB5 | 0559e993 | Linux-gated integration tests against systemctl --user |
| NDB6 | dbc33ef3 | CI lane: node-dbus-integration on ubuntu-24.04 |
| NDB7 | 103005d8 | activate systemd-dbus default + operator doc + closeout |
| NDB7 | 6050c28d | fix: idempotent Manager.Subscribe (found by live CI) |
...
Within each band: one commit, one entry in the Execution Log. This
plan executes on the isolated `node-dbus-binding` worktree branch (not
directly on `main`) to stay clear of concurrent work on `main`. CI is
the verification mechanism: the branch is pushed so the full suite —
including the `node-dbus-integration` lane that actually exercises the
live systemd binding on Linux — runs and goes green. A PR
(`node-dbus-binding → main`) is opened **as the last step**, only after
NDB1–NDB7 are landed, the local verifier reads `10 passed, 0 failed`,
and branch CI is green. See "Branch, CI, and PR workflow" below.

## Branch, CI, and PR workflow

- **Isolation.** All NDB work lands on the `node-dbus-binding` worktree
  branch. `main` is being actively churned by concurrent work; the
  branch keeps NDB conflict-free until the closeout PR.
- **CI is verification, not local compile.** NDB5's integration tests
  are gated on `target_os = "linux"` + `systemd-dbus-integration-tests`,
  and the dev host is macOS — they cannot run locally. The live binding
  is only *proven* by the `node-dbus-integration` lane going green on a
  pushed branch run. "Green local `make check`/`clippy`" is necessary
  but not sufficient; a green branch CI run is the real evidence.
- **PR as the last step.** After NDB1–NDB7 land, the local verifier is
  `10 passed, 0 failed`, and branch CI is green, open a PR
  `node-dbus-binding → main`. Never push NDB commits directly to `main`.
- **Base-dependency caveat.** NDB builds on `crates/nimbus-node/`, which
  exists only in the not-yet-on-`origin/main` node-extraction work this
  branch is based on. The closeout PR targets `main`; keep its reviewable
  scope to NDB by opening it once that base has reached `main` (or by
  scoping the comparison base accordingly), so the diff is the binding,
  not the unrelated extraction history.
- **Verifier vs. process.** The 10-condition verifier is the durable
  completion gate and stays passable on `main` after merge (condition 10's
  "CI green on main" becomes true once the lane runs there). The
  push → green-branch-CI → PR sequence is the integration process that
  the `/goal` drives; it is enforced by the goal, not encoded as an
  extra verifier condition.
