...
| gvproxy | Bundled with the neovex macOS archive/cask | Networking + port forwarding |
...
4. On Linux today: installs distro dependencies via apt/dnf, then installs
   released `neovex` + `neovex-crun` artifacts directly from GitHub Releases
5. On macOS today: installs or upgrades `agentstation/tap/neovex` via
...
```bash
# Download the released Neovex host binary bundle
curl -L -o neovex.tar.gz \
  https://github.com/agentstation/neovex/releases/download/v0.1.14/neovex_linux_x86_64.tar.gz

# Download the matching Linux private runtime separately
curl -L -o neovex-crun \
  https://github.com/agentstation/neovex-crun/releases/download/v1.27-neovex.1/neovex-crun-linux-amd64
...
The released `neovex` tarball includes `neovex`, `README.md`, and `LICENSE`.
On macOS it also includes `libexec/gvproxy`. The Linux private runtime
(`neovex-crun`) remains a separate release asset from `agentstation/neovex-crun`.
...
**neovex-crun release status:** `done` — `agentstation/neovex-crun`
publishes the Linux `neovex-crun-linux-amd64` and `neovex-crun-linux-arm64`
...
**Acceptance criteria:**
- a tagged `agentstation/neovex-crun` release exists and publishes
  `neovex-crun-linux-amd64` + `neovex-crun-linux-arm64`
- `git tag v0.1.14 && git push --tags` triggers neovex build
- Neovex release assets include `neovex_linux_x86_64.tar.gz`,
  `neovex_linux_arm64.tar.gz`, `neovex_darwin_arm64.tar.gz`,
...
| 2026-04-18 | D4a | `done` | Revalidated the shipped macOS distribution contract against the public `v0.1.14` release and the live Homebrew cask. The released `neovex_darwin_arm64.tar.gz` asset was downloaded, matched against the published `checksums-sha256.txt`, and confirmed to contain `neovex`, `README.md`, `LICENSE`, and `libexec/gvproxy`. The checked-in isolated proof harness then installed those exact bits under a temporary Homebrew tap/token at `/tmp/neovex-v0.1.14-homebrew-proof/run`, proved host `neovex 0.1.14`, `machine init`, `machine start`, guest SSH, guest `neovex 0.1.14`, forwarded machine API `reachable: true`, guest machine-API `HTTP/1.1 200 OK`, packaged `gvproxy`, and `/Users` virtiofs, then cleaned up the proof tap/cask. Finally, the real named cask path was refreshed with `brew update` and `brew upgrade --cask neovex`, moving the installed machine from `0.1.11` to `0.1.14`; `/opt/homebrew/bin/neovex --version` returned `neovex 0.1.14`, and the installed `neovex` plus `libexec/gvproxy` matched the downloaded release bytes exactly. Durable conclusion: Channel 4 is no longer just an internal proof lane; the published release archive, Homebrew tap metadata, and live operator upgrade path are aligned. | `gh release view v0.1.14 --repo agentstation/neovex --json tagName,assets,url`; `curl --fail -L -o /tmp/neovex-v0.1.14-homebrew-proof/release/checksums-sha256.txt https://github.com/agentstation/neovex/releases/download/v0.1.14/checksums-sha256.txt`; `curl --fail -L -o /tmp/neovex-v0.1.14-homebrew-proof/release/neovex_darwin_arm64.tar.gz https://github.com/agentstation/neovex/releases/download/v0.1.14/neovex_darwin_arm64.tar.gz`; `shasum -a 256 -c <(grep ' neovex_darwin_arm64.tar.gz$' /tmp/neovex-v0.1.14-homebrew-proof/release/checksums-sha256.txt)`; `env NEOVEX_MACHINE_API_READY_TIMEOUT_SECS=180 bash scripts/collect-neovex-homebrew-cask-proof.sh --output-dir /tmp/neovex-v0.1.14-homebrew-proof/run --host-binary /tmp/neovex-v0.1.14-homebrew-proof/release/unpack/neovex --gvproxy /tmp/neovex-v0.1.14-homebrew-proof/release/unpack/libexec/gvproxy`; `brew update`; `HOMEBREW_NO_AUTO_UPDATE=1 brew upgrade --cask neovex`; `/opt/homebrew/bin/neovex --version`; `diff -q /tmp/neovex-v0.1.14-homebrew-proof/release/unpack/neovex /opt/homebrew/Caskroom/neovex/0.1.14/neovex`; `diff -q /tmp/neovex-v0.1.14-homebrew-proof/release/unpack/libexec/gvproxy /opt/homebrew/Caskroom/neovex/0.1.14/libexec/gvproxy` | Keep Channel 4 stable, then implement Channel 1 as a bootstrapper that reuses this Homebrew path on macOS and the existing release artifacts on Linux instead of inventing a second macOS install mechanism |
