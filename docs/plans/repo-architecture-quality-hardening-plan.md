...
| RAQ9 | `done` | Added an enterprise evidence taxonomy pass across audit, error reasons, deny reasons, and operator diagnostics. Tenant-isolation events now expose stable `nimbus.tenant_isolation.<kind>.<result>` event names in OCSF/OTel exports, reason codes are canonicalized and redacted when non-machine-readable, and external policy evidence carries stable reason codes while redacting backend-supplied credential text. | `cargo check -p nimbus-server`, `cargo test -p nimbus-server tenant_isolation -- --nocapture` (116 passed), `cargo test -p nimbus-server operator_policy -- --nocapture` (25 passed), `bash scripts/verify-enterprise-policy-egress.sh` outside the local sandbox (8 gates passed, including 6 audit export/redaction tests), and `cargo fmt --all --check` passed. |
| RAQ10 | `done` | Final documentation and verification closure. Updated the architecture index and active plan status to reflect the new concept-owned module map, canonical server construction seams, guardrail script, and public compatibility surfaces. | `cargo fmt --all --check`, `make check`, `make clippy`, `npm run typecheck`, `npm run test`, `npm run build`, `./scripts/verify-repo-architecture-quality.sh`, `bash scripts/verify-enterprise-policy-egress.sh`, `bash scripts/verify-artifact-provenance.sh`, `bash scripts/verify-bun-jsc-runtime-contract.sh`, targeted local-file docs reference checks, and `git diff --check` passed. `npm run docs:validate-refs:strict` remains unavailable because this checkout has no such npm script. |
...
| 2026-05-26 | RAQ9 | `done` | Added `tenant_isolation/evidence.rs` as the enterprise evidence taxonomy helper. Tenant-isolation audit events now export stable names such as `nimbus.tenant_isolation.host_bridge_operation.denied`, include `nimbus.event.name` in exported attributes, canonicalize non-machine-readable reason codes to redacted fallback codes, and preserve decision/workload/correlation fields without leaking credential-bearing text. Operator external-policy evidence now records `external_policy_allowed`/`external_policy_denied` reason codes and redacts backend-supplied bearer, token, query-string, secret-handle, credential, and private-key text before summaries or fail-closed errors. Verification passed: `cargo check -p nimbus-server`; `cargo test -p nimbus-server tenant_isolation -- --nocapture` (116 passed, including 21-scenario conformance and new taxonomy/redaction tests); `cargo test -p nimbus-server operator_policy -- --nocapture` (25 passed); `bash scripts/verify-enterprise-policy-egress.sh` outside the local sandbox after the expected socket-bind denial in the sandboxed rerun (8 gates passed, including 6 audit export/redaction tests); and `cargo fmt --all --check`. |
| 2026-05-26 | RAQ10 | `done` | Closed the architecture-quality wave. `docs/architecture/README.md` now records the concept-owned module map, canonical server construction seams, guardrail script, and compatibility-surface posture. `docs/plans/README.md` marks this plan as a completed baseline. Final verification passed: `cargo fmt --all --check`; `make check`; `make clippy`; `npm run typecheck`; `npm run test` (including 42 UI test files and 278 UI tests); `npm run build`; `bash scripts/verify-repo-architecture-quality.sh`; `bash scripts/verify-enterprise-policy-egress.sh` outside the local sandbox (8 gates passed); `bash scripts/verify-artifact-provenance.sh` (5 gates passed); `bash scripts/verify-bun-jsc-runtime-contract.sh` outside the local sandbox after the expected listener-bind denial in the sandboxed rerun (7 gates passed); targeted local-file docs reference checks for this plan's current posture references; and `git diff --check`. `npm run docs:validate-refs:strict` remains unavailable because the root package has no such script. |
