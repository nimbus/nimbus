...
- 2026-04-13: Ran M5/SCP5 Linux-host verification on Debian 13 x86_64.
  Fixed compilation: made `RuntimeServiceRegistry` trait public, re-exported
  from facade, fixed imports in `neovex-bin`.
  ...
  tenant deletion teardown.
  Recovery drill (`bash scripts/verify-microvm-m5-recovery-drill-helper.sh`)
  passed: no leaked ports, no orphan processes, manifest persists (stopped,
  shutdown_requested: true), logs persist, project layout
  `smoke-app-444d8e7fafaa` intact.
  Artifact paths:
  `/tmp/neovex-sandbox-smoke/m5-compose-serve-verification/compose-serve.log`,
  `/tmp/neovex-sandbox-smoke/m5-compose-serve-verification/summary.txt`,
  `/tmp/neovex-sandbox-smoke/m5-recovery-drill/summary.txt`,
  `/tmp/neovex-sandbox-smoke/m5-compose-control/services/projects/smoke-app-444d8e7fafaa/`.
  M5 is now done. All phases M1 through M5 are complete.
