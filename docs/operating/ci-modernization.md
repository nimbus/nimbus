...
The `Bun/JSC Runtime Contract` and `Rust Workspace Tests` jobs
intentionally override the Linux target `RUSTFLAGS` back to
`-C link-arg=-fuse-ld=lld`. Those gates link OpenSSL-backed test
binaries with vendored OpenSSL, and the ubuntu-24.04 mold lane has
failed there with an unresolved OpenSSL assembly symbol
(`bn_sqrx8x_internal`). Keep mold as the workspace default for CI
acceleration, but keep these lanes on lld unless a future runner and
mold version are explicitly reproven against the OpenSSL-backed test
suite.
