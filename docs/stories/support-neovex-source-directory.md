# Story: Native `neovex/` source roots with `convex/` compatibility

## Summary

As a Neovex user, I want to place my app source in a first-party `neovex/`
directory without losing support for upstream-style `convex/` projects, so
that Neovex-native apps look like Neovex apps while Convex-compatible apps
still work unchanged.

This story is the first user-facing step in that direction:

- `neovex/` becomes the preferred native source root
- `convex/` remains a supported compatibility source root
- internal runtime artifacts stay in `.neovex/convex/` for now

The current repo already ships both a Neovex-native JS surface
(`neovex/server`, `neovex/browser`, `neovex/react`, `neovex/values`) and the
...
makes Neovex-native apps look like compatibility apps even though the repo
already exposes a separate `neovex/*` package namespace.

That creates three problems:

1. New Neovex projects cannot use a first-party `neovex/` folder convention.
2. Generated `_generated/*` files inherit Convex branding even for Neovex
   source trees.
3. The public contract is muddy: the repo appears to support a Neovex-native
...
| Internal runtime artifacts | emitted into `{appDir}/.neovex/convex/` |
...
1. If both `{appDir}/neovex/` and `{appDir}/convex/` exist, use `neovex/` and
   emit a user-facing informational message explaining that both were detected
   and `neovex/` was selected.
2. Else if `{appDir}/neovex/` exists, use it.
3. Else if `{appDir}/convex/` exists, use it.
4. Else fail with a clear error naming both supported directories.

If both directories exist, `neovex/` wins. That keeps the native Neovex path
...
| --- | --- | --- |
| `{appDir}/neovex/` | `{appDir}/neovex/_generated/` | `neovex/*` |
...
- Add first-class source-root detection for `neovex/` and `convex/`
- Emit `_generated/*` under the selected source root
- Make generated imports match the selected package namespace
- Add tests for native Neovex mode, compatibility mode, both-roots-present
  behavior, and missing-root errors
- Update user-facing help text and docs so they describe the new contract

## Out of scope

- Renaming `.neovex/convex/` internal artifacts
- Renaming the Rust `--convex-app-dir` flag
- Adding a top-level `neovex codegen` command
- Adding a manual `--source-dir` selector flag
- Shipping Neovex-only runtime capabilities beyond the directory and generated
  code contract
...
async function resolveSourceRoot(appDir) {
  ...
  const neovexExists = await pathExists(neovexDir);
...
consumers of `@neovex/codegen` from getting unexpected console output while
...
- `internalDir` from `{appDir}/.neovex/convex`
...
```js
import { makeQueryReference } from "neovex/browser";
export { query, mutation } from "neovex/server";
import type { GenericId } from "neovex/values";
```

This is what makes a `neovex/` source tree feel genuinely native instead of
...
1. `neovex/` root only
   - codegen succeeds
   - `_generated/*` lands in `neovex/_generated/`
   - internal artifacts still land in `.neovex/convex/`
   - generated files import from `neovex/*`
...
3. both roots present
   ...
   - the CLI emits a short informational message saying both roots were found
     and `neovex/` will be used
   - generated files land under `neovex/_generated/`

4. neither root present
   - codegen fails with the explicit error message

At least one native-mode fixture should author user code with `neovex/server`
and `neovex/values` imports so we prove the mode end to end rather than only
...
tests are validating the compatibility surface, not the Neovex-native mode.
...
- describe `--convex-app-dir` as an app directory whose runtime artifacts are
  generated under `.neovex/convex/`, while the user source root may be either
  `convex/` or `neovex/`
...
| `packages/codegen/src/emit/generated_files.mjs` | emit `neovex/*` or `convex/*` imports based on selected mode |
...
| `crates/**/*.rs` | still consume `.neovex/convex/`, which stays unchanged in this story |
...
1. `convex codegen --app <dir>` succeeds when `<dir>/neovex/` exists and
   `<dir>/convex/` does not.
2. `convex codegen --app <dir>` succeeds when `<dir>/convex/` exists and
   `<dir>/neovex/` does not.
3. `neovex-codegen --app <dir>` succeeds with either supported source root.
4. When both roots exist, `neovex/` is selected.
5. When both roots exist, the CLI emits a short informational message saying
   both roots were detected and `neovex/` was selected.
