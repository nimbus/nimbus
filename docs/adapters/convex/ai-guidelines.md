...
- System fields are automatically added to all documents and are prefixed with an underscore. The covered system fields are `_creationTime` with validator `v.number()`, `_updateTime` with validator `v.number()`, and `_id` with validator `v.id(tableName)`.
