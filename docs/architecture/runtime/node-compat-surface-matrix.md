...
| `globalThis.Buffer` | unsupported | supported | supported by same bootstrap contract | `runtime::tests::basic_invocation::web_standard_target_does_not_expose_node_globals`, `runtime::tests::basic_invocation::node22_target_exposes_minimal_node_globals`, `runtime::tests::node_compat::node22_primary_lane_executes_manifested_core_semantics_subset`, `docs/architecture/runtime/node-lts-compat/manifests/core-semantics.md` |
...
| Core `process` metadata / warning / scheduling subset: `process.release`, `process.features`, `process.uptime()`, `process.nextTick()`, `process.emitWarning()`, and `process.on("warning")` | unsupported | supported for the currently manifested `NLC4` subset | supported by same bootstrap contract | `runtime::tests::node_compat::node22_primary_lane_executes_manifested_process_and_timing_subset`, `runtime::tests::node_compat::node20_validation_lane_executes_official_process_and_timing_subset`, `runtime::tests::node_compat::node20_process_features_watchpoint`, `docs/architecture/runtime/node-lts-compat/manifests/process-and-timing.md`, `docs/architecture/runtime/node-lts-compat/failures/process-and-timing.md` |
| `process.stdout` / `process.stderr` writable stream objects | unsupported | supported | supported by same bootstrap contract | `runtime::tests::basic_invocation::node22_target_exposes_minimal_node_globals`, `runtime::tests::node_compat::node22_primary_lane_executes_manifested_core_semantics_subset`, `docs/architecture/runtime/node-lts-compat/manifests/core-semantics.md` |
| `process.on("warning")` delivery for `emitWarning()` and `require("punycode")` deprecations | unsupported | supported | supported by same bootstrap contract | `runtime::tests::basic_invocation::node22_target_delivers_manual_process_warning_events`, `runtime::tests::basic_invocation::node22_target_delivers_process_warning_events_for_deprecated_modules`, `runtime::tests::node_compat::node22_primary_lane_executes_manifested_core_semantics_subset`, `docs/architecture/runtime/node-lts-compat/manifests/core-semantics.md` |
...
- most `node:` builtin usage beyond the verified `node:stream`, bundle-root-safe `node:fs`, `node:readline`, `node:tty`, `node:os`, `node:module`, `node:path`, the core semantics slice above, and tooling-profile `node:child_process` / `buffer` / `crypto` surfaces
...
- The carried loader-context denominator now also includes the first
  Node22-primary `node:domain` foundation wave: add/remove behavior, timer and
  ...
  primary lane. This is intentionally lane-scoped for now; Node20 and Node24
...
- The carried loader-context denominator now also includes the first
  Node22-primary `node:sqlite` foundation subset:
  ...
  and the checked-in sqlite build profile widened the bundled SQL function
...
- The carried loader-context denominator now also includes the first
  Node22-primary `node:sea` non-SEA contract:
...
- The carried loader-context denominator now also includes the first
  Node22-primary `node:test` helper, context-metadata, `run()`
  ...
  Node22-primary `node:test` reporter-edge, reporter-output, CLI-options,
  ...
  support claim.
- The carried loader-context denominator now also includes the first
  Node22-primary pure `repl.start()` foundation batch:
...
- The carried loader-context denominator now also includes the first
  Node22-primary `node:wasi` validation wave, the first executable wave, the
  narrower Node22-primary argv contract, the first filesystem wave, and the
...
- The first pinned Node20 validation lane is also green for the paired
...
- The ignored Node24 preview lane now stages a `160`-file official
  `nodejs/node v24.15.0` subset, but it is not a green claim: the explicit
  preview run still aborts early through a `rusty_v8` weak-handle panic near
...
- The `NLC3` failure inventory now also records the active official-runtime
  watchpoints instead of burying them in red lanes: shared `test-assert-deep.js`
...
- The first explicit split-LTS assert bodies now cover
  ...
  public application-profile `process.env` contract.
...
- The completed `NLC10` Tooling canary registry now also carries pinned package
  ...
  primary lane. These are trust-layer evidence lanes, not a blanket claim that
  every package feature is fully supported in `RuntimeProfile::Tooling`; consult
