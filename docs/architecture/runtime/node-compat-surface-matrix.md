...
| Core `process` metadata / warning / scheduling subset: `process.release`, `process.features`, `process.uptime()`, `process.nextTick()`, `process.emitWarning()`, and `process.on("warning")` | unsupported | supported for the currently manifested `process-and-timing` subset | supported by same bootstrap contract | `runtime::tests::node_compat::node22_default_lane_executes_manifested_process_and_timing_subset`, `runtime::tests::node_compat::node20_supported_lane_executes_official_process_and_timing_subset`, `runtime::tests::node_compat::node20_process_features_watchpoint`, `docs/architecture/runtime/node-lts-compat/manifests/process-and-timing.md`, `docs/architecture/runtime/node-lts-compat/failures/process-and-timing.md` |
...
- The verified core-semantics row above is also intentionally narrow: it
  ...
  subpaths, but it does not yet close the whole `core-semantics` family or imply a
  Node-upstream pass-rate claim on its own.
- The current pinned upstream `core-semantics` family subset from `nimbus/deno`
...
- The first pinned Node20 supported lane is also green for the paired
  ...
  slices are documented in the `core-semantics` family failure inventory because those files are
...
- The `core-semantics` family failure inventory now also records the active official-runtime
...
- The remaining-file inventory is now explicit: no imported public-core
  official files remain unstaged for the `core-semantics` family. The only remaining tracked items
...
- The same source-first batching rule now also covers
  ...
  pinned in the `core-semantics` family failure inventory instead of being counted green in
  either lane.
...
- The completed validation and closeout phase Tooling canary registry now also carries pinned package
