...
| Network identity, plans, port or segment leases, attachment lifecycle, provider capability evidence, sovereignty, or reconciliation | [`network/control-plane.md`](network/control-plane.md), then the completed [`../plans/archive/nimbus-network-control-plane-plan.md`](../plans/archive/nimbus-network-control-plane-plan.md) | Keep portable connectivity-resource authority separate from concrete effects, policy, naming, projections, and cluster transport. |
...
| Horizontal scaling, cluster transport, node identity, placement, gossip invalidation, or cluster-mode integration | [`../plans/horizontal-scaling-plan.md`](../plans/horizontal-scaling-plan.md) | The design document referenced by that deferred plan is not present in this checkout; the plan remains the current routing owner. Single-node remains the launch baseline. |
...
| [network/](network/) | `nimbus-network` | Transport-free connectivity-resource identity, plans, leases, state, capability evidence, and reconciliation |
