...
| Runtime manager (`nimbus-compute`) | Canonical runtime configuration, lane executors, Engine-issued owner lowering, deployment authority, cancellation, worker retirement, and diagnostics | Engine tenant runtime lease plus admitted runtime projection | Mint tenant incarnation, derive ownership from a label/route/bundle, or let an adapter own executor lifecycle |
| Runtime primitives (`nimbus-runtime`) | V8/Deno/Node/Bun execution mechanics, owner-partitioned retained state, heap/pool primitives, runtime host-call boundary | Manager-issued runtime owner/deployment leases plus exact runtime authority | Reuse across owner incarnations, treat routing as authority, or decide tenant access from runtime context |
...
Mutable retained substrates carry a mandatory runtime owner independently of
their routing affinity. Tenant owners combine owner class, stable Engine subject,
and Engine/storage incarnation. Inside that owner partition, exact deployment,
bundle, runtime-shape, permission, capability/service, and construction
authority must match. V8 and Wasmtime share this contract; Bun/JSC retained mode
cannot become product-selectable without it.

Runtime pools also carry a trust classification. Exposure to any of the following
...
Tenant deletion is a single lifecycle chain: Engine fences new operations and
holds the load/recreation gate; the compute runtime manager revokes the exact
owner, cancels runtime work, and obtains retirement acknowledgements from every
lane worker; services are torn down; Engine drains any remaining operation
leases and completes storage deletion. A timeout is fail-closed and retryable.
Deployment replacement retires a distinct deployment authority so old work can
drain without admitting its retained state into the new generation.
...
- runtime-owner tests prove different subjects with equal numeric incarnations,
  same subject with a new incarnation, `None`/unscoped-script routing,
  V8/Wasmtime retention, queued/active/background races, and served
  delete/recreate all fail closed
