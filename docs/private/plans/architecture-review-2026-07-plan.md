...
Next action: Execute SMR0 in `storage-metadata-retention-plan.md`: author its
contract verifier, capture the exact fail-before result, and ratify the bounded
shipped profile before production behavior changes. SA4 remains the only active
Band SA task. Keep the dirty IMV7 closeout worktree unchanged.
