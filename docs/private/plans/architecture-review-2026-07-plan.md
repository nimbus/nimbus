...
Next action: Execute SMR1 in `storage-metadata-retention-plan.md`: add the
provider-neutral checkpoint and desired/confirmed/physical floor contract,
then implement atomic memory, redb, and SQLite compaction plus nonzero-base
PITR. SA4 remains the only active Band SA task. Keep the dirty IMV7 closeout
worktree unchanged.
