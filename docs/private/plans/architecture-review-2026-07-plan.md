...
Next action: Execute SA2 in a dedicated `codex/sa2-convex-object-surface`
branch and `nimbus-worktrees/sa2-convex-object-surface` worktree. Prove and
repair concurrent logical-file manifest ownership and the O(n) full-bucket
operation path. SA2 is the only active Band SA task. Keep the dirty IMV7
closeout worktree unchanged.
...
| SA2 | Close the two convex.rs object-surface mediums: a fresh ULID per store orphans a manifest when the same logical file is stored concurrently, and every operation performs an O(n) full-bucket list. | `crates/nimbus-s3/src/convex.rs:333-336,534-546` | medium / confirmed | in_progress |
