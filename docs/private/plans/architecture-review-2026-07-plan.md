...
| CO13 | De-duplicate the three Convex client classes: extract one coerce-and-delegate helper shared by `ConvexHttpClient`/`ConvexClient`/`ConvexReactClient` (~150 identical lines ×3). | `packages/convex/src/browser.ts:161,288,427` | small | implemented (js-lane, 2026-07-07: `withConvexInvocationMethods`/`withConvexLiveQuery` mixins composed onto the three Nimbus base classes — one body per method regardless of client count; public API byte-compatible; typecheck/test/build green; uncommitted in `.worktrees/js-lane`, PR pending) |
| CO14 | Split the control-plane `Nimbus` class + ~50 `NimbusService*`/`NimbusSandbox*`/`NimbusSession*` types out of the SDK barrel into `./control-plane`; keep `index.ts` a thin barrel (1,003 lines today). | `packages/nimbus/src/index.ts:32-433` | small | implemented (js-lane, 2026-07-07: `packages/nimbus/src/control-plane/{types,client,discovery,index}.ts`; `index.ts` now a 2-line barrel; added `./control-plane` export map entry + `entryContracts` surface-contract row; `assertNimbusRootSdkBoundary()` updated to span `client.ts`+`discovery.ts` post-split; all 5 required gates green; uncommitted in `.worktrees/js-lane`, PR pending) |
...
| DE16 | Stop committing generated Firestore protobuf (15.7k lines under `packages/firebase/src/gen/`): generate at build time via the existing `@bufbuild/protoc-gen-es` dev dep, gitignore `gen/`. | `packages/firebase/src/gen/**` | medium | implemented (js-lane, 2026-07-07: `@bufbuild/buf generate` replaces raw protoc; wired into `build`/`test`/`typecheck` via `codegen:proto` pre-step; `git rm -r` of 25 committed `_pb.ts` files staged, `gen/` gitignored; added scripted double-generate determinism check (`selftest/codegen_determinism.mjs`, sha256-diffs two disposable generate runs, wired into `selftest.mjs` so it runs under `npm run test`/`typecheck` in CI); CI check: `ci.yml`'s `ui-artifacts` and `js` jobs both regenerate fresh before any consumption, no lane consumes stale/committed output; `dual-target-nightly.yml`'s "firebase" adapter only does raw HTTP probes, never imports the package; all 5 required gates green; uncommitted in `.worktrees/js-lane`, PR pending) |
