...
Next action: Execute SMR3 in `storage-metadata-retention-plan.md`: wire the
bounded shipped profile and single-flight maintenance controller into the
production Engine tenant lifecycle, with ordered finalization, cancellation,
retry, diagnostics, metrics, and a manual operator trigger. SA4 remains the
