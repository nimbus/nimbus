...
Next action: Execute SMR5 in `storage-metadata-retention-plan.md`: qualify the
retention contract across generated histories, crash/restart faults, all six
backends, measured budgets, documentation, and repository gates. SA4 remains
the only active Band SA task. Keep the dirty IMV7 closeout worktree unchanged.
