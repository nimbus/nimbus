...
| CP2 | Split `AppState`: `ComputeState` (engine + registries, zero axum types) lives in nimbus-compute; server `AppState` wraps it plus transport concerns. `construction.rs` splits the same way. | `nimbus-server/src/{state.rs,construction.rs}` | medium | in_progress (PR #161 open 2026-07-08; ComputeError+ComputeState per amended spec, Deref zero-churn, autoreview clean, gates independently verified post-rebase) |
