...
Next action: Band SA is complete. Keep its proof ledger closed and return future
work to the owning plan map. Keep BLI `proposed`; completed prerequisite BLI-SA6
does not start BLI0 or the whole BLI campaign. RR30 remains deferred until BLI3
merges; RR31 is outside this Band SA goal.
...
| 2026-08-25 | SA8 | completed | PR #310 merged as `2e8d744ea`. Scalar `SpecialDouble::NegativeZero` now keeps its sign bit in the digest; GeoPoint coordinates still normalize both zero spellings. Proof: `proof/architecture-review-2026-07/sa8-negative-zero-digest.md`. |
| 2026-08-25 | SA1 | started | Accepted the three S3 wire repairs and their conformance lane as the next high task. SA1 is the only active Band SA task. |
| 2026-08-25 | SA1 | completed | PR #311 merged as `d5edda726`. Pagination now uses the last consumed key, DeleteObject conditions are decided inside the tenant committer, supplied multipart CRC64NVME is recomputed or rejected, and unsupported stock-SDK checksum defaults are documented as a fail-closed compatibility decision. Proof: `proof/architecture-review-2026-07/sa1-s3-wire.md`. |
| 2026-08-25 | SA3 | started | Accepted the embedded process fence as the next high task. Bootstrap must lock every distinct local persistence or control root before opening providers and retain the guards for the Engine lifetime. SA3 is the only active Band SA task. |
| 2026-08-25 | SA3 | completed | PR #312 merged as `c4f9a29a3`. Every Engine bootstrap now locks its canonical local roots before provider open, including encrypted redb and libSQL replica caches; contention is a machine-readable `Busy` error. Local `make ci` and Nimbus autoreview passed. Proof: `proof/architecture-review-2026-07/sa3-process-fence.md`. |
| 2026-08-25 | SA4 | started | Accepted the unbounded commit-log, unwired MVCC pruning, and genesis-only PITR cost as one metadata-retention outcome. A dedicated owner plan must define and execute the contract before this row can close. SA4 is the only active Band SA task. |
| 2026-08-25 | SA4 / SMR | promoted | Promoted `storage-metadata-retention-plan.md` at baseline `cc7ae36a3`. The plan owns checkpoint-before-delete, four durable windows, Engine lifecycle wiring, trimmed-history validation, and six-backend qualification. SMR0 is the only active task; no production behavior changed. |
| 2026-08-25 | SA4 / SMR0 | completed | Baseline PR #313 merged as `0ff18d1a7`: verifier `7 passed, 11 failed`; capacity calibration and bounded profile are recorded in `proof/storage-metadata-retention/smr0-baseline.md`. SMR1 now owns the checkpoint and embedded compaction seam. |
| 2026-08-26 | SA4 / SMR1 | completed | PR #314 merged as `0d4b9a112`: the provider-neutral checkpoint, desired/confirmed/physical floors, embedded atomic compaction, and nonzero-base PITR are proved in `proof/storage-metadata-retention/smr1-embedded-checkpoint.md`. Verifier advanced to 12/6. |
| 2026-08-26 | SA4 / SMR2 | completed | PR #317 merged as `f97b2db67`: PostgreSQL, MySQL, and libSQL now finalize retained history in one lease-fenced provider transaction; nine focused live tests cover stale generations, exact restart floors, injected and native-SQL rollback, MVCC pruning, and libSQL cache rebuild. Verifier advanced to 13/5. Proof: `proof/storage-metadata-retention/smr2-provider-parity.md`. SMR3 now owns production lifecycle wiring. |
| 2026-08-26 | SA4 / SMR3 | completed | PR #319 merged as `820549575`: the bounded shipped profile, explicit retain-all mode, one controller per loaded tenant, ordered finalization, shutdown drainage, operator trigger, diagnostics, metrics, and late-pin guard are proved in `proof/storage-metadata-retention/smr3-production-lifecycle.md`. Verifier advanced to 16/2. SMR4 now owns fail-closed paged consumers. |
| 2026-08-26 | SA4 / SMR4 | completed | PR #320 merged as `e6b9fcb05`: durable per-resource read floors, process-local publication, pre/post-page validation, contiguous journal checks, typed Engine recovery, and deterministic concurrent-prune tests cover memory, redb, SQLite, PostgreSQL, MySQL, and libSQL. Verifier advanced to 17/1. Proof: `proof/storage-metadata-retention/smr4-consumer-safety.md`. SMR5 now owns qualification and the launch-readiness verdict. |
| 2026-08-26 | SA4 / SMR5 | started | Accepted the final generated-history, crash/restart, six-backend, benchmark, documentation, repository-gate, and review matrix. SA4 remains the only active Band SA row until SMR5 and SMR9 close it with a measured `SAFE` or `NOT SAFE` verdict. |
| 2026-08-26 | SA4 / SMR5 | completed | PR #321 merged as `4fe11923d`. Generated and fault matrices passed; PostgreSQL 84/84 and MySQL 54/54 passed; every libSQL retention case passed. Steady-state latest-path overhead was 2.81% and bounded redb plateaued at 4.75 MB versus 34.22 MB retain-all. `make ci`, docs, and final autoreview passed. Verifier reached 18/0; verdict `SAFE`. SMR9 now owns plan cleanup. |
| 2026-08-26 | SA4 / SMR9 | started | Accepted terminal cleanup. SA4 remains the only active Band SA row until the SMR plan is archived and routing points to the next accepted SA item. |
| 2026-08-26 | SA4 / SMR9 | completed | Archived `storage-metadata-retention-plan.md`, retained its proof root, removed its active index entry, and closed SA4 with PRs #313–#321, verifier 18/0, and the measured `SAFE` verdict. |
| 2026-08-26 | SA2 | started | Accepted the confirmed Convex object-surface repairs as the next independent Band SA item. SA2 is the only active row; it must prove stable logical-file ownership under concurrent stores and remove the O(n) bucket-wide lookup path without expanding BLI scope. |
| 2026-08-26 | SA2 | completed | PR #322 merged as `dfa00606e`. Each `ConvexStorageId` now owns one deterministic manifest slot; concurrent imports replace that slot, point operations do not scan the bucket, and export keeps its intentional enumeration path. Full Node 24 `make ci` and Nimbus autoreview passed. Proof: `proof/architecture-review-2026-07/sa2-convex-object-surface.md`. |
| 2026-08-26 | SA5 | started | Accepted erasure parity byte stability as the next independent Band SA item. SA5 is the only active row; it must document the durable `reed-solomon-simd` pin and prove exact parity bytes with a checked-in golden vector, including a non-64-divisible shard size. |
| 2026-08-26 | SA5 | completed | PR #323 merged as `ad43a052c`. NBLE1 now records its exact `reed-solomon-simd = 3.1.0` codec commitment and a checked-in k=3/m=2 golden vector pins both parity shards at Nimbus's accepted 22-byte non-64 boundary. Focused blob tests, repository gates, and Nimbus autoreview passed. Proof: `proof/architecture-review-2026-07/sa5-erasure-codec-stability.md`. |
| 2026-08-26 | SA9 | started | Accepted format-version validation as the next independent Band SA item. SA9 is the only active row; it must reject unsupported journal record versions before replay and preserve a non-corruption version-skew classification at every deployment-crossing format reader in scope. |
| 2026-08-26 | SA9 | completed | PR #324 merged as `c967908bc`. Journal records reject unsupported versions before integrity validation; recognized object-backup and framed-AEAD version skew is operator-legible `InvalidInput`, while unrecognized magic remains corruption. Proof: `proof/architecture-review-2026-07/sa9-format-version-validation.md`. |
| 2026-08-26 | SA7 | started | Accepted the archive restore message repair as a post-IMV diagnostic rider under Band SA. SA7 is the only active row; it must validate the outer container version before nested payload decode and name the digest-codec incompatibility without changing the archived IMV plan. |
| 2026-08-26 | SA7 | completed | PR #325 merged as `8ca40cc21`. Whole-deployment backups, tenant PITR archives, and object-backup manifest snapshots now validate their owning container version before nested materialized-position decode; old archives identify the digest-codec incompatibility. Proof: `proof/architecture-review-2026-07/sa7-archive-version-diagnostics.md`. |
| 2026-08-26 | SA10 | started | Accepted the missed server-side digest assertions and operator visibility for the report-semantics consistency endpoint as the next post-IMV rider. SA10 is the only active Band SA row; the archived IMV plan and dirty IMV7 closeout worktree remain unchanged. |
| 2026-08-26 | SA10 | completed | PR #326 merged as `56682f558`. The server route now asserts and compares serialized materialized positions, emits structured INFO/WARN verification outcomes, and documents the warning event as an alert hook. Proof: `proof/architecture-review-2026-07/sa10-consistency-observability.md`. |
| 2026-08-26 | SA6 / BLI-SA6 | started | The BLI owner accepted error classification as a pre-promotion safety prerequisite. SA6 preserves transient verification and rollback read failures as errors, lets only wrong bytes authorize corruption, and treats only `NotFound` as absence. BLI remains `proposed`; BLI0 and the whole campaign have not started. |
| 2026-08-26 | SA6 / BLI-SA6 | corrected | Nimbus autoreview rejected the proposed heal change: a missing indexed physical pack is repair evidence, not a transient verification outcome. Restored the repair behavior and added `erasure_heal_repairs_missing_backing_pack`. The accepted SA6 scope is scrub/rebuild verification classification plus erasure-manifest rollback preimage reads. |
| 2026-08-26 | SA6 / BLI-SA6 | completed | PR #327 merged as `619a0a4c8`. Only typed corruption now becomes a direct-verification finding or quarantine; transient verification errors propagate. Manifest publish preflights every non-identical replica before its first write and treats only `NotFound` as no preimage. Focused regressions, strict blob Clippy, Nimbus autoreview, and Node 22 `make ci` passed. Proof: `proof/architecture-review-2026-07/sa6-error-classification.md`. BLI remains `proposed`. |
| 2026-08-26 | SA11 | started | Accepted the remaining NKV-owned contract work as the only active Band SA row. A minimal owner plan must distinguish current redb transaction durability from absent journal, sequence, PITR, replication, and cross-provider guarantees; it must not reopen archived NKV0 implementation scope. |
| 2026-08-26 | SA11 / NKVD | completed | The NKV owner accepted and published the current durability boundary: atomic local redb transactions, direct standalone and Engine compositions, no document committer lease/sequence/journal, no current PITR/replication/non-redb parity, and future NKV-DR/NKV6 ownership. Archived the one-task plan at `archive/nimbus-kv-durability-contract-plan.md`; proof: `proof/architecture-review-2026-07/sa11-tenant-kv-durability.md`. Band SA is terminal. |
| 2026-08-26 | SA band | completed | SA1-SA11 are done and SA12-SA15 retain their routed, reference, refuted, or elsewhere-owned no-action verdicts. Every accepted repair has focused proof; SA6 closed with full `make ci`. No Band SA implementation task remains active. |
