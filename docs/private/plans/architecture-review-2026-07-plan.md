...
Next action: Execute SA7 in a dedicated `codex/sa7-archive-version-diagnostics`
branch and `nimbus-worktrees/sa7-archive-version-diagnostics` worktree. Parse
the archive container header and reject unsupported versions before nested
payload decoding, with an operator-legible digest-codec diagnostic. SA7 is the
only active Band SA task. It is a post-IMV diagnostic rider owned by Band SA;
