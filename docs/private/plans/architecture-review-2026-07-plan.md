...
Next action: Execute SA3 as the next high lane. Acquire one exclusive advisory
lock for every distinct local Engine persistence or control root before a
provider opens, retain the guards for the Engine lifetime, and add a fresh-
process refusal test that covers the encrypted-redb path. Do not add committer
lease machinery. Keep the dirty IMV7 closeout worktree unchanged.
