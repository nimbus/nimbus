...
Next action: Execute SA9 in a dedicated `codex/sa9-format-version-validation`
branch and `nimbus-worktrees/sa9-format-version-validation` worktree. Reject
unsupported `TenantEventRecord` versions before replay and keep version skew
distinct from corruption at deployment-crossing format readers. SA9 is the
only active Band SA task. Keep the dirty IMV7 closeout worktree unchanged.
