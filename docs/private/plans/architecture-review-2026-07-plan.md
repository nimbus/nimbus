...
Next action: Execute SA6 as the BLI-accepted pre-promotion safety prerequisite.
Preserve transient scrub, heal, and rollback read errors as errors; classify
only successful-read-wrong-bytes as corruption and only `NotFound` as absence.
SA6 is the only active Band SA task. Keep BLI `proposed`; this prerequisite
does not start BLI0 or the whole BLI campaign.
