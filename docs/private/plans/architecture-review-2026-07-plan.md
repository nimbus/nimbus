...
Next action: Execute SMR9 in `storage-metadata-retention-plan.md`: archive the
completed owner plan, preserve its proof root, and close SA4 with the green
18/0 verifier and measured `SAFE` verdict. SA4 remains the only active Band SA
task until cleanup is complete. Keep the dirty IMV7 closeout worktree unchanged.
...
| SA4 | Execute [`storage-metadata-retention-plan.md`](storage-metadata-retention-plan.md): the commit log is never truncated, `retention.rs` pruning is unit-tested (`retention.rs:528` pins hard-delete denial under the floor) but wired to no production lifecycle so `durable_journal_cursor_floor()` is always 0, and PITR replays from genesis so restore cost grows without bound. The accepted contract adds four explicit durable windows, a materialized checkpoint before journal deletion, desired/confirmed/physical floors, production lifecycle ownership, bounded PITR, and post-page fail-closed validation. BLI remains blob-plane only. HS owns future distributed authority; this task uses the existing process and committer fences. Keep this row active until SMR closes. | `storage-metadata-retention-plan.md`, `nimbus-storage/src/retention.rs`, commit log, PITR paths | high / accepted; SMR5 `SAFE`, SMR9 cleanup in progress | in_progress |
