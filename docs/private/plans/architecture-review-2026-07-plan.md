...
Next action: Execute SA11 through a minimal NKV-owned durability-contract plan.
State that current tenant-KV writes are direct redb transactions outside the
document committer, journal, sequence, PITR, and replication contracts. SA11 is
the only active Band SA task. Keep BLI `proposed`; completed prerequisite
BLI-SA6 does not start BLI0 or the whole BLI campaign.
