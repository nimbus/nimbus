...
Next action: Execute SMR4 in `storage-metadata-retention-plan.md`: make every
journal, changefeed, bootstrap, historical-read, and PITR page fail closed
after a concurrent retention-floor advance. SA4 remains the only active Band
SA task. Keep the dirty IMV7 closeout worktree unchanged.
