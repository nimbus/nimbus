...
Next action: Execute SA5 in a dedicated `codex/sa5-erasure-codec-stability`
branch and `nimbus-worktrees/sa5-erasure-codec-stability` worktree. Record the
durable codec pin rationale and add a checked-in golden parity vector that
fails if a dependency upgrade changes durable shard bytes. SA5 is the only
active Band SA task. Keep the dirty IMV7 closeout worktree unchanged.
