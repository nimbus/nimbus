...
Next action: Execute SA10 as the next post-IMV Band SA rider. Repair the
server-side digest assertions so they inspect the serialized `position` field,
then add operator-visible tracing, metrics, or alert integration for the
report-semantics consistency endpoint. SA10 is the only active Band SA task;
keep the archived IMV plan and dirty IMV7 closeout worktree unchanged.
