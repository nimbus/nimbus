...
Status: `active — SUC0–SUC2 + SUC4 complete (PRs #248–#253); SUC3.1 step 4/5 in_progress (steps 1-3 merged, PRs #254/#257/#258); SUC5.1 complete (PR #255); SUC6.1 complete (PR #256); SUC6.2 closed by U7; SUC6.3 closed`
...
| SUC3.1 Provider facade extraction | `in_progress` (steps 1-2 merged, PRs #254/#257: SqlStoreCore + shared executor + libsql on the seam incl. shared sql_commit and its new StorageCommitBeforeVisibility; step 3 landed the U5 witness (3 of 4 paths compiler-linked; direct path excluded per U8) + U4 gates (StorageCommit* ban + Journal* per-owner pins); step 4 = pg/my transaction-half twins) | delete the ~855-line triplicated blocks into one facade; sqlite (with #244/#245 optimizations) is the reference; provider feature gates so embedded builds/measures in isolation | ~−1,700 LoC net; conformance harness green for all providers; sqlite paired A/B unchanged | `suc3/facade.md` | after SUC2; sqlite fork is canonical, not blocked-on |
