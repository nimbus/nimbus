...
| EX3.2 | `examples/convex/tasks` (from the React demo): clean tasks app authored via `convex/_generated/server` + schema | Same acceptance; parity-only exercises moved out of the user-facing app; README carries the loud copy-out warning (XD1: `convex` npm name collision — copied out of the monorepo, `npm install` silently substitutes the real Convex Cloud package; monorepo-only until EX7.1) | ~250 | blocked(commit 2a894fccd. App built to spec: schema/functions/generated API, React/Vite UI, reactive headless smoke, README with the XD1 copy-out warning. Static verification green — codegen, typecheck, test, build. Live smoke blocked at setup: 403 `auth.forbidden: Convex silo 'demo' is not registered to a team`. Root cause is NOT app-specific — it is the pre-existing #41 team-binding gate (`crates/nimbus-convex/src/tenancy.rs`, `crates/nimbus-server/src/adapters/convex/handlers/registry_auth.rs:121-133`), which is all-fail-closed and applies to every application-Convex request including anonymous ones (an anonymous principal has no `subject`/`issuer` claim and can never resolve a team, so no operator config admits it). The only way to satisfy it (`crates/nimbus-server/src/tests/local_server_security.rs:352-442`) is a signed customJwt-backed principal plus `NIMBUS_CONVEX_SILO_TEAMS`/`NIMBUS_CONVEX_PRINCIPAL_TEAMS` env provisioning at boot — nothing in `examples/convex/{html,http,node}` sets this up either, and no CI job exercises any of these demos live, so this affects every application-Convex example in the repo today, not just this one. Escalated to team-lead with the full evidence + three resolution options (scope EX3.2 to add real JWT auth+provisioning; ship static-verified-only and file the gap separately; treat the fail-closed-for-loopback-anonymous behavior itself as the bug); awaiting decision) |
