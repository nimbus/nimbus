...
Status: `active; NNC6.1e complete; NNC6.2 in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC6.1e complete; NNC6.2 in progress` |
| Current band | `NNC6 — compute orchestration and exact lifecycle order` |
| Current item | `NNC6.2 — admitted intent to NetworkPlan compiler` |
| Last completed item | `NNC6.1e — durable tenant discovery and pure recovery decisions` |
| Next action | Commit NNC6.2's completed audit proof, then land its named expected-red compiler and verifier packet before any product implementation. |
| Current acceptance checkpoint | NNC6.2's read-only audit is complete. The linked proof freezes C1-C18, F1-F15, the source allowlist, the transitional OCI caller baseline, and the separate NNC6.2a durability owner. No product criterion is claimed before its expected-red proof. |
...
| Latest verification checkpoint | Portable store `26/26`, compute saga/recovery `20/20`, full workloads `73/73`, full compute `94/94`, server saga store `28/28` with one intentionally ignored child. The 30-case distinct-process digest is `44ad8f3b626c2b503441c96a2bbb88181f1f3887040947e7c0323e533f2a9ea3`; task and checkpoint ledgers remain 99/99 after the prospective NNC6.2a split. |
| Blocking decision | No blocker. The NNC6.2 audit is complete and its expected-red packet is dependency-ready. |
...
| NNC6.2 | `in_progress` | **Dependency:** NNC6.1e complete. **Owner:** pure compute compilation from already-admitted tenant/service/sandbox sources to one exact canonical `NetworkPlan` payload. **Evidence:** `docs/private/plans/proof/nimbus-network-control-plane/nnc6.2-admitted-network-plan-compiler.md`. **Owned paths:** the proof's frozen compute, workloads payload, tenant projection, sandbox capability projection, portable identity, and verifier allowlist only. **Last green:** the read-only owner/dependency/caller audit completed from clean HEAD `9b2f6f91f`; three independent inventories changed no path. **Next:** commit the audit checkpoint, then land F1-F15 expected red before product implementation. **Forbidden:** lifecycle/provider effects, saga durable embedding, tenant policy inside `nimbus-network`, placeholder intent, service naming ownership, transitional OCI caller growth, and any new `nimbus-network` workspace edge. **Blocker:** none. |
| NNC6.2a | `todo` | **Dependency:** NNC6.2. Owns the workloads-owned durable complete compiled-plan carrier and distinct-process exact replay before first reservation. It may not route lifecycle effects. |
