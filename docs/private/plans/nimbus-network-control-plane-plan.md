...
Status: `active; NNC4.2 complete; NNC4.3 capability registration and exact selection in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC4.2 complete; NNC4.3 capability registration and exact selection in progress` |
| Current band | `NNC4 — evidence-based capability registry and sovereignty profile` |
| Current item | `NNC4.3 — register sandbox host-managed attachment and server/local ingress capabilities` |
| Last completed item | `NNC4.2 — promote only capability interfaces with real substitution` |
| Next action | Inventory the exact sandbox host-managed attachment and server/local ingress composition roots, stable provider IDs, and capability facts read-only. Before source edits, define a fail-before selection matrix that registers concrete reports, selects one exact satisfying registration, rejects incomplete combinations, and invokes no speculative effect trait. |
| Current acceptance checkpoint | NNC4.2 clauses S1-S6 pass in `docs/private/plans/proof/nimbus-network-control-plane/nnc4.2-capability-interface-substitution.md`. Its thirteen-row implementation/consumer census admits no new public product interface; `NetworkSegmentAllocator` remains the already-earned NNC2 seam, upper-owner seams remain local, and the deferred `ClusterLeaseProvider` prototype stays under NNC2.8/HS2/HS5. The one Sol/xhigh/fast review found two proof-only defects—missing consumer-side counts and a wrong row count—and both are corrected without executable change or repeat review. |
...
| Last checkpoint commit | `8907ccada3004238b6442ccbb3e5c9e7f79dff8d` — NNC4.1 capability satisfaction and digest-bound requirements; tree `d4c854d22fab`. |
| Audit dirty state | NNC4.1 is durable in `HEAD` at `8907ccada3004238b6442ccbb3e5c9e7f79dff8d`. NNC4.2 closes with exactly three docs/proof paths: this plan, its force-tracked proof, and the routing index. No Rust source, manifest, verifier, or behavior path changed. NNC4.3 owns only this plan/index recovery transition until its read-only registration audit admits exact source/tests. |
| Latest dirty-state checkpoint | NNC4.2 is complete: the structural non-promotion proof covers all thirteen candidate rows and both interface-admission paths. NNC4.3 is activated read-only; it may register concrete capability reports and selection behavior but may not invent provider effect interfaces or cross owner/dependency boundaries. |
| Corrected-candidate checkpoint | The one 36,874-byte GPT-5.6 Sol/xhigh/fast NNC4.2 review ran in one pass and reported two valid documentation findings: the production-consumer admission path was incomplete for three rows, and an eleven-row claim miscounted a thirteen-row table. The exact thirteen-row consumer census and corrected count resolve both. Because no executable code changed, the owner-directed cadence forbids another review. |
...
| Last verification | NNC4.2 exact gates prove one network public trait; five allocator implementations (three production/two test); zero upper capability consumers; zero speculative candidate/registry symbols; two service-registry implementations; zero production ingress resolver/constructions; three wire adapters; four HTTP adapters; two machine VMM adapters; zero generic lifecycle/status upper consumers; four shared port-authority owner groups; one raw-store upper owner; and the blocked two-fake cluster prototype. |
| Latest verification checkpoint | S1-S6 pass; live verifier 15/15, verifier self-test 45/45, docs 108 pages, site 17/17, cargo format, and both diff checks pass. The one item review's two proof findings are accepted and corrected; no executable behavior changed and no review finding remains undispositioned. |
| Blocking decision | No blocker. NNC4.2 is complete. NNC4.3 begins with a dependency-safe read-only registration/selection audit. |
...
| NNC4 — capabilities/sovereignty | `in_progress` | Named negative matrix, evidence-based seams, machine modes, offline local profile. | NNC4.1 capability satisfaction and NNC4.2 evidence-based seam review are complete. NNC4.3 now registers concrete sandbox/server capability facts and exact fail-closed selection without speculative provider interfaces. |
...
| NNC4.2 | `done` | Final evidence: `docs/private/plans/proof/nimbus-network-control-plane/nnc4.2-capability-interface-substitution.md`. S1-S6 pass; the thirteen-row implementation/consumer census keeps every unearned interface concrete and concept-owned, reuses existing upper/allocator seams, and preserves the exact core-only edge and all effect/trust/transport owners. Structural gates, verifier 15/15 plus self-test 45/45, docs 108, site 17/17, format/diff pass. The one Sol/xhigh/fast review's two proof-only findings are accepted and corrected; no repeat is warranted. |
| NNC4.3 | `in_progress` | **Owned paths:** canonical plan and routing index only until the read-only registration census admits an exact proof plus source/tests. **Expected red:** not yet designed; first name the concrete sandbox host-managed attachment and server/local ingress registration facts, stable provider IDs, incomplete-combination cases, exact selection behavior, and dependency-safe composition root without adding an effect trait. **Last green:** NNC4.2 S1-S6, structural gates, verifier 15/15 plus self-test 45/45, docs 108, site 17/17, format/diff, and its one fully dispositioned review pass. **Next:** inventory current attachment/ingress composition roots and capability facts, then write NNC4.3 acceptance and fail-before selection tests before source edits. **Blocker:** none. |
| NNC4.4 | `todo` | — |
| NNC4.5 | `todo` | — |
| NNC4.6 | `todo` | — |
| NNC4.7 | `todo` | — |
