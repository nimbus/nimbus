...
Status: `active; NNC6.3a complete; NNC6.3b in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC6.3a complete; NNC6.3b in progress` |
| Current band | `NNC6 — compute orchestration and exact lifecycle order` |
| Current item | `NNC6.3b — pure provision decision protocol and exact admitted composition inputs` |
| Last completed item | `NNC6.3a — strict workloads-owned executable carrier and closed desired digest` |
| Next action | Commit the exact staged NNC6.3a item. Then read the NNC6.3b row and NNC6.3 substitution-audit proof, inspect the pure decision/composition call graph, freeze an acceptance-bearing NNC6.3b proof, and capture fail-before evidence before product edits. |
| Current acceptance checkpoint | NNC6.3a E1-E20 are complete after its sole full and sole narrow reviews. Focused corrections pass codec `2/2`, successor `1/1`, Debug `1/1`, and real Engine/store corruption `1/1`; affected suites pass workloads `106/106`, compute `122/122` plus one ignore, and server `549/549` plus 30 ignores. NNCV031 is `25/25` plus `13/13`; live aggregate is `32/32`; exact retained-plus-additive adversarial coverage is `178 + 62 + 1 = 241`. Strict Clippy, proof lint, scripts, format/diff, docs `108`, and site `17/17` pass. NNC6.3b is dependency-ready and has no product edit yet. |
...
| Last checkpoint commit | NNC6.3 item commit `81e4f2f9ca6c998973a67f0c58e4918998bcca5e`. |
| Audit dirty state | NNC6.3a's complete reviewed 30-path item is staged for its exact commit: workloads carrier/saga and fixtures, compute codec and fixtures, server codec/schema/durability tests, the test-only shared-manager composition correction, this plan/proof/routing record, NNCV031, the pinned historical ingress contract, and aggregate verifier. No unrelated path or unstaged product change exists. NNC6.3b has no dirty path. |
| Latest dirty-state checkpoint | NNC6.3 is durable at `81e4f2f9ca6c998973a67f0c58e4918998bcca5e`. NNC6.3a's one full review found eight issues at confidence `0.98`: seven accepted corrections are implemented and their affected behavior, static contract, live aggregate, and strict Clippy gates are green; the nested-successor strictness claim was source-rejected and an exact regression test was added. |
| Current item review | The sole complete-item Sol/xhigh/fast review is complete in thread `019fc43f-afaa-79b1-8e7d-68175eb8d694`: five P2 and three P3 findings at confidence `0.98`; seven accepted and one source-rejected. The sole narrow correction review in thread `019fc45c-f0b9-7e62-abc3-e2c6822bb47b` validated all executable corrections and found one accepted P3 in stale staged-state wording at confidence `0.98`. The wording is corrected. No further review is warranted. |
| Candidate-frozen identity | Reviewed 30-path staged tree `567401c569424243749186c01fe5097e32fb0401`; complete binary patch SHA-256 `c9a68a7379b0b0638c4e4c16f5c7488f7a237cfac0f84ea241bd7a94f1bc3c99`. The earlier pre-identity tree was `ebc64264e8b27e5ecf81ffe079a47b71c9880070`. |
| Final post-review candidate identity | The sole narrow review inspected corrected 30-path staged tree `fdbc2e6d8484eeddf253e18579ecf0cd821e0ad1`; complete binary patch SHA-256 `5601df1dce4ae29621133c353f73210b39cb0086419bff998d48ab2d4fc6fac0`. The earlier tree before identity wording was `b76473da806c8e76486f910242136ebd83bdbb67`. Only ledger-only closeout wording may differ at commit. |
...
| Last verification | NNC6.1e affected behavior `195/195` with one child-only ignore; all-target/all-feature check; strict Clippy; warning-denied rustdoc; Bash syntax/ShellCheck; format/diff; direct `recovery-decisions` `1/1`; live verifier `28/28`; focused mutations `10/10`; aggregate mutations `198/198`; docs `108`; site `17/17`. |
| Latest verification checkpoint | Corrected focused behavior passes codec `2/2`, successor strictness `1/1`, Debug redaction `1/1`, and real Engine/store corruption `1/1`. Full affected libraries pass workloads `106/106`, compute `122/122` plus one ignore, and server `549/549` plus 30 ignores. NNCV031 passes `25/25` and `13/13`; live verifier passes `32/32`; retained split-bound mutations plus the new compatibility mutation prove `178 + 62 + 1 = 241`; strict affected Clippy, dependency/effect/manual audits, proof lint `0`, Bash/ShellCheck, format/diff, docs `108`, and site `17/17` pass. Full and narrow review findings are dispositioned; only the final ledger/docs rerun and exact commit remain. |
| Blocking decision | No blocker. NNC6.3a is complete and staged for its exact item commit; NNC6.3b is dependency-ready. |
...
| NNC6.4 | After NNC6.3a-NNC6.3b, atomically install generation-fenced provider commands, the compute provision dispatcher, every provision caller replacement, and every legacy deletion. | One candidate and commit introduce small capabilities earned by real Container, Krun, server-ingress, and forwarded-machine adapters; cut native service/sandbox, Convex async activation, local/forwarded Compose, Machine API, and guest-node command paths to compute; keep Convex sync naming and every invocation snapshot read-only; and delete `SandboxBackend::start`, coarse Machine API start routes, `start_service_launch`, direct services provider starts, ServiceManager activation claims/effectful registry methods, and every other old provision authority. A deterministic observer proves admit→compile→persist→reserve→prepare→attach→activation-ready→activate→workload-ready→publish→observe with an exact phase CAS after every effect. Prepare, attach, and activate cannot make an endpoint host-routable; only an idempotent owner-local publish command may do so after exact workload-readiness evidence, and ambiguous publication inspects before retry. Container/Krun and forwarded-machine/node tests prove exact identity/generation/attempt fencing, idempotent replay, stale/crossed rejection before mutation, crash recovery after every effect, one provider attempt under concurrency, and fresh-process Engine recovery. Every definite reserve/prepare/attach/activate/readiness/publish failure retains the exact phase, records recoverable failure evidence when durability is available, issues no later command or publication, and leaves NNC6.5 as the sole compensation owner. Cloud Functions HTTP/callable/trigger remain snapshot-only with zero activation/store/provider calls. The guest retains no saga store or second coordinator; its DirectProcess/Systemd backend starts are fenced provider sinks. |
| NNC6.4a | After NNC6.4, route eligible container/krun and explicit service restarts through the same compute saga. | Restart is one durable desired transition, not a local stop/start pair; withdrawal racing exit/inspection cannot restart; a fenced generation is vetoed; and an admitted restart reacquires same-generation attachment and required PEP evidence before tenant execution. |
| NNC6.5 | Implement teardown choreography and cut over every teardown caller. | Observer asserts persist withdrawal→withdraw→drain→stop→detach→release→record for service/sandbox stop, force definition deletion, standalone Compose down, failed start, and tenant retirement; unresolved persistence makes zero stop effects, and definition removal waits for safe lifecycle progression. |
...
| NNC6 — compute choreography | `in_progress` | Exact provision/teardown observers across every workload/retirement path. | NNC6.1-NNC6.3a and NNC6.1e1 are complete. NNC6.3b is active and owns pure provision decisions and exact admitted composition inputs; NNC6.4 remains one atomic provider-command/caller replacement. No item retains a temporary legacy authority. NNC6.4a/NNC6.5/NNC6.6 retain restart, teardown, and resolver fencing; NNC6.1e2 is the final convergence gate. Provider effects remain with their named owners. |
...
| NNC6.3b | `in_progress` | **Dependency:** NNC6.3a complete. Owns the pure provision decision protocol, exact node/provider/publication inputs, definitive/ambiguous outcome vocabulary, fail-before composition, and no product effect interface or caller cutover. **Owned paths:** none yet; prospective ownership is workloads decision vocabulary, compute pure selector/composition validation, fixtures, one proof/helper if required, plan, and routing checkpoint. **Last green:** NNC6.3a E1-E20, live verifier `32/32`, aggregate `241/241`, and all quality/docs/review gates. **Dirty paths:** none. **Next:** freeze its written acceptance/proof contract and fail-before census before product edits. **Blocker:** none. |
| NNC6.4 | `todo` | **Dependency:** NNC6.3a and NNC6.3b. Owns one atomic provider-command and provision-caller replacement across Container, Krun, native APIs, Convex, Compose, Machine API, and guest node; explicit publication and definite-error behavior; and deletion of every coarse provision authority in the same item. |
| NNC6.4a | `todo` | **Dependency:** NNC6.4. Owns eligible provider restart and explicit service restart cutover through the same fenced saga. |
| NNC6.5 | `todo` | Owns every teardown caller cutover: service/sandbox stop, force deletion, Compose down, failed provision, and tenant retirement choreography. |
