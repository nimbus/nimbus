...
Status: `active; NNC6.2a complete; NNC6.1e1 in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC6.2a complete; NNC6.1e1 in progress` |
| Current band | `NNC6 — compute orchestration and exact lifecycle order` |
| Current item | `NNC6.1e1 — compute-owned workload-saga ingress` |
| Last completed item | `NNC6.2a — durable complete compiled-plan carrier` |
| Next action | If this audit checkpoint is dirty, commit it. Then add item-local fail-before tests and NNCV030 without invoking structured review. |
| Current acceptance checkpoint | NNC6.1e1's three-lane source audit is complete. The frozen I1-I20 matrix narrows this item to durable submission and confirmed pure decision derivation; effectful caller cutovers are assigned to NNC6.3/NNC6.4a/NNC6.5/NNC6.6/NNC6.1e2. Historical caller-cutover verification remains expected red at `0/1`. |
...
| Last checkpoint commit | `b0a97e4404cd8afc3b11fdbf2053fa12e0b1c3d7` (routing checkpoint that activates NNC6.1e1). |
| Audit dirty state | Prospective boundary freeze: canonical plan, routing index, new NNC6.1e1 proof, and two historical verifier routing/range corrections. No product, manifest, provider/effect, or `nimbus-network` path changed. |
| Latest dirty-state checkpoint | Three independent read-only lanes plus owner inspection found one coordinator, zero production compiler/intent construction, one effectful runtime registry, all native/Compose lifecycle bypasses, and the missing executable-spec recovery owner. The current coarse manager calls cannot safely follow `IntentCommitted`; the corrected boundary preserves every cutover under a later choreography owner. |
| Current item review | Not run. NNC6.1e1 is in audit/fail-before preparation and is not review-eligible. Exactly one full GPT-5.6 Sol/xhigh/fast review runs only after I1-I18 are green and the item is candidate-frozen. |
| Candidate-frozen identity | Not established; product implementation has not started. |
| Final post-review candidate identity | Not established; no NNC6.1e1 review has run. |
...
| Latest verification checkpoint | Audit freeze: task/ledger `99/99`; historical caller cutover expected red `0/1`; new proof writing lint passes; NNCV028 direct `18` plus mutations `7/7`; NNCV029 direct `23` plus mutations `10/10`; live aggregate `30/30`; retained prior `213/213` plus two new targeted mutations gives `215/215`; docs `108`; site `17/17`; syntax, scoped ShellCheck, and diff checks pass. The unsplit aggregate replay exceeded 600 seconds in the unchanged bind-exemption prefix and is not claimed as a fresh full pass. |
| Blocking decision | No blocker. The audit boundary is frozen prospectively before product edits. |
...
NNC6.3, NNC6.4a, NNC6.5, and NNC6.6 retain every runtime, native API,
standalone Compose, restart, teardown, and resolution caller cutover together
with provision, restart, and teardown effects;
...
| NNC6.3 | Implement provision choreography and cut over every provision caller. | Observer asserts exact admit→compile→persist→reserve→start→attach→publish→observe order for Convex, Cloud Functions, sandbox API, service API, and standalone Compose up; lazy async activation uses compute while sync naming stays services-owned; standalone sandbox identity exists before effects; a durable/reconstructable executable-spec owner is named; and `ServiceManager` no longer supplies effectful runtime-registry lifecycle authority. |
...
| NNC6 — compute choreography | `in_progress` | Exact provision/teardown observers across every workload/retirement path. | NNC6.1-NNC6.2a are complete. NNC6.1e1 is the sole current item and owns durable submission plus confirmed pure decision only. NNC6.3/NNC6.4a/NNC6.5/NNC6.6 own every effectful caller cutover, including standalone Compose; NNC6.1e2 is the final convergence gate. Provider effects remain with their named owners. |
...
| NNC6.2a | `done` | Final evidence: `docs/private/plans/proof/nimbus-network-control-plane/nnc6.2a-durable-compiled-network-plan.md`; durable commit `ba78303608a2a48f319e452fc585593c5140445e`, tree `e1e1cb0d9035dcb1a34018da9dcc9fadd58f1b65`. One workloads-owned strict saga-v2 carrier persists the exact compiled plan through the unchanged Engine execution-unit adapter; references remain derived tuples; pure recovery reconstructs exact reservation material in a distinct process before effects. Focused durability `3/3`; affected `844/844` with 29 declared skips; live verifier `30/30`; exact mutations `213/213`; all quality/docs gates pass. The full review's two accepted P2 executable/proof defects and sole narrow review's one P2 test-only content-boundary gap are corrected and proven. No third review, dependency/effect edge, push, or PR. |
| NNC6.1e1 | `in_progress` | **Dependencies:** NNC6.1e and NNC6.2a complete. **Owner:** one compute-owned durable submission/confirmation seam on the existing coordinator; no dispatch. **Owned paths:** canonical plan, routing index, `proof/nimbus-network-control-plane/nnc6.1e1-durable-workload-saga-ingress.md`, and the two historical verifier helpers whose route/range checks must admit later items; no product path changed. **Audit:** three independent source lanes plus owner inspection froze I1-I20, the exact initial allowlist, complete outcome matrix, cancellation/crash semantics, and later-owner map. **Last green:** NNC6.2a `844/844`, live verifier `30/30`, exact mutations `213/213`; NNC6.1e `195/195`. **Expected red:** historical caller cutover `0/1`; item-local fail-before and NNCV030 not yet written. **Next:** if this audit checkpoint is dirty, commit it; then add item-local behavioral/process fail-before plus NNCV030. **Forbidden:** effectful caller edits, placeholder/reconstructed plan, provider effects, service-name ownership, a second store/coordinator, or a decision before exact confirmed durability. **Blocker:** none. |
| NNC6.3 | `todo` | Owns lazy/native provision caller cutover plus standalone Compose up and the durable/reconstructable executable-spec decision. |
| NNC6.4 | `todo` | — |
| NNC6.4a | `todo` | Owns eligible provider restart and explicit service restart cutover through the same fenced saga. |
