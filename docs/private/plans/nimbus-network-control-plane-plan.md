...
Status: `active; NNC7.4 complete; NNC7.5 in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC7.4 complete; NNC7.5 in progress` |
| Current band | `NNC7 — integration and observed projection cutover` |
| Current item | `NNC7.5 — prove projection loss, lag, deletion, rebuild, and stale updates cannot change authority` |
| Last completed item | `NNC7.4 — stable typed connectivity projections`; the commit containing its completed ledger row is the durable item commit. |
| Next action | Audit projection writer failure paths and authority inputs, then freeze NNC7.5 acceptance before product edits. |
| Current acceptance checkpoint | NNC7.4 F1-F7 and A1-A14 pass. Both authorized Sol/xhigh/fast reviews are dispositioned, all accepted findings are corrected, and review cadence is exhausted. |
...
| Execution base | Current `origin/main` is `8877eaff43a36d9606a1feaa0ab31d0377539d9d`; NNC7.2 item commit `d031a1bdeee0e322fa822254668a1f971169067d` is `0 behind / 159 ahead`. No merge or rebase is required. |
| Last checkpoint commit | NNC7.3 recovery checkpoint `3d8e621a1e4c003768bedbae91586e1641909c40` is the named predecessor; the commit containing the completed NNC7.4 row is the current durable checkpoint. |
| Owned paths | NNC7.5 begins with a read-only failure-path and substitution audit. No product paths are frozen yet. It cannot add projection authority, mutate desired/lease/provider state, or take NNC7.6/NNC8 behavior. |
| Latest dirty-state checkpoint | NNC7.4 is candidate-complete. Its final accepted-review corrections and all affected gates are green; only the exact item commit remains. |
| Last completed item review | NNC7.4 used one full and one narrow GPT-5.6 Sol/xhigh/fast review. Each reported four P2 correlation defects at confidence `0.97` and `0.98`. All eight are corrected and proven. The narrow review used tree `368a79b12202a104378853049634b1f81f3b279d`, patch `f7f20b3fece7590104aa05eb1b9a295af81c62fab2ca72b3df246f60cf58b7d5`, and thread `019ffc37-8e52-7142-896b-4c9d6ff3a73a`. Review cadence is exhausted. |
...
| Last green | NNC7.4: connectivity `7/7`; machine projection `1/1`; drift `3/3`; system `78/78`; serialized server `752 + 35 ignored`; strict affected quality; verifier `36/36`; docs `108`; site `17/17`; proof lint zero. Earlier affected network, machine, and CLI suites remain green. |
| Latest verification checkpoint | Final connectivity `7/7`, machine projection `1/1`, drift `3/3`, system `78/78`, serialized server `752 + 35 ignored`, strict affected Clippy/Rustdoc, format/diff, live verifier `36/36`, proof lint, docs `108`, and site `17/17` pass. |
| Next | Commit NNC7.4, then start the NNC7.5 read-only substitution audit. |
| Blocker | none |
...
| NNC7.5 | `in_progress` | **Dependency:** NNC7.4 is complete in the commit containing the prior row. **Current scope:** read-only writer/caller/failure-path audit and fail-before design for projection loss, lag, deletion, rebuild, and stale update. **Forbidden:** desired/lease/provider mutation, new provider effects, NNC7.6 TLS/telemetry, or NNC8 recovery. **Last green:** NNC7.4 affected behavior, quality, docs, and verifier gates are green. **Next:** freeze acceptance and owned paths before product edits. **Blocker:** none. |
| NNC7.6 | `todo` | — |
| NNC8.1 | `todo` | — |
