...
Status: `active; NNC3 complete; NNC4.1 capability dimensions and satisfaction errors in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC3 complete; NNC4.1 capability dimensions and satisfaction errors in progress` |
| Current band | `NNC4 — evidence-based capability registry and sovereignty profile` |
| Current item | `NNC4.1 — land capability dimensions and deterministic satisfaction errors` |
| Last completed item | `NNC3.9 — delete all old production port allocators and probe/drop decision paths` |
| Next action | Commit the exact NNC3.9 checkpoint, then begin NNC4.1 with a read-only inventory of existing `nimbus-network` capability vocabulary, `NetworkPlan` requirements, machine provider modes, and consumer fallback/error behavior. Record the positive/negative dimension matrix and fail-before cases before editing source. |
| Current acceptance checkpoint | NNC3.9 passes D1-D5 and closes the NNC3 band: no old authority vocabulary/shim; one `LocalPortLeaseAuthority`; census 66/66 + 35/35 across 26 sites; live verifier 15/15; self-test 45/45; focused 47/2; sandbox 683/24; AST 8/8; MIR/generated/dependency boundaries clean; quality/docs green. The full Sol review's three accepted P2 findings are corrected, and the narrow three-file correction review is clean with zero findings at 0.98. NNC4.1 has no source change yet and must first derive exact capability dimensions plus named unsatisfied-requirement errors without silent fallback. No blocker. |
...
| Last checkpoint commit | `129a06ea2620e0c250de7e6ed496b143431a2511` — NNC3.8 restart/cleanup-pending reconciliation and NNC3.9 activation. |
| Audit dirty state | NNC3.8 is durable in `HEAD` at `129a06ea2620e0c250de7e6ed496b143431a2511`. The reviewed NNC3.9 candidate was exactly 42 staged paths: 30 modified sandbox callers, eight Git-detected old-to-new module renames, inventory, verifier, plan, and proof. Final status adds only `docs/private/plans/README.md`, so the exact item checkpoint is 43 staged paths. Every path is inside admitted NNC3.9 proof/routing scope; no `nimbus-network` or NNC4 implementation path changed. |
| Latest dirty-state checkpoint | NNC3.9 is candidate-complete and ready for its one checkpoint commit. The old vocabulary/path scan is empty; live verifier is 15/15; exact injected child is 14/1 only at NNCV005; full self-test is 45/45; inventory is 66/66 + 35/35; both modularity exceptions are explicit; all full-review findings are dispositioned; and the narrow correction review is clean. NNC4.1 owns no dirty source path yet. |
| Corrected-candidate checkpoint | The one 582,286-byte GPT-5.6 Sol/xhigh/fast full-review invocation used two internal bundle chunks. Chunk 1 (`019fa883-f7ad-7b20-a528-d16559205cf6`) is clean at 0.94. Chunk 2 (`019fa884-9bbe-7211-9bbe-4bc664d06952`) found three valid P2 closeout defects: non-exclusive NNCV005 self-test evidence, a missing separate exception for the 1,921-line test root, and a stale pre-staging path count. All three are corrected; exact child 14/1, live 15/15, Bash/ShellCheck/diff, and full self-test 45/45 pass. The required narrow 18,190-byte one-pass correction review (`019fa893-eecb-7810-a881-e61f228dc23e`) reports zero findings, `patch is correct`, confidence 0.98. No further review is required. |
...
| Last verification | NNC3.9 focused `port_lifecycle` behavior passes 47/47 with two intentional child/characterization ignores; full `nimbus-sandbox --lib` passes 683/683 with 24 intentional ignores; AST scanner tests pass 8/8; the source-derived census passes at 66/66 authorities plus 35/35 risks; and the live verifier passes 15/15. Exact legacy type/module/probe names and old filesystem paths have zero matches. |
| Latest verification checkpoint | `cargo rustc -p nimbus-sandbox --lib --all-features -- --emit=mir=/tmp/nnc39-sandbox.mir` produced 19,608,127 bytes of resolved MIR with zero old allocator/probe symbols and exactly one resolved `TcpListener::bind`, the already-classified MachinePortProxy effect. All 15 current generated Firebase/Tonic Rust outputs (405,012 bytes across three identical output sets) have zero socket/bind/host-port/mapping/reserve/allocation patterns. `nimbus-network` still has exactly one normal workspace edge, to `nimbus-core`. Final verifier self-test is 45/45; workspace strict Clippy, affected compile/rustdoc, format/diff, Bash/ShellCheck/JSON, docs 108, and site 17/17 pass. |
| Blocking decision | No blocker. NNC3.9 is complete and ready to commit; NNC4.1 has a dependency-safe read-only audit immediately after that checkpoint. |
...
| NNC4 — capabilities/sovereignty | `in_progress` | Named negative matrix, evidence-based seams, machine modes, offline local profile. | NNC4.1 begins with a no-source-change inventory and explicit positive/named-negative capability matrix. |
...
| NNC4.1 | `in_progress` | **Owned paths:** no NNC4 source path is dirty; only the final NNC3.9 proof/ledger/routing checkpoint is staged. **Expected red:** not yet admitted—first derive the current capability/fallback matrix and name each unsatisfied requirement before writing fail-before tests. **Last green:** NNC3.9 D1-D5, verifier 15/15 + self-test 45/45, workspace Clippy, affected behavior/compiler evidence, and docs gates all pass. **Next:** read-only inventory of `nimbus-network` capability types/tests, `NetworkPlan` requirements, `nimbus-machine` networking mode, and every consumer fallback/error path; open `nnc4.1-capability-dimensions-satisfaction.md` only after the matrix is exact. **Blocker:** none. |
| NNC4.2 | `todo` | — |
| NNC4.3 | `todo` | — |
| NNC4.4 | `todo` | — |
| NNC4.5 | `todo` | — |
