...
| Next action | After the R0 checkpoint commit, implement NNC6.4a Band R1 only: portable restart policy/state/identity/transition vocabulary, pure decisions, strict wire behavior, and exact store content. Do not add a provider effect, route, SDK method, or caller cutover in R1. |
| Current acceptance checkpoint | The read-only audit and Band R0 are complete at `proof/nimbus-network-control-plane/nnc6.4a-fenced-restart-substitution-audit.md`. R0 adds one shared source scanner, NNCV034 with `19` contract groups, a `33/33` named mutation suite, a source-derived path allowlist, and aggregate integration. The live aggregate is expected red with NNCV000-NNCV033 green and only NNCV034 failing; the aggregate mutation suite passes `360/360`. Product source remains unchanged. The last green executable product baseline is NNC6.4 at `6f4f909a06a20de1003d5aafc2f5ffcba43cf0bd`. |
...
| Last checkpoint commit | `8723bc9a8ac27abc8ecbbd59d5f8d8d159e98cc1` is the durable NNC6.4a read-only audit checkpoint. |
| Audit dirty state | The owner worktree was clean at audit checkpoint `8723bc9a8ac27abc8ecbbd59d5f8d8d159e98cc1` before Band R0. The original checkout remains untouched with its user-owned `AGENTS.md`, concurrent-write benchmark, and browser bundle changes. |
| Latest dirty-state checkpoint | The R0 candidate contains only the aggregate verifier, its existing source-mode router, the extracted shared lexical scanner, the concept-owned restart checker and shell contract, this proof, this plan, and the routing index. Product paths are unchanged. R1 product paths remain forbidden until this exact checkpoint is committed. |
| Current item review | NNC6.4a has no review and no candidate. The audit is partial item preparation and does not authorize autoreview. One full Sol/xhigh/fast review runs only after A1-A20 are green and the complete item is candidate-frozen. The completed NNC6.4 review cadence remains closed. |
...
| Latest verification checkpoint | R0 source and shell syntax, scoped ShellCheck, Prettier, diff, and path-integrity checks pass. NNCV034 passes its green fixture and `33/33` exact named mutations. The live aggregate has the required expected-red shape: `34` passed and `1` failed, with only NNCV034 red. The complete aggregate mutation suite passes `360/360`. Proof lint passes with zero errors, docs pass `108`, and the site gate passes `17/17`. |
| Blocking decision | No blocker. R0 is candidate-complete; R1 is dependency-ready after the R0 checkpoint commit. |
...
| NNC6.4a | `in_progress` | **Dependency:** NNC6.4 complete at `6f4f909a06a20de1003d5aafc2f5ffcba43cf0bd`. **Audit:** complete at `docs/private/plans/proof/nimbus-network-control-plane/nnc6.4a-fenced-restart-substitution-audit.md`; zero product changes. A1-A20 freeze one nested same-generation restart state, a separate execution-attempt fence, compute-owned schedule/count/watch and command authority, Container/Krun substitutions, service/SDK ingress, race/crash/recovery behavior, deletion gates, later-owner boundaries, and the candidate path allowlist. **R0:** candidate-complete with one shared scanner, NNCV034 `19` contract groups, `33/33` named mutations, live aggregate expected red at `34` passed and `1` failed, and aggregate mutations `360/360`; product source unchanged. **Current step:** commit R0. **Owned paths after commit:** R1 allowlist only; no provider effect, route, SDK, or caller cutover. **Next:** implement Band R1 portable state and pure decisions. **Last green product:** NNC6.4 item commit `6f4f909a06a20de1003d5aafc2f5ffcba43cf0bd`. **Review:** none; do not review partial work. **Blocker:** none. |
