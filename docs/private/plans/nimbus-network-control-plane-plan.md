...
Status: `active; NNC6.3b complete; NNC6.4 in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC6.3b complete; NNC6.4 in progress` |
| Current band | `NNC6 — compute orchestration and exact lifecycle order` |
| Current item | `NNC6.4 — atomic provider-command and provision-caller replacement` |
| Last completed item | `NNC6.3b — pure provision decision protocol and exact admitted composition inputs` |
| Next action | Commit the exact NNC6.3b item after its ledger-only closeout gates, then begin NNC6.4 with the canonical read-only provider/caller substitution audit. Do not run another NNC6.3b structured review. |
| Current acceptance checkpoint | NNC6.3b E1-E20 are green after its item-level review and sole permitted narrow correction review. Full affected suites pass network `239` plus one ignore, workloads `125`, compute `147` plus one child-only ignore, and server `645` plus 30 declared skips. NNCV032 passes `32/32` direct conditions and `36/36` mutations; the live aggregate passes `33/33`, and the exact bounded retained-plus-additive run passes `277/277`. Affected check, strict Clippy, warning-denied rustdoc, format/diff, Bash syntax, scoped ShellCheck, proof lint, dependency/path scans, docs `108`, and site `17/17` pass. |
...
| Last checkpoint commit | NNC6.3a item commit `ed0560b4e45f7ec571934624962de72d021a71a8`. |
| Audit dirty state | NNC6.3b owns exactly 55 paths: its proof, plan/routing checkpoint, NNCV032 contract/self-test, aggregate routing, narrow predecessor repairs, and only the frozen network/workloads/compute/server product and test paths recorded in the item proof. No sandbox, service, machine, node, proxy, egress, system, CLI, manifest, provider-caller, or dependency-manifest path is dirty. The original checkout remains untouched. |
| Latest dirty-state checkpoint | The pure provider evidence, endpoint semantics, portable saga protocol, composition seam, reducer, exact physical persistence, reducer-driven history fixtures, structural verifier, full-review corrections, and retained-prerequisite correction are implemented. E1-E20 are green. Only ledger closeout and the exact item commit remain. |
| Current item review | The one full GPT-5.6 Sol/xhigh/fast item review found eight accepted executable defects, one expected pre-closeout ledger placeholder, and one source-rejected admission-digest claim. All accepted defects received fail-before and corrected proofs. Because the executable corrections were material, one narrow GPT-5.6 Sol/xhigh/fast correction review ran; it found one accepted activation-bypass defect and one source-rejected constructor-validation claim. The accepted defect is corrected and re-proven. Review cadence is exhausted; no further NNC6.3b review is warranted. |
| Candidate-frozen identity | Full item review: tree `eba9b42d97de4d1713c1122878fc2668b9bccba5`, patch SHA-256 `b849013b7b49aef78b05044e1fa9fc2aa6b2477f2cb756f65fdd747b8e3993b7`. Narrow correction review: tree `41e234bf0fb9389217a6476babe953e0aacb52c0`, patch SHA-256 `4d79b482e77b5c8dd338c21c949c4119125687e727e221fcfaab79c8084bf4a3`. |
| Final post-review candidate identity | After the sole accepted narrow-review defect correction and before ledger-only closeout: staged tree `00d64578012338e23c17b506447fbe5a38bc08b0`, full patch SHA-256 `df042085bb132faae91a3141f6a644a1fea5199b62ff47c2928a82c7c7aa9417`, executable/non-private-doc patch SHA-256 `ff0551ba284866427b2a62fc147e94ab1695f68f1089f2238bb97f0c81be1de3`. |
...
| Last verification | NNC6.3b affected behavior passes network `239` plus one ignore, workloads `125`, compute `147` plus one child-only ignore, and server `645` plus 30 declared skips. Affected check, strict Clippy, warning-denied rustdoc, format/diff, Bash syntax, scoped ShellCheck, proof lint, dependency/path scans, docs `108`, and site `17/17` pass. |
| Latest verification checkpoint | NNCV032 direct `32/32` and self-test `36/36` pass. The live aggregate passes `33/33`; the bounded retained-plus-additive mutation run passes `277/277`. The sole accepted narrow-review defect has a direct `0/1` fail-before and corrected `1/1` proof, and the final workloads suite passes `125/125`. |
| Blocking decision | No blocker. NNC6.3b is complete; NNC6.4 is dependency-ready after the exact NNC6.3b commit. |
...
| NNC6.4 | `in_progress` | **Dependency:** NNC6.3a and NNC6.3b complete. **Owned paths:** none yet; begin with the read-only substitution census and freeze the exact product/test/proof allowlist before edits. **Last green:** NNC6.3b E1-E20, NNCV032 `32/32` plus `36/36`, live aggregate `33/33`, bounded aggregate `277/277`, affected behavior, quality, and docs gates. **Next:** after the exact NNC6.3b commit, audit every Container, Krun, native API, Convex, Compose, Machine API, and guest-node provision caller and freeze NNC6.4's fail-before/acceptance matrix before product edits. **Blocker:** none. The item atomically owns one provider-command and provision-caller replacement, explicit publication and definite-error behavior, and deletion of every coarse provision authority in the same item. |
