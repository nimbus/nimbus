...
Status: `active; NNC7.1a complete; NNC7.2 in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC7.1a complete; NNC7.2 in progress` |
| Current band | `NNC7 — integration and observed projection cutover` |
| Current item | `NNC7.2 — preserve service-owned naming/readiness while carrying stable endpoint identity and generation; read-only substitution audit next` |
| Last completed item | `NNC7.1a — server-owned structured listener group`; the commit containing its completed ledger row is the durable item commit. |
| Next action | Trace current service publication, readiness, logical resolution, endpoint types, and generation checks. Freeze the exact seam and fail-before case before product edits. |
| Current acceptance checkpoint | `proof/nimbus-network-control-plane/nnc7.1a-structured-listener-group.md` records F1-F8 green, focused behavior `42/42`, full server `752 + 35 ignored`, strict affected quality, live architecture `36/36`, docs `108`, site `17/17`, one accepted F7 proof correction, and a clean narrow review at `0.98`. |
...
| Last checkpoint commit | NNC7.1 item commit `e2af495c39996c0692ca4756391e0d705629dfad`; current divergence is `0 behind / 157 ahead` of `origin/main`. |
| Audit dirty state | The staged NNC7.1a item contains ten modified server source paths, two new concept-owned listener-group paths, the exact bind inventory, this plan, and its proof. NNC7.2 adds only its initial proof checkpoint. No unrelated path is dirty; the original checkout remains untouched. |
| Latest dirty-state checkpoint | NNC7.1a is complete and review-exhausted. NNC7.2 owns no dirty product path and starts with a read-only service endpoint-generation audit. |
| Last completed item review | NNC7.1 used one GPT-5.6 Sol/xhigh/fast review over staged tree `2ec4b2dfa373d6b87bf803b3204f3872f82db267`, patch SHA-256 `cf6873d05059bfe79fd176c1b457030bf534e523855925a116023fa4f2c04a60`, two paths, one pass, and no fallback. It reported no actionable finding at confidence `0.88`. Review cadence is exhausted. |
...
| Last verification | NNC7.1a focused behavior passes `42/42`; full server passes `752 + 35 ignored`; all-target check, strict Clippy/Rustdoc, format/diff, exact bind/composition censuses, live architecture `36/36`, docs `108`, and site `17/17` pass. |
| Latest verification checkpoint | The one full Sol/xhigh/fast review accepted one P3 scheduler-barrier defect. The test-only correction is green at listener-group `10/10`, strict Clippy, format, and diff. The one narrow correction review is clean at `0.98`; cadence is exhausted. |
| Blocking decision | No blocker. Commit NNC7.1a and continue the NNC7.2 read-only audit. |
...
| NNC7 — integrations/projections | `in_progress` | Protocol parity, stable service/machine handles, rebuild-safe projections, TLS guard. | NNC7.2 is the sole active item; its service endpoint-generation substitution audit is next. |
...
| NNC7.2 | `in_progress` | **Dependency:** NNC7.1a is complete. **Evidence:** `proof/nimbus-network-control-plane/nnc7.2-service-endpoint-generation.md`. **Current scope:** read-only trace of service name, readiness, publication, resolution, endpoint identity, and generation checks. **Owned paths:** proof plus concise recovery/ledger rows until the audit freezes product paths. **Forbidden:** network effects, tenant policy, ingress transport, NNC7.3 machine/sandbox status, and NNC7.4-NNC7.5 projections. **Last green:** NNC7.1a item gates; NNC7.2 has no product change yet. **Next:** freeze current-versus-target seam and deterministic stale-generation fail-before. **Blocker:** none. |
| NNC7.3 | `todo` | — |
| NNC7.4 | `todo` | — |
| NNC7.5 | `todo` | — |
| NNC7.6 | `todo` | — |
