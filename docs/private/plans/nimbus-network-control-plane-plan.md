...
Status: `active; NNC6.5b fail-before recorded; implementation pending`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC6.5b fail-before recorded; implementation pending` |
| Current band | `NNC6 — compute orchestration and exact lifecycle order` |
| Current item | `NNC6.5b — compute confirmed-command driver and registry` |
| Last completed item | `NNC6.5a — strict portable teardown protocol and durable reducer` at `eefbb7178e304450632f266bdfec35ada2a55d71` |
| Next action | Replace the seven source-level fail-before assertions with observable tests while implementing the six frozen compute concepts. Start with portable decision delegation and the confirmed command gate, then the exact registry, dispatcher, driver, retained runtime, and real process harness. |
| Current acceptance checkpoint | The NNC6.5b source audit, exact paths, B1-B24, 34-test roster, ten-cut process matrix, and post-item NNCV035 target `0/8` are frozen. At checkpoint `76626d33541a126d32fb9aa694429f4a93b44292`, all six modules and four root APIs were absent, and all eight raw recovery actions were present. The seven named tests then failed for their intended missing seam: six compute `0/1` cases with 309 filtered and one server `0/1` case with 634 filtered. Implementation has not started. |
...
| Last checkpoint commit | `76626d33541a126d32fb9aa694429f4a93b44292` is the exact NNC6.5b acceptance-freeze checkpoint. |
| Audit dirty state | The dirty tree contains only seven test-only fail-before modules/helpers, two module registrations, the NNC6.5b proof, canonical plan, and routing index. No production behavior exists in the new module prefixes. The original checkout remains untouched with its user-owned `AGENTS.md`, concurrent-write benchmark, and browser bundle changes. |
| Latest dirty-state checkpoint | The owner worktree was clean at acceptance-freeze commit `76626d33541a126d32fb9aa694429f4a93b44292` before the fail-before tests and ledger edits. |
| Current item review | NNC6.5b has no structured review. Per the item cadence, do not review partial audit, fail-before, implementation, or cleanup work. Run one full Sol/xhigh/fast review only after every frozen NNC6.5b acceptance criterion is green. |
...
| Final closeout identity | NNC6.5a item commit `eefbb7178e304450632f266bdfec35ada2a55d71`; tree `036c5077cd70e4b2a30ad6a3f57f0ea155f8fd94`; final executable/script patch SHA-256 `af284c1f7687c801a13aec5528fb3c03e9116828e828f1a54ef2edf7443fcb91`; 32 paths. |
...
| Last verification | The pre-edit census passed for six absent module paths, four absent root APIs, and eight present raw actions. Each of seven fail-before tests reports exact `0/1`, exit 101, and its intended assertion: six compute cases have 309 filtered, and the server case has 634 filtered. Rustfmt passes after the test additions. The prior acceptance-freeze gates remain NNCV035 `0/11`, self-test `55/55`, aggregate `35/36` sole NNCV035 red, docs `108`, and site `17/17`. |
| Latest verification checkpoint | `/tmp/nnc65b-fail-before-decision.out`, `/tmp/nnc65b-fail-before-command.out`, `/tmp/nnc65b-fail-before-registry.out`, `/tmp/nnc65b-fail-before-dispatch.out`, `/tmp/nnc65b-fail-before-driver.out`, `/tmp/nnc65b-fail-before-runtime.out`, and `/tmp/nnc65b-fail-before-process.out`. |
| Blocking decision | No blocker. NNC6.5b is dependency-ready for implementation against the recorded fail-before state. |
...
| NNC6.5b | Add the compute-owned teardown decision, confirmed-command gate, dispatcher, driver, exact capability registry, and explicit runtime. | B1-B24 in `proof/nimbus-network-control-plane/nnc6.5b-compute-confirmed-teardown-driver.md` pass. Only a direct confirmed CAS winner can receive Execute; replay and ambiguous store/effect outcomes receive Inspect; five exact small capabilities have no fallback; contender, cancellation, CAS conflict, stale/crossed, and ten crash-after-claim/effect cuts prove one effect and durable convergence. NNCV035 becomes exact `0/8` and remains the sole aggregate red. This item adds no provider implementation or product caller cutover. |
...
| NNC6 — compute choreography | `in_progress` | Exact provision/teardown observers across every workload/retirement path. | NNC6.1-NNC6.5a and NNC6.1e1 are complete. NNC6.5b has a frozen source audit, B1-B24 contract, path boundary, and fail-before protocol; implementation has not started. NNC6.5c-NNC6.5g remain later units. No completed item retains a temporary legacy authority. NNC6.6 retains resolver fencing; NNC6.1e2 is the final convergence gate. Provider effects remain with their named owners. |
...
| NNC6.5b | `in_progress` | **Dependency:** NNC6.5a complete at `eefbb7178e304450632f266bdfec35ada2a55d71`. **Evidence:** `docs/private/plans/proof/nimbus-network-control-plane/nnc6.5b-compute-confirmed-teardown-driver.md`. **Owned value:** compute confirmed-command derivation, durable CAS driver, and immutable exact five-capability registry only; no provider implementation. **Frozen acceptance:** B1-B24, 34 named tests, two parent tests covering ten real-process cuts, exact post-item NNCV035 `0/8`, and aggregate `35/36` sole NNCV035 red. **Last green:** at checkpoint `76626d33541a126d32fb9aa694429f4a93b44292`, six modules and four APIs were absent, eight raw actions were present, six compute tests failed exact `0/1` with 309 filtered, and one server test failed exact `0/1` with 634 filtered. **Owned paths:** six compute concept modules and children, compute module exports/recovery, a test-support child, narrow raw-action test replacement, the workload teardown verifier, and a server test-only Engine-store process proof; exact paths are in the evidence. **Next:** replace source assertions with observable tests while implementing decision, command, registry, dispatch, driver, runtime, and process recovery in dependency order. **Forbidden:** provider adapters/effects, `ComputeState`, server ingress, node/systemd, sandbox, machine/guest, native service callers, Compose, tenant retirement, compatibility paths, Cargo edges, and `nimbus-network` edits. **Blocker:** none. |
