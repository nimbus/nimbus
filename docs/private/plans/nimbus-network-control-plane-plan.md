...
Status: `active; NNC4.4 complete; NNC4.5 egress readiness audit in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC4.4 complete; NNC4.5 egress readiness audit in progress` |
| Current band | `NNC4 — evidence-based capability registry and sovereignty profile` |
| Current item | `NNC4.5 — compose egress readiness as a dependency handle without moving PDP/PEP authority` |
| Last completed item | `NNC4.4 — confirm krunkit/vfkit host-managed versus WSL2/provider-managed networking modes` |
| Next action | Begin NNC4.5 with a read-only call-graph and ownership audit of egress PDP decisions, proxy PEP lifecycle/readiness, network capability vocabulary, and every consumer that may publish readiness; write the acceptance contract and expected-red packet before source edits. |
| Current acceptance checkpoint | NNC4.4 M1-M12 are complete on candidate tree `3cd1418c0175ebec3a117f8971df037f3b7831f2`: machine 21/21, CLI 879/1, focused 40/40 plus sandbox refusal 1/1, all quality/static/docs gates, and the one full Sol/xhigh/fast review are green. NNC4.5 has no admitted source paths until its audit contract is written. |
...
| Last checkpoint commit | `f0d36a5bff4ff96dc4c1eb0ca668e391f81af4e4` — NNC4.3 exact capability registration/selection; tree `5bfaa90f7d1f`. |
| Audit dirty state | NNC4.4's exact 14-path owner diff is staged with no unstaged diff. It owns plan/routing/proof, `nimbus-machine` provider/connectivity values and tests, narrow CLI readiness/stop/non-Unix-stub callers/tests, and one test-only lease-effect accessor/assertion. No manifest, network-crate, sandbox implementation, server, compute, system, egress/proxy, services, or cluster source changed. NNC4.5 has made no source edit. |
| Latest dirty-state checkpoint | Frozen staged tree `3cd1418c0175ebec3a117f8971df037f3b7831f2`, diff SHA-256 `12cadbed16162fde8bd9a86032f6c4cdd948e9fe32f01d6403fc2020c6eed124`, passed the sole full review. This proof/ledger/routing closeout is docs-only; per cadence it does not trigger another review. |
| Current item review | Actual autoreview thread `019fab85-be1b-7030-a823-c61733826669` reviewed the complete 79,296-byte NNC4.4 bundle in one pass with `gpt-5.6-sol`, `xhigh`, and fast service tier. It reported zero findings and `patch is correct` at 0.96. No accepted executable defect exists, so no correction review is permitted or needed. |
...
| Last verification | NNC4.4 focused proof passes: connectivity 5/5, non-Unix stub 3/3, provider-managed post-start 1/1, provider/bootstrap 19/19, stop/cleanup 4/4, provider selection 7/7, lease-scope distinction 1/1, and ordinary sandbox machine-forwarder refusal 1/1. |
| Latest verification checkpoint | Machine 21/21 and CLI 879 passed with one child-only ignore; affected all-target/all-feature check, strict Clippy, warning-denied rustdoc, exact core-only dependency and zero-forbidden-authority scans pass. Docs pass 108 pages and site 17/17. The live verifier passes 15/15 and its adversarial self-test passes 45/45 after catching and repairing the missing `Last green` and duplicate full-hash recovery fields. |
| Blocking decision | No blocker. Commit the exact NNC4.4 item, then continue NNC4.5's dependency-ready read-only audit. |
...
| NNC4 — capabilities/sovereignty | `in_progress` | Named negative matrix, evidence-based seams, machine modes, offline local profile. | NNC4.1-NNC4.4 are complete. NNC4.5 now audits the exact egress-readiness dependency handle while preserving egress PDP and proxy PEP authority. |
...
| NNC4.5 | `in_progress` | **Owned paths:** plan and a new NNC4.5 proof only during the read-only audit; no source path is admitted before the written contract. **Last green:** NNC4.4 is candidate-complete with all behavior/quality/static/docs gates and its one clean full review. **Next:** inventory PDP decisions, PEP lifecycle/readiness evidence, current dependency handles, and readiness/publication consumers; reconcile egress/proxy and compute/service owners before expected-red tests. **Blocker:** none. |
| NNC4.6 | `todo` | — |
| NNC4.7 | `todo` | — |
| NNC5.1 | `todo` | — |
| NNC5.2 | `todo` | — |
