...
Status: `active; NNC5.3 complete; NNC5.3a in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC5.3 complete; NNC5.3a in progress` |
| Current band | `NNC5 — sandbox-owned deep attachment lifecycle` |
| Current item | `NNC5.3a — complete machine-forwarded readiness evidence` |
| Last completed item | `NNC5.3 — complete host-managed attachment readiness evidence` |
| Next action | Record the resulting NNC5.3 commit hash, then begin NNC5.3a with a read-only source/substitution audit and exact fail-before packet before changing executable code. Preserve the common attachment/pin/PEP base, authenticate current gvproxy instance/generation plus exact routes/workers/listener lifetimes, and do not enter NNC5.4, NNC5.6, NNC8.3 cleanup/reuse, push, or PR work. |
| Current acceptance checkpoint | NNC5.3 R1-R18 are complete in `proof/nimbus-network-control-plane/nnc5.3-complete-attachment-readiness.md`: one read-only host-managed composer authenticates exact desired/durable attachment, attempt-bound Netavark/IPAM/status, active-table and real-shape nft pin, listener-lifetime, and PEP evidence for both Container and Krun; Active reconciliation authenticates exact retained FreshLaunch or claim-free RestartRetained authority before effects, reconciles dead-owner listener lifetimes, and reapplies the pin without a second Netavark setup; status/endpoints withdraw and restore with exact evidence. The historical false-ready is `0/2` then green `2/2`; full-review defects are red `0/4` then green `4/4`; sole-narrow-review defects are red `0/2` then green `2/2`. Final executable SHA-256 is `9cc7cf33173f493ebcf152bff5d0714df8a272f432df65425a391d6b6483590d`. Machine-forwarded Container remains the now-active NNC5.3a owner and was untouched by NNC5.3. |
...
| Last checkpoint commit | `fc4827b06c672fae7b5f68c9e718100cec3ba83b` — exact reviewed NNC5.2d checkpoint; tree `25a7acfb8c9a`. |
| Audit dirty state | The exact NNC5.3 code, tests, verifier/census truth-ups, proof, and canonical recovery ledger are staged as one checkpoint; the commit containing this row makes the owner worktree clean before NNC5.3a source work begins. The original dirty checkout retains exactly its four user-owned paths and the `machine-os` companion remains clean. |
| Latest dirty-state checkpoint | NNC5.3's complete corrected host-managed item is closed on top of durable NNC5.2d. R1-R18 and all six accepted review findings are green; its coherent composition roots are below the modularity thresholds after concept-owned extraction. NNC5.3a is the single ledger owner but has no executable edit yet. No cleanup/reuse, unrelated, rebase, destructive recovery, push, or PR work occurred. |
| Current item review | The sole full GPT-5.6 Sol/xhigh/fast item review, thread `019fb601-90ee-7313-bb1a-a2877c01da02` at `0.97`, found four accepted defects, red `0/4` then corrected `4/4`. The sole narrow correction review, thread `019fb62f-556a-79b0-8c37-bf95fb7fde68` at `0.99`, found real anonymous-set JSON and dormant-table defects, red `0/2` then corrected `2/2`. Final executable SHA-256 is `9cc7cf33173f493ebcf152bff5d0714df8a272f432df65425a391d6b6483590d`. No third review or docs/ledger review is permitted or warranted. |
...
| Last verification | NNC5.3 full-review correction regressions pass `4/4`; narrow correction regressions are red `0/2` then green `2/2`; complete readiness/pin/lifetime passes `30/30`; Netavark/provider-operation passes `47/47`; historical production regressions `2/2`; pin `10/10`; listener lifetime `5/5`; sandbox `859/859` with 21 declared skips; portable network `235/235` with one declared skip. All-target/all-feature check, strict no-deps Clippy, warning-denied rustdoc, exact `nimbus-network -> nimbus-core`, live verifier `20/20`, adversarial mutations `78/78`, Bash/Node/ShellCheck with documented pre-existing exclusions, format/diff, docs `108`, and site `17/17` pass. |
| Latest verification checkpoint | The common collector emits an exact portable `Ready=True` observation only after all desired/durable/provider/pin/listener/PEP facets authenticate. Every named identity substitution and lost/unknown facet fails closed without mutation. The real pin inspector requires the whole active table and real nft anonymous-set shape; chain-only, dormant, hidden-allow, jump, and substituted evidence fail closed. Both live backends withdraw and restore endpoints with exact evidence. Active restart reclaims only dead-owner lifetimes at a higher generation and reapplies the pin while preserving attachment bytes and avoiding a second Netavark setup. |
| Blocking decision | No blocker. NNC5.3 is complete; NNC5.3a is dependency-ready and begins with read-only audit/fail-before work. |
...
| NNC5.3 | `done` | Final evidence: `docs/private/plans/proof/nimbus-network-control-plane/nnc5.3-complete-attachment-readiness.md`. One read-only host-managed Container/Krun composer authenticates exact desired/durable attachment, attempt-bound Netavark/IPAM/status, active-table and real-shape nft pin, listener lifetimes, and PEP before emitting a portable observation; exact Active attach authority precedes dead-owner lifetime/pin reconciliation without a second setup. Historical false-ready is `0/2` then `2/2`; full-review defects are `0/4` then `4/4`; sole-narrow defects are `0/2` then `2/2`; focused `30/30`, Netavark/provider `47/47`, sandbox `859/859` with 21 skips, portable `235/235` with one skip, verifier `20/20` plus mutations `78/78`, affected quality/dependency/docs gates pass. Full review thread `019fb601-90ee-7313-bb1a-a2877c01da02` at `0.97`; sole narrow thread `019fb62f-556a-79b0-8c37-bf95fb7fde68` at `0.99`; no third review. Final executable SHA-256 `9cc7cf33173f493ebcf152bff5d0714df8a272f432df65425a391d6b6483590d`. Machine-forwarded publication remains NNC5.3a-owned; no push or PR. |
| NNC5.3a | `in_progress` | **Dependencies:** NNC5.3 is complete in the exact checkpoint containing this row; its commit hash is the first recovery truth-up. **Scope/evidence:** complete Container machine-forwarded readiness over current gvproxy instance/generation, exact exposed receipts, live local proxy routes/workers/listener lifetimes, and the common attachment/pin/PEP base. A historical receipt alone is never current forwarding evidence. **Owned paths:** canonical plan/recovery ledger only until the read-only audit and fail-before packet name exact source/test owners. **Last green:** prospective split was recorded before NNC5.3 executable work; NNC5.3 common base is green and durable in this checkpoint. **Next:** record the NNC5.3 commit hash, audit real gvproxy/route/worker/listener substitution seams, and write exact acceptance/fail-before proof before production edits. **Blocker:** none. |
| NNC5.4 | `todo` | — |
| NNC5.5 | `todo` | — |
| NNC5.6 | `todo` | — |
| NNC6.1 | `todo` | — |
