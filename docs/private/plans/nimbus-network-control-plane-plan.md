...
Status: `active; NNC6.5d complete; NNC6.5d1 in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC6.5d complete; NNC6.5d1 in progress` |
| Current band | `NNC6 — compute orchestration and exact lifecycle order` |
| Current item | `NNC6.5d1 — exact Container execution drain/stop and shared sandbox teardown command substrate` |
| Last completed item | `NNC6.5d — sandbox and forwarded-machine teardown substitution audit`; the commit containing this checkpoint is its durable item commit. |
| Next action | Start NNC6.5d1 with the frozen read-only call graph and owned paths. Record its exact fail-before baseline before behavior edits. Build only the shared sandbox command/provider-journal substrate and Container execution drain/stop adapters. Do not add Krun behavior, network detach/release, caller cutover, or product deletion. |
| Current acceptance checkpoint | NNC6.5d A1-A20 are green in `proof/nimbus-network-control-plane/nnc6.5d-sandbox-machine-teardown-substitution-audit.md`. The audit freezes honest provider-admission drain, exact execution stop, `Deleting` plus compound detached evidence, release-only terminal authority, one provider-command journal, retained execution locator evidence, authenticated signal identity, and distinct host-managed/forwarded provider realms. It prospectively splits NNC6.5d1-NNC6.5d4 with exact outcomes, independent capability recovery, and a two-realm forwarded-machine matrix. Product source is unchanged. |
...
| Audit dirty state | The commit containing this checkpoint owns only this plan, `docs/private/plans/README.md`, and `proof/nimbus-network-control-plane/nnc6.5d-sandbox-machine-teardown-substitution-audit.md`. Product source is unchanged. No NNC6.5d1 dirty path is expected after the commit. The original checkout remains untouched. |
| Latest dirty-state checkpoint | NNC6.5d full-review input tree `d3eec42a26c70e81b667c96847b9c9b147eb0518`; final closeout is the commit containing this checkpoint. |
| Current item review | NNC6.5d used exactly one full GPT-5.6 Sol/xhigh/fast review. Its three P2 documentation findings are accepted, corrected, and proven. No executable code changed, so no narrow correction review ran or is warranted. NNC6.5d1 has no candidate and no review. |
| Full-review input identity | Staged tree `d3eec42a26c70e81b667c96847b9c9b147eb0518`; complete patch SHA-256 `aa5f7c2564c31c2f012f45bc3ddb4dd8a7e60cc66ebc7ff707a77b0986d72bc2`; documentation-only executable patch is empty. Actual reviewer: GPT-5.6 Sol, xhigh, fast; one 59,598-byte pass; thread `019fe7a4-67eb-77f1-b284-2e16684abcdb`. It reported three P2 findings at overall confidence 0.99. |
| Narrow-review input identity | None. The accepted NNC6.5d findings changed only the audit contract, plan ledger, and routing prose. The cadence forbids a narrow review for documentation-only corrections. |
| Final closeout identity | The commit containing this checkpoint is the exact NNC6.5d item commit. |
...
| Last verification | NNC6.5d source and final checks confirm exactly three owned documentation paths and no product edits. NNCV035 self-test passes `55/55`; direct expected red is exact `0/7`; aggregate is exact `35/36` with sole NNCV035 red; NNCV008 passes; proof lint has zero diagnostics; format and diff checks pass; docs pass `108`; site passes `17/17`. All three accepted review corrections pass the affected gates. |
| Latest verification checkpoint | Artifacts: `/tmp/nnc65d-full-autoreview.out`, `/tmp/nnc65d-full-autoreview.json`, `/tmp/nnc65d-proof-lint.json`, `/tmp/nnc65d-teardown-self-test.out`, `/tmp/nnc65d-teardown-check.out`, `/tmp/nnc65d-aggregate.out`, `/tmp/nnc65d-check-docs.out`, and `/tmp/nnc65d-docs-site.out`. |
| Blocking decision | No blocker. Commit the exact NNC6.5d checkpoint, then start NNC6.5d1 fail-before work. |
...
| NNC6 — compute choreography | `in_progress` | Exact provision/teardown observers across every workload/retirement path. | NNC6.1-NNC6.5c and NNC6.1e1 are complete. NNC6.5c exhausted its one full plus one narrow Sol/xhigh/fast review; all seven accepted defects are corrected and proven. NNC6.5d is the audit-only active item; it prospectively splits provider work into NNC6.5d1-NNC6.5d4 before any product edit. NNC6.5e-NNC6.5g remain later caller/convergence units. No completed item retains a temporary legacy authority. NNC6.6 retains resolver fencing; NNC6.1e2 is the final convergence gate. Provider effects remain with their named owners. |
...
| NNC6.5d1 | `in_progress` | **Dependency:** NNC6.5d. Container execution drain/stop plus the shared exact sandbox teardown command and provider-journal substrate. **Owned paths:** compute `workload_saga/teardown_sandbox.rs` and Container children, the narrow confirmed-command locator, sandbox `provider_command.rs`, a neutral sandbox teardown contract, Container `runtime/teardown.rs` children, and narrow manifest/composition/conmon identity edits. Forbidden paths, failure outcomes, independent process cuts, and acceptance commands are frozen in the NNC6.5d proof. **Last green:** NNC6.5d final gates are proof lint zero, NNCV035 `55/55` and expected `0/7`, aggregate sole expected NNCV035 red, docs `108`, and site `17/17`; no NNC6.5d1 product path is dirty. **Next:** record the read-only call graph and exact fail-before baseline before behavior edits. **Blocker:** none. |
| NNC6.5d2 | `todo` | **Dependency:** NNC6.5d1. Krun execution drain/stop over the same substrate, with no second journal or network teardown. |
