...
| Next action | Force-add the ignored NNC6.4 proof, stage and inspect the exact product-free preparation checkpoint, commit it, then implement the portable dispatch protocol and move its intact provision-state test group to a concept-owned child. |
...
| Last checkpoint commit | NNC6.3b item commit `c42c61fb2d97d037069f3b27b9055d6e58f11d1d`. |
| Audit dirty state | NNC6.4 currently owns only its new proof, NNCV033 contract/self-test, aggregate/verifier completion pins, this plan, and the routing index. Product source remains untouched until the expected-red checkpoint is durable. The original checkout remains untouched. |
| Latest dirty-state checkpoint | NNC6.3b is durable at `c42c61fb2d97d037069f3b27b9055d6e58f11d1d`, tree `78b2c099c3541346a642c92e8fdda587dde0b1f9`; the NNC6.4 activation transition is durable at `76c920a12ed21eb8b81c1de088bcad52fd0d81e4`. The NNC6.4 source census and proof contract are dirty preparation work with no product behavior change. |
...
| NNC6.4 | `in_progress` | **Dependency:** NNC6.3a and NNC6.3b complete. **Evidence:** `docs/private/plans/proof/nimbus-network-control-plane/nnc6.4-atomic-provision-caller-cutover.md`. **Owned paths:** proof; NNCV033 contract/self-test; NNCV032 historical pin; aggregate verifier; plan; routing index. Product allowlist is frozen. **Last green:** corrected provider-target NNCV033 `48/48`; pinned NNCV032 `32/32` plus `36/36`; expected-red live aggregate `33/34` with NNCV033 as the sole failure; aggregate self-test `325/325`; docs `108`; site `17/17`; NNC6.3b affected behavior and quality gates; NNC6.4 proof lint. **Expected red:** current product passes `4/40` NNCV033 groups and fails the exact 36 implementation groups because it has no dispatcher/caller cutover, CAS provenance collapses ambiguity confirmation into `Applied`, portable state cannot authorize retry after exact absence, and every coarse authority remains. **Next:** commit the exact product-free preparation checkpoint, then implement the portable protocol first. **Blocker:** none. No structured review before E1-E34 are green. |
