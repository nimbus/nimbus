...
Status: `active; NNC4.1 complete; NNC4.2 capability seam substitution review in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC4.1 complete; NNC4.2 capability seam substitution review in progress` |
| Current band | `NNC4 — evidence-based capability registry and sovereignty profile` |
| Current item | `NNC4.2 — promote only capability interfaces with real substitution` |
| Last completed item | `NNC4.1 — capability dimensions and deterministic satisfaction errors` |
| Next action | After the NNC4.1 item commit, inventory existing network-facing traits, concrete consumers, and real adapters read-only. Record which capability boundaries have at least two substitutable implementations/consumers and which must remain concrete; do not add a trait or source edit before that proof. |
| Current acceptance checkpoint | NNC4.1 meets C1-C6. Initial fail-before exited 101 for exactly `E0432`, `E0061`, and `E0599`; the first actual Sol/xhigh/fast review's sole P2 then received its own failing digest regression. The correction makes `NetworkPlanContentDigest` distinct and derives the final domain-separated `NetworkPlanDigest` from content plus canonical requirements, making inconsistent constructor/wire pairs unrepresentable. Capability 21/21, plan 7/7, public integration 3/3, full network 169/169, quality/static/docs gates, and the clean 0.98 narrow correction review pass. |
...
| Last checkpoint commit | `c642433b230bd175b4a4a5fe968a3996d611664c` — NNC3.9 single-authority deletion and NNC4.1 activation. |
| Audit dirty state | NNC3.9 is durable in `HEAD` at `c642433b230bd175b4a4a5fe968a3996d611664c`. The completed NNC4.1 candidate owns exactly ten paths: `capability.rs`, its concept-owned test child, `lib.rs`, `plan.rs`, the `state.rs`/`status.rs` fixture callers, the public integration test, this plan, its force-tracked proof, and the plan routing index. No upper crate, machine owner, verifier, inventory, dependency baseline, or unrelated path changed. |
| Latest dirty-state checkpoint | NNC4.1 is candidate-complete and clean-reviewed. Production capability code is 1,055 lines and its 724-line private test matrix is split into a concept-owned child. Satisfaction errors are output-only; requirements/provider facts use closed enums/ordered sets; complete plan identity is derived from a type-distinct content digest plus those requirements. The only remaining mutation before NNC4.2 audit is the exact NNC4.1 item commit. |
...
| Last verification | NNC4.1 capability matrix passes 21/21, corrected plan behavior/wire passes 7/7, public integration passes 3/3, and full `nimbus-network --all-features` passes 169/169 with zero ignores. Strict affected Clippy, warning-denied rustdoc, format, and diff checks pass after the C5 correction. |
| Latest verification checkpoint | Network metadata retains exactly one workspace edge (`nimbus-network -> nimbus-core`). Forbidden effect/trait/default scans are empty. The live verifier is 15/15 and its fail-closed self-test 45/45. Format/diff checks pass; docs are 108 pages link-clean and site validation is 17/17. |
| Blocking decision | No blocker. NNC4.1 is complete and clean-reviewed; NNC4.2 has a dependency-safe read-only substitution audit after the item commit. |
...
| NNC4 — capabilities/sovereignty | `in_progress` | Named negative matrix, evidence-based seams, machine modes, offline local profile. | NNC4.1 is complete with the 14-dimension matrix and digest-bound requirements; NNC4.2 now audits real substitution before promoting any interface. |
