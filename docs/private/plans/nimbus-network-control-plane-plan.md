...
| Next action | Implement the exact restart capability registry, dispatcher, bounded driver, deterministic clock, and durable watch against the confirmed command/result seam. Then generalize the existing sandbox provider-command journal with a restart ordinal and earn the real Container/Krun capabilities. Keep the service route, SDK method, machine transport, scheduler deletion, caller cutover, and full live convergence in R3/R4. |
| Current acceptance checkpoint | R2 portable command/store state is durable at `14e6236d4e3d1199a7ae40674bcdedd50b98fd58`, normalized compute admission is durable at `8935e0c77dd188f50566b72c917b2005a213ecdd`, and private command confirmation plus exact result reduction is durable at `e1e9c95167972c2566a468d01aa0b91e559dd9be`. Only the direct claim-CAS winner receives execute mode. Replay, ambiguous confirmation, and fresh-process recovery first persist exact inspection state. Results authenticate the saga transition, desired/source/attempt/provider fences, claim, and dispatch epoch. Ambiguous and in-progress inspection never retry; only authenticated absence retains the same attempt and advances one dispatch epoch; definite failure stops later commands. Command/ambiguity behavior passes `10/10`; full compute passes `251` with one declared child-only ignore; strict compute Clippy, Rustfmt, Prettier, JavaScript syntax, diff, and NNCV034 `68/68` pass. The live contract exits `1` on exactly readiness, capabilities, service, watch, machine, scheduler, and behavior. No provider effect, route, SDK method, machine transport, scheduler deletion, or caller cutover entered this checkpoint. |
...
| Last checkpoint commit | `e1e9c95167972c2566a468d01aa0b91e559dd9be` is the durable R2 confirmed-command/result checkpoint. Its tree is `48f14195cfebd71e6ecaca0e012d83d6deefac56`; its patch from the prior ledger checkpoint is SHA-256 `3d5ed0766c5d4dcbbace69b4a1c69cb16662de499ad3d9f881accd7dc6238853` across `5` paths. Earlier R2 checkpoints remain `14e6236d4e3d1199a7ae40674bcdedd50b98fd58` for portable command/store state and `8935e0c77dd188f50566b72c917b2005a213ecdd` for compute admission. |
| Audit dirty state | The owner worktree was clean at the R2 confirmed-command/result checkpoint `e1e9c95167972c2566a468d01aa0b91e559dd9be` before this ledger update. The original checkout remains untouched with its user-owned `AGENTS.md`, concurrent-write benchmark, and browser bundle changes. |
| Latest dirty-state checkpoint | Only this plan, its routing index, and the NNC6.4a proof contain the post-product recovery update. The dispatcher, driver, watch, provider, route, SDK, machine, and scheduler paths remain unchanged. |
...
| Latest verification checkpoint | R2 command-state tests pass `23/23`; portable candidate-page tests pass `4/4`; durable Engine candidate-query tests pass `4/4`; compute admission/progress tests pass `8/8`; confirmed command/ambiguity tests pass `10/10`; full workloads passes `165/165`; full compute passes `251` with one declared child-only ignore; server workload-store tests pass `52` with `5` declared child-only ignores. Strict compute Clippy, Rustfmt, Prettier, JavaScript syntax, and diff checks pass. NNCV034 passes `68/68` sole-diagnostic mutations. Its live contract exits `1` on exactly the seven intended later groups and has no path violation. Known vendored Brotli warnings are unchanged. R1 aggregate and docs/site evidence remains valid; no final R4 gate or item-completion claim is made. |
| Blocking decision | No blocker. The portable/store, compute-admission, and confirmed-command/result dependencies are durable; dispatcher, driver, and watch work is dependency-ready. |
...
| NNC6.4a | `in_progress` | **Dependency:** NNC6.4 complete at `6f4f909a06a20de1003d5aafc2f5ffcba43cf0bd`. **Audit:** `docs/private/plans/proof/nimbus-network-control-plane/nnc6.4a-fenced-restart-substitution-audit.md`. A1-A20 freeze one nested same-generation restart state, a separate execution-attempt fence, compute-owned schedule/count/watch and command authority, Container/Krun substitutions, service/SDK ingress, race/crash/recovery behavior, deletion gates, later-owner boundaries, and the candidate path allowlist. **R0:** durable at `6d8961bd6d4da819b2524128cb398e22e0a9382f`. **R1:** durable at `d117ba369eaf5acc5ede9ec3edad32a11ddfbeb2`. **R2 portable/store:** durable at `14e6236d4e3d1199a7ae40674bcdedd50b98fd58`; exact record claim/result transitions and one bounded global candidate page are green. **R2 compute admission:** durable at `8935e0c77dd188f50566b72c917b2005a213ecdd`; the normalized reducer and sole-coordinator CAS pass `8/8`. **R2 confirmed command/result:** durable at `e1e9c95167972c2566a468d01aa0b91e559dd9be`; only the direct CAS winner executes, replay/recovery inspects, exact absence alone advances one dispatch epoch, crossed results fail closed, and definite failure stops. Command/ambiguity passes `10/10`; full compute passes `251` with one declared ignore. NNCV034 remains `68/68`; the live contract has exactly seven intended R2-R4 diagnostics. **Current step:** implement the exact capability registry, dispatcher, bounded driver, and watch before the provider journal and real adapters. **Owned paths:** exact R2 allowlist only; no service route, SDK, machine transport, scheduler deletion, or caller cutover. **Review:** none; one full review only after A1-A20 are green. **Blocker:** none. |
