...
Status: `active; NNC7 complete; NNC8.1 in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC7 complete; NNC8.1 in progress` |
| Current band | `NNC8 — full recovery, ambiguity, and state-machine proof` |
| Current item | `NNC8.1 — rehydrate and reconcile every persisted phase` |
| Last completed item | `NNC7.6 — TLS authority and telemetry cardinality`; the commit containing its completed ledger row is the durable item commit. |
| Next action | Audit the persisted network and workload phases against the existing NNC0 crash-cut runner, then freeze the smallest NNC8.1 behavior matrix before product edits. |
| Current acceptance checkpoint | NNC7.6 is complete. Closed KV command labels, bounded unknown input, pinned proxy fairness state, distinct TLS authorities, `15/15` static mutations, and all affected closeout gates pass. NNC8.1 has not changed product source. |
...
| Execution base | Current `origin/main` is `8877eaff43a36d9606a1feaa0ab31d0377539d9d`; NNC7.5 checkpoint `8dbe562ca4b55147eb92ba01922e9d0fa1043ec0` is `0 behind / 163 ahead`. No merge or rebase is required. |
| Last checkpoint commit | The NNC7.6 item commit is `HEAD` and contains its completed row. Its parent durable checkpoint is `8dbe562ca4b55147eb92ba01922e9d0fa1043ec0`. |
| Owned paths | NNC8.1 is in read-only audit. It owns no product path until its existing crash-cut runner, persisted-phase census, and deterministic convergence matrix are frozen in its proof. It cannot absorb provider effects or begin NNC8.2 ambiguity work. |
| Latest dirty-state checkpoint | NNC7.6 closeout is the only dirty item. Its product Rust stayed unchanged after the full review. The four narrow-review proof gaps are corrected; self-tests pass `15/15`; no unrelated path is dirty. |
| Last completed item review | NNC7.6 used one full and one narrow GPT-5.6 Sol/xhigh/fast review. Ten proof or routing findings were accepted, corrected, and proven. Review cadence is exhausted; details remain in the item proof. |
...
| Last green | NNC7.6: KV `30 + 3 ignored`; proxy `164`; server TLS `6`; strict affected Clippy/Rustdoc; static contract `15/15` plus live source; aggregate verifier `36/36`; docs `108`; site `17/17`; proof lint zero. |
| Latest verification checkpoint | The narrow-review corrections pass the final `15/15` mutation contract and live source contract. Product Rust did not change after its full affected gates. |
| Next | Freeze the NNC8.1 persisted-phase and crash-cut behavior matrix. |
| Blocker | none |
...
| NNC7 — integrations/projections | `in_progress` | Protocol parity, stable service/machine handles, rebuild-safe projections, TLS guard. | NNC7.4 is complete. NNC7.5 is the sole active item; its read-only projection-independence audit is next. |
...
| NNC8.1 | `in_progress` | **Dependency:** NNC7 is complete. **Current step:** read-only census of persisted phases, the NNC0 crash-cut runner, and existing convergence tests. **Acceptance:** every named boundary converges to one desired generation or a fenced cleanup-pending state. **Owned paths:** none until the proof matrix is frozen. **Last green:** NNC7.6 closeout verifier `36/36`; NNC8.1 has no product change. **Next:** freeze the behavior matrix before product edits. **Blocker:** none. |
