...
Status: `active; NNC5.2 complete; NNC5.2a in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC5.2 complete; NNC5.2a in progress` |
| Current band | `NNC5 — sandbox-owned deep attachment lifecycle` |
| Current item | `NNC5.2a — persist exact attachment-to-segment association and provider attempt before effects` |
| Last completed item | `NNC5.2 — persisted attachment phase/generation and opaque provider handle` |
| Next action | Begin NNC5.2a with a read-only substitution and recovery audit: inventory reservation/attachment association writes, first namespace/Netavark effects, filename-derived orphan classification, and the eight required recovery rows before mutating code. |
| Current acceptance checkpoint | NNC5.2 criteria 1-17 are complete over predecessor `3cf209513c103f581245ca963a082517e7f031e6`. One tenant-qualified concrete attachment authority persists exact resource version, phase, selected provider, and stable opaque handle in the shared network store. The shared OCI lifecycle injects that authority into both real backend routes and authenticates durable desired state plus provider observation before retry effects. The sole full item review accepted one production-route proof gap; four fresh Container/Krun constructor and private `configure_network` tests corrected it and the one permitted narrow review is clean at 0.99. Production routes pass 4/4 after an informative fixture-prerequisite red 0/4; portable authority/state passes 15/15; lifecycle/routes pass 38/38; the backend lane passes 332/332; full affected crates pass 1003/1003 with 24 declared skips; all quality, verifier, and docs gates pass. NNC5.2a and later owners remain honestly red. |
...
| Last checkpoint commit | `3cf209513c103f581245ca963a082517e7f031e6` — exact reviewed NNC5.1 checkpoint. |
| Audit dirty state | NNC5.2 owns only this recovery-header/ledger update and new in-progress proof before executable work. The source tree was clean at audit start. Three read-only audit agents reported no changed paths; no agent retains path ownership. The original dirty checkout and clean `machine-os` companion remain untouched. |
| Latest dirty-state checkpoint | NNC5.2 closes as one exact 32-path item checkpoint; the reviewed correction had 31 paths and the sole post-review addition is the plan-index routing truth-up required by the static verifier. Executable work is localized to `nimbus-network` attachment state/store/manager modules, the shared sandbox OCI lifecycle plus concept children, Container/Krun authority injection, capability compilation, directly related stale fixtures, one process-composition assertion, and the mechanically refreshed composition census/classifier. The lifecycle composition root is 1,488 lines after privileged effects and teardown recovery decisions moved into 68-line `host.rs` and 477-line `recovery.rs` owners. No rebase, push, or PR has occurred; the original dirty checkout and clean `machine-os` companion remain untouched. |
| Current item review | The sole full GPT-5.6 Sol/xhigh/fast review ran as one 193,201-byte pass under `019fb301-f7f5-72c2-a4bb-f9cfa3dc7819`. Its one P1 at 0.98 was accepted: the matrix did not exercise the new backend fields/constructors through production routes. Four Container/Krun fresh-reopen and corrupt-construction route tests correct that proof gap and pass 4/4. The one permitted narrow Sol/xhigh/fast review ran as one 211,240-byte pass under `019fb314-eac2-79b3-bc10-27e8d3e2e88e` and is clean at 0.99. No further NNC5.2 review ran or is warranted. |
...
| Last verification | Audit start was clean at `3cf209513c103f581245ca963a082517e7f031e6`. Last executable evidence remains NNC5.1: contract `30/30`; exact-authority row `2/2`; port lifecycle `47/47`; affected container/krun lane `328/328`; direct krun regression `1/1`; full sandbox `758/758` with `24` declared skips, plus affected quality/static/docs gates. |
| Latest verification checkpoint | NNC5.2 final green: production routes 4/4 after informative fixture-prerequisite red 0/4; portable authority/state 15/15; lifecycle plus routes 38/38, including 66 Container/Krun phase/observation decisions; affected backend lane 332/332; final affected crates 1003/1003 with 24 declared skips; affected check, strict Clippy, warning-denied rustdoc, format, and diff checks pass; verifier 17/17 plus self-test 62/62; docs 108; site 17/17. One earlier loaded full run transiently failed the pre-existing concurrent preselected-identity test; it immediately passed 1/1 in isolation, in both backend lanes, and in both final full runs, so no change was made on an unreproduced result. Corrected executable SHA-256 is `fb7838cab3bd63940d8c6d41dc414e876ca522e1c53d33feea9bef22ce7fe0b7`. NNC5.2a durable association/attempt-before-first-effect and orphan matrix, NNC5.3 complete readiness, NNC5.4 process-kill cuts, NNC5.5 band-wide locality, and NNC5.6 side-effect-free workload inspection remain honestly deferred. |
| Blocking decision | No blocker. NNC5.2a is the next dependency-ready item. |
...
| NNC5.2a | `in_progress` | **Dependencies:** NNC5.2 is complete in the pending exact local predecessor checkpoint. **Scope:** persist exact reservation and attachment-to-segment association plus provider attempt before the first namespace/Netavark effect; replace filename-only orphan classification with durable owner evidence; prove the eight required recovery rows adopt, remove, or quarantine. **Owned paths:** none yet; the read-only audit must freeze an exact non-overlapping path/proof set before implementation. **Last green:** NNC5.2 production routes 4/4, portable authority/state 15/15, lifecycle/routes 38/38, backend lane 332/332, affected crates 1003/1003 with 24 skips, and all closeout gates. **Next:** inventory reservation/association writes, first effects, orphan classification, and current fail-before seams before any source mutation. **Blocker:** none. |
| NNC5.3 | `todo` | — |
| NNC5.4 | `todo` | — |
| NNC5.5 | `todo` | — |
| NNC5.6 | `todo` | — |
