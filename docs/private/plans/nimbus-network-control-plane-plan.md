...
| Next action | Implement NNC6.5d4 band 5 from `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`: add the guest composite execution drain/stop state machine and exact phase sink. Reuse the retained Systemd traits, Container journal, and current node/Container progress stores. Add replay, contention, and fresh-process cuts. Do not add forwarded attachment composition, the private route, client, parent adapter, or caller cutover yet. |
| Current acceptance checkpoint | NNC6.5d4 audit and fail-before are durable at `29137fa000341e48bd9ee41ef37451f5611977f1`. Bands 1-4 are green. K3-K4 pass. The operation-family and injected-journal prerequisites of K7 pass. K11-K14 have their strict wire vocabulary, digest, response correlation, and closed outcomes. K15 has a closed translation without caller-selected provider IDs. K16 passes: Linux production opens one deterministic durable Systemd teardown root; one concrete backend is retained behind lifecycle, drain, and stop traits; exact teardown capability keeps store and backend blockers distinct; and the operation remains unavailable until its strict sink exists. |
...
| Last checkpoint commit | `0eb5cab62` is the durable NNC6.5d4 band 1 checkpoint. `fb7acb395` is band 2. `690ac9f22` is band 3. The commit containing this header is the durable band 4 recovery checkpoint. The prior audit checkpoint is `29137fa000341e48bd9ee41ef37451f5611977f1`. |
| Audit dirty state | The owner worktree was clean after `690ac9f22`. Preserve only the band 4 Systemd and guest-composition paths, their concept-owned tests, the two source-derived census reconciliations, routing, proof, and plan until their recovery commit. The original checkout remains untouched. |
| Latest dirty-state checkpoint | Band 4 is candidate-complete. Three read-only packets changed zero paths. The owned diff opens the deterministic production Systemd teardown store, retains one concrete backend behind the lifecycle, drain, and stop traits, and reports exact teardown readiness without adding a route or supported operation. It also refreshes one shifted listener fingerprint and classifies one new test-only backend construction. No client, dispatch effect, parent adapter, caller, protocol version, dependency, coarse-stop path, or `nimbus-network` source changed. |
...
| Last verification | NNC6.5d4 band 4 fail-before reports `root=1 drain=1 stop=1 production=1`. Focused Systemd capability/store behavior passes `3/3`; guest composition passes `4/4`; existing capability tests pass `5/5`; restart capability passes `1/1`; and Systemd reopen ambiguity passes `2/2`. Full node passes `108/108`; full CLI passes `952/952` with one declared ignore. Format, diff, strict affected Clippy, warning-denied affected Rustdoc, NNCV008, proof lint with zero diagnostics, docs `108`, and site `17/17` pass. NNCV035 self-test passes `55/55`; direct remains exact expected red at `0/7`; aggregate is `35/36` with only NNCV035 red. Linux cross-check is environment-blocked before Nimbus compilation because `aarch64-linux-gnu-gcc` and its sysroot are absent; the Linux call shape is source-checked. |
| Latest verification checkpoint | Active record: `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. It records bands 1-3 fail-before, implementation, behavior counts, quality gates, sparse portable semantics, strict forwarded full-chain semantics, injected-journal ownership, and the band 4 recovery point. |
| Blocking decision | No blocker. Finish band 4 static/docs gates, commit its exact recovery checkpoint, then implement only band 5. Do not run structured review on partial NNC6.5d4 work. |
...
| NNC6.5d4 | `in_progress` | **Dependency:** NNC6.5d3 complete at `c1c7f1397`. **Evidence:** `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. **Checkpoints:** audit `29137fa00`; band 1 `0eb5cab62`; band 2 `fb7acb395`; band 3 `690ac9f22`; the commit containing this row is the band 4 checkpoint. Three read-only audit packets freeze K1-K35 and the exact two-realm contract. **Bands 1-3:** one strict receipt prefix, one teardown-family final-withdraw operation with injected compute journal, and one transport-only Machine API envelope establish the portable and wire prerequisites without effects or callers. **Band 4:** Linux production opens the existing crash-safe Systemd teardown store at `control/service-sandboxes/systemd/teardown`; one concrete backend allocation is retained behind `HostLifecycleBackend`, `HostExecutionDrainProvider`, and `HostExecutionStopProvider`. General lifecycle capability remains independent. Exact teardown capability adds `durable_teardown_state`, preserves required Systemd blockers, and reports a precise missing-store blocker. The Machine API reports `workload-teardown.phase` unavailable with separate missing-sink and provider blockers, but it does not add the operation to `supported_operations` or the router. **Owned paths:** band 4 changes only Systemd construction/capability tests, guest API composition/state/capability reporting, one concept-owned composition test child, source-derived bind and composition census entries, routing, proof, and this recovery ledger. No route, client, composite teardown effect, parent adapter, caller, provider journal, `nimbus-network`, dependency, protocol version, or coarse-stop path changed. **Last green:** focused Systemd capability/store `3`, guest composition `4`, capability regression `5`, restart capability `1`, reopen ambiguity `2`; full node `108`; full CLI `952` with one declared ignore; format, diff, strict affected Clippy, warning-denied affected Rustdoc, NNCV008, proof lint zero, docs `108`, and site `17/17` pass. NNCV035 self-test is `55/55`; direct is expected red at `0/7`; aggregate is `35/36` with only NNCV035 red. Linux cross-check is environment-blocked before Nimbus compilation by the absent cross C compiler/sysroot. **Next:** band 5 guest composite drain/stop state and exact sink only. **Required seams:** distinct admitted parent attachment/execution/ingress provider IDs; parent publication withdrawal before guest drain/stop; exact remote guest envelopes and provider-journal reuse; parent release only after exact guest/provider absence; independent parent/guest durable realms; sibling-batch, response-loss, process-death, stale-generation, crossed-forwarder, and ambiguity convergence. **Forbidden:** Compose down, product caller cutover, physical-machine stop, tenant policy, service naming, a CLI-local saga store, a public route, or coarse-stop deletion. **Review:** no structured review has run; the only full item review is deferred until K1-K34 are green and the complete item is candidate-frozen. **Blocker:** none. |
