...
| Current item | `NNC6.5 — teardown caller cutover` |
| Last completed item | `NNC6.4a — fenced workload and explicit service restart cutover` |
| Next action | Create the exact NNC6.4a closeout commit, record its durable hash, then start NNC6.5 with the read-only teardown substitution audit and fail-before acceptance freeze. |
| Current acceptance checkpoint | NNC6.4a A1-A20 are green. All `22` full-review findings and all four narrow-review findings have exact corrections and affected proofs. NNCV033 passes `40/40 + 50/50`; NNCV034 passes `86/86`; the live aggregate passes `35/35`; aggregate mutations pass `413/413`; affected behavior, quality, SDK, docs, real read surfaces, fencing, ambiguity, crash/recovery, and modularity proofs pass. Review cadence is exhausted after one full and one narrow Sol/xhigh/fast review. |
...
| Last checkpoint commit | `02de45f1f2977cf78ad594c97854830f467b5d14` remains the last durable recovery checkpoint. The complete NNC6.4a candidate is acceptance-green and will become the next exact closeout commit. |
| Audit dirty state | The dedicated owner worktree contains only NNC6.4a-owned staged and final-correction paths. The original checkout remains untouched with its user-owned `AGENTS.md`, concurrent-write benchmark, and browser bundle changes. |
| Latest dirty-state checkpoint | The complete item differs from audit checkpoint `8723bc9a8ac27abc8ecbbd59d5f8d8d159e98cc1` on `221` paths and from R2 recovery commit `73f53796392eae1b7c6df06e15450f272e228710` on `151` R3-and-later paths. Every owned path is staged before the closeout commit; no unrelated or original-checkout path is included. |
| Current item review | The one full GPT-5.6 Sol/xhigh/fast review reported `22` accepted findings at confidence `0.99`. The one permitted narrow Sol/xhigh/fast correction review reported four accepted findings at confidence `0.99`. Review cadence is exhausted. Rerun affected proofs after correction; do not run another structured review. |
| Full-review input identity | Pre-ledger staged tree `63488a338db76cc65baea73afee257f0af46a00d`; complete item patch SHA-256 `5cad6e9616c8fbb2f391fc67ecc76b293a42962ec981552f3199e52264aa6e0b`; complete executable/script patch SHA-256 `de61345c0f81622d9d6a14aa988a8291c2af490f6fd07e6fd430caa060691bae`; R4 staged patch SHA-256 `cb7911351a31fcce1d8d5ae88ce2968d4ad80f652e844df285f223af6b9e82bc`. |
| Narrow-review input identity | Corrected pre-ledger tree `090a9eecc96f95d2e0fc2c3da65e5cd309114f82`; complete item patch SHA-256 `d6ac954fe23f1180e88643a85ee6fbd8c7268de1074aa59e206467eb73aeac95`; complete executable/script patch SHA-256 `2d4a608aa084ffbdcad07dbccb977478185ef42492a30fbc573e5b5ef152f28e`; correction patch SHA-256 from the first review snapshot `1df8d49325330a920065ade6eca4de9e7594b7c0048c706f0afac9c021e4fb9b`; correction executable/script SHA-256 `26a5814227f40e6fe421d3029b430ebf52af2fa3a90ed9be8f763dac0033275d`. Synthetic commit `73247dfc846b1b8c2b799b3c33e84428c4f29a08` is the sole narrow-review input. |
| Final closeout identity | The final correction candidate has `221` complete item paths and `151` R3-and-later paths. Its exact durable commit and tree are recorded in the immediate post-commit recovery checkpoint; no additional structured review is authorized. |
...
| Last verification | Final full suites pass workloads `172`, compute `303 + 1 ignore`, sandbox `1,004 + 27 ignores`, machine `34`, node `72`, server `692 + 32 ignores`, CLI `948 + 1 ignore`, and services `82`. Real GET and logical-name reads pass `1/1` each. Affected all-target checks, strict Clippy, warning-denied rustdoc, SDK build/test/typecheck with `24`-route parity, docs `108`, site `17/17`, format, diff, syntax, Prettier, scoped ShellCheck, and proof lint pass. NNCV033 is `40/40 + 50/50`; NNCV034 is `86/86`; the aggregate is `35/35 + 413/413`. Known vendored Brotli warnings are unchanged. |
| Latest verification checkpoint | The narrow-review corrections prove observation absence republish, inspect-before-terminal successor veto, later-veto read-only node inspection across compute/machine/guest/node, and assertion-quality fail-closed verification. The final aggregate mutation log is `/tmp/nnc64a-final-aggregate-selftest.log` and ends `self-test: 413 passed, 0 failed`. All A1-A20 obligations are green. |
| Blocking decision | No blocker. NNC6.5 is active at its read-only audit boundary; its first source change remains gated on the exact NNC6.4a closeout commit. |
...
| NNC6 — compute choreography | `in_progress` | Exact provision/teardown observers across every workload/retirement path. | NNC6.1-NNC6.4a and NNC6.1e1 are complete. NNC6.5 is active for teardown caller cutover. No completed item retains a temporary legacy authority. NNC6.6 retains resolver fencing; NNC6.1e2 is the final convergence gate. Provider effects remain with their named owners. |
...
| NNC6.4a | `done` | **Evidence:** `docs/private/plans/proof/nimbus-network-control-plane/nnc6.4a-fenced-restart-substitution-audit.md`. The complete item has `221` paths and `151` R3-and-later paths. A1-A20 pass. NNCV033 passes `40/40 + 50/50`; strengthened NNCV034 passes `86/86`; the aggregate passes `35/35 + 413/413`. Final behavior passes workloads `172`, compute `303 + 1 ignore`, sandbox `1,004 + 27 ignores`, machine `34`, node `72`, server `692 + 32 ignores`, CLI `948 + 1 ignore`, and services `82`. Quality, SDK, docs `108`, site `17/17`, read-surface, fencing, crash/recovery, and modularity proofs pass. The sole full review's `22` accepted findings and the sole narrow review's four accepted findings are corrected and proven. No third review ran or is warranted. The immediate post-commit checkpoint records the exact closeout commit. |
| NNC6.5 | `in_progress` | **Owned paths:** none yet; read-only audit preparation does not claim NNC6.5 implementation paths. **Last green:** NNC6.4a A1-A20, NNCV034 `86/86`, aggregate `35/35 + 413/413`, affected behavior/quality, docs `108`, and site `17/17`. **Current boundary:** read-only teardown substitution audit and fail-before acceptance freeze. **First source-action gate:** record the exact NNC6.4a closeout commit before any NNC6.5 source edit. **Next:** commit NNC6.4a with its ledger/evidence, record the exact hash, then freeze NNC6.5 callers, expected-red cases, and owned paths before editing source. **Owned scope:** every teardown caller cutover: service/sandbox stop, force deletion, Compose down, failed provision, and tenant retirement choreography. **Blocker:** none. |
