...
| Next action | Implement NNC6.5d4 band 2 from `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`: add a teardown-family final-withdraw provider operation and extract one narrow confirmed-teardown journal seam. Start with expected-red restart-separation and five-operation claim-rule tests. Do not change Machine API, provider composition, or callers yet. |
| Current acceptance checkpoint | NNC6.5d4 audit and fail-before are durable at `29137fa000341e48bd9ee41ef37451f5611977f1`. Band 1 is green: K3 passes, and the portable half of K4 passes while forwarded full-chain enforcement remains open. One workloads-owned validated prefix projects exact durable receipt bytes through compute command, provider observation, and result authentication. |
...
| Last checkpoint commit | The commit containing this header is the durable NNC6.5d4 band 1 recovery checkpoint. The prior audit checkpoint is `29137fa000341e48bd9ee41ef37451f5611977f1`. |
| Audit dirty state | The owner worktree was clean after `29137fa0`. Preserve only the band 1 `nimbus-workloads`, `nimbus-compute`, proof, and plan paths until their recovery commit. The original checkout remains untouched. |
| Latest dirty-state checkpoint | Band 1 is candidate-complete. Three read-only packets changed zero paths. The owned diff adds the portable prefix and record projection, carries it through compute command, callback, and result fences, and splits prefix tests into a `94`-line concept-owned child so the parent remains at `1,436` lines. |
...
| Last verification | NNC6.5d4 band 1 expected-red compilation exits `101` for both missing seams. Green focused behavior is workloads `2/2`, compute prefix `5/5`, and crossed result `1/1`; full workloads is `218/218`; full compute is `374/374` plus one declared child-only ignore. Format, diff, strict affected Clippy, warning-denied affected Rustdoc, NNCV008, proof lint zero, docs `108`, and site `17/17` pass. NNCV035 self-test passes `55/55`; direct remains exact expected red at `0/7`; aggregate is `35/36` with only NNCV035 red. |
| Latest verification checkpoint | Active record: `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. It records band 1 fail-before, implementation, behavior counts, quality gates, sparse resource-free semantics, and the band 2 recovery point. |
| Blocking decision | No blocker. Commit the exact band 1 recovery checkpoint, then implement only band 2. Do not run structured review on partial NNC6.5d4 work. |
...
| NNC6.5d4 | `in_progress` | **Dependency:** NNC6.5d3 complete at `c1c7f1397`. **Evidence:** `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. **Audit checkpoint:** `29137fa000341e48bd9ee41ef37451f5611977f1`. Three read-only audit packets freeze K1-K35 and the exact two-realm contract. **Band 1:** one workloads-owned strict prefix projects the exact durable ordered receipt subset, preserves resource-free gaps, and rejects malformed, reordered, duplicate, stale, or crossed history. Compute authenticates identical prefix bytes through confirmed command, provider observation, and result before reducer or CAS. **Owned paths:** the portable prefix, exact record projection, compute command/result/observation fences, concept-owned tests, proof, and recovery ledger only. No Machine API, provider effect, caller, or `nimbus-network` path changed. **Last green:** fail-before exits `101`; focused passes workloads `2`, compute `5`, crossed result `1`; full workloads passes `218`; full compute passes `374` plus one ignore; format, diff, strict Clippy, warning-denied Rustdoc, NNCV008, proof lint zero, docs `108`, and site `17/17` pass. NNCV035 is `55/55`, direct `0/7`, and aggregate `35/36` sole NNCV035 red. **Next:** band 2 teardown-family final-withdraw operation and narrow confirmed-teardown journal seam. **Required seams:** distinct admitted parent attachment/execution/ingress provider IDs; parent publication withdrawal before guest drain/stop; exact remote guest envelopes and provider-journal reuse; parent release only after exact guest/provider absence; independent parent/guest durable realms; sibling-batch, response-loss, process-death, stale-generation, crossed-forwarder, and ambiguity convergence. **Forbidden:** Compose down, product caller cutover, physical-machine stop, tenant policy, service naming, a CLI-local saga store, a new public route, or coarse-stop deletion. **Blocker:** none. |
