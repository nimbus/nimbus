...
Status: `active; NNC4.6c complete; NNC4.6d in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC4.6c complete; NNC4.6d in progress` |
| Current band | `NNC4 — evidence-based capability registry and sovereignty profile` |
| Current item | `NNC4.6d — wire the manager/root through CLI start/dev/Compose, server listeners, and standalone KV` |
| Last completed item | `NNC4.6c — inject one sandbox-owned OCI process composition into container and krun` |
| Next action | Commit the exact staged NNC4.6c item, then record that commit as NNC4.6d's last-green checkpoint and perform a read-only construction/root/capability call-graph audit across CLI start/dev/Compose, server listeners, and standalone KV before freezing NNC4.6d fail-before tests. |
| Current acceptance checkpoint | NNC4.6c C1-C14 are green. The full review's alias-reconciliation and two-layer proof findings plus the narrow review's retained-alias TOCTOU finding all have exact fail-before and corrected behavioral proof. Final production composition is 5/0/0, IPAM 13/0/0, process contract 4/0/0, and sandbox 722/0/24. Affected check, strict Clippy, warning-denied rustdoc, exact core-only metadata, verifier 15/15 plus self-test 45/45, format, diff, docs 108, and site 17/17 pass. NNC4.6d has not begun executable work. |
...
| Last checkpoint commit | `5166150290952ca4638cb000601f5c38f518ac6a` — NNC4.6b staged local-manager composition; tree `1946832f35ac`. |
| Audit dirty state | The exact NNC4.6c closeout spans the canonical plan/routing/proof plus container, krun, and OCI process-composition, segment, IPAM, port-lifecycle, PEP, Netavark, machine-lifetime, and focused test paths. Constructor logic and test-heavy additions are in concept-owned children. No generated artifact or NNC4.6d-f executable path is modified. NNC4.6d currently owns no dirty executable path. |
| Latest dirty-state checkpoint | HEAD remains the committed NNC4.6b checkpoint `5166150290952ca4638cb000601f5c38f518ac6a`, tree `1946832f35ac7f7c98471ef2ff2b9a3656838fe9`, until the exact staged NNC4.6c item commit. Final NNC4.6c executable digest is `128b7973b101a33efb713f8601fc9a2579d812c1bd4262e549b44f084e59f5f0`; the complete owned item is staged after this ledger closeout. The branch remains unpushed with no PR. |
| Current item review | NNC4.6c full review thread `019fad3e-5ea0-7932-82b9-4b4ab9d45188` and narrow correction thread `019fad58-6d79-74e2-a070-77394bac5b64` are fully dispositioned. Three accepted findings have exact fail-before and corrected proof; the final TOCTOU correction passed affected/static/owner gates. The permitted narrow review is consumed, so no further NNC4.6c review is allowed or warranted. NNC4.6d has no review because implementation has not started. |
...
| Latest verification checkpoint | Final NNC4.6c is green. Initial alias fail-before is 0/1; corrected alias and cross-root cases are 1/0/0 each; retargeted-alias fail-before is 0/1 then 1/0/0 across container and krun; IPAM is 13/0/0; process contract is 4/0/0; production composition is 5/0/0; full sandbox is 722/0/24. Affected all-target check, strict Clippy, warning-denied rustdoc, exact core-only metadata, live verifier 15/15, unchanged self-test 45/45, format, diff, docs 108, and site 17/17 pass. Pre-review network 200/0/0, capability 3/0/0, and real-process store/port 8/0/3 remain frozen evidence. |
| Blocking decision | No blocker. Commit NNC4.6c, then begin NNC4.6d's read-only audit. |
...
| NNC4 — capabilities/sovereignty | `in_progress` | Named negative matrix, evidence-based seams, machine modes, offline local profile. | NNC4.1-NNC4.6c are complete. NNC4.6d now owns production CLI/start/dev/Compose/server/KV manager wiring; NNC4.6e owns host/guest machine authority realms; NNC4.6f owns constructor/root/primitive-handle census closure. NNC4.7 remains the sovereignty tripwire owner. |
...
| NNC4.6c | `done` | Final evidence: `docs/private/plans/proof/nimbus-network-control-plane/nnc4.6c-oci-network-process-composition.md`. C1-C14 pass: one manager-derived OCI process composition injects exact segment/IPAM/port/PEP/Netavark/machine-lifetime authority into container and krun while workload roots remain artifact-only and direct/runner boundaries stay explicit. Initial reviewed executable digest `c05b3ce10397cf4057e861fab0396055536074dcd84c568f54ebd8f2b54ddf19`; final digest `128b7973b101a33efb713f8601fc9a2579d812c1bd4262e549b44f084e59f5f0`. Full review thread `019fad3e-5ea0-7932-82b9-4b4ab9d45188` and narrow correction thread `019fad58-6d79-74e2-a070-77394bac5b64` accepted three findings; canonical-alias reconciliation, two-layer authority proof, and retarget-after-authentication fencing each have exact failing regressions and corrected behavior. Final process 4/0/0, IPAM 13/0/0, production 5/0/0, sandbox 722/0/24, affected check/strict Clippy/rustdoc, exact core-only edge, verifier 15/15 plus self-test 45/45, format/diff, docs 108, and site 17/17 pass. No further review is warranted. |
| NNC4.6d | `in_progress` | **Dependency:** NNC4.6c is green and staged for its exact commit. **Owned paths:** none yet; intended scope is production CLI start/dev/standalone Compose root resolution and capability freeze, server listener manager-derived handles, standalone KV manager-derived handle, plus a concept-owned NNC4.6d proof. Machine host/guest, cluster, provider-effect, policy, naming, and NNC4.6e-f implementation are forbidden. **Last green:** NNC4.6c final candidate on HEAD `5166150290952ca4638cb000601f5c38f518ac6a`, tree `1946832f35ac`, with staged executable digest `128b7973b101a33efb713f8601fc9a2579d812c1bd4262e549b44f084e59f5f0`; replace this with the exact NNC4.6c commit/tree before any NNC4.6d source edit. **Next:** commit NNC4.6c, record its exact hash/tree, then perform a read-only constructor/root/capability and listener call-graph audit before freezing fail-before tests. **Acceptance:** start/dev/Compose/KV resolve one typed logical-node root; Compose workload roots stay distinct but share authority; two projects conflict durably before bind; every server/KV listener carries the manager-derived handle; exact source-owned local attachment/ingress reports freeze without fabricated providers. **Review:** none; not eligible before all written criteria are green. **Blocker:** none. |
