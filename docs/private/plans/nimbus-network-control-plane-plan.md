...
Status: `active; NNC5.2d complete; NNC5.3 in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC5.2d complete; NNC5.3 in progress` |
| Current band | `NNC5 — sandbox-owned deep attachment lifecycle` |
| Current item | `NNC5.3 — complete readiness evidence` |
| Last completed item | `NNC5.2d — startup quarantine application and filename-authority deletion` |
| Next action | Commit the exact NNC5.2d item, then begin NNC5.3 with a read-only substitution audit of every readiness producer and consumer. Record the current netns-only false-ready behavior and design fail-before tests for required Netavark, IPAM, firewall/pin, forwarding, and PEP evidence before changing executable code. Do not run cleanup/release/finalization/reuse, revisit NNC5.2d, push, or open a PR. |
| Current acceptance checkpoint | NNC5.2d is candidate-complete. Its exact fail-before, shared startup adapter, exact read-only adoption, version/claim-fenced quarantine, two-backend durable admission fence, unmatched/unknown preservation, crash/restart convergence, filename-authority deletion, existing-work inspection/cleanup availability, behavioral matrix, affected gates, and verifier/docs proofs are green. The complete 38-path item was frozen at tree `fbf5fb030acd3dfc3abf31ebef3e89fa0bcc5ffb` with staged diff SHA-256 `8fe8c14b1a790001787c342a49e76f55cb19ee9feb82a591940efe78f7129a05`. Its sole full GPT-5.6 Sol/xhigh/fast review returned zero findings, `patch is correct`, and confidence `0.87`; no correction review is warranted. The exact item commit is the remaining transition, after which NNC5.3 begins with read-only readiness substitution audit and fail-before design. |
...
| Last checkpoint commit | `ae29108f3bd2037557727e0036cf0f7ebfc039c0` — exact reviewed NNC5.2c checkpoint; tree `528b4f10a2dd`. |
| Audit dirty state | NNC5.2d's exact 38-path candidate is staged for its item commit. The original dirty checkout retains exactly its four user-owned paths and the `machine-os` companion remains clean. |
| Latest dirty-state checkpoint | NNC5.2d owns exactly 38 staged paths: 26 Rust production/test paths, four visible recovery/prior-proof/census documents, its ignored item proof, and seven verifier paths. Its pre-review frozen tree is `fbf5fb030acd3dfc3abf31ebef3e89fa0bcc5ffb`; its staged diff SHA-256 is `8fe8c14b1a790001787c342a49e76f55cb19ee9feb82a591940efe78f7129a05`. The startup state machine and both backend roots are implemented; obsolete filename-derived authority is deleted. The Krun test composition root was reduced from 2,030 to 1,552 lines by moving its coherent network group into a 488-line concept-owned child. Final affected behavior passes 1,070/1,070 with 24 declared skips. The stale multi-tenant verifier now follows the shared lifecycle and evidence-aware startup seams at 16/16. No unrelated path, rebase, destructive recovery, push, or PR occurred. |
| Current item review | NNC5.2d's one full GPT-5.6 Sol/xhigh/fast review ran only after every preceding acceptance criterion was green and the complete item was frozen. Thread `019fb58a-cf6a-7ad3-92c7-d8856064a1cb` returned zero findings, `patch is correct`, and confidence `0.87`. No accepted defect exists, so no narrow correction review is warranted. NNC5.3 has no structured review; it is entering read-only audit. |
...
| Last verification | NNC5.2d expected-red evidence is 18 pass/1 intended verifier fail, isolated mutations 5/5, and both backend cases 0/2 before implementation. Startup reconciliation passes 8/8, classifier passes 12/12, and real Container/Krun startup authority passes 4/4. The first full affected run was 1,059 pass/9 fail/24 skip and exposed a false quarantine of the legitimate reserved pre-effect restart state. Its real fail-before is 0/1; the corrected positive and six-case negative matrix pass. A second full run was 1,067 pass/2 fail/24 skip, with only two desired-only tests expecting the later planning fence; both now prove the earlier provider-attempt startup fence and byte-preserving authority. Final affected behavior is 1,070/1,070 with 24 declared skips. All-target/all-feature check, strict Clippy, warning-denied rustdoc, exact core-only workspace dependency, live static verifier 19/19, aggregate mutation self-tests 72/72, multi-tenant verifier 16/16 after its 10/16 fail-before, Bash/Node/ShellCheck, format/diff, docs 108, and site 17/17 pass. The one full candidate review is clean; no repeat review is warranted. |
...
| Blocking decision | No blocker. Commit NNC5.2d exactly once, then execute NNC5.3's read-only audit and fail-before work. |
...
| NNC5.2d | `done` | Final evidence: `docs/private/plans/proof/nimbus-network-control-plane/nnc5.2d-startup-quarantine-application.md`. One 227-line concept-owned adapter collects/classifies once and applies only exact version-fenced desired and claim-fenced allocator quarantine; Container and Krun inject the same authorities and durably fence unmatched evidence. Exact adoption, unmatched preservation, crash/restart convergence, existing-work cleanup/inspection, and deletion of every filename-derived live-set authority are proven. State machine 8/8, classifier 12/12, backend startup authority 4/4, affected crates 1,070/1,070 with 24 declared skips, check/strict Clippy/rustdoc, core-only edge, verifier 19/19 plus 72/72 mutations, multi-tenant verifier 16/16, format/diff/docs pass. The sole full Sol/xhigh/fast review is clean with zero findings at confidence 0.87; no correction review is warranted. Cleanup/release/finalization/reuse remains NNC8.3-owned. |
| NNC5.3 | `in_progress` | **Dependencies:** NNC5.2d is candidate-complete and closes in the exact preceding item commit. **Scope/evidence:** define complete readiness evidence; netns existence alone cannot report ready, and required Netavark, IPAM, firewall/pin, forwarding, and PEP conditions must be asserted without moving provider effects or policy into `nimbus-network`. **Owned paths:** read-only audit first; exact implementation paths and proof artifact will be recorded before executable edits. **Last green:** NNC5.2d state machine 8/8, classifier 12/12, backend authority 4/4, affected crates 1,070/1,070 with 24 declared skips, all quality/dependency/verifier/docs gates, and one clean full review. **Next:** inventory every readiness producer/consumer and substitution point, identify current netns-only false-ready behavior, and add exact fail-before criteria/tests before implementation. **Blocker:** none. |
| NNC5.4 | `todo` | — |
| NNC5.5 | `todo` | — |
| NNC5.6 | `todo` | — |
| NNC6.1 | `todo` | — |
