...
Status: `active; NNC7.5 complete; NNC7.6 in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC7.5 complete; NNC7.6 in progress` |
| Current band | `NNC7 — integration and observed projection cutover` |
| Current item | `NNC7.6 — guard TLS authorities and telemetry cardinality` |
| Last completed item | `NNC7.5 — projection independence`; the commit containing its completed ledger row is the durable item commit. |
| Next action | Derive the NNC7.6 ingress-certificate, interception-CA, and metric-label census from current source; freeze exact acceptance and expected-red evidence before product edits. |
| Current acceptance checkpoint | NNC7.5 F1-F6 and A1-A12 are complete in `proof/nimbus-network-control-plane/nnc7.5-projection-independence.md`. Projection loss, lag, deletion, automatic or explicit rebuild, stale updates, and writer failure cannot change listener, machine, services, saga, lease, provider, or network authority. |
...
| Execution base | Current `origin/main` is `8877eaff43a36d9606a1feaa0ab31d0377539d9d`; the pre-NNC7.5 checkpoint is `0 behind / 162 ahead`. No merge or rebase is required. |
| Last checkpoint commit | NNC7.4 item commit `7057fece3897f116a51cde06e972c88258cd913a` is the current durable checkpoint. |
| Owned paths | NNC7.6 initially owns a read-only source census and one proof artifact. Any product or verifier path must be justified by a frozen defect. It cannot merge ingress certificate authority with the proxy interception CA, move TLS effects into `nimbus-network`, add unbounded identity labels, or enter NNC8 recovery. |
| Latest dirty-state checkpoint | The exact NNC7.5 candidate is staged across its `19` owned paths. No unrelated path is dirty. NNC7.6 product work has not started. |
| Last completed item review | NNC7.5 used one full and one narrow GPT-5.6 Sol/xhigh/fast review. Five P2 findings were accepted, corrected, and proven. Review cadence is exhausted; details remain in the item proof. |
...
| Last green | NNC7.5: focused behavior `35/35`; full system `84/84`, compute `473 + 1 ignored`, server `753 + 35 ignored`; strict affected Clippy/Rustdoc; format/diff; verifier `36/36`; docs `108`; site `17/17`; proof lint zero. External provider execution is explicitly disabled and is not claimed. |
| Latest verification checkpoint | The final review correction advances rebuild revisions so older in-flight success cannot clear requested repair. Its filtered connectivity set passes `14/14`; full system passes `84/84`; final system strict Clippy/Rustdoc pass. |
| Next | Commit NNC7.5, then run the read-only NNC7.6 authority/cardinality census and freeze its acceptance contract. |
| Blocker | none |
...
| NNC7.6 | `in_progress` | **Dependency:** NNC7.5 completes in the commit containing the prior row. **Scope:** prove ingress certificate providers remain distinct from the proxy interception CA and every network metric label has bounded cardinality. **Owned paths:** read-only census, item proof, plan, and only defect-proven owner tests or verifier rules. **Forbidden:** moving TLS/provider effects into `nimbus-network`, merging certificate authorities, public provider growth, telemetry redesign, or NNC8 recovery. **Last green:** NNC7.5 closeout gates above. **Next:** derive the complete source census and freeze falsifiable acceptance before edits. **Blocker:** none. |
| NNC8.1 | `todo` | — |
