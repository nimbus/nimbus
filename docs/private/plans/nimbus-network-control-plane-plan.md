...
Status: `active; NNC7.2 complete; NNC7.3 in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC7.2 complete; NNC7.3 in progress` |
| Current band | `NNC7 — integration and observed projection cutover` |
| Current item | `NNC7.3 — return portable endpoint and attachment handles from sandbox and machine status; read-only audit next` |
| Last completed item | `NNC7.2 — services-owned stable endpoint identity and generation`; the commit containing its completed ledger row is the durable item commit. |
| Next action | Trace sandbox and machine status carriers, provider evidence, address-derived identity, redaction, and current consumers. Freeze the exact substitution seam before product edits. |
| Current acceptance checkpoint | NNC7.2 G1-G8 are green. The full review's one P2 identity-loss defect is corrected. The one narrow Sol/xhigh/fast review is clean at confidence `0.96`. |
...
| Execution base | Current `origin/main` is `8877eaff43a36d9606a1feaa0ab31d0377539d9d`; NNC6.1e2 item commit `fb9880e16b9b0a95768fe4f217318fa4532229aa` is `0 behind / 155 ahead`. No merge or rebase is required. |
| Last checkpoint commit | NNC7.1a item commit `43f07a5b5883e8d60ff2e1f38b8af516256482f3`; current divergence is `0 behind / 158 ahead` of `origin/main`. |
| Audit dirty state | The staged NNC7.2 item contains seventeen Rust product/test paths, its proof, this plan, the routing transition, and the initial NNC7.3 proof. No unrelated path is dirty; the original checkout remains untouched. |
| Latest dirty-state checkpoint | NNC7.2 is acceptance-green and review-exhausted. NNC7.3 owns no dirty product path and starts with a read-only status-carrier audit. |
| Last completed item review | NNC7.2 used one full and one narrow GPT-5.6 Sol/xhigh/fast review. The full review accepted one P2 withdrawal identity-loss defect at confidence `0.93`. The correction is proven. The narrow review used staged tree `42cdf9827b42515467ee1ffd0825609984bbf229`, patch SHA-256 `70ef5e0c5209bbd31ffd7b2ca36696360065b9795b53624821b74458eb156516`, nineteen paths, and reported no finding at confidence `0.96`. Review cadence is exhausted. |
...
| Last verification | NNC7.2 G1-G8 pass. Network `275 + 1 ignored`, services `101`, compute `469 + 1 ignored`, server `685 + 35 ignored`, CLI `1,007 + 4 ignored`, workspace check, strict quality, architecture `36/36`, docs `108`, and site `17/17` pass. |
| Latest verification checkpoint | Stable endpoint identity survives withdrawal and exact republish. Stale, crossed, and mismatched evidence fail before mutation. The narrow correction review is clean at `0.96`; review cadence is exhausted. |
| Blocking decision | No blocker. Commit NNC7.2 and continue the NNC7.3 read-only audit. |
...
| NNC7 — integrations/projections | `in_progress` | Protocol parity, stable service/machine handles, rebuild-safe projections, TLS guard. | NNC7.3 is the sole active item; its sandbox and machine status-carrier audit is next. |
...
| NNC7.3 | `in_progress` | **Dependency:** NNC7.2 is complete. **Evidence:** `proof/nimbus-network-control-plane/nnc7.3-portable-provider-status-handles.md`. **Current scope:** read-only trace of sandbox and machine status carriers, provider evidence, address-derived identity, opaque-handle redaction, and consumers. **Owned paths:** initial proof plus concise plan/routing transition; no product path until the audit freezes one exact seam. **Forbidden:** provider effects, transport, tenant policy, logical naming, NNC7.4 projection schemas, and NNC8 recovery changes. **Last green:** NNC7.2 item gates. **Next:** freeze current-versus-target ownership, acceptance matrix, and deterministic address-change fail-before. **Blocker:** none. |
| NNC7.4 | `todo` | — |
| NNC7.5 | `todo` | — |
| NNC7.6 | `todo` | — |
| NNC8.1 | `todo` | — |
