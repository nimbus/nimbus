...
Status: `active; NNC8.1 complete; NNC8.2 in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC8.1 complete; NNC8.2 in progress` |
| Current band | `NNC8 — full recovery, ambiguity, and state-machine proof` |
| Current item | `NNC8.2 — exact inspection and live-claim authority across provider commands` |
| Last completed item | `NNC8.1 — rehydrate and reconcile every persisted phase`; the commit containing its completed ledger row is the durable item commit. |
| Next action | Audit every provider-command producer and freeze the NNC8.2 ambiguity and delayed-claim matrix before product edits. |
| Current acceptance checkpoint | NNC8.1 K1-K12 are green. The common process owner has five direct consumers; all recovery matrices, the affected mutation lane `4/4`, aggregate `37/37`, and one full plus one clean narrow review pass. |
...
| Execution base | Current `origin/main` is `8877eaff43a36d9606a1feaa0ab31d0377539d9d`; the NNC8.1 parent checkpoint `23bf486f8752f1fcafb0dc02e9a157f4d225ccac` is `0 behind / 164 ahead`. No merge or rebase is required. |
| Last checkpoint commit | The commit containing the completed NNC8.1 row is the durable item checkpoint. Its parent clean checkpoint is `23bf486f8752f1fcafb0dc02e9a157f4d225ccac`. |
| Owned paths | NNC8.2 is in read-only producer and ambiguity audit. It owns no product path until its exact producer census, claim lifetime, ambiguous outcome, and deterministic race matrix are frozen in its proof. |
| Latest dirty-state checkpoint | The completed NNC8.1 diff is fully attributed, both verifier corrections pass direct proofs, and review cadence is exhausted. NNC8.2 has no product edit. |
| Last completed item review | NNC8.1 used one full and one narrow GPT-5.6 Sol/xhigh/fast review. Two P2 verifier-proof findings were accepted and corrected; the narrow review is clean at `0.98`. Review cadence is exhausted. |
...
| Last green | NNC8.1 K1-K12: full affected behavior, strict quality, affected mutations `4/4`, NNCV020 `9/9`, aggregate `37/37`, docs `108`, site `17/17`, proof lint zero, and clean narrow review at `0.98`. |
| Latest verification checkpoint | Harness `13 + 2 ignored`; state/port `8 + 3`; provider-private `19 + 3`; CLI `1,007 + 4`; sandbox `1,162 + 30`; server `659 + 35`; KV `10 + 2`; NNCV020 mutations `9/9`; NNC8.1 mutations `4/4`; aggregate `37/37`; strict Clippy and authored-crate Rustdoc; docs `108`; site `17/17`. |
| Next | Commit NNC8.1, then freeze the NNC8.2 provider-command producer and ambiguity matrix. |
| Blocker | none |
...
| NNC8 — recovery/fencing | `in_progress` | Failure table and crash/stale/ambiguity model fully closed. | NNC8.1 is complete. NNC8.2 is active in read-only producer and ambiguity audit. |
...
| NNC8.2 | `in_progress` | **Dependency:** NNC8.1 is complete. **Routed discovery from NNC6.5d1:** earlier provision and restart producers discard `ExecuteClaimed` before their effects. **Current step:** read-only census of every provider-command producer, exact inspection branch, and delayed claim lifetime. **Owned paths:** none until the proof freezes the producer inventory and deterministic race matrix. **Acceptance:** every producer holds current authority across its effect boundary, and delayed epoch N cannot act after exact inspection authorizes epoch N+1. **Last green:** NNC8.1 K1-K12, aggregate `37/37`, docs `108`, and site `17/17`. **Next:** freeze fail-before evidence and the smallest complete behavior matrix before product edits. **Blocker:** none. |
| NNC8.3 | `todo` | — |
| NNC8.4 | `todo` | — |
| NNC8.5 | `todo` | — |
| NNC8.6 | `todo` | — |
