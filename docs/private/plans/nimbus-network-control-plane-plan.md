...
| Next action | Implement NNC6.5d2 from `proof/nimbus-network-control-plane/nnc6.5d2-krun-execution-teardown.md`. First retain strict backend failure codes in the existing provider observation. Then add the Krun exact teardown state, admission fences, authenticated effects, compute adapter, and fail-before test roster. Do not run structured review before K1-K24 are green. |
| Current acceptance checkpoint | NNC6.5d2 K1-K3 are green from three read-only audits; K4-K23 are expected red from six exact source checks. The audit proves that NNC6.5d1's conmon process identity and `krun-runtime` provider journal are sufficient. It also finds one shared result defect: backend-specific `DefiniteFailure` codes are not durable today. NNC6.5d2 will repair the existing observation envelope, not add a second journal. Product source is unchanged at this checkpoint. |
...
| Last checkpoint commit | The commit containing this checkpoint is the durable NNC6.5d2 read-only audit and fail-before commit. Its parent, `79b122bdc49d45a6009c203ee413d454de136c99`, is the complete NNC6.5d1 item checkpoint. |
| Audit dirty state | Only the NNC6.5d2 proof, this plan, and the routing index belong to the expected-red checkpoint. NNC6.5d2 has no product diff. The original checkout remains untouched. |
| Latest dirty-state checkpoint | The commit containing this header is the durable NNC6.5d2 read-only audit and fail-before checkpoint. Its parent is the clean NNC6.5d1 item commit `79b122bdc49d45a6009c203ee413d454de136c99`. |
...
| Last verification | The clean NNC6.5d1 baseline remains green with provider journal `19` plus one child-only ignore, Container teardown `19` plus one child-only ignore, compute adapter `6`, full compute `352` plus one ignore, and serialized full sandbox `1,049` plus 44 ignores. NNC6.5d2 records six expected-red source checks at exit `1`, a current Krun inventory of `149` test attributes with `3` child-only ignores, proof lint zero, docs `108`, site `17/17`, NNCV035 self-test `55/55`, direct expected `0/7`, aggregate `35/36` with only NNCV035 red, zero product changes, and zero structured reviews. |
| Latest verification checkpoint | Canonical record: `proof/nimbus-network-control-plane/nnc6.5d2-krun-execution-teardown.md`. The prior completed item remains `proof/nimbus-network-control-plane/nnc6.5d1-container-execution-teardown.md`. |
| Blocking decision | No blocker. Implement the durable NNC6.5d2 contract. Do not reopen NNC6.5d1 and do not review partial NNC6.5d2 work. |
...
| NNC6.5d2 | `in_progress` | **Dependency:** NNC6.5d1. **Evidence:** `proof/nimbus-network-control-plane/nnc6.5d2-krun-execution-teardown.md`. Krun execution drain/stop uses the same neutral command, conmon process identity, and `krun-runtime` provider journal, with no second journal or network teardown. **Owned paths:** `crates/nimbus-compute/src/workload_saga/teardown_sandbox/krun.rs`, narrow shared compute mapping/exports, and attributed tests; `crates/nimbus-sandbox/src/backends/krun/vm/teardown.rs` and concept-owned children/tests; narrow Krun manifest, provision, creator, restart, inspection, lifecycle, composition-root, and test-support edits. The audit also owns the strict failure-code field in the existing provider observation, its journal tests, and the narrow Container consumer regression needed to keep the shared contract exact. Shared conmon changes are not authorized because NNC6.5d1 is sufficient. Attachment detach/release, machine transport, caller cutover, coarse-stop deletion, provision/restart producer migration, and a second journal are forbidden. **Last green:** clean NNC6.5d1 baseline; NNC6.5d2 proof lint, format, diff, docs `108`, site `17/17`, and NNCV035 self-test `55/55` pass; direct NNCV035 remains expected `0/7`. **Expected red:** six source checks exit `1`; exact Krun teardown and compute adapter paths are absent, the manifest has no teardown state, admissions have no drain fence, and coarse stop uses raw PID plus final network release. **Next:** implement K4-K23, run K24 gates, freeze the candidate, then run the one K25 Sol/xhigh/fast item review. **Blocker:** none. |
