...
Status: `active; NNC7.3 complete; NNC7.4 in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC7.3 complete; NNC7.4 in progress` |
| Current band | `NNC7 — integration and observed projection cutover` |
| Current item | `NNC7.4 — extend connectivity projections without overloading HTTP route inventory; read-only substitution audit starts next` |
| Last completed item | `NNC7.3 — portable endpoint and attachment handles from sandbox and machine status`; the commit containing its completed ledger row is the durable item commit. |
| Next action | Read the NNC7.4 projection owners and active projection plans, then freeze a fail-before contract and exact owned paths before product edits. |
| Current acceptance checkpoint | NNC7.3 A1-A12 pass. The full review accepted one P2 A3 endpoint-correlation defect at confidence `0.97`; the correction is proven. The sole narrow Sol/xhigh/fast review is clean at confidence `0.98`. Review cadence is exhausted. |
...
| Last checkpoint commit | NNC7.2 item commit `d031a1bdeee0e322fa822254668a1f971169067d`; the staged NNC7.3 item commit is the next durable checkpoint. |
| Owned paths | NNC7.4 starts read-only. Until its scope freezes, only this plan and a concept-owned NNC7.4 proof are eligible edits. Product projection paths remain forbidden until the audit names their existing owners and proves that no canonical desired-state or lease authority will move into `nimbus-system`. |
| Latest dirty-state checkpoint | NNC7.3 is candidate-complete and fully staged across exactly 39 attributed paths. The item commit should leave a clean worktree before the NNC7.4 read-only audit. |
| Last completed item review | NNC7.3 used one full and one narrow GPT-5.6 Sol/xhigh/fast review. The full review accepted one P2 endpoint-correlation defect at confidence `0.97`. The corrected candidate used staged tree `cb2bc40e162b576b99edaec82e23f559a5069b16`, patch SHA-256 `820e2a28dfc5ced8978982eafa5a46b354c2585d6cbed4450fa8e786a961055b`, and 39 paths. The narrow review reported no findings at confidence `0.98`. Review cadence is exhausted. |
...
| Last green | NNC7.3: network `276 + 1 ignored`; sandbox `1,172 + 45 ignored`; compute `472 + 1 ignored`; machine `48`; CLI `1,007 + 4 ignored`; serialized server `752 + 35 ignored`; strict quality; verifier `36/36`; docs `108`; site `17/17`; proof lint zero. |
| Latest verification checkpoint | NNC7.3 A1-A12 pass after correction. The corrected constructor and persisted decode path reject crossed endpoint identity. The narrow review is clean at `0.98`. |
| Next | Begin the NNC7.4 read-only projection substitution audit from current source and active owner plans. |
| Blocker | none |
...
| NNC7 — integrations/projections | `in_progress` | Protocol parity, stable service/machine handles, rebuild-safe projections, TLS guard. | NNC7.4 is the sole active item; its read-only projection substitution audit is next. |
...
| NNC7.4 | `in_progress` | **Dependency:** NNC7.3 is complete in the commit containing the prior row. **Owned paths:** read-only audit only until scope freeze; this plan and a concept-owned NNC7.4 proof may record findings. **Forbidden:** product edits before the audit maps `nimbus-system`, server, services, machine, and network projection owners; moving desired state, lease authority, or provider effects into observed projections. **Last green:** NNC7.3 gates above. **Next:** freeze the current-versus-target projection contract, fail-before cases, and exact product paths. **Blocker:** none. |
| NNC7.5 | `todo` | — |
| NNC7.6 | `todo` | — |
| NNC8.1 | `todo` | — |
