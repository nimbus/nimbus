...
Status: `active; NNC5.2a complete; NNC5.2b in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC5.2a complete; NNC5.2b in progress` |
| Current band | `NNC5 — sandbox-owned deep attachment lifecycle` |
| Current item | `NNC5.2b — durable-evidence startup orphan classification` |
| Last completed item | `NNC5.2a — exact association and first-effect ordering` |
| Next action | Commit the exact reviewed NNC5.2a 61-path checkpoint locally, record its commit/tree in the recovery header, then resume NNC5.2b's read-only durable-evidence classifier audit from its recorded eight-row fail-before. Do not push or open a PR. |
| Current acceptance checkpoint | NNC5.2a criteria 1-17 are green and this exact staged commit closes criterion 18. One immutable portable attachment association authenticates claim, segment, epoch, plan/version/generation/digest, attachment, tenant, and selected provider. The sandbox IPAM journal remains the sole transient provider-attempt authority and is prepared before namespace, listener, Netavark, machine-forwarding, or cleanup callbacks. Prepared no-effect attempts reopen idempotently; crossed pre-effect fences are inspection-only and cannot duplicate effects. Container host-managed, Container machine-forwarded, and Krun routes share the lifecycle. The full item review found one real final-detach cross-authority crash wedge; the narrow review found one retry continuation of that wedge. Both reproduce and are corrected. Exact host-managed plus machine-forwarded recovery proofs pass `2/2`, including fallible callbacks, while restart remains fenced. Required proofs 1-11 pass; portable `233/233` with one skip, lifecycle `41/41`, provider-cleanup `30/30`, Netavark `12` with three intentional ignores, actual backend lane `332/332`, and full sandbox `785/785` with 24 skips pass. Check, strict Clippy, warning-denied rustdoc, format/diff, script quality, live verifier `18/18`, verifier self-test `67/67`, docs `108`, and site `17/17` pass. |
...
| Last checkpoint commit | `94263586d4f53eb30d504f4f23f4d5ac1fb5bb10` — exact reviewed NNC5.2 checkpoint; tree `501734a47b0a`. |
| Audit dirty state | The three bounded NNC5.2a audits and prospective split are preserved in the proof. The implementation now changes only named portable attachment/segment paths; sandbox lifecycle, allocator, IPAM, Netavark, actual backend composition, and focused tests; concept-owned verifier helpers; two source-derived census locators; and canonical plan/proof routing. Existing staged routing edits were preserved. The original dirty checkout retains exactly its four user-owned paths and the `machine-os` companion remains clean. |
| Latest dirty-state checkpoint | NNC5.2 is durable as exact 32-path commit `94263586d4f53eb30d504f4f23f4d5ac1fb5bb10`. NNC5.2a's initial 61-path freeze had executable SHA-256 `22ddd3732d985af93e409a5d045373d87e4bb51d7599adb5e42fdfcd73972712`; its narrow-reviewed correction was `359592d2b6bd1614c0c0903bab1b2e216c410d2b7901bcc15597e5517ae41509`. The final candidate remains exactly 61 staged paths with zero unstaged changes, cached diff integrity green, and executable SHA-256 `f0c8e7078b55e457776ae7d7cd83cca6f213ce3d8d56e965f1b3a741c90eaf85`. No unrelated path, rebase, destructive recovery, push, or PR occurred. |
| Current item review | NNC5.2a's sole full GPT-5.6 Sol/xhigh/fast review ran as one integrated 429,453-byte pass under thread `019fb3a3-d6c1-7d03-b54d-b98cf74b9637`. One P1 final-detach crash wedge was accepted, reproduced, and corrected; one alleged missing-forwarder panic was rejected with public-dispatch evidence. The one permitted narrow review ran as a 445,782-byte Sol/xhigh/fast pass under thread `019fb3cf-b1da-78b2-b1a1-9dfb37dd4863` and found one accepted P1 `CleanupPending` retry continuation. Its fail-before is `0/2`; the final correction passes `2/2` plus every affected gate. Per item cadence, no third review ran or is warranted. |
...
| Last verification | NNC5.2a corrected executable candidate: accepted-defect proofs `2/2`; required proofs 1-11; portable `233/233` with one skip; lifecycle `41/41`; provider cleanup `30/30`; Netavark `12` with three intentional ignores; actual Container/Krun `332/332`; full sandbox `785/785` with 24 skips. |
| Latest verification checkpoint | Affected all-target/all-feature check, strict Clippy, warning-denied rustdoc, format/diff, Node/shell syntax, ShellCheck, exact dependency/effect verifier `18/18`, adversarial verifier self-test `67/67`, docs `108`, and site `17/17` pass. Exact final detach reopens after allocator finalization for host-managed Container/Krun and machine-forwarded Container; callback failures retain `CleanupPending` and retry to `Released`; restart rejects before provider inspection/effects. No production workaround was added. Only the exact local commit remains. |
| Blocking decision | No blocker. NNC5.2a is candidate-complete and proceeding through its written closeout sequence. |
...
| NNC5.2b | Replace filename-only startup orphan classification with durable evidence. | A pure classifier consumes canonical attachment intent, exact allocator association, sandbox provider-attempt/locator evidence, and untrusted artifact observations. The deterministic matrix covers hold+desired+effect, hold+no desired+effect, hold+no netns, effect+no hold, manifest+no hold, hold+netns+no manifest, stale-generation evidence, and unknown inspection; only exact current evidence adopts, every other row quarantines without provider cleanup or capacity reuse, and NNC8.3 remains the cleanup-convergence owner. |
...
| NNC5.2b | `in_progress` | **Dependencies:** NNC5.2a is complete in the exact staged checkpoint. **Scope:** replace the filename-derived live set with a pure durable-evidence classifier and exact quarantining mutations; provider cleanup/release convergence remains NNC8.3. **Fail-before:** `nnc0_7_orphan_recovery_must_classify_the_complete_evidence_matrix` freshly exited 101 at 0/1 and reported all eight rows mismatched. **Owned paths:** plan/proof routing only until the read-only audit freezes exact implementation paths; no executable source mutation has begun. **Last green:** NNC5.2a's exact candidate gates remain green; NNC5.2b has no correction green yet. **Next:** after the NNC5.2a durability commit, audit candidate enumeration, sandbox locator evidence, all eight decisions, and verifier mutations against the completed association/attempt evidence. **Blocker:** none. |
| NNC5.3 | `todo` | — |
| NNC5.4 | `todo` | — |
| NNC5.5 | `todo` | — |
| NNC5.6 | `todo` | — |
