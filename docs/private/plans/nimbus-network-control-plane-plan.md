...
Status: `active; NNC6.5e complete; NNC6.5f in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC6.5e complete; NNC6.5f in progress` |
| Current band | `NNC6 — compute orchestration and exact lifecycle order` |
| Current item | `NNC6.5f — Compose, guest/forwarded composition, and physical-machine boundary cutover` |
| Last completed item | `NNC6.5e — native service, sandbox, and definition caller cutover`; the commit containing this ledger transition is its durable item checkpoint. |
| Next action | Start the NNC6.5f read-only substitution audit. Freeze the complete local/forwarded Compose down, guest/machine, and physical-machine stop call graphs, durable-store composition, exact provider-envelope use, active-workload conflict behavior, path ownership, fail-before proofs, and prospective acceptance-bearing split before product edits. Keep compensation, tenant retirement, coarse-authority deletion, and final NNCV035 convergence in NNC6.5g. |
| Current acceptance checkpoint | NNC6.5e K1-K32 are green in `proof/nimbus-network-control-plane/nnc6.5e-native-source-retirement-cutover.md`. Its exact 23-test roster and all seven affected suites pass. Compute is `403 + 1 ignore`, services `89`, server `719 + 33 ignores`, and CLI `995 + 3 ignores`; the unchanged full workloads, sandbox, and network evidence remains green. Strict Clippy and warning-denied rustdoc pass. The aggregate self-test is `502/502`; the teardown helper is `88/88`; the native stage is `1/1`; and the live aggregate is the exact expected `35/36` with only the six NNC6.5f-NNC6.5g owner groups red. Both authorized NNC6.5e review invocations are fully dispositioned, and its review cadence is exhausted. NNC6.5f starts with no product edit. |
...
| Last checkpoint commit | The commit containing the NNC6.5e `done` row is its exact self-authenticating item checkpoint. The prior exact checkpoint is NNC6.5d4 at `6b685627c94ea53705019778b108e47f252fd08a`; its last recovery slice was band 8 at `3efa99911`. |
| Audit dirty state | The exact NNC6.5e product, tests, static contracts, proof, ledger, and routing closeout are staged for one item commit. NNC6.5f has no product edit. The original checkout remains untouched. |
| Latest dirty-state checkpoint | NNC6.5e implementation, behavioral, static, quality, complexity, review disposition, documentation, routing, and K32 recovery acceptance are complete. The exact `59`-path item is ready for its self-authenticating commit. |
| Last completed item review | NNC6.5e used one full and one narrow GPT-5.6 Sol/xhigh/fast invocation. The narrow wrapper split its large input into two internal passes, but this remains one correction-review invocation. Six narrow findings are accepted and corrected; the fixed-count-poll claim against the server deadline-bounded semantic loop is rejected with exact source and `1/1` test evidence. Review cadence is exhausted. |
...
| Last verification | Exact roster `23/23`; workloads `219`; compute `403 + 1 ignore`; services `89`; serialized server `719 + 33 ignores`; CLI `995 + 3 ignores`; serialized sandbox `1,164 + 48 ignores`; network `274 + 1 ignore`; aggregate self-test `502/502`; teardown mutations `88/88`; native static stage `1/1`; strict quality and dependency/effect scans green. The final live aggregate is the expected `35/36`, docs are `108`, the site is `17/17`, proof lint has zero diagnostics, and plan/ledger gates pass. |
| Latest verification checkpoint | Completed NNC6.5e record: `proof/nimbus-network-control-plane/nnc6.5e-native-source-retirement-cutover.md`. It records K1-K32 green, both review inputs and dispositions, final corrected candidate identity, exact failure evidence, and later-owner exclusions. |
| Blocking decision | No blocker. Commit the exact NNC6.5e item, then start the NNC6.5f read-only substitution audit. |
...
| NNC6 — compute choreography | `in_progress` | Exact provision/teardown observers across every workload/retirement path. | NNC6.1-NNC6.5e and NNC6.1e1 are complete. NNC6.5f is active; NNC6.5g remains the later compensation, tenant-retirement, deletion, and convergence unit. No completed item retains a temporary legacy authority. NNC6.6 retains resolver fencing; NNC6.1e2 is the final convergence gate. Provider effects remain with their named owners. |
...
| NNC6.5f | `in_progress` | **Dependency:** NNC6.5c and NNC6.5d4 are complete. **Scope:** local/forwarded Compose down, guest/forwarded composition, and physical-machine boundaries only. **Owned paths:** none at activation. **Last green:** the NNC6.5e closeout has K1-K32 green, live aggregate `35/36` with only the exact six NNC6.5f-NNC6.5g groups red, docs `108`, site `17/17`, and recovery/routing gates green. **Next safe action:** create the source-derived NNC6.5f read-only substitution audit, freeze all callers and failure proofs, and split prospectively before product edits if one coherent review unit cannot own the complete value. **Forbidden:** compensation, tenant retirement, coarse-authority deletion, final NNCV035 convergence, policy, naming, resolver authority, network-crate effects, and a CLI-local saga store. **Blocker:** none. |
