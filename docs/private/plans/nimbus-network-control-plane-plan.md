...
Status: `active; NNC3.7a complete; NNC3.7b bind/allocation census closure in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC3.7a complete; NNC3.7b bind/allocation census closure in progress` |
| Current band | `NNC3 — cross-process host-global PortLease` |
| Current item | `NNC3.7b — close the source-derived bind/allocation census` |
| Last completed item | `NNC3.7a — CLI dev/start port migration` |
| Next action | Commit the exact reviewed NNC3.7a checkpoint, then begin NNC3.7b with a read-only regeneration/reconciliation of every baseline bind, allocation, probe, exemption, and shared-authority consumer against `nnc0.1-bind-owner-inventory.json` and NNCV005/NNCV006. Freeze the mechanical fail-before case for any missing or misclassified site before editing production code. Do not start producer-wide restart convergence (NNC3.8), old-authority deletion (NNC3.9), or structured sibling-task unwind (NNC7.1a). |
...
| Last checkpoint commit | `77f075750109c56abe0d4086a4e244a4f99ba20e` — reviewed NNC3.7 completion and NNC3.7a activation. |
| Audit dirty state | The exact reviewed NNC3.7a completion candidate has 18 owned paths listed in `docs/private/plans/proof/nimbus-network-control-plane/nnc3.7a-cli-dev-start-port-migration.md`; the routing-index transition is the only path added after the clean review and all are staged. The first Sol review's one material lifecycle finding is fixed and the required rerun is clean. NNC3.7b is activated in this ledger but has no implementation-owned dirty path. No NNC3.8, NNC3.9, or NNC7.1a path is admitted. |
...
| Last verification | NNC3.7a written acceptance passes 3/3. Focused behavioral acceptance passes 12/12, including continuous provider-assigned ownership, pure start desired state, conflict guidance, incarnation rejection, explicit cleanup, bundle drop, and `ServeOptions` drop/replacement cleanup. The complete affected lane passes 1,648/1,648 with 29 declared skips and no leak diagnostic. Check, strict Clippy, warning-denied rustdoc, format/diff, inventory JSON, exact core-only workspace edge, verifier self-tests 16/16 plus live 14/1 solely later-owned NNCV005, docs 108, and site 17/17 pass. The first Sol/xhigh/fast review's one finding is accepted and fixed; its required identical rerun is clean with zero findings. Exact evidence is in the NNC3.7a proof. |
| Blocking decision | No blocker. NNC3.7a is complete and NNC3.7b is active. Source-derived census closure is the only newly executable implementation scope; fresh-process cleanup-pending convergence and old-authority deletion remain owned by NNC3.8 and NNC3.9. |
...
| NNC3.7b | `in_progress` | Activation checkpoint: the reviewed NNC3.7a completion commit is being formed; after it lands, `HEAD` is the recovery proof. Owned paths: none. Last green: the complete NNC3.7a evidence above. Next action: read-only bind/allocation census regeneration, inventory/verifier reconciliation, and deterministic missing/misclassified-site fail-before proof before production edits. Blocker: none. |
| NNC3.8 | `todo` | — |
| NNC3.9 | `todo` | — |
| NNC4.1 | `todo` | — |
| NNC4.2 | `todo` | — |
