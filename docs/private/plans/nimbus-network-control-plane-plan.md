...
Status: `active; NNC7.1 complete; NNC7.1a in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC7.1 complete; NNC7.1a in progress` |
| Current band | `NNC7 — integration and observed projection cutover` |
| Current item | `NNC7.1a — server-owned structured listener group; read-only audit next and product implementation not started` |
| Last completed item | `NNC7.1 — preserve WireProtocolAdapter and socket owners while consuming leases/status`; the commit containing its completed ledger row is the durable item commit. |
| Next action | At the clean NNC7.1 checkpoint, audit the existing sibling startup, guard, projection, spawn, supervision, shutdown, and inherited-socket paths. Freeze acceptance before product edits. |
| Current acceptance checkpoint | `proof/nimbus-network-control-plane/nnc7.1-wire-protocol-listener-parity.md` records P1-P12 green. The verification-only conclusion passed one Sol/xhigh/fast review at `0.88`. NNC7.1a retains the named NNCF17 expected-red. |
...
| Last checkpoint commit | NNC7.1 start checkpoint `f51b680b41c862a958c88e99e1f010885c6d09fd`; replace this value with the NNC7.1 item hash at the first clean NNC7.1a checkpoint. |
| Audit dirty state | The exact NNC7.1 proof/plan closeout is staged for its item commit. Product implementation was not needed. The original checkout remains untouched. |
| Latest dirty-state checkpoint | `proof/nimbus-network-control-plane/nnc7.1-wire-protocol-listener-parity.md` records P1-P12, source ownership, exact verification commands, scope boundaries, and the next acceptance-bearing action. |
...
| Last verification | NNC7.1 focused server parity passes `45/45`; complete KV passes `26 + 3 ignored`; exact-product full server evidence remains `741 + 36 ignored`; NNC7.1a reproduces only NNCF17 at `0/1`; affected check, strict Clippy/Rustdoc, format/diff, live architecture `36/36`, proof lint zero, docs `108`, and site `17/17` pass. |
| Latest verification checkpoint | Candidate NNC7.1 record: `proof/nimbus-network-control-plane/nnc7.1-wire-protocol-listener-parity.md`. |
| Blocking decision | No blocker. Commit NNC7.1, then begin the read-only NNC7.1a audit. |
...
| NNC7 — integrations/projections | `in_progress` | Protocol parity, stable service/machine handles, rebuild-safe projections, TLS guard. | NNC7.1 is the sole active item; its read-only ownership and protocol-parity audit is next. |
...
| NNC7.1a | `in_progress` | **Dependency:** NNC7.1 is complete. **Current scope:** read-only audit of sibling preparation, guard, projection, spawn, supervision, shutdown, and inherited-socket ownership around the existing NNCF17 expected-red. Product implementation has not started. **Owned paths:** none until the audit freezes one concept-owned listener-group seam and proof. **Last green:** NNC7.1 P1-P12; NNCF17 reproduces at `0/1` only because the first sibling remains live after the kth failure. **Next:** freeze source ownership, acceptance criteria, failure matrix, and non-goals before edits. **Blocker:** none. |
| NNC7.2 | `todo` | — |
| NNC7.3 | `todo` | — |
| NNC7.4 | `todo` | — |
| NNC7.5 | `todo` | — |
