...
| Next action | Implement NNC6.5d4 band 8 from `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`: add the parent confirmed-publication progression and one real `ForwardedMachineTeardownAdapter` for all five compute capabilities. Persist the exact request and request-may-exist boundary before remote Execute, retain a current execution token, enforce exact guest Inspect before retry after ambiguity or restart, and preserve complete-batch parent port fencing. Do not add compute registry substitution, product caller cutover, Compose down, physical-machine stop, or coarse-stop deletion. |
| Current acceptance checkpoint | NNC6.5d4 audit and fail-before are durable at `29137fa000341e48bd9ee41ef37451f5611977f1`. Bands 1-7 are green. K3-K4 pass. The operation-family and injected-journal prerequisites of K7 pass. K11-K14 have their strict wire vocabulary, digest, response correlation, closed outcomes, private route, and bounded one-shot client. K15 has a closed translation without caller-selected provider IDs. K16 passes with deterministic durable Systemd production composition. Bands 5a-5c close Systemd and Container activation admission, provide a cancellation-safe generic-journal seam, and add the strict guest composite execution sink. Band 6 adds exact forwarded Container attachment detach and release with publication-absence proof and retained leases. Band 7 adds strict pre-effect route decoding and authority checks, truthful production capability composition, complete response correlation, bounded response handling, and explicit ambiguous Execute followed by caller-supplied exact Inspect. K22 remains open for band 8 because the parent adapter must durably own request-may-exist state and enforce recovery order. No second journal exists. |
...
| Last checkpoint commit | `0eb5cab62` is the durable NNC6.5d4 band 1 checkpoint. `fb7acb395` is band 2. `690ac9f22` is band 3. `1100cbc04` is band 4. `bf7b61808` is band 5a. `824c2f6f0` is band 5b. `62d21cb7a` is band 5c. `a98841aaa` is band 6. The commit that contains this header is band 7. The prior audit checkpoint is `29137fa000341e48bd9ee41ef37451f5611977f1`. |
| Audit dirty state | The commit that contains this header is the exact band 7 recovery checkpoint. It owns only the strict private teardown route, truthful route capability composition, bounded one-shot client transport, exact route/client/capability tests, two reconciled source-derived censuses, and matching ledger paths. The original checkout remains untouched. |
| Latest dirty-state checkpoint | Band 7 authenticates strict request bytes and the installed forwarder before guest facade or provider effects. It bounds both request and response bodies, correlates every response fence, and turns all post-send uncertainty into one explicit `Ambiguous` outcome without automatic retry. An explicit caller-supplied Inspect can follow an ambiguous Execute, but the client owns no durable retry state. Band 8 remains the owner of the parent request-may-exist boundary and enforced recovery order. No parent adapter, compute substitution, product caller, coarse stop, dependency, or `nimbus-network` path changed. NNC6.5d4 remains one review unit. |
...
| Last verification | Band 7 client transport passes `5`; the private route passes `2`; real capability composition passes `5`; guest execution teardown remains `12` with one declared child entry point; and Machine API teardown wire passes `7`. Full Machine passes `41` library plus `5` integration tests. Full CLI passes `978` with two declared ignores. Affected all-target checking, strict all-feature Clippy, warning-denied Rustdoc, format, and diff pass. The aggregate verifier self-test passes `469/469`; NNCV035 passes `55/55`; live NNCV006 and NNCV015 pass; direct NNCV035 remains expected red at `0/7`; and the aggregate is `35/36` with only NNCV035 red. Proof lint reports zero diagnostics. Docs pass `108`; the site passes `17/17`. No structured review ran. |
| Latest verification checkpoint | Active record: `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. It records bands 1-7 fail-before, implementation, behavior counts, quality gates, exact route/client correlation, one-shot ambiguity, truthful capability composition, complete forwarded attachment absence and release proofs, read-only inspection, and the inherited full-Sandbox timing limitation. |
| Blocking decision | No blocker. The commit containing this header closes band 7. Implement only band 8 next. Do not run structured review on partial NNC6.5d4 work. |
...
| NNC6.5d4 | `in_progress` | **Dependency:** NNC6.5d3 complete at `c1c7f1397`. **Evidence:** `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. **Checkpoints:** audit `29137fa00`; band 1 `0eb5cab62`; band 2 `fb7acb395`; band 3 `690ac9f22`; band 4 `1100cbc04`; band 5a `bf7b61808`; band 5b `824c2f6f0`; band 5c `62d21cb7a`; band 6 `a98841aaa`; the commit containing this row is band 7. **Bands 1-6:** strict prior-receipt fencing, a teardown-family final-withdraw stream, injected compute journal ownership, the closed Machine API vocabulary, durable guest Systemd composition, activation barriers, one Container-rooted guest journal, composite execution drain/stop, and forwarded retained attachment detach/release are green. **Band 7:** the private POST route strictly decodes and bounds one exact request, authenticates the installed forwarder before facade or provider effects, and returns only a fully correlated response. Production capability availability requires the real guest execution/attachment composition, both durable journals, a reachable forwarder, and matching instance/generation. The bounded client authenticates before socket I/O, sends once, validates every response fence, and returns explicit `Ambiguous` for any post-send uncertainty. It never retries or manufactures Inspect. One proof sends an ambiguous Execute and then one caller-supplied exact Inspect. **Owned paths:** private machine route and capability composition; guest Container journal readiness; concept-owned one-shot client child; exact route/client/capability tests; NNCV006 and NNCV015 census evidence; this plan, proof, and routing index. No parent adapter, confirmed-publication progression, compute substitution, product caller, dependency manifest, `nimbus-network`, public route, protocol version, provider selection, or coarse-stop path changed. **Last green:** client `5`; route `2`; capability composition `5`; guest execution teardown `12 + 1`; Machine wire `7`; full Machine `41 + 5`; full CLI `978 + 2`; affected all-target check; strict all-feature Clippy; warning-denied Rustdoc; format/diff; aggregate self-test `469/469`; NNCV035 `55/55`; live NNCV006 and NNCV015; direct expected `0/7`; aggregate `35/36` sole NNCV035 red; proof lint zero; docs `108`; site `17/17`. **Truth boundary:** K22 remains open. Band 8 owns durable parent request-may-exist state, the current execution token, and enforced Inspect-before-retry after ambiguity or restart. **Next:** band 8 adds confirmed-publication progression and one five-capability `ForwardedMachineTeardownAdapter`; band 9 owns real compute substitution. **Forbidden:** compute substitution before band 9, Compose down, product caller cutover, physical-machine stop, tenant policy, service naming, a CLI-local saga store, a public product route, or coarse-stop deletion. **Review:** no structured review has run; the one full item review remains deferred until K1-K34 are green and the complete item is candidate-frozen. **Blocker:** none. |
