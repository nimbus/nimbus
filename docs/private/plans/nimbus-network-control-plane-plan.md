...
Status: `active; NNC3.7 complete; NNC3.7a CLI dev/start resolver migration in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC3.7 complete; NNC3.7a CLI dev/start resolver migration in progress` |
| Current band | `NNC3 — cross-process host-global PortLease` |
| Current item | `NNC3.7a — migrate CLI dev conventional/ephemeral resolver and start-adapter availability decisions` |
| Last completed item | `NNC3.7 — machine SSH/forwarding listener migration` |
| Next action | Commit the exact reviewed NNC3.7 checkpoint, then begin NNC3.7a with a read-only call graph of CLI dev conventional/ephemeral selection and start-adapter availability probes. Reconcile every site with the bind inventory, freeze fail-before conventional-conflict and ephemeral-adoption behavior, and identify the shared lease/provider-bind seam before mutating production code. Do not start census closure (NNC3.7b), producer-wide restart convergence (NNC3.8), old-authority deletion beyond the item gate (NNC3.9), or structured sibling-task unwind (NNC7.1a). |
...
| Last checkpoint commit | `9ded055bf386ab8ccc6dc9c36d569707e2755f89` — reviewed NNC3.6 completion and NNC3.7 activation. The reviewed NNC3.7 completion commit is being formed; after it lands, `HEAD` is the recovery proof. |
| Audit dirty state | The exact NNC3.7 completion candidate has 32 owned paths, including its ignored/force-tracked proof; every path is recorded under `Candidate Changed Paths` in `docs/private/plans/proof/nimbus-network-control-plane/nnc3.7-machine-listener-port-migration.md`. They are confined to Cargo dependency promotion, `nimbus-cli` machine lifecycle/composition/tests, `nimbus-machine` root/runtime vocabulary, the narrow confirmed-stop network transition/test, fixture projections, the bind inventory, routing index, this plan, and the proof. NNC3.7a has no implementation-owned dirty path yet. No census closure, producer-wide NNC3.8 reconciliation, NNC3.9 deletion beyond the machine-local allocator, or NNC7.1a task-group path is admitted. |
...
| Last verification | NNC3.7 written acceptance passes 2/2: production machine probe/drop and its JSON/lock authority are deleted, and the real machine/server shared-authority conflict passes. Focused machine port tests pass 8/0/1 child-only ignore, stop tests 4/0/0, exact network rebind 1/0/0, and provider-managed pre-authority rejection 1/0/0. One canonical affected lane passes 1,655/1,655 with 29 declared skips and no live external-provider claim. All-target check, strict Clippy, warning-denied rustdoc, format/diff, inventory JSON, exact core-only workspace edge, verifier 16/16 self-tests plus live 14/1 solely later-owned NNCV005, docs 108 pages, and site 17/17 pass. The one actual Sol/xhigh/fast review produced two accepted findings: the invalid optional provider-managed state is structurally removed and its pre-authority rejection is tested; failed-aggregate arithmetic is replaced by the clean canonical lane. No material architecture change required repeat review. Exact commands and dispositions are in the NNC3.7 proof. |
| Blocking decision | No blocker. NNC3.7 is complete and NNC3.7a is active. CLI dev/start resolver decisions are the only newly executable implementation scope; census closure, fresh-process cleanup-pending convergence, and old-authority deletion remain owned by their later NNC3 items. |
...
| NNC3.7a | `in_progress` | Recovery checkpoint: the reviewed NNC3.7 completion commit is being formed. Owned paths: none. Last green: the NNC3.7 proof above. Next action: read-only CLI dev/start conventional/ephemeral resolver and availability-probe call graph, inventory reconciliation, and deterministic fail-before design before production edits. Blocker: none. |
| NNC3.7b | `todo` | — |
| NNC3.8 | `todo` | — |
| NNC3.9 | `todo` | — |
| NNC4.1 | `todo` | — |
