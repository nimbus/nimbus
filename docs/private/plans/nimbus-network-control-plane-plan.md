...
| Next action | Implement NNC6.5d4 band 7 from `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`: add only the strict private Machine API route and client child. Prove complete request and response correlation, durable request-may-exist ordering, exact guest Inspect before retry after an ambiguous request, and truthful capability availability. Do not add the parent adapter, compute substitution, caller cutover, Compose down, physical-machine stop, or coarse-stop deletion. |
| Current acceptance checkpoint | NNC6.5d4 audit and fail-before are durable at `29137fa000341e48bd9ee41ef37451f5611977f1`. Bands 1-6 are green. K3-K4 pass. The operation-family and injected-journal prerequisites of K7 pass. K11-K14 have their strict wire vocabulary, digest, response correlation, and closed outcomes. K15 has a closed translation without caller-selected provider IDs. K16 passes with deterministic durable Systemd production composition. Bands 5a-5c close Systemd and Container activation admission, provide a cancellation-safe generic-journal seam, and add the strict guest composite execution sink. Band 6 adds the forwarded Container attachment composition: exact prior Stop and Detach journal results, complete machine-publication absence, retained listener leases, compound detached proof, read-only Inspect, proof-gated final release, and zero-listener identity fencing are green. No second journal exists. The missing private route remains an explicit capability blocker until band 7. |
...
| Last checkpoint commit | `0eb5cab62` is the durable NNC6.5d4 band 1 checkpoint. `fb7acb395` is band 2. `690ac9f22` is band 3. `1100cbc04` is band 4. `bf7b61808` is band 5a. `824c2f6f0` is band 5b. `62d21cb7a` is band 5c. The commit that contains this header is band 6. The prior audit checkpoint is `29137fa000341e48bd9ee41ef37451f5611977f1`. |
| Audit dirty state | The commit that contains this header is the exact band 6 recovery checkpoint. It owns only the forwarded Container attachment composition, guest attachment sink, retained-publication strategy, complete absence witness, read-only journal adoption hardening, exact Stop and Detach authentication, recovery tests, two source-derived census updates, and matching ledger paths. The original checkout remains untouched. |
| Latest dirty-state checkpoint | Band 6 reuses the NNC6.5d3 retained detach/release lifecycle, the Container-rooted provider journal, and the existing machine-publication authority. Forwarded Detach authenticates exact prior composite Stop success and complete publication absence, retains leases, and binds a complete compound proof. Release authenticates exact prior Detach success and continued absence before exact plan-member lease release. Inspect is read-only and fails closed on unresolved stages. Host-managed authentication remains sealed, no Netavark classifier runs for machine publication, and no route, client, parent adapter, compute substitution, or caller changed. NNC6.5d4 remains one review unit. |
...
| Last verification | Band 6 forwarded attachment passes `9`; the complete Container network teardown module passes `13` with one declared child entry point; read-only journal adoption passes `1`; guest attachment passes `6`; guest execution teardown remains `12` with one declared child entry point. Full CLI passes `970` with two declared ignores; full compute passes `381` with one declared ignore. Every NNC6.5d4 Sandbox test passes. Repeated full Sandbox runs retain one unrelated process-fixture or contender timeout after exact focused reruns pass; this band checkpoint does not claim final K33 full-suite completion. Affected all-target/all-feature check, strict all-feature Clippy, warning-denied Rustdoc, format, diff, NNCV006, NNCV008, NNCV021, NNCV035 self-test `55/55`, and direct expected red `0/7` pass. The aggregate is `35/36` with only NNCV035 red. Proof lint has zero diagnostics. Docs pass `108`; the site passes `17/17`. No structured review ran. |
| Latest verification checkpoint | Active record: `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. It records bands 1-6 fail-before, implementation, behavior counts, quality gates, complete forwarded attachment absence and release proofs, read-only inspection, exact publication-mode classification, zero-listener fencing, and the inherited full-Sandbox timing limitation. |
| Blocking decision | No blocker. The commit containing this header closes band 6. Implement only band 7 next. Do not run structured review on partial NNC6.5d4 work. |
...
| NNC6.5d4 | `in_progress` | **Dependency:** NNC6.5d3 complete at `c1c7f1397`. **Evidence:** `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. **Checkpoints:** audit `29137fa00`; band 1 `0eb5cab62`; band 2 `fb7acb395`; band 3 `690ac9f22`; band 4 `1100cbc04`; band 5a `bf7b61808`; band 5b `824c2f6f0`; band 5c `62d21cb7a`; the commit containing this row is band 6. **Bands 1-5:** strict receipt-prefix fencing, one teardown-family final-withdraw stream, injected compute journal ownership, the transport-only Machine API envelope, durable production Systemd composition, both activation-admission barriers, the cancellation-safe current-claim seam, the nonpublishing Container child, and one strict guest composite execution sink are green without a route or product caller. **Band 6:** one forwarded Container attachment child authenticates exact Stop and Detach history, the installed forwarder and translation, and complete machine-publication absence before the shared retained detach and proof-gated final release. It retains machine listener leases through Detach, releases exact plan members only at Release, binds zero-listener identity into the compound proof, keeps Inspect read-only, and never sends machine publication through Netavark classification. Host-managed behavior stays sealed. **Owned paths:** guest attachment dispatch; forwarded Container attachment composition; shared retained-publication and release action bundles; exact machine-publication classification and absence witness; machine plan-member release; read-only provider-journal adoption; focused recovery tests; NNCV006 and NNCV021 census evidence; this plan, proof, and routing index. No route, client, parent adapter, compute substitution, product caller, dependency manifest, `nimbus-network`, protocol version, provider selection, or coarse-stop path changed. **Last green:** forwarded attachment `9`; complete Container network teardown `13 + 1`; read-only journal `1`; guest attachment `6`; guest execution teardown `12 + 1`; full CLI `970 + 2`; full compute `381 + 1`; affected all-target/all-feature check; strict all-feature Clippy; warning-denied Rustdoc; format/diff; NNCV006, NNCV008, and NNCV021; NNCV035 `55/55`; direct expected `0/7`; aggregate `35/36` sole NNCV035 red; proof lint zero; docs `108`; site `17/17`. Every NNC6.5d4 Sandbox test passes. Repeated full Sandbox runs retain one unrelated inherited timing failure after exact focused reruns pass, so this recovery checkpoint does not claim final K33 full-suite completion. **Next:** band 7 adds only the strict private route and client child, request/response correlation, and exact Inspect-before-retry after ambiguity. **Forbidden:** parent adapter before band 8, compute substitution before band 9, Compose down, product caller cutover, physical-machine stop, tenant policy, service naming, a CLI-local saga store, a public product route, or coarse-stop deletion. **Review:** no structured review has run; the one full item review remains deferred until K1-K34 are green and the complete item is candidate-frozen. **Blocker:** none. |
