...
| Next action | Commit this exact NNC6.5d3 checkpoint. Then start NNC6.5d4 with a read-only substitution audit of `crates/nimbus-machine/src/machine/backend.rs`, its `provision` owner, Machine API service-workload handlers and client translation, parent `publication_authority`, provider-journal reuse, and compute teardown substitution. Create the canonical NNC6.5d4 proof and freeze acceptance and fail-before criteria before product edits. |
| Current acceptance checkpoint | NNC6.5d3 K1-K29 are green and its review cadence is exhausted. NNC6.5d4 has not changed product source. Its frozen audit contract requires distinct parent and guest provider realms, exact remote phase envelopes, parent publication withdrawal before guest drain/stop, final parent release only after exact guest/provider absence, and two-realm replay, ambiguity, contention, and process-recovery proofs. |
...
| Last checkpoint commit | The commit containing this header is the durable NNC6.5d3 item commit. Its parent `df9b7bee3031ec0bbae62eb66ef0c395e3aca677` is the durable audit and fail-before checkpoint. |
| Audit dirty state | Before the item commit, preserve all 79 NNC6.5d3 paths: 68 Rust paths plus the exact manifest, inventory, verifier, proof, and ledger coordinates. After the commit, require a clean owner worktree before any NNC6.5d4 edit. The original checkout remains untouched. |
| Latest dirty-state checkpoint | The corrected NNC6.5d3 candidate is fully staged. It adds one authenticated port-plan snapshot, pinned no-follow parent and final-target namespace inspection/read/remove, semantic provider-lock contention probes, and an empty-selection release no-op. The item commit containing this header closes NNC6.5d3. |
...
| Last verification | Focused sandbox: contract `1`, attachment lifecycle `81 + 5`, egress `92 + 4`, Container `1`, Krun `2`, fresh-process parents `2`, and combined network teardown `15 + 2`. Correction proofs pass for batch snapshot `1`, K14 `12`, and generic/Container/Krun contention `1` each. Compute teardown passes `23`; substitution passes `8`. Network passes `273 + 1`; final default-concurrency sandbox passes `1,117 + 47`; compute passes `369 + 1`. Strict affected Clippy/rustdoc, format/diff, bind/composition scans, NNCV035 `55/55`, direct expected `0/7`, and aggregate expected `35/36` sole NNCV035 red pass. |
| Latest verification checkpoint | Completed record: `proof/nimbus-network-control-plane/nnc6.5d3-host-managed-attachment-detach-release.md`. It records both reviews, every finding disposition, deterministic fail-before corrections, and restored K1-K29 evidence. |
| Blocking decision | No blocker. Commit the exact NNC6.5d3 checkpoint, require a clean owner worktree, and begin only the NNC6.5d4 read-only substitution audit. Do not reopen NNC6.5d3 or run another NNC6.5d3 review. |
...
| NNC6.5d4 | `in_progress` | **Dependency:** NNC6.5d3 complete. **Owned value:** end-to-end forwarded-machine provider adapters and exact guest phase envelopes without caller cutover. **Last green:** NNC6.5d3 K1-K29, affected behavior and quality gates, and aggregate `35/36` with sole expected NNCV035 red; NNC6.5d4 product source is unchanged. **Next:** perform the read-only parent/guest substitution audit frozen in `proof/nimbus-network-control-plane/nnc6.5d-sandbox-machine-teardown-substitution-audit.md`; create `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`; freeze exact acceptance, fail-before cases, crash cuts, owned paths, and forbidden paths before product edits. **Required seams:** distinct admitted parent attachment/execution/ingress provider IDs; parent publication withdrawal before guest drain/stop; exact remote guest envelopes and provider-journal reuse; parent release only after exact guest/provider absence; independent parent/guest durable realms; sibling-batch, response-loss, process-death, stale-generation, crossed-forwarder, and ambiguity convergence. **Forbidden:** Compose down, product caller cutover, physical-machine stop, tenant policy, service naming, a CLI-local saga store, a new public route, or coarse-stop deletion. **Blocker:** none. |
