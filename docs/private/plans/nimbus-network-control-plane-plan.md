...
| Next action | Implement NNC6.5d4 band 4 from `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`: add durable Systemd production composition and retain the exact lifecycle, drain, and stop trait objects. Add capability and unavailable-mode tests. Do not add the private teardown route, client, guest composite state machine, parent adapter, or caller cutover yet. |
| Current acceptance checkpoint | NNC6.5d4 audit and fail-before are durable at `29137fa000341e48bd9ee41ef37451f5611977f1`. Bands 1-3 are green. K3 passes. K4 now has both the portable sparse-prefix validator and the strict forwarded full-chain and exact-subject validator. The operation-family and injected-journal prerequisites of K7 pass. K11-K14 have their strict wire vocabulary, complete domain-separated digest, exact response correlation, and closed mode-specific outcomes. K15 has a closed translation without caller-selected provider IDs; guest composition remains later. |
...
| Last checkpoint commit | `0eb5cab62` is the durable NNC6.5d4 band 1 recovery checkpoint. `fb7acb395` is the durable band 2 recovery checkpoint. The commit containing this header is the durable band 3 recovery checkpoint. The prior audit checkpoint is `29137fa000341e48bd9ee41ef37451f5611977f1`. |
| Audit dirty state | The owner worktree was clean after `fb7acb395`. Preserve only the band 3 `nimbus-machine`, `nimbus-workloads`, proof, and plan paths until their recovery commit. The original checkout remains untouched. |
| Latest dirty-state checkpoint | Band 3 is candidate-complete. Three read-only packets changed zero paths. The owned diff adds strict Machine API teardown command, request, digest, response, translation, outcome, and wire-error vocabulary plus concept-owned pure wire tests. It also makes two digest-bound optional attempt fields required on the wire and exports one shared success-evidence correlation check. No route, client, capability advertisement, provider composition, effect, caller, or `nimbus-network` source changed. |
...
| Last verification | NNC6.5d4 band 3 expected-red path and symbol checks exit `1` four times. Focused behavior passes Machine API teardown wire `7/7` and workloads optional-field strictness `1/1`. Full workloads passes `219/219`; full Machine passes `41/41 + 5/5`. Format, diff, strict affected Clippy, and warning-denied affected Rustdoc pass. NNCV008 passes; NNCV035 self-test passes `55/55`; direct remains exact expected red at `0/7`; aggregate is `35/36` with only NNCV035 red. Proof lint reports zero diagnostics; docs pass `108`; site passes `17/17`. |
...
| Blocking decision | No blocker. Commit the exact band 3 recovery checkpoint, then implement only band 4. Do not run structured review on partial NNC6.5d4 work. |
...
| NNC6.5d4 | `in_progress` | **Dependency:** NNC6.5d3 complete at `c1c7f1397`. **Evidence:** `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. **Checkpoints:** audit `29137fa00`; band 1 `0eb5cab62`; band 2 `fb7acb395`; the commit containing this row is the band 3 checkpoint. Three read-only audit packets freeze K1-K35 and the exact two-realm contract. **Band 1:** one workloads-owned strict prefix projects exact durable ordered receipts, preserves valid resource-free gaps, and is authenticated by compute through result CAS. **Band 2:** one distinct final-withdraw operation and one compute-owned injected-journal seam derive all five exact operations without moving provider effects or journal ownership. **Band 3:** `nimbus-machine::api` owns one strict transport-only teardown envelope, domain-separated request digest, closed provider translation, closed Execute and Inspect outcomes, and fully correlated response. The guest accepts only Drain, Stop, Detach, and Release; every forwarded phase requires the complete exact prior receipt chain and the same execution locator and network plan. Parent-local withdrawal cannot enter the guest. Two digest-bound optional attempt fields now require explicit wire presence, and portable success evidence exposes one narrow shared step-and-subject check. **Owned paths:** band 3 changes only the `nimbus-machine` API vocabulary and concept-owned tests; the `nimbus-workloads` teardown wire validation, shared evidence check, and focused test; this proof; and this recovery ledger. No route, client, capability advertisement, provider composition, effect, caller, or `nimbus-network` path changed. **Last green:** band 3 expected-red exits `1` four times; focused wire passes `7 + 1`; full workloads passes `219`; full Machine passes `41 + 5`; format, diff, strict affected Clippy, and warning-denied affected Rustdoc pass. NNCV008 and NNCV035 self-test `55/55` pass; direct remains `0/7`; aggregate is `35/36` with only NNCV035 red; proof lint is zero; docs are `108`; site is `17/17`. **Next:** band 4 durable Systemd production composition and retained exact lifecycle/drain/stop traits. **Required seams:** distinct admitted parent attachment/execution/ingress provider IDs; parent publication withdrawal before guest drain/stop; exact remote guest envelopes and provider-journal reuse; parent release only after exact guest/provider absence; independent parent/guest durable realms; sibling-batch, response-loss, process-death, stale-generation, crossed-forwarder, and ambiguity convergence. **Forbidden:** Compose down, product caller cutover, physical-machine stop, tenant policy, service naming, a CLI-local saga store, a new public route, or coarse-stop deletion. **Review:** no structured review has run; the only full item review is deferred until K1-K34 are green and the complete item is candidate-frozen. **Blocker:** none. |
