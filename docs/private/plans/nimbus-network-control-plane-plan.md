...
Status: `active; NNC5.1 complete; NNC5.2 in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC5.1 complete; NNC5.2 in progress` |
| Current band | `NNC5 — sandbox-owned deep attachment lifecycle` |
| Current item | `NNC5.2 — persist attachment phase/generation and opaque provider handle` |
| Last completed item | `NNC5.1 — consolidated container/krun attachment phases and compensation` |
| Next action | Begin NNC5.2 with a read-only substitution/durability audit: inventory the existing container and krun attachment phase, generation, provider-attempt, and restart evidence; reconcile NNC5.2a ordering; design the fail-before matrix before mutating code. |
| Current acceptance checkpoint | NNC5.1 criteria 1-13 are complete. One 1,449-line crate-private OCI lifecycle owns common reservation, setup, reverse compensation, typed detach progress, backend-explicit restart admissibility, Netavark/netns/IPAM/port-lifetime cleanup, and terminal authority release. Raw context construction occurs once and the adapter constructor is private; actual Container/Krun backend types implement the small host-managed route, while only Container implements machine forwarding. Exact fresh/restart authority authenticates adopted attachment, claim, published ports, and auxiliary PEP tenant/sandbox/address/provider identity before effects. Two independent fail-before runs each failed 0/2 when fresh or restart PEP identity was weakened; corrected row 12 is 2/2 and the identical 15-row/two-profile contract is 30/30. Missing registered-lifetime recovery preserves the primary plus retained-fence diagnostic. The later NNC5.2-NNC5.6 gaps remain explicitly red. |
...
| Last checkpoint commit | `14789e07aafd38740a44469d83353d2d11b44a82` — NNC4.7; the exact NNC5.1 successor is this pending local checkpoint. |
| Audit dirty state | The exact NNC5.1 checkpoint owns this plan/index/proof; `backends/oci/network.rs`; new `backends/oci/network/attachment_lifecycle.rs`, its concept-owned `tests.rs`, and `tests/authority.rs`; `backends/oci/port_lifecycle/batch_state.rs` for terminal replay plus exact PEP listener/binding authentication; container `runtime.rs`, `network_launch.rs`, `execution_cleanup.rs`, `restart.rs`, and minimal explicit-authority fixture adaptations in `runtime/lifecycle.rs` plus `runtime/planning.rs`; krun `vm.rs`, `lifecycle.rs`, minimal `vm/tests.rs` adaptation, and `tests/launch_compensation.rs`; plus exactly four mechanically shifted line anchors in the frozen NNC4.6f census. The original checkout and clean `machine-os` companion remain untouched. |
| Latest dirty-state checkpoint | NNC5.1 closes as one exact nineteen-path item checkpoint. The private lifecycle constructor occurs only inside the deep owner; production and the complete contract route through small capabilities implemented by the actual backend types. No active agent owns paths. No rebase, push, or PR occurred. |
| Current item review | The sole full GPT-5.6 Sol/xhigh item review accepted three executable findings and one ledger finding. Its material corrections triggered the sole narrow GPT-5.6 Sol/xhigh review, which accepted two remaining P1 gaps: incomplete exact PEP identity and a matrix that bypassed backend-type routes. Both reproduced, were corrected, and were owner-verified with focused plus aggregate gates. Per cadence, no third review ran or is warranted. The structured helper's external-limit and nested-session refusals returned no content and are not verdicts. |
...
| Last verification | Final contract `30/30`; exact-authority row `2/2`; port lifecycle `47/47`; affected container/krun lane `328/328`; direct krun regression `1/1`; full sandbox `758/758` with `24` declared skips. All-target/all-feature check, strict Clippy, warning-denied rustdoc, exact core-only metadata, private-constructor/backend-route scans, format, and diff checks pass. |
| Latest verification checkpoint | Live verifier `17/17`; adversarial verifier `62/62`. Historical F1/F3 fail-before was `26/30`. Narrow-review PEP fail-before separately produced fresh `0/2` and restart `0/2`; corrected row 12 is `2/2`. Raw context construction is one private occurrence, raw adapter construction is confined to two owner-trait defaults, and source-string assertions/test-only publication constructors are zero. The NNCV015 historical fail-before was `16/17` solely because four unchanged krun census anchors shifted. NNC5.2/NNC5.2a durable shared evidence, NNC5.3 complete readiness, NNC5.4 crash cuts, NNC5.5 band-wide locality, and NNC5.6 side-effect-free inspection remain honestly deferred. |
| Blocking decision | No blocker. NNC5.2 is the next dependency-ready item. |
...
repository's 1,500–1,999-line explicit-justification band at 1,954 lines. It is a deep,
...
| NNC5.2 | `in_progress` | **Dependencies:** NNC5.1 is complete. **Scope now:** read-only substitution/durability audit of container and krun attachment phase, generation, provider-attempt, restart, and orphan evidence; reconcile NNC5.2a ordering and write the exact fail-before matrix before implementation. **Owned paths:** this recovery row/header only until the audit freezes an explicit NNC5.2 proof and code-path list; no NNC5.2 executable mutation belongs in the NNC5.1 checkpoint. **Last green:** NNC5.1 final contract 30/30, affected lane 328/328, sandbox 758/758 with 24 skips, verifier 17/17 plus self-test 62/62, and affected quality/docs gates. **Next:** inventory source-derived desired/durable/observed attachment evidence and substitution paths, reconcile NNC5.2a ordering, then record exact expected-red tests and owned paths before editing code. **Acceptance target:** persist one shared attachment phase/generation and opaque provider handle so restart from every phase inspects rather than blindly recreates. **Blocker:** none. |
| NNC5.2a | `todo` | — |
| NNC5.3 | `todo` | — |
| NNC5.4 | `todo` | — |
| NNC5.5 | `todo` | — |
