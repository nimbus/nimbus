...
| Next action | Implement NNC6.5d4 band 5b from `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`: close reservation, preparation, network-attachment, and machine-ingress publication producers under the Container lifecycle lock and durable drain barrier. Then add one asynchronous exact-current-claim seam around the existing Container-rooted provider journal and one Container execution-teardown substep that cannot publish the generic result after only its child effect. Prove exact replay, process contention, no delayed Execute after an Inspect `NotCompleted`, and no second guest journal. Do not add the guest composite sink, route, client, parent adapter, or caller cutover yet. |
| Current acceptance checkpoint | NNC6.5d4 audit and fail-before are durable at `29137fa000341e48bd9ee41ef37451f5611977f1`. Bands 1-5a are green. K3-K4 pass. The operation-family and injected-journal prerequisites of K7 pass. K11-K14 have their strict wire vocabulary, digest, response correlation, and closed outcomes. K15 has a closed translation without caller-selected provider IDs. K16 passes: Linux production opens one deterministic durable Systemd teardown root; one concrete backend is retained behind lifecycle, drain, and stop traits; exact teardown capability keeps store and backend blockers distinct; and the operation remains unavailable until its strict sink exists. Band 5a now binds the exact prior-receipt prefix into every host claim and provider fence, closes one durable Systemd activation-admission barrier before drain success, blocks initial and restart activation after closure, and requires the matching drain receipt before stop. |
...
| Last checkpoint commit | `0eb5cab62` is the durable NNC6.5d4 band 1 checkpoint. `fb7acb395` is band 2. `690ac9f22` is band 3. `1100cbc04` is band 4. The commit containing this header and its matching proof row is band 5a. The prior audit checkpoint is `29137fa000341e48bd9ee41ef37451f5611977f1`. |
| Audit dirty state | The owner worktree was clean after `1100cbc04`. Preserve only the active band 5a host claim, Systemd activation-admission/store, compute lowering, tests, routing, proof, and plan paths until their recovery commit. The original checkout remains untouched. After that commit, band 5b owns only the audited Container producer fences, one-journal async seam, authorized Container substep, tests, and matching ledger paths. |
| Latest dirty-state checkpoint | Band 5a is candidate-green. The exact prior receipt prefix is part of the closed host claim and operation fence. The Systemd store has one durable execution-scoped activation admission and absorbing drain barrier. Initial and restart activation serialize against drain; drain checkpoints closure before success; stop authenticates the exact prior drain receipt. Focused contention, reopen, crossed-history, corruption, and restart cases pass. Full node and compute suites and affected quality/static/docs gates pass. The commit containing this header is the band 5a recovery checkpoint. Band 5b is next; NNC6.5d4 remains one review unit. |
...
| Last verification | NNC6.5d4 band 5a focused activation-barrier behavior passes `7/7`; host claim identity passes `4/4`; restart activation fencing, corrupt-store fail-before, and prior-receipt reopen pass `1/1` each. Full node passes `118/118`; full compute passes `381/381` with one declared child-only ignore. Format, diff, strict affected Clippy, warning-denied affected Rustdoc, NNCV008, technical-writing proof lint with zero diagnostics, docs `108`, and site `17/17` pass. NNCV035 self-test passes `55/55`; direct remains exact expected red at `0/7`; aggregate is `35/36` with only NNCV035 red. No structured review ran. |
| Latest verification checkpoint | Active record: `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. It records bands 1-5a fail-before, implementation, behavior counts, quality gates, sparse portable semantics, strict forwarded full-chain semantics, injected-journal ownership, production Systemd composition, and the activation-admission recovery point. |
| Blocking decision | No blocker. Commit band 5a, then implement only band 5b. Do not run structured review on partial NNC6.5d4 work. |
...
| NNC6.5d4 | `in_progress` | **Dependency:** NNC6.5d3 complete at `c1c7f1397`. **Evidence:** `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. **Checkpoints:** audit `29137fa00`; band 1 `0eb5cab62`; band 2 `fb7acb395`; band 3 `690ac9f22`; band 4 `1100cbc04`; the commit containing this row is the band 5a checkpoint. Three read-only audit packets freeze K1-K35 and the exact two-realm contract. **Bands 1-4:** one strict receipt prefix, one teardown-family final-withdraw operation with injected compute journal, one transport-only Machine API envelope, and one durable production Systemd composition establish the prerequisites without a route or product caller. **Band 5a:** the exact prior-receipt prefix is part of the closed host claim and evidence fence. One execution-scoped Systemd activation admission serializes exact initial and restart activation against an absorbing durable drain barrier. Drain checkpoints closure before success; stop requires the matching prior drain receipt before inspection or effect. **Owned paths:** host claim, Systemd activation/teardown store and state machine, compute lowering, concept-owned tests, routing, proof, and this recovery ledger. No route, client, guest composite sink, shared async journal seam, Container producer, parent adapter, caller, `nimbus-network`, dependency, protocol version, or coarse-stop path changed. **Last green:** activation barrier `7`, host claim `4`, restart/corruption/reopen `1` each; full node `118`; full compute `381` with one declared ignore; format, diff, strict affected Clippy, warning-denied affected Rustdoc, NNCV008, proof lint zero, docs `108`, and site `17/17` pass. NNCV035 self-test is `55/55`; direct is expected red at `0/7`; aggregate is `35/36` with only NNCV035 red. **Next:** band 5b closes the four audited Container producers, adds one async exact-current-claim seam around the existing Container-rooted journal, and adds an authorized Container substep without a generic result publication. **Forbidden:** guest composite sink before 5c, Compose down, product caller cutover, physical-machine stop, tenant policy, service naming, a CLI-local saga store, a public route, or coarse-stop deletion. **Review:** no structured review has run; the only full item review is deferred until K1-K34 are green and the complete item is candidate-frozen. **Blocker:** none. |
