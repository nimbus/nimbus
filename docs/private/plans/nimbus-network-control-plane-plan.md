...
| Next action | Commit the exact NNC6.5d4 audit and fail-before checkpoint. Then implement band 1 in `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`: add the validated prior receipt-prefix contract and bind it through confirmed command and result authentication before any Machine API or provider edit. |
| Current acceptance checkpoint | NNC6.5d4 audit and fail-before are complete at clean source base `c1c7f1397`; product source is unchanged. K1-K35 are frozen in `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. The audit adds exact prior-receipt transport, a distinct teardown-family final-withdraw operation, composite Systemd/Container guest execution proof, a forwarded attachment evidence bridge, strict private wire correlation, and two-realm recovery to the existing frozen parent/guest contract. |
...
| Last checkpoint commit | `c1c7f13979137140bdd4df58e65fa0b6141596b4` is the durable NNC6.5d3 item commit. The commit containing this updated header, the new NNC6.5d4 proof, and the routing-index edit is the durable NNC6.5d4 audit and fail-before checkpoint. |
| Audit dirty state | Preserve exactly the new NNC6.5d4 proof plus this plan and `docs/private/plans/README.md`. No product source changed. The three read-only audit packets reported zero changed paths. The original checkout remains untouched. |
| Latest dirty-state checkpoint | The NNC6.5d4 source and complexity census, seven absent-seam fail-before checks, retained coarse-stop check, K1-K35 contract, two-realm crash matrix, batch matrix, path boundaries, and implementation bands are recorded. Product source remains identical to `c1c7f1397`. |
...
| Last verification | NNC6.5d3 remains green at its committed counts. NNC6.5d4 format/diff and zero-diagnostic proof lint pass. NNCV008 passes. NNCV035 self-test is `55/55`; direct remains exact expected red at `0/7`; aggregate is `35/36` with only NNCV035 red. Docs pass `108`; site passes `17/17`. Seven absent-seam checks exit `1`, the retained coarse-stop check exits `0`, and three read-only audits report zero product changes. |
| Latest verification checkpoint | Active record: `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. It records the current and target graphs, K1-K35, failure and crash matrices, batch fencing, exact paths, complexity decisions, fail-before evidence, verification commands, and reviewer cadence. |
| Blocking decision | No blocker. Commit the exact NNC6.5d4 audit/fail-before checkpoint, then start only implementation band 1. Do not reopen or review NNC6.5d3, and do not run structured review on partial NNC6.5d4 work. |
...
| NNC6.5d4 | `in_progress` | **Dependency:** NNC6.5d3 complete at `c1c7f1397`. **Evidence:** `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. **Audit result:** three read-only packets freeze K1-K35, exact current/target ownership, prior-receipt transport, a teardown-family final-withdraw operation, pairwise provider identity, strict private Machine API correlation, composite Systemd/Container guest execution proof, forwarded attachment evidence bridging, complete-batch fencing, two independent durable realms, crash/concurrency matrices, and concept-owned paths. Seven absent-seam checks are expected red; the retained coarse-stop check is expected green. Product source is unchanged. **Last green:** NNC6.5d3 K1-K29 and its committed behavior/quality counts; the NNC6.5d4 audit base is clean. **Next:** commit this exact audit/fail-before checkpoint, then implement band 1: the validated ordered prior receipt-prefix contract through confirmed command and result authentication. **Required seams:** distinct admitted parent attachment/execution/ingress provider IDs; parent publication withdrawal before guest drain/stop; exact remote guest envelopes and provider-journal reuse; parent release only after exact guest/provider absence; independent parent/guest durable realms; sibling-batch, response-loss, process-death, stale-generation, crossed-forwarder, and ambiguity convergence. **Forbidden:** Compose down, product caller cutover, physical-machine stop, tenant policy, service naming, a CLI-local saga store, a new public route, or coarse-stop deletion. **Blocker:** none. |
