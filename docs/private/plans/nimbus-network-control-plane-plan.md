...
Status: `active; NNC6.5d2 complete; NNC6.5d3 in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC6.5d2 complete; NNC6.5d3 in progress` |
| Current band | `NNC6 — compute orchestration and exact lifecycle order` |
| Current item | `NNC6.5d3 — shared host-managed Container/Krun attachment detach and final release` |
| Last completed item | `NNC6.5d2 — exact Krun execution drain/stop over the shared sandbox teardown substrate`; the commit containing this checkpoint is its durable item commit. |
| Next action | Start NNC6.5d3 implementation band 1: add the neutral sandbox attachment command and observation, then prove exact compute lowering. |
| Current acceptance checkpoint | The NNC6.5d3 audit checkpoint is complete. K1 is green. K2-K29, the exact nine-detach/six-release crash matrices, path ownership, and nine expected-red seams are frozen. Product source remains unchanged until implementation band 1 starts. |
...
| Last checkpoint commit | The commit containing this recovery row is the durable NNC6.5d3 audit and fail-before checkpoint. NNC6.5d2 completed at `15872e4192a0c66865a3aadff30ca79d8f9a08e3`. |
| Audit dirty state | The checkpoint owns only the NNC6.5d3 audit proof, canonical plan, and routing index. Product source is unchanged. Three read-only audit packets reported zero changed or staged paths. The original checkout remains untouched. |
| Latest dirty-state checkpoint | Recover from the commit containing this row and `proof/nimbus-network-control-plane/nnc6.5d3-host-managed-attachment-detach-release.md`. Start with implementation band 1. Do not discard, rebase, or duplicate the audit checkpoint. |
| Current item review | NNC6.5d2 review cadence is complete and exhausted. NNC6.5d3 uses the canonical-item cadence: no structured review during audit, fail-before, implementation, or convergence. One full Sol/xhigh/fast review is authorized only after K1-K28 are green and the complete item is candidate-frozen. |
| Full-review input identity | Complete `266,439`-byte NNC6.5d2 bundle; thread `019fe916-d4c5-79b0-91bf-38cac9c5abf4`; GPT-5.6 Sol/xhigh/fast; accepted P1 stale provision snapshot and P2 graceful-retry rebase defects; overall incorrect confidence `0.97`. |
| Narrow-review input identity | Complete `280,464`-byte corrected NNC6.5d2 bundle; reviewed candidate tree `4dd82b219dfb8f8e46194da7e12da2d8fe757f3c`; thread `019fe935-23dd-73c2-8170-c5a08e5f77a3`; GPT-5.6 Sol/xhigh/fast; zero findings; correct at confidence `0.99`. |
| Final closeout identity | The containing commit is the exact NNC6.5d2 item checkpoint. Reviewed corrected candidate: `31` paths, raw staged binary patch `276,429` bytes, SHA-256 `9af49413c28fe982a6a41cc8d032c31a96ebbbc9221cd73242c1ade4b5356b8d`, tree `4dd82b219dfb8f8e46194da7e12da2d8fe757f3c`. |
...
| Last verification | NNC6.5d3 audit checkpoint: format and diff pass; proof lint passes with three advisory passive-voice warnings; NNCV035 self-test is `55/55`; direct is the expected `0/7`; aggregate is `35/36`, with only NNCV035 red; docs pass `108`; site passes `17/17`. The unchanged NNC6.5d2 baseline includes exact Krun `27 + 1 ignore`, full Krun `173 + 4`, serialized full sandbox `1,078 + 45`, provider journal `25 + 1`, runtime identity `6`, schema `5`, Container `19 + 1`, compute substitution `11`, server process `2 + 1`, and full compute `357 + 1`. |
| Latest verification checkpoint | Active record: `proof/nimbus-network-control-plane/nnc6.5d3-host-managed-attachment-detach-release.md`. It records the clean source base, three read-only audits, current/target graphs, exact ownership, K1-K29, fail-before evidence, and the nine-detach/six-release proof matrices before product-source edits. |
| Blocking decision | No blocker. Start implementation band 1 after the audit checkpoint commit. Do not reopen or re-review NNC6.5d2. Do not run NNC6.5d3 autoreview before candidate freeze. |
...
| NNC6 — compute choreography | `in_progress` | Exact provision/teardown observers across every workload/retirement path. | NNC6.1-NNC6.5d2 and NNC6.1e1 are complete. NNC6.5d3 is the active host-managed detach/release provider unit. NNC6.5d4 remains the later forwarded-machine provider unit, and NNC6.5e-NNC6.5g remain later caller/convergence units. No completed item retains a temporary legacy authority. NNC6.6 retains resolver fencing; NNC6.1e2 is the final convergence gate. Provider effects remain with their named owners. |
...
| NNC6.5d3 | `in_progress` | **Dependency:** NNC6.5d1 and NNC6.5d2 complete. **Evidence:** `proof/nimbus-network-control-plane/nnc6.5d3-host-managed-attachment-detach-release.md`. **Owned value:** shared host-managed Container/Krun attachment detach and final release, exact capability adapters, compound detached evidence, retained reusable authority, and no-premature-reuse proofs. **Audit result:** three read-only packets at `15872e4192a0c66865a3aadff30ca79d8f9a08e3` report zero changed paths. K1-K29, exact tenant-qualified manifest location, one-journal authority, current/target graphs, owned and forbidden paths, nine expected-red seams, and the nine-detach/six-release crash matrices are frozen. Product source is unchanged. **Owned paths:** neutral sandbox attachment contract; shared OCI attachment lifecycle concept children; narrow PEP/listener/IPAM/segment observation and release seams; Container/Krun attachment-teardown children and strict manifest state; compute attachment adapter/lowering and tests; canonical proof/ledger/routing/static coordinates. **Last green:** audit proof lint passes; format and diff pass; NNCV035 self-test is `55/55`; direct is expected `0/7`; aggregate is `35/36`, with only NNCV035 red; docs pass `108`; site passes `17/17`. **Next:** commit the audit-only checkpoint, then implement band 1 fail-before tests and the neutral command/lowering seam. **Review:** forbidden until K1-K28 are green and the item is candidate-frozen. **Blocker:** none. |
