...
Status: `active; NNC6.3 complete; NNC6.3a in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC6.3 complete; NNC6.3a in progress` |
| Current band | `NNC6 — compute orchestration and exact lifecycle order` |
| Current item | `NNC6.3a — strict workloads-owned executable carrier and closed desired digest` |
| Last completed item | `NNC6.3 — provision-choreography substitution audit and prospective split` |
| Next action | Read the NNC6.3a band row and frozen proof contract, inspect the exact workloads/compute/server codec and saga call graph, define owned paths, and capture fail-before evidence for missing, unknown, duplicate, crossed, oversized, and digest-divergent executable content before the first product edit. |
| Current acceptance checkpoint | NNC6.3 A1-A14 are green. The sole full review's two P1 and two P2 planning findings are corrected; the sole narrow verifier-correction review is clean at `0.99`. Focused NNCV030 passes `10/10` and `13/13`, live verifier `31/31`, aggregate mutations `228/228`, docs `108`, site `17/17`, proof lint, syntax, helper ShellCheck, format, and diff gates pass. NNC6.3a is active but has not yet changed product source. |
...
| Last checkpoint commit | NNC6.1e1 item commit `26df5075d7dab582a4c9602e248993eabd8eab49`. |
| Audit dirty state | NNC6.3's exact five owned paths are staged for its item commit; product source remains byte-unchanged. NNC6.3a has no product dirty path yet. The proof is under the ignored private-doc fence and is force-tracked deliberately. |
| Latest dirty-state checkpoint | NNC6.3 began from clean owner HEAD. Its narrow-reviewed staged tree before closeout ledger wording is `313c351d2c980cae97c15d1e28d94fceb91dd5b4`, with binary patch SHA-256 `f90e11fdbaf7df832a25a068e5aefb8e909e5dbf402b126f60a0c70cafc41b2c`. Product source remains unchanged. |
| Current item review | NNC6.3's sole full actual GPT-5.6 Sol/xhigh/fast review reported two P1 and two P2 planning findings at `0.98`; all four are accepted and corrected. Its sole narrow Sol/xhigh/fast verifier-correction review is clean with no findings at `0.99`. No further NNC6.3 review is warranted. NNC6.3a has not reached candidate review. |
| Candidate-frozen identity | Corrected five-path substantive staged tree before identity wording `ec67da6dcf74b5ed0ac1daac3aad404da0534bc5`; binary patch SHA-256 `78941ed4e8036bc92648ccff6d75fe77a2bfd027280cb06b8e13b8f4898b3c9b`. Only candidate identity and later review/ledger evidence may change before closeout. |
| Final post-review candidate identity | Narrow-reviewed staged tree before closeout wording `313c351d2c980cae97c15d1e28d94fceb91dd5b4`; binary patch SHA-256 `f90e11fdbaf7df832a25a068e5aefb8e909e5dbf402b126f60a0c70cafc41b2c`. Closeout wording changes only plan/proof/routing state. |
...
| Latest verification checkpoint | NNC6.3 product source is unchanged. Exact source census is recorded; new-proof writing lint passes with zero errors and 10 advisories; focused NNCV030 passes `10/10` and mutations `13/13`; live static verifier passes `31/31`; retained aggregate mutations pass `228/228`; Bash syntax, helper ShellCheck, `cargo fmt --all --check`, `git diff --check`, docs `108`, and site `17/17` pass. The sole narrow review is clean at `0.99`. |
| Blocking decision | No blocker. NNC6.3 is complete and NNC6.3a is dependency-ready. |
...
| NNC6 — compute choreography | `in_progress` | Exact provision/teardown observers across every workload/retirement path. | NNC6.1-NNC6.3 and NNC6.1e1 are complete. NNC6.3a is active and owns the strict executable carrier plus closed desired digest; NNC6.3b remains pure preparation, and NNC6.4 remains one atomic provider-command/caller replacement. No item retains a temporary legacy authority. NNC6.4a/NNC6.5/NNC6.6 retain restart, teardown, and resolver fencing; NNC6.1e2 is the final convergence gate. Provider effects remain with their named owners. |
...
| NNC6.3a | `in_progress` | **Dependency:** NNC6.3 complete. **Owned paths:** none dirty yet; prospective ownership is workloads saga executable vocabulary, compute executable codec/digest derivation, server Engine codec/round-trip tests, NNCV031 contract, proof, plan, and routing checkpoint. **Last green:** NNC6.3 live verifier `31/31`, mutations `228/228`, docs `108`, site `17/17`, and clean narrow review `0.99`. **Next:** inspect the exact carrier/codec/store call graph and capture the written fail-before matrix before product edits. **Blocker:** none. |
| NNC6.3b | `todo` | **Dependency:** NNC6.3a. Owns the pure provision decision protocol, exact node/provider/publication inputs, definitive/ambiguous outcome vocabulary, fail-before composition, and no product effect interface or caller cutover. |
