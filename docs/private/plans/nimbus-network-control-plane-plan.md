...
| Next action | Implement NNC6.5d4 band 5c from `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`: add the concept-owned guest composite execution drain/stop sink. Authenticate the installed forwarder and local node, use the one Container-rooted generic journal, compose the exact Systemd and Container substeps, require both child receipts before one generic publication, and implement the deterministic read-only inspection join. Keep the private route absent and retain its separate capability blocker. Do not add forwarded attachment composition, a route, client, parent adapter, caller cutover, or coarse-stop deletion. |
| Current acceptance checkpoint | NNC6.5d4 audit and fail-before are durable at `29137fa000341e48bd9ee41ef37451f5611977f1`. Bands 1-5b are green. K3-K4 pass. The operation-family and injected-journal prerequisites of K7 pass. K11-K14 have their strict wire vocabulary, digest, response correlation, and closed outcomes. K15 has a closed translation without caller-selected provider IDs. K16 passes with deterministic durable Systemd production composition. Band 5a closes Systemd activation admission before drain and authenticates stop with the exact prior drain receipt. Band 5b now fences all four remaining Container provision producers under the one lifecycle lock and durable drain barrier, provides one cancellation-safe asynchronous exact-current-claim seam around the existing Container-rooted journal, and provides one authorized Container substep that cannot publish generic success. One process claimant wins, a still-startable Execute cannot cross an Inspect `NotCompleted` classification, exact child replay has no second effect, and no second journal exists. |
...
| Last checkpoint commit | `0eb5cab62` is the durable NNC6.5d4 band 1 checkpoint. `fb7acb395` is band 2. `690ac9f22` is band 3. `1100cbc04` is band 4. `bf7b61808` is band 5a. The commit containing this header and its matching proof row is band 5b. The prior audit checkpoint is `29137fa000341e48bd9ee41ef37451f5611977f1`. |
| Audit dirty state | The owner worktree was clean after band 5a commit `bf7b61808`. Preserve only the active band 5b Container provision fences, lifecycle-lock module extraction, asynchronous exact-current-claim seam, authorized Container substep, concept-owned tests, NNCV024 source-coordinate correction, routing, proof, and plan paths until their recovery commit. The original checkout remains untouched. After that commit, band 5c owns only the guest composite execution sink, its exact composition and inspection tests, capability blocker reconciliation, and matching ledger paths. |
| Latest dirty-state checkpoint | Band 5b is candidate-green. Reservation, preparation, private attachment, and machine publication hold the same Container lifecycle lock from current-state authentication through child settlement and reject the durable drain barrier before replay or effect. The asynchronous journal worker retains exact lock and publication ownership through caller cancellation; Claimed inspection returns explicit still-startable authority without polling. The Container substep authenticates the lock-scoped claim, reuses the existing state machine, and leaves generic journal bytes Claimed until its caller publishes. Focused, process-contention, full Sandbox and compute, affected quality, static, and source-verifier mutation gates pass. The commit containing this header is the band 5b recovery checkpoint. Band 5c is next; NNC6.5d4 remains one review unit. |
...
| Last verification | NNC6.5d4 band 5b async journal passes `7/7` with one declared child-only ignore; provision barrier passes `3/3`; Container child execution passes `3/3`; exact child stop ordering passes `1/1`; provision phases pass `27/27`. Full Sandbox passes `1,133` executed with `48` declared platform or child-process ignores. Full compute passes `381/381` with one declared child-only ignore. All-target Sandbox check, strict affected Clippy, warning-denied affected Rustdoc, format, diff, NNCV008, NNCV024 direct and exact creating-lock mutation, NNCV035 self-test `55/55`, and NNCV035 direct expected red `0/7` pass. The aggregate is `35/36` with only NNCV035 red. Proof lint reports zero diagnostics. Docs pass `108`; the site passes `17/17`. No structured review ran. |
| Latest verification checkpoint | Active record: `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. It records bands 1-5b fail-before, implementation, behavior counts, quality gates, sparse portable semantics, strict forwarded full-chain semantics, injected-journal ownership, production Systemd composition, both activation-admission barriers, the one asynchronous journal, and the nonpublishing Container child seam. |
| Blocking decision | No blocker. Commit band 5b, then implement only band 5c. Do not run structured review on partial NNC6.5d4 work. |
...
| NNC6.5d4 | `in_progress` | **Dependency:** NNC6.5d3 complete at `c1c7f1397`. **Evidence:** `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. **Checkpoints:** audit `29137fa00`; band 1 `0eb5cab62`; band 2 `fb7acb395`; band 3 `690ac9f22`; band 4 `1100cbc04`; band 5a `bf7b61808`; the commit containing this row is band 5b. Three read-only audit packets freeze K1-K35 and the exact two-realm contract. **Bands 1-5a:** strict receipt-prefix fencing, one teardown-family final-withdraw stream, injected compute journal ownership, the transport-only Machine API envelope, durable production Systemd composition, and its activation-admission barrier are green without a route or product caller. **Band 5b:** all four remaining Container provision producers authenticate current durable state and open admission under the one lifecycle lock before replay or effect. One internally retained async worker keeps the exact provider stream locked through awaited effect and durable publication despite caller cancellation; Claimed inspection returns explicit still-startable authority without polling. One lock-scoped Container substep reuses the existing teardown state machine and cannot publish generic success after only its child effect. **Owned paths:** Container runtime provision, teardown, runner, composition, and tests; Sandbox provider-command current-claim code and tests; the source-contract verifier; this plan, proof, and routing index. The existing Container-rooted provider journal remains the sole guest journal. No guest sink, route, client, parent adapter, caller, dependency, `nimbus-network`, protocol version, or coarse-stop path changed. **Last green:** async journal `7 + 1`; provision barrier `3`; child execution `3`; child stop order `1`; provision phases `27`; full Sandbox `1,133 + 48`; full compute `381 + 1`; all-target check, strict affected Clippy, warning-denied Rustdoc, format/diff, NNCV008, NNCV024 direct plus exact creating-lock mutation, NNCV035 `55/55`, direct expected `0/7`, aggregate `35/36` sole NNCV035 red, proof lint zero, docs `108`, and site `17/17`. **Next:** band 5c adds only the guest composite execution drain/stop sink and deterministic inspection join while the route blocker stays explicit. **Forbidden:** forwarded attachment composition before its band, Compose down, product caller cutover, physical-machine stop, tenant policy, service naming, a CLI-local saga store, a public route, or coarse-stop deletion. **Review:** no structured review has run; the only full item review remains deferred until K1-K34 are green and the complete item is candidate-frozen. **Blocker:** none. |
