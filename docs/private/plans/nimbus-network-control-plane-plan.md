...
Status: `active; NNC6.5f complete; NNC6.5f1 in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC6.5f complete; NNC6.5f1 in progress` |
| Current band | `NNC6 — compute orchestration and exact lifecycle order` |
| Current item | `NNC6.5f1 — canonical forwarded composition and foreground retirement facade` |
| Last completed item | `NNC6.5f — read-only Compose, guest/forwarded, and physical-machine substitution audit`; the commit containing this ledger transition is its durable item checkpoint. |
| Next action | Audit the two forwarded composition roots and the retained server foreground runtime without product edits. Freeze fail-before tests for one returned canonical `PreparedForwardedWorkloadProfile` and one narrow `ComputeResourceRetirer` facade, then implement only after the NNC6.5f1 seam and path roster are exact. |
| Current acceptance checkpoint | NNC6.5f A1-A20 pass. The strengthened NNCV035 helper passes `138/138`; the complete aggregate passes `552/552`; live NNCV035 is exact `0/7`; and the live aggregate is exact `35/36` with only NNCV035 red. Rust format, Prettier, diff, syntax, focused ShellCheck, strict proof lint, dependency/effect, ledger/routing, docs `108`, and site `17/17` pass. The full review's eight and narrow review's nine accepted findings are corrected and proven. The cadence forbids another NNC6.5f review. Product source remains unchanged. |
...
| Last checkpoint commit | NNC6.5e is durably complete at `18377b1a2b17e21249c582ba49b8158280e0ff6d`. The prior exact checkpoint is NNC6.5d4 at `6b685627c94e`; its last recovery slice was band 8 at `3efa99911`. |
| Audit dirty state | The NNC6.5f closeout contains exactly the canonical plan, routing index, ignored/private proof, three concept-owned NNCV035 verifier files, and the aggregate verifier's exact-count update. It changes no product source. NNC6.5f1 starts from that item checkpoint. The original checkout remains untouched. |
| Latest dirty-state checkpoint | Seven audit-owned paths close NNC6.5f. Four verifier paths have binary patch SHA-256 `f952fee2b597fe8a097d5f04ccc23e7afcab6340ce4a89e8d094aadb3d9b14e5`; no crate path changed. The item commit that contains this transition is the durable checkpoint before NNC6.5f1 product work. |
| Last completed item review | NNC6.5f used one full and one narrow GPT-5.6 Sol/xhigh/fast review. The full review accepted eight findings, and the narrow review accepted nine P2 findings. All seventeen findings are corrected and proven. Review cadence is exhausted; no third review ran or is warranted. |
...
| Last verification | NNC6.5f helper `138/138`; aggregate `552/552`; live NNCV035 exact `0/7`; live aggregate exact `35/36` with only NNCV035 red; Rust format, Prettier, diff, Node/Bash syntax, focused ShellCheck, strict proof lint, NNCV004/NNCV008/NNCV009/NNCV012, docs `108`, and site `17/17` green. NNC6.5e behavior remains unchanged. |
| Latest verification checkpoint | Completed NNC6.5f record: `proof/nimbus-network-control-plane/nnc6.5f-compose-machine-caller-substitution-audit.md`. It records A1-A20, all seventeen review-finding corrections, the cross-owner barrier protocol, exact rosters, 138 sole-diagnostic mutations, and recovery state. |
| Blocking decision | No blocker. Commit the exact NNC6.5f closeout, then begin the read-only NNC6.5f1 substitution audit. |
...
| NNC6 — compute choreography | `in_progress` | Exact provision/teardown observers across every workload/retirement path. | NNC6.1-NNC6.5e and NNC6.1e1 are complete. NNC6.5f is the active read-only caller-substitution audit; it prospectively owns NNC6.5f1 forwarded composition, NNC6.5f2 Compose/coarse guest cutover, and NNC6.5f3 physical-machine fencing. NNC6.5g remains the later compensation, tenant-retirement, remaining deletion, and convergence unit. No completed item retains a temporary legacy authority. NNC6.6 retains resolver fencing; NNC6.1e2 is the final convergence gate. Provider effects remain with their named owners. |
...
| NNC6.5f1 | `in_progress` | **Dependency:** NNC6.5f is complete. Install one canonical forwarded teardown composition and the narrow foreground `ComputeResourceRetirer` facade. **Owned paths:** read-only audit first; expected implementation is limited to `nimbus-cli` forwarded network/Compose preparation, the narrow machine registry accessor, `nimbus-server/workload_composition.rs`, concept-owned tests, and the canonical proof/ledger/verifier coordinates. **Last green:** NNC6.5f A1-A20, helper `138/138`, aggregate `552/552`, live `0/7` and `35/36`, quality/docs gates. **Next:** freeze the exact current/target call graph, returned-result dataflow, fail-before tests, and candidate path roster before product edits. **Forbidden:** Compose down caller behavior, coarse guest deletion, physical-machine stop/barrier, compensation, tenant retirement, provider effects, and `nimbus-network` changes. **Blocker:** none. |
| NNC6.5f2 | `todo` | **Dependency:** NNC6.5f1. Open the canonical Engine/store for local and forwarded Compose down, retire through compute, report durable identity/outcome, and atomically delete the coarse guest stop envelope. Do not change physical stop. |
