...
Status: `active; NNC6.2 complete; NNC6.2a in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC6.2 complete; NNC6.2a in progress` |
| Current band | `NNC6 — compute orchestration and exact lifecycle order` |
| Current item | `NNC6.2a — durable complete compiled-plan carrier` |
| Last completed item | `NNC6.2 — admitted intent to exact NetworkPlan compiler` |
| Next action | Commit the exact reviewed NNC6.2 item, record its durable hash in the routing transition, then perform NNC6.2a's read-only durable-carrier/replay audit before any product edit. |
| Current acceptance checkpoint | NNC6.2 is complete: C1-C18 are green after five accepted full-review corrections and the narrow review's accepted count defect. Full affected behavior passes `1,477/1,477` with 27 declared skips; affected and workspace checks, strict Clippy, warning-denied rustdoc, NNCV028 `18/18` plus `6/6`, aggregate verifier `29/29`, exact aggregate arithmetic `198 + 6 = 204`, complete split-bound mutation coverage, format/diff, Bash/ShellCheck, docs 108, and site `17/17` pass. |
...
| Last checkpoint commit | `e183c7b8fe28cf88ff7dcb81f01c25d6b47b4cd1` (NNC6.2 frozen source allowlist after the durable audit/split checkpoints `c2dc4b4c0` and `e403741ba`). |
| Audit dirty state | Post-review NNC6.2 changes remain inside its frozen compute, workloads, tenant, sandbox, identity, verifier, census, proof, and recovery paths. Cargo manifests and `saga.rs` are unchanged; NNC6.2a has no product edit. |
| Latest dirty-state checkpoint | The pure compiler, source-sovereignty refinement, tenant-qualified retained plan identity, complete portable envelope correlation, tenant validation, sandbox capability projection, deterministic endpoint/route identities, and NNCV028 exist. The bounded child-process proof is concept-owned and explicitly ignored outside its parent. No lifecycle/provider effect, saga embedding, Cargo manifest, push, or PR occurred. |
| Current item review | NNC6.2's review cadence is exhausted: one full GPT-5.6 Sol/xhigh/fast review reported six findings at confidence `0.98`, five accepted/corrected and one source-contract rejection; the sole narrow review confirmed the behavioral corrections and reported one accepted P3 exact-count defect at confidence `0.96`, now corrected and proven. No third review is warranted. NNC6.2a has not reached candidate freeze and must not run structured review during audit or implementation. |
| Corrected candidate identity | Exact 23-path pre-ledger staged tree `0ebb9ba62ce52476b9b060a59946dabd00e4e9cf`; complete staged patch SHA-256 `c80a0425150b33a8cb6ec2705244411003cc5d6aa2a0a0f24ab2ced107e53574`; frozen `crates/` plus `scripts/` executable/static-proof SHA-256 `37a0eb67766208c4484854da0371884025a670ccba2f4b6788cdac11428fd481`. Ledger-only hash recording does not alter the executable digest. |
| Final post-review candidate identity | Exact 23-path pre-ledger staged tree `1ae690409c2c05c1474b52af7cec5242b97605a0`; complete staged patch SHA-256 `9a749f90cdeaca9b2e210c6359a892693f30dbce5526f48748f1fc136d2fc048`; final `crates/` plus `scripts/` executable/static-proof SHA-256 `c149dd56fc0908581c0a0ac0d168891b52446e20ec86707cdbb2877d43a7e813`. Final ledger-only recording does not alter the executable digest. |
...
| Latest verification checkpoint | Corrected affected suites pass network `236/236` with one child-only skip, tenant `97/97`, sandbox `948/948` with 25 intentional child/provider/scale skips, workloads `88/88`, and compute `108/108` with one explicit child-only ignore: total `1,477/1,477`. Focused workloads pass `14/14`; focused compute passes `15/15`. NNCV028 passes `18/18` with `6/6` mutations; aggregate verifier passes `29/29`; exact arithmetic proves HEAD `198` plus six counted NNC6.2 mutations equals/reports `204`; the complete contract is covered by a bounded prefix through 8/9 NNCV020 plus an exact continuation from the final NNCV020 mutation through NNCV028. Affected all-feature and workspace all-target checks, strict Clippy, warning-denied rustdoc, Bash/ShellCheck, format/diff, docs 108, and site `17/17` pass. The documented dormant `nimbus-fs/fuse` feature makes workspace-wide all-feature check fail at missing system `fuse.pc`, recorded rather than hidden. |
| Blocking decision | No blocker. NNC6.2a is dependency-ready for read-only audit after the exact NNC6.2 item commit. |
...
- NNC6.1e1 consumes NNC6.2a's durable compiled plan and owns the
...
| NNC6.2a | Persist the complete compiled network plan payload in workloads-owned saga intent. | A distinct process reopening only Engine durability reconstructs the exact plan and resource bytes at `IntentCommitted` before any network command; strict decoding rejects partial, digest-only, crossed, or unknown-version payloads; and the plan tuple remains a derived exact view rather than a second desired-state authority. |
| NNC6.1e1 | Route lazy activation and explicit service/sandbox lifecycle requests through a compute-owned saga ingress after NNC6.2a. | Logical naming and read-only resolution remain services-owned; compute persists the complete admitted intent before issuing any command; store failure, ambiguity, stale generation, or conflict makes zero effects; and `ServiceManager` is no longer installed as an effectful runtime registry or an independent lifecycle decision authority. |
...
| NNC6 — compute choreography | `in_progress` | Exact provision/teardown observers across every workload/retirement path. | NNC6.1-NNC6.1e are complete. NNC6.2 is the sole current item and owns admitted intent to exact `NetworkPlan` compilation without effects. NNC6.1e1 follows NNC6.2; NNC6.1e2 is the post-NNC6.3-NNC6.6 convergence gate. Provider effects remain with their named owners. |
...
| NNC6.2 | `done` | Final evidence: `docs/private/plans/proof/nimbus-network-control-plane/nnc6.2-admitted-network-plan-compiler.md`. One pure compute compiler validates already-admitted source correlation, monotonic sovereignty, and exact provider selection before producing a workloads-owned portable payload. Retained tenant-qualified identity rederives every plan/resource ID; retained requirements/resources derive and authenticate the complete envelope. Affected behavior is `1,477/1,477` with 27 declared skips; NNCV028 is `18/18` plus `6/6`; aggregate is `29/29` with exact `198 + 6 = 204` arithmetic and complete split-bound coverage; every quality/docs gate passes. One full Sol/xhigh/fast review reported six findings, five accepted/corrected and one source-contract rejection; the sole narrow review's one accepted P3 count defect is corrected/proven. No lifecycle/provider effect, saga embedding, Cargo edge, push, or PR. |
| NNC6.2a | `in_progress` | **Dependency:** NNC6.2 complete. **Owner:** workloads-owned durable complete compiled-plan carrier and distinct-process exact replay before first reservation. **Owned paths:** none; read-only audit only until one NNC6.2a proof freezes the strict codec/store/process allowlist. **Last green:** NNC6.2 C1-C18, review cadence, `1,477/1,477` affected behavior, NNCV028 `18/18` plus `6/6`, aggregate `29/29` plus exact 204-case coverage, and every quality/docs gate; NNC6.2a has no product edit. **Next:** audit the existing saga tuple, durable Engine adapter, codec/version ownership, crash cut, and zero-command replay seam; freeze its expected-red proof before product edits. **Forbidden:** lifecycle routing, provider effects, tuple-only or digest-only recovery, permissive/partial decoding, a second desired-state authority, or changes to NNC6.2's pure compiler seam. **Blocker:** none. |
| NNC6.1e1 | `todo` | **Dependency:** NNC6.1e and NNC6.2a. Owns compute saga ingress for lazy and explicit service/sandbox lifecycle requests; it may not invent a placeholder `NetworkPlan` or absorb provider effects. |
| NNC6.3 | `todo` | — |
| NNC6.4 | `todo` | — |
| NNC6.4a | `todo` | — |
| NNC6.5 | `todo` | — |
