...
| Next action | Implement NNC6.5d4 band 3 from `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`: add the strict Machine API teardown wire vocabulary, complete request digest, closed mode-specific outcomes, response correlation, and pure wire tests. Do not add the private route, client, provider composition, or caller cutover yet. |
| Current acceptance checkpoint | NNC6.5d4 audit and fail-before are durable at `29137fa000341e48bd9ee41ef37451f5611977f1`. Bands 1 and 2 are green. K3 passes; the portable half of K4 passes while forwarded full-chain enforcement remains open; and the operation-family plus injected-journal prerequisites of K7 pass while the future parent adapter remains open. One workloads-owned validated prefix projects exact durable receipt bytes through compute authentication. One compute-owned seam derives all five teardown journal claims from the confirmed command and receives the existing provider-owned journal. |
...
| Last checkpoint commit | `0eb5cab62` is the durable NNC6.5d4 band 1 recovery checkpoint. The commit containing this header is the durable band 2 recovery checkpoint. The prior audit checkpoint is `29137fa000341e48bd9ee41ef37451f5611977f1`. |
| Audit dirty state | The owner worktree was clean after `0eb5cab62`. Preserve only the band 2 `nimbus-sandbox`, `nimbus-compute`, proof, and plan paths until their recovery commit. The original checkout remains untouched. |
| Latest dirty-state checkpoint | Band 2 is candidate-complete. Three read-only packets changed zero paths. The owned diff adds a distinct teardown-family final-withdraw operation, one compute-owned confirmed-teardown command and injected-journal seam, and focused five-stream, restart-separation, crossed-command, mode-fence, and final-withdraw tests. Existing Container and Krun execution and attachment adapters use the seam without moving effects or journal ownership. |
...
| Last verification | NNC6.5d4 band 2 expected-red compilation exits `101` for both missing seams. Focused behavior passes shared seam `6/6`, real final-withdraw lowering `1/1`, sandbox operation family `4/4`, restart/final stream separation `1/1`, restart-domain rejection `1/1`, network retry authority `1/1`, sandbox teardown `24/24`, and restart regressions `1/1 + 1/1`. Full sandbox passes `1,109` plus `31` declared ignores; full compute passes `381` plus one declared ignore. Format, diff, strict affected Clippy, and warning-denied affected Rustdoc pass. NNCV008 passes; NNCV035 self-test passes `55/55`; direct remains exact expected red at `0/7`; aggregate is `35/36` with only NNCV035 red. Proof lint reports zero diagnostics; docs pass `108`; site passes `17/17`. |
| Latest verification checkpoint | Active record: `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. It records bands 1-2 fail-before, implementation, behavior counts, quality gates, sparse resource-free semantics, journal injection, restart/final operation separation, and the band 3 recovery point. |
| Blocking decision | No blocker. Commit the exact band 2 recovery checkpoint, then implement only band 3. Do not run structured review on partial NNC6.5d4 work. |
...
| NNC6.5d4 | `in_progress` | **Dependency:** NNC6.5d3 complete at `c1c7f1397`. **Evidence:** `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. **Audit checkpoint:** `29137fa000341e48bd9ee41ef37451f5611977f1`. **Band 1 checkpoint:** `0eb5cab62`. Three read-only audit packets freeze K1-K35 and the exact two-realm contract. **Band 1:** one workloads-owned strict prefix projects the exact durable ordered receipt subset, preserves resource-free gaps, and rejects malformed, reordered, duplicate, stale, or crossed history. Compute authenticates identical prefix bytes through confirmed command, provider observation, and result before reducer or CAS. **Band 2:** `WithdrawFinalPublication` is a distinct teardown-family journal operation; restart withdrawal and its lineage stay unchanged. One compute-owned confirmed-teardown seam derives the five exact operations and portable claim fences, receives the provider-owned journal, and is used by existing Container and Krun execution and attachment adapters. It does not open a store, select a namespace, or move an effect. **Owned paths:** band 2 changes only `nimbus-sandbox` provider-operation vocabulary and tests; the `nimbus-compute` confirmed-teardown command/journal seam, existing sandbox lowerers, and tests; this proof; and this recovery ledger. No Machine API, provider effect, caller, or `nimbus-network` path changes. **Last green:** band 2 fail-before exits `101`; focused seam/operation/restart checks pass `6 + 1 + 4 + 1 + 1 + 1 + 24 + 1 + 1`; full sandbox passes `1,109` plus `31` ignores; full compute passes `381` plus one ignore; format, diff, strict Clippy, and warning-denied Rustdoc pass. NNCV008 and NNCV035 self-test `55/55` pass; NNCV035 direct remains `0/7`; aggregate is `35/36` with only NNCV035 red; proof lint is zero; docs are `108`; site is `17/17`. **Next:** band 3 strict Machine API teardown wire vocabulary and pure wire tests. **Required seams:** distinct admitted parent attachment/execution/ingress provider IDs; parent publication withdrawal before guest drain/stop; exact remote guest envelopes and provider-journal reuse; parent release only after exact guest/provider absence; independent parent/guest durable realms; sibling-batch, response-loss, process-death, stale-generation, crossed-forwarder, and ambiguity convergence. **Forbidden:** Compose down, product caller cutover, physical-machine stop, tenant policy, service naming, a CLI-local saga store, a new public route, or coarse-stop deletion. **Review:** no structured review has run; the only full item review is deferred until K1-K34 are green and the complete item is candidate-frozen. **Blocker:** none. |
