...
Status: `active; NNC6.4 complete; NNC6.4a in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC6.4 complete; NNC6.4a in progress` |
| Current band | `NNC6 — compute orchestration and exact lifecycle order` |
| Current item | `NNC6.4a — fenced workload and explicit service restart cutover` |
| Last completed item | `NNC6.4 — atomic provider-command and provision-caller replacement` |
| Next action | Start R3 by extracting the NNCV034 fixture data before its single source-contract owner reaches 2,000 lines. Then add the exact native service restart facade/route and SDK method, route local and forwarded Compose through the same compute authority, add the fully fenced forwarded-machine command, and delete provider-local scheduler state. Keep final A1-A20 convergence and the one item-level review in R4. |
| Current acceptance checkpoint | R2 is durable through product commit `5826dff6019d453f9eba575ecf67850ac3b19e6a`. Compute owns the exact capability registry, dispatcher, bounded driver, deterministic clock, supervisor, and 64-page bounded durable watch. One generalized provider command journal authenticates source attempt, target attempt, workload generation, restart ordinal, provider realm, and observation. Real Container and Krun adapters implement the same small withdrawal, quiescence, preparation, retained attachment/PEP, activation-prerequisite, activation, readiness, publication, and observation roles. Server listener retention and exact ServiceManager execution projection preserve desired/durable/observed separation. Provider realms opt into restart only after they earn the complete set; forwarded machine remains truthfully unregistered until R3. Focused restart behavior passes compute `55/55`, sandbox `69/69`, and server `20/20`; full compute passes `289` with one ignore, sandbox `1,005` with `27` ignores, server `594` with `31` ignores, and services `82/82`. Final R2 composition/CLI adaptations pass `10/10`, `6/6`, and `3/3`. Affected all-target check, strict Clippy, warning-denied rustdoc, Rustfmt, diff, Bash/JavaScript syntax, and NNCV034 `71/71` pass. The live contract exits `1` only on service, machine, scheduler, and behavior. No service route, SDK method, machine restart transport, scheduler deletion, or final caller cutover entered R2. |
...
| Last checkpoint commit | `5826dff6019d453f9eba575ecf67850ac3b19e6a` is the durable R2 provider-orchestration checkpoint. Its tree is `0a78ba83822755b9a0c730eee19fa0de5c077305`; its patch from the prior ledger commit is SHA-256 `59b5ee9e59b40f979d6cdbc0f0d832f5a3d9d5307f937409e293b48b36a37ae9` across `93` paths. Earlier R2 checkpoints remain `14e6236d4e3d1199a7ae40674bcdedd50b98fd58` for portable command/store state, `8935e0c77dd188f50566b72c917b2005a213ecdd` for compute admission, and `e1e9c95167972c2566a468d01aa0b91e559dd9be` for confirmed command/result reduction. |
| Audit dirty state | The owner worktree was clean at R2 product checkpoint `5826dff6019d453f9eba575ecf67850ac3b19e6a` except for the already-open NNC6.4a proof update. The original checkout remains untouched with its user-owned `AGENTS.md`, concurrent-write benchmark, and browser bundle changes. |
| Latest dirty-state checkpoint | Only this plan, its routing index, and the NNC6.4a proof contain the post-product recovery update. R3 service, SDK, machine restart transport, and scheduler-deletion paths remain unchanged. |
| Current item review | NNC6.4a has no structured review. R1 is an internal candidate checkpoint, not the item review unit. One full Sol/xhigh/fast review runs only after A1-A20 are green and the complete item is candidate-frozen. The completed NNC6.4 review cadence remains closed. |
| Candidate-frozen identity | Executable/script patch SHA-256 `744fe56d40dbbb418d4450700f9a0662471fa93afa4814ec7ca9dd5a301fb0dd`; pre-ledger staged tree `44008a58a544e6023ddc0baf625bf35d13204c22`; pre-ledger full patch SHA-256 `43cb1fcb6cd15f3e15537eabf2c376bfc9619c2c23743321e477caafee294276`. Exactly `322` paths were staged with zero unstaged paths before recording this identity. |
| Final post-review candidate identity | NNC6.4 item commit `6f4f909a06a20de1003d5aafc2f5ffcba43cf0bd`; pre-ledger tree `c2bbd4ff3cbb9e5c49d59a1d737333cc3621fe9f`; full patch SHA-256 `1e899c0a2b29f93552bf0402414cddd94f70792157dd9aa83a82484c53d71b66`; executable/script patch SHA-256 `0486001f13ad2d72919784fdee9d1e68830669786102db284289ad40619691e4`; `326` owned paths. |
...
| Last verification | Direct NNCV033 `40/40`; mutations `50/50`; aggregate `34/34`; aggregate self-test `327/327`; affected non-CLI `2,502/2,502` with `79` skips; CLI `936/936` with one skip; system `73/73`; listener `18/18`; affected all-target check; strict affected Clippy; warning-denied rustdoc; Rustfmt, Prettier, diff, Bash syntax, scoped ShellCheck, dependency/effect/static scans; SDK build/typecheck/test with `24`-route parity and package closure; docs `108`; site `17/17`. Known vendored Brotli warnings are unchanged. |
| Latest verification checkpoint | R2 command/store/admission/confirmation evidence remains green. Provider orchestration adds compute restart `55/55`, sandbox restart `69/69`, server restart `20/20`, full compute `289` with one ignore, full sandbox `1,005` with `27` ignores, full server `594` with `31` ignores, full services `82/82`, server composition `10/10`, CLI Compose lifecycle `6/6`, and guest provision adaptation `3/3`. Affected all-target check, strict Clippy, warning-denied rustdoc for six public crates, Rustfmt, diff, Bash syntax, and JavaScript syntax pass. NNCV034 passes `71/71` sole-diagnostic mutations. Its live contract exits `1` on exactly service, machine, scheduler, and behavior, with no path violation. Known vendored Brotli warnings are unchanged. No final aggregate/docs/R4 or item-completion claim is made, and no structured review has run. |
| Blocking decision | No blocker. R2 is durable and R3 service/SDK, Compose, machine, and scheduler cutover work is dependency-ready. |
...
| NNC6 — compute choreography | `in_progress` | Exact provision/teardown observers across every workload/retirement path. | NNC6.1-NNC6.4 and NNC6.1e1 are complete. NNC6.4a is active for the fenced restart substitution audit. No item retains a temporary legacy authority. NNC6.5/NNC6.6 retain teardown and resolver fencing; NNC6.1e2 is the final convergence gate. Provider effects remain with their named owners. |
...
| NNC6.4a | `in_progress` | **Dependency:** NNC6.4 complete at `6f4f909a06a20de1003d5aafc2f5ffcba43cf0bd`. **Audit:** `docs/private/plans/proof/nimbus-network-control-plane/nnc6.4a-fenced-restart-substitution-audit.md`. A1-A20 freeze one nested same-generation restart state, a separate execution-attempt fence, compute-owned schedule/count/watch and command authority, Container/Krun substitutions, service/SDK ingress, race/crash/recovery behavior, deletion gates, later-owner boundaries, and the candidate path allowlist. **R0:** durable at `6d8961bd6d4da819b2524128cb398e22e0a9382f`. **R1:** durable at `d117ba369eaf5acc5ede9ec3edad32a11ddfbeb2`. **R2 portable/store, admission, command:** durable at `14e6236d4e3d1199a7ae40674bcdedd50b98fd58`, `8935e0c77dd188f50566b72c917b2005a213ecdd`, and `e1e9c95167972c2566a468d01aa0b91e559dd9be`. **R2 provider orchestration:** durable at `5826dff6019d453f9eba575ecf67850ac3b19e6a`; compute owns the exact registry/dispatcher/driver/supervisor/clock/bounded watch, the shared provider journal owns restart ordinals and attempt fences, and real Container/Krun plus server ingress/retained-listener capabilities are green. Provider realms opt in only after earning the complete set; forwarded machine remains unregistered until R3. Focused restart is `55 + 69 + 20`; full compute/sandbox/server/services is `289 + 1 ignore`, `1,005 + 27 ignores`, `594 + 31 ignores`, and `82`; final composition/CLI adaptation is `10 + 6 + 3`. Affected check, strict Clippy, rustdoc, format/diff/syntax, and NNCV034 `71/71` pass. Live red is exactly service, machine, scheduler, and behavior. **Current step:** R3 fixture extraction, service/SDK, Compose, machine, then scheduler deletion. **Review:** none; one full review only after A1-A20 are green. **Blocker:** none. |
