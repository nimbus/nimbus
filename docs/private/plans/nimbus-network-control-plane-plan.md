...
Status: `active; NNC4 complete; NNC5.1 in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC4 complete; NNC5.1 in progress` |
| Current band | `NNC5 — sandbox-owned deep attachment lifecycle` |
| Current item | `NNC5.1 — consolidate container/krun attachment phases and compensation` |
| Last completed item | `NNC4.7 — Linux sovereignty integration tripwire` |
| Next action | Begin NNC5.1 with a read-only container/krun attachment-phase, compensation, caller, and test-contract audit; record expected-red duplication and exact owned paths before editing sandbox code. |
| Current acceptance checkpoint | NNC4.7 is complete in `proof/nimbus-network-control-plane/nnc4.7-local-sovereignty-tripwire.md`. The fail-closed adapter records explicit `PASS`/`FAIL`/`SKIPPED`, independently authenticates source/runner/process/tool/topology/rule/counter/capture/trace/cleanup evidence, and derives assertions from exact raw artifacts. Deterministic 70/70 with zero skips, aggregate verifier 17/17 plus adversarial 62/62, website 109, docs 108, and site 17/17 pass. Two final source-matched LinuxKit processes on the same named runner each pass two complete attempts (96 commands, 210 artifacts, exact control counters, zero reset/profile counters); the ordered pair proves fresh-process cleanup/re-entry. All fourteen findings from the one full and one narrow Sol/xhigh/fast reviews are accepted, corrected, and dispositioned; the exact cadence permits no third review. |
...
| Last checkpoint commit | `df85ca12028c9433b083dcb1dab7a23c71d4c98f` — NNC4.6f. |
| Audit dirty state | NNC4.7 owns exactly twenty paths: modified `Makefile`, this plan, plan index, ignored/force-tracked proof, and aggregate verifier; one thin wrapper; ten concept-owned live/evidence Python modules; external verifier; deterministic owner, wrapper harness, and shell entry. No production Rust or Cargo dependency changed. The original checkout and clean `machine-os` companion remain untouched. |
| Latest dirty-state checkpoint | Starting Nimbus HEAD `df85ca12028c9433b083dcb1dab7a23c71d4c98f`, tree `27d1bced4d77b6633c1e624d8b5bac839f9808f8`. Final live executable harness digest `ae1c4124b3a8708325aea8ccd7259661f14ca12382741f4902c89cabbf9e17a5`; successor evidence SHA-256 `c633937360b3cae0eba662054ed5fd3040ab1117db58e41bc68baa32d3321c3d`; processes `82387@1398829` and `84795@1402754` independently pass and the ordered pair validator passes. Branch remains unpushed with no PR. |
| Current item review | NNC4.7 used exactly one full and one narrow correction review, both actual GPT-5.6 Sol/xhigh/fast. The full review found eight accepted defects; the narrow review found five additional executable defects and one stale-ledger defect. All fourteen are corrected and dispositioned with focused, aggregate, and fresh live proof. No third structured review ran or is permitted by the item cadence. |
...
| Last verification | NNC4.7 deterministic suite 70/70 with zero skips; Ruff format/lint, Python compile, ShellCheck, static tripwire contract, aggregate verifier 17/17 plus self-test 62/62, capability dimensions 15/15, capability registry/selection/registration 10/10, exact `nimbus-network -> nimbus-core` edge, website 109, docs 108, site 17/17, and diff check pass. Two final independent named LinuxKit invocations each pass 20 assertions, 96 commands, and 210 authenticated artifacts; ordered fresh-process re-entry passes. |
| Latest verification checkpoint | Runner `nnc47-linuxkit` on `docker-desktop`, LinuxKit `6.12.76-linuxkit` aarch64, boot `1000f2ed-b6bc-4e4d-8e5d-ca6b269d69a6`; predecessor process `82387@1398829`, successor `84795@1402754`; exact control counters are 1/1/1/1/1/0 and reset/profile counters zero in all four attempts. Evidence/manifests/archives/source hashes and every full/narrow review disposition are recorded in the item proof. The first 900-second aggregate self-test bound expired honestly during NNCV015; sufficiently bounded runs completed `62 passed, 0 failed`, including the final exact 70-test helper gate before test-only modular extraction; post-extraction focused/static gates pass unchanged. |
| Blocking decision | No blocker. All pre-review acceptance is green; the one item-level structured review, ledger transition, and exact commit are ordinary closeout work. |
...
| NNC5.1 | `in_progress` | **Dependencies:** NNC2 and NNC4 are complete. **Scope:** audit and consolidate duplicated container/krun attachment phases and compensation into one sandbox-owned deep module while preserving network portability and sandbox-owned effects. **Owned paths:** this recovery header/row and the future `proof/nimbus-network-control-plane/nnc5.1-sandbox-attachment-lifecycle.md`; the read-only audit must name exact sandbox implementation/test paths before any source edit. **Last green:** NNC4.7 closes with deterministic 70/70, verifier 17/17 + 62/62, and two-process privileged proof; no NNC5.1 implementation baseline is claimed before its audit. **Next:** read current container/krun phase callers, compensation paths, and contract tests; record the exact duplication/expected-red matrix before source edits. **Acceptance:** duplicated caller-local setup/cleanup switchboards are deleted and both backends run one shared behavioral contract suite. **Blocker:** none. |
| NNC5.2 | `todo` | — |
| NNC5.2a | `todo` | — |
| NNC5.3 | `todo` | — |
| NNC5.4 | `todo` | — |
