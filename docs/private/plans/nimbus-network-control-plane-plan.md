...
Status: `active; NNC5.4a complete; NNC5.5 in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC5.4a complete; NNC5.5 in progress` |
| Current band | `NNC5 — sandbox-owned deep attachment lifecycle` |
| Current item | `NNC5.5 — preserve sandbox effect ownership/locality` |
| Last completed item | `NNC5.4a — make machine-forwarded publication/withdrawal batches inspectable and idempotent` |
| Next action | Start NNC5.5 with a read-only substitution audit of every Netavark, nft, namespace, IPAM, gvproxy, and probe production effect plus the `nimbus-network` dependency/import surface. Freeze exact fail-before/static proof and owned paths before any executable edit. |
| Current acceptance checkpoint | NNC5.4a R1-R20 are green. The strict durable batch, per-slot ambiguity, SHA-256 envelope, exact provider observation, process-death cuts, two-process contention, dead-owner rebuild, lifecycle ordering, no-release fences, modularity, core-only edge, full Sandbox suite, static verifier/mutations, quality, docs, one full review, and one warranted narrow correction review are complete. NNC5.5 has not begun executable work. |
...
| Last checkpoint commit | This ledger transition is the exact NNC5.4a item checkpoint; its parent is NNC5.4 commit `239c9a5523d38350c0a74348f1501f0cb014ff2a`, tree `4b7e54e5d1db`. |
| Audit dirty state | The NNC5.4a checkpoint owns the canonical plan/routing/proof, Container durable publication and lifecycle integration, OCI provider adapter/receipts, port-lifecycle authentication, affected regression fixtures, and frozen verifier paths. The old terminal-only evidence files are deleted; `forwarding.rs` tests moved intact to concept-owned `forwarding/tests.rs`. The original checkout retains its two user-owned paths (`docs/private/plans/research/concurrent-write-throughput-benchmark.md` and `demos/convex/vendor/browser.bundle.js`), and the `machine-os` companion remains clean. |
| Latest dirty-state checkpoint | NNC5.4a is complete and staged as one exact item checkpoint. Production/test owners are concept-split below 1,500 lines; the 1,551-line aggregate verifier retains its explicit single-orchestration-owner exception and delegates NNCV021 logic/mutations. No unrelated change, rebase, destructive recovery, push, or PR occurred. |
| Current item review | NNC5.4a's sole full Sol/xhigh/fast item review returned six findings: four accepted and corrected, two rejected against explicit repo/authority contracts. The accepted executable changes warranted one narrow correction review; its sole IPv4-mapped finding is corrected and proven. No third review ran or is warranted. NNC5.5 review cannot run until that complete future item is candidate-frozen and green. |
...
| Last verification | Final NNC5.4a: Sandbox `898` passed, `0` failed, `27` intentional ignores; `nnc5_4a_` `17` passed with one child-only ignore; fresh-process named cuts `5` passed with one child-only ignore; forwarding `20/20`; provider cleanup `30/30`; live verifier `22/22`; adversarial mutations `111/111`; verifier self-test `112/112`; AST `12/12`; docs `108` and site `17/17`. Check, strict Clippy, warning-denied rustdoc, format/diff, census, syntax, ShellCheck, and core-only/effect scans pass. |
| Latest verification checkpoint | Full-review cleanup, wildcard, verifier-owner, and bind-census defects plus the narrow-review IPv4-mapped address defect each have exact fail-before and corrected proof. Strict legacy rejection and terminal-drift fencing remain deliberate. Exact executable/script SHA-256 is `a5eadd2b4795589ce7cf1244a74d54a3ca5d82edf851cd0cae92944304f940e5`. |
| Blocking decision | No blocker. Commit this exact NNC5.4a checkpoint, then execute the dependency-ready NNC5.5 read-only ownership/substitution audit. |
...
| NNC5.5 | `in_progress` | **Dependency:** NNC5.4a is complete in the immediately preceding exact checkpoint. **Owner:** preservation proof for sandbox-local Netavark, nft, namespace, IPAM, gvproxy, and probe effects plus the transport-free `nimbus-network` boundary. **Acceptance:** enumerate every production effect and call path, prove `nimbus-network` imports none and retains only the `nimbus-core` workspace edge, prove capability substitution cannot move effect ownership, and freeze exact fail-before/static mutation cases before any executable edit. **Owned paths:** canonical plan/routing plus a new NNC5.5 proof packet only until the read-only audit freezes exact source, test, and verifier ownership; no production path is yet claimed. **Last green:** NNC5.4a Sandbox `898/898` with 27 intentional skips, verifier `22/22`, mutations `111/111`, self-test `112/112`, AST `12/12`, affected quality/dependency/docs gates, and fully dispositioned full+narrow reviews. **Next:** perform the read-only source/dependency/substitution audit and record the item proof packet. **Blocker:** none. |
| NNC5.6 | `todo` | — |
| NNC6.1 | `todo` | — |
| NNC6.1a | `todo` | — |
| NNC6.1b | `todo` | — |
