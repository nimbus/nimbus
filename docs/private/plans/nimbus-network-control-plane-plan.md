...
Status: `active; NNC6.5c complete; NNC6.5d in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC6.5c complete; NNC6.5d in progress` |
| Current band | `NNC6 — compute orchestration and exact lifecycle order` |
| Current item | `NNC6.5d — Container/Krun execution and host-managed/forwarded-machine network teardown adapters` |
| Last completed item | `NNC6.5c — final-ingress and DirectProcess/Systemd teardown adapters`; the commit containing this checkpoint is its durable item commit. |
| Next action | Start NNC6.5d with a read-only source and substitution audit. Freeze its owned paths, provider-journal boundaries, exact fail-before roster, crash/restart matrix, and acceptance interpretation before product edits. Resume no NNC6.5c implementation work. |
| Current acceptance checkpoint | NNC6.5c C1-C24 are green. The full review's four accepted defects and narrow review's two P2 plus one P3 defects are corrected and proven. Final behavior is network 270 plus one declared ignore, serialized server 709 plus 33 declared ignores, node 105, and compute 345 plus one declared ignore. Affected quality gates, NNCV035 55/55 and exact 0/7, aggregate 35/36 sole NNCV035 red, strict proof lint, docs 108, and site 17/17 pass. Review cadence is exhausted; no third review ran. |
...
| Last checkpoint commit | The commit containing this checkpoint is the durable NNC6.5c item commit. Its parent, `3976a8b6c475bd60dcee347ba39182f3944a701d`, is the durable NNC6.5b checkpoint and clean NNC6.5c audit base. |
| Audit dirty state | The commit containing this checkpoint owns the complete NNC6.5c network lease, server ingress/listener, node DirectProcess/Systemd, compute substitution, test, proof, census, manifest, and lockfile diff. After that commit, no NNC6.5d dirty path is expected. Inspect status before the read-only audit. The original checkout remains untouched. |
| Latest dirty-state checkpoint | Narrow-review input tree `5eafeecef1177daeec94ceba850d5e21fabed145`; final closeout is the commit containing this checkpoint. |
| Current item review | NNC6.5c used exactly one full and one narrow GPT-5.6 Sol/xhigh/fast review. The narrow review's three accepted findings are corrected and proven. Review cadence is exhausted. NNC6.5d has no candidate and no review yet. |
| Full-review input identity | Staged tree `fb9a8e5a8a042aba84e66448931cae7831ff0e03`; complete patch SHA-256 `d637ae538e21b0c1aa09b8e1c932e4b36056a818325c101ae1f93dd76ad59d7c`; executable patch SHA-256 `f872c77c955dcfacc598b7b9327a6fe19cc70e90aaea5066401a849315e1dd4e`. Actual reviewer: GPT-5.6 Sol, xhigh, fast; one 412,059-byte pass; thread `019fe730-6430-77d1-be9a-8dc2251ef133`. |
| Narrow-review input identity | Staged tree `5eafeecef1177daeec94ceba850d5e21fabed145`; complete patch SHA-256 `60db86bd3ecc69553f871b09905b7351542d94f2e334ece1347604bfd9bb8302`; executable patch SHA-256 `f6e8a0bdc7817bb09b381ff4977631b939747663b28a0e41629c2e0b728c73f5`. Actual reviewer: GPT-5.6 Sol, xhigh, fast; one pass; three accepted findings at overall confidence 0.93. |
| Final closeout identity | The commit containing this checkpoint is the exact NNC6.5c item commit. |
...
| Last verification | Corrected behavior and quality are green: network 270 plus one declared ignore; serialized server 709 plus 33 declared ignores across all binaries; node 105; compute 345 plus one declared ignore; affected check, strict Clippy, warning-denied Rustdoc, format, and diff checks; NNCV035 55/55 and exact 0/7; aggregate 35/36 sole NNCV035 red; proof lint zero; docs 108; site 17/17. |
| Latest verification checkpoint | Full-review artifacts: `/tmp/nnc65c-full-autoreview.out` and `.json`. Narrow-review artifacts: `/tmp/nnc65c-narrow-correction-autoreview.out` and `.json`; focused/final artifacts are named in the proof. |
| Blocking decision | No blocker. NNC6.5c is complete. Start the NNC6.5d read-only audit. |
...
| NNC6 — compute choreography | `in_progress` | Exact provision/teardown observers across every workload/retirement path. | NNC6.1-NNC6.5c and NNC6.1e1 are complete. NNC6.5c exhausted its one full plus one narrow Sol/xhigh/fast review; all seven accepted defects are corrected and proven. NNC6.5d is in progress; NNC6.5e-NNC6.5g remain later units. No completed item retains a temporary legacy authority. NNC6.6 retains resolver fencing; NNC6.1e2 is the final convergence gate. Provider effects remain with their named owners. |
...
| NNC6.5d | `in_progress` | **Dependency:** NNC6.5c. Container/Krun drain/stop and host-managed/forwarded-machine detach/release adapters. **Owned paths:** pending read-only source audit and explicit path freeze; do not edit product paths before the freeze. **Last green:** inherited NNC6.5c closeout above. **Next:** audit the existing sandbox and machine provider journals, effect boundaries, callers, crash/restart semantics, and exact substitution points; then record the fail-before roster and acceptance mapping. **Blocker:** none. |
