...
| Next action | Implement NNC6.5d4 band 9 from `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. Add real compute registry substitution and the complete two-realm crash, contention, sibling-batch, empty-batch, and effect-order proofs. Keep product caller cutover, Compose down, physical-machine stop, public-route growth, and coarse-stop deletion out of this item. Do not run structured review until K1-K34 are green and the whole item is candidate-frozen. |
| Current acceptance checkpoint | NNC6.5d4 bands 1-8 are green. Band 8 completes K5-K10 and the direct parent portions of K22-K24. One five-capability adapter owns one parent journal namespace, exact request-before-send state, current execution tokens, and Inspect-before-retry recovery. Confirmed publication has explicit withdrawal and release progress. Complete parent forwarding and port batches stay fenced, including zero-listener workloads. Exact guest release and independent provider absence gate final parent port release. Band 9 owns real compute substitution and the remaining end-to-end K22-K29 proofs. No second journal or allocation authority exists. |
...
| Last checkpoint commit | `0eb5cab62` is band 1. `fb7acb395` is band 2. `690ac9f22` is band 3. `1100cbc04` is band 4. `bf7b61808` is band 5a. `824c2f6f0` is band 5b. `62d21cb7a` is band 5c. `a98841aaa` is band 6. `b5d607418` is band 7. The commit that contains this header is the exact band 8 recovery checkpoint. The audit checkpoint is `29137fa000341e48bd9ee41ef37451f5611977f1`. |
| Audit dirty state | The commit that contains this header is the exact band 8 recovery checkpoint. It owns the parent five-capability adapter, confirmed-publication progression, strict request-before-send recovery, complete forwarding and port-batch fencing, exact Release response evidence, Unix gvproxy retirement provider, runtime/socket confinement, focused tests, source-derived verifier updates, and matching proof and routing records. The original checkout remains untouched. |
| Latest dirty-state checkpoint | Band 8 authenticates the complete tenant-qualified command, retained history, parent publication batch, durable bindings, provider incarnation, and guest release evidence before each effect. Fresh complete partial forwarding observations authorize only an adjacent fenced retry; live older effects remain `InProgress`. Parent and guest state roots remain separate. No compute registry substitution, product caller, coarse stop, dependency manifest, or `nimbus-network` path changed. NNC6.5d4 remains one review unit. |
...
| Last verification | Parent forwarding passes `2`; direct adapter substitution and ordering pass `3`; sandbox forwarding passes `27`; provider-command passes `42` with two ignores; and the strict Release wire regression passes `1`. Full Machine passes `42 + 5`; Sandbox passes `1,164` with `48` ignores; Compute passes `381` with one ignore; and CLI passes `986` with two ignores. Affected checks, strict all-feature Clippy, warning-denied Rustdoc, format, diff, dependency/effect scans, source-derived censuses, and batch/readiness verifiers pass. Aggregate self-test passes `469/469`; NNCV035 passes `55/55`; direct NNCV035 remains expected `0/7`; and live aggregate remains `35/36` with only NNCV035 red. Proof lint reports zero diagnostics. Docs pass `108`; the site passes `17/17`. No structured review ran. |
| Latest verification checkpoint | Active record: `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. It records bands 1-8 fail-before, implementation, recovery behavior, exact counts, quality gates, source-derived verifier reconciliation, and explicit complexity ownership. Band 9 remains open. |
| Blocking decision | No blocker. The commit containing this header closes band 8. Implement only band 9 next. Do not run structured review on partial NNC6.5d4 work. |
...
| NNC6.5d4 | `in_progress` | **Dependency:** NNC6.5d3 complete at `c1c7f1397`. **Evidence:** `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. **Checkpoints:** audit `29137fa00`; band 1 `0eb5cab62`; band 2 `fb7acb395`; band 3 `690ac9f22`; band 4 `1100cbc04`; band 5a `bf7b61808`; band 5b `824c2f6f0`; band 5c `62d21cb7a`; band 6 `a98841aaa`; band 7 `b5d607418`; the commit containing this row is band 8. **Bands 1-7:** prior receipts, one final-withdraw stream, injected journals, strict private wire, durable guest composition, producer barriers, exact guest execution and attachment phases, and one-shot transport are green. **Band 8:** one parent five-capability adapter owns the exact request-before-send state, Inspect-before-retry recovery, confirmed-publication progression, complete forwarding-batch withdrawal, and proof-gated complete parent port release. Strict Release response evidence, Unix gvproxy retirement, runtime/socket confinement, fresh-partial retry authority, and exact direct adapter tests pass. **Last green:** parent forwarding `2`; direct adapter `3`; sandbox forwarding `27`; provider command `42 + 2 ignores`; Release wire `1`; full Machine `42 + 5`; full Sandbox `1,164 + 48 ignores`; full Compute `381 + 1 ignore`; full CLI `986 + 2 ignores`; affected checks; strict Clippy; warning-denied Rustdoc; format/diff; aggregate self-test `469/469`; NNCV035 `55/55`; direct expected `0/7`; aggregate `35/36` sole NNCV035 red; proof lint zero; docs `108`; site `17/17`. **Truth boundary:** K5-K10 and direct parent K22-K24 are green. Band 9 owns real compute registry substitution and remaining two-realm K22-K29 proofs. **Next:** band 9. **Forbidden:** product caller cutover, Compose down, physical-machine stop, tenant policy, service naming, a CLI-local saga store, public-route growth, coarse-stop deletion, dependency changes, or `nimbus-network` effects. **Review:** no structured review has run. Run one full Sol/xhigh/fast review only after K1-K34 are green and the complete item is candidate-frozen. **Blocker:** none. |
