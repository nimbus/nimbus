...
Status: `active; NNC5.2b complete; NNC5.2c in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC5.2b complete; NNC5.2c in progress` |
| Current band | `NNC5 — sandbox-owned deep attachment lifecycle` |
| Current item | `NNC5.2c — pure exhaustive orphan classifier` |
| Last completed item | `NNC5.2b — durable orphan evidence enumeration` |
| Next action | Commit the exact completed NNC5.2b checkpoint, then read NNC5.2c's acceptance row and current NNC0.7 eight-row expected-red proof. Audit the classifier's minimum input/output vocabulary before changing executable code; keep it total, pure, I/O-free, and incapable of classification side effects. Do not start NNC5.2d quarantine/startup wiring, run another NNC5.2b review, push, or open a PR. |
| Current acceptance checkpoint | NNC5.2b criteria 1-12 are green. One sandbox-owned locator persists tenant, sandbox, exact OCI provider kind, and a versioned SHA-256 artifact-realm identity before effects; its breaking `v2` realm authenticates the directory capability actually scanned, so ambient path retarget cannot splice evidence across realms. Every Netavark lifecycle transition reauthenticates that locator, and provider-attempt handles bind provider/action/tenant/attachment plus a domain-separated digest of the exact claim, segment, and locator generation. Live-to-terminal IPAM transition, replay, reconciliation, and retirement authenticate exact realm/backend and accept only no-effect `Reserved` or state-machine-proven `Detached` phases. Typed store inventory validates every durable key and returns every tenant-IPAM partition deterministically. The orphan collector accepts only least-authority read traits, forms one tenant-qualified desired/provider union, performs exact claim-qualified allocator inspection, and retains artifacts/errors as untrusted present/absent/typed-unknown evidence. It has no classifier, quarantine, cleanup, release, finalization, or reuse capability. Full-review fail-before/correction is `0/5` then `5/5`; narrow-review fail-before/correction is `0/2` then `3/3`; placement provider/config cleanup is `1/1`; orphan `18/18`, focused network/IPAM/Netavark `88/88`, affected crates `1048/1048` with 26 skips, check, strict Clippy, warning-denied rustdoc, exact core-only dependency, live verifier `18/18`, adversarial self-test `67/67`, and format/diff pass. |
...
| Last checkpoint commit | `154a5af0c8137fbdaf998f635796dd18d108d043` — exact reviewed NNC5.2a checkpoint; tree `d784f7799852`. The exact NNC5.2b commit is the next command and will replace this field after it is durable. |
| Audit dirty state | The only changes after exact NNC5.2a commit `154a5af0c8137fbdaf998f635796dd18d108d043` are the 40 NNC5.2b-owned paths: 36 executable paths plus the plan, routing index, proof, and existing production-authority census. Before this ledger truth-up, all 40 were staged with zero unstaged paths. The executable SHA-256 is `90029aaeb486d651bee4c237e8d6a224ffc17b17b6ad2bb48fab852119f69156`. The original dirty checkout retains exactly its four user-owned paths and the `machine-os` companion remains clean. |
| Latest dirty-state checkpoint | NNC5.2b was prospectively split before executable edits, so evidence enumeration, pure classification, and startup fencing remain separate acceptance-bearing units. The sole full review's four and sole narrow review's two accepted defects are corrected and proven. Provider evidence now authenticates the pinned directory handle; every terminal IPAM transition/replay/retirement authenticates exact realm/backend in both safe phases; and provider identity is paired with placement config construction before IPAM. The final candidate is 40 owned paths with zero unstaged changes before ledger truth-up, executable SHA-256 `90029aaeb486d651bee4c237e8d6a224ffc17b17b6ad2bb48fab852119f69156`, and crate-only SHA-256 `80d9dde4502ee6f19b7550e15b6d0977cdad6f6698fbaf384f28c897b6deae9b`. The parent eight-row classifier fail-before intentionally remains red for NNC5.2c/d. No unrelated path, rebase, destructive recovery, push, or PR occurred. |
| Current item review | NNC5.2b's sole full GPT-5.6 Sol/xhigh/fast review produced four accepted findings, all corrected. Its one permitted narrow Sol/xhigh/fast correction review found two accepted residual bypasses; both have exact fail-before and corrected proofs. The review cadence is exhausted: no third structured review ran or is warranted. NNC5.2c has not yet reached its candidate-review gate. |
...
| Last verification | NNC5.2b final candidate: full-review fail-before/correction `0/5` then `5/5`; narrow-review fail-before `0/2` with exit 101/101 and correction `3/3`; placement pairing `1/1`; orphan `18/18`; focused lane `88/88`; exact unrelated Conmon reproduction `1/1`; complete affected retry `1048/1048` with 26 skips; all-target/all-feature check; strict Clippy; warning-denied rustdoc; exact `nimbus-network -> nimbus-core`; verifier `18/18`; adversarial self-test `67/67`; format/diff. |
| Latest verification checkpoint | The first full affected attempt under concurrent external CPU stress passed `1047/1048`; the sole failure was an unrelated Conmon timing case that immediately passed `1/1` alone. With no source change, the bounded two-test-concurrency full rerun passed `1048/1048`. Static and architectural gates are green; documentation gates are the final pre-commit command. |
| Blocking decision | No blocker. NNC5.2b needs only documentation/restaging/commit closeout; NNC5.2c is dependency-ready immediately after that commit. |
...
| NNC5 — sandbox attachment lifecycle | `in_progress` | Shared implementation, complete readiness, ambiguity/crash convergence. | NNC5.1 begins from completed NNC2 and NNC4 dependencies. |
...
| NNC5.2c | `in_progress` | **Dependencies:** NNC5.2b is complete and its exact local checkpoint commit is the next command. **Scope/evidence:** pure total eight-row `Adopt`/named-`Quarantine` classifier only; the existing NNC0.7 matrix and `docs/private/plans/proof/nimbus-network-control-plane/nnc5.2b-durable-orphan-evidence-enumeration.md` define its inputs. **Fail-before:** NNC0.7 remains intentionally red at exit 101, `0/1`, with all eight required rows mismatched; recover that exact evidence before edits. **Owned paths:** none beyond the staged NNC5.2b checkpoint; NNC5.2c begins with a read-only vocabulary audit. **Last green:** NNC5.2b final gates are recorded in the prior row; no NNC5.2c acceptance claim exists yet. **Review:** none; structured review waits for the complete candidate and all written criteria. **Boundaries:** no I/O, mutation, provider capability, quarantine application, cleanup, release, finalization, or capacity reuse. **Next:** commit NNC5.2b, read the eight-row test and classifier seam, then create the NNC5.2c proof/expected-red checkpoint before executable edits. **Blocker:** none. |
| NNC5.2d | `todo` | Depends on NNC5.2c. Exact quarantine application, two-backend startup wiring, no-hold admission fence, filename-authority deletion, and NNCV018; NNC8.3 retains cleanup/release convergence. |
| NNC5.3 | `todo` | — |
| NNC5.4 | `todo` | — |
| NNC5.5 | `todo` | — |
| NNC5.6 | `todo` | — |
