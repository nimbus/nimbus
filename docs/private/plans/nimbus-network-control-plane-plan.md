...
Status: `active; NNC6.5d4 complete; NNC6.5e in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC6.5d4 complete; NNC6.5e in progress` |
| Current band | `NNC6 — compute orchestration and exact lifecycle order` |
| Current item | `NNC6.5e — native service, sandbox, and definition caller cutover` |
| Last completed item | `NNC6.5d4 — end-to-end forwarded-machine teardown provider adapters and exact guest phase envelopes`; the commit containing this checkpoint is its durable item commit. |
| Next action | After the commit containing this header proves NNC6.5d4 durability, resume NNC6.5e with a read-only native service, sandbox, and definition caller-cutover audit. Keep Compose down, forwarded composition, physical-machine stop, tenant retirement, failed-provision compensation, public-route growth, and coarse-stop deletion in their later owners. |
| Current acceptance checkpoint | NNC6.5d4 K1-K35 are green. Band 9 composes the real five-capability compute registry, exact request-before-send and Inspect-before-retry recovery, uniform complete-batch retain/release, explicit zero-listener publication evidence, and a fresh-process two-realm crash matrix. Full Workloads, Network, Compute, Sandbox, Machine, Node, and corrected serialized CLI suites pass. Strict quality and proof gates, aggregate mutation `481/481`, docs `108`, and site `17/17` pass. NNCV035 passes `67/67`; direct remains the intended `0/7`; and the live aggregate remains exact `35/36` with only NNCV035 red. The one full review's P1 is source-rejected with behavioral evidence; its accepted P2 is corrected. The sole narrow review is clean at `0.99`. No second journal or allocation authority exists. |
...
| Last checkpoint commit | The commit containing this recovery header is the exact NNC6.5d4 item checkpoint. Its last recovery slice was band 8 at `3efa99911fbacef60bdd911ddef43af391b745c2`. Short checkpoints: audit `29137fa00`; band 1 `0eb5cab62`; band 2 `fb7acb395`; band 3 `690ac9f22`; band 4 `1100cbc04`; band 5a `bf7b61808`; band 5b `824c2f6f0`; band 5c `62d21cb7a`; band 6 `a98841aaa`; band 7 `b5d607418`. |
| Audit dirty state | Band 9 owns only the forwarded-machine registry/substitution tests, guest prepared-start recovery, complete provider-managed port-batch retain/release correction, explicit empty publication reference, source-derived NNCV035 and bind-census coordinates, and matching proof/ledger/routing closeout. Product caller cutover and every NNC6.5e-NNC6.5g seam remain untouched. The original checkout remains untouched. |
| Latest dirty-state checkpoint | Band 9 authenticates all five real compute capabilities through the forwarded adapter, persists exact prepared request bytes before guest I/O, invalidates delayed tokens after inspected absence, retains siblings until exact complete-batch absence, and recovers terminal-response-loss and pre-effect process death from separate parent and guest roots. K1-K35 are green. The exact item commit is the remaining durability action. |
| Last completed item review | The one full GPT-5.6 Sol/xhigh/fast NNC6.5d4 review reported one P1 and one P2 at confidence `0.99`. The P1 is rejected by the unchanged two-cut process proof and composite inspection join. The accepted P2 bounded-wait defect is corrected. The sole narrow review is clean at `0.99`. Review cadence is exhausted. |
| Full-review input identity | Staged tree `7440423203e010ad6d42cc13d97b22ec7e0c5613`; patch SHA-256 `7e455251c72fb15af7c8ce75800deb62d7d09750de8f7c3f5f7c3bf2e1e254ad`; thread `019feeef-9501-7772-bca7-ecfa538657d5`; `34` paths, including `26` Rust paths. The wrapper ran one item-bundle pass. |
| Narrow-review input identity | The sole narrow review used staged tree `cddb5db89fe32db1743ca45e6c39ed926194b27c`, patch SHA-256 `085d9a6a6f9c89982982848743b6784db8c2dbbd4af291fbe79e2912d181b895`, and thread `019feeff-debb-7021-bc42-1733d2766e85`. The wrapper ran one correction-bundle pass. |
| Narrow-review result | Zero findings. The review confirms the P2 correction and the P1 rejection at confidence `0.99`. No further review is authorized or needed. |
...
| Last verification | Full Workloads passes `219`; Network `274 + 1 ignore`; Compute `381 + 1 ignore`; Sandbox `1,164 + 48 ignores`; Machine `47`; Node `121`; and corrected serialized CLI `995 + 3 ignores`. The focused forwarded lane passes `27 + 1 ignore`; the missing-call timeout regression passes `1/1` in `0.11` seconds. Strict Clippy/rustdoc/format/diff, dependency/effect scans, NNCV035 `67/67`, aggregate mutation `481/481`, direct `0/7`, live aggregate `35/36` sole NNCV035 red, proof lint zero, docs `108`, and site `17/17` pass. |
| Latest verification checkpoint | Active record: `proof/nimbus-network-control-plane/nnc6.5d4-forwarded-machine-teardown-provider.md`. It records bands 1-9; full affected behavior; strict quality; exact direct and aggregate source-contract state; modularity dispositions; aggregate mutation `481/481`; proof lint zero; docs `108`; and site `17/17`. |
| Blocking decision | No blocker. Commit the exact NNC6.5d4 checkpoint, then resume the NNC6.5e read-only audit. |
...
| NNC6 — compute choreography | `in_progress` | Exact provision/teardown observers across every workload/retirement path. | NNC6.1-NNC6.5d4 and NNC6.1e1 are complete. NNC6.5e is next; NNC6.5f-NNC6.5g remain later caller/convergence units. No completed item retains a temporary legacy authority. NNC6.6 retains resolver fencing; NNC6.1e2 is the final convergence gate. Provider effects remain with their named owners. |
...
| NNC6.5e | `in_progress` | **Dependency:** NNC6.5c and NNC6.5d3 are complete. **Scope:** native service, sandbox, and definition caller cutover only. **Owned paths:** the initial audit owns only this plan, its routing row, and a new concept-owned NNC6.5e proof record; freeze exact product and test paths before any edit. **Last green:** NNC6.5d4 closes with Workloads `219`, Network `274 + 1 ignore`, Compute `381 + 1 ignore`, Sandbox `1,164 + 48 ignores`, Machine `47`, Node `121`, serialized CLI `995 + 3 ignores`, NNCV035 `67/67`, aggregate mutation `481/481`, docs `108`, and site `17/17`. **Next:** run a read-only caller, authority, retry, and deletion-order audit before product edits; define explicit acceptance criteria and fail-before proofs in the canonical plan and a concept-owned proof record. Compose, forwarded composition, physical-machine stop, tenant retirement, failed-provision compensation, and coarse-stop deletion remain later-owned. **Blocker:** none. |
