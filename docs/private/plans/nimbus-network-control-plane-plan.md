...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC8.4 complete; NNC8.5 in progress` |
| Current band | `NNC8 — full recovery, ambiguity, and state-machine proof` |
| Current item | `NNC8.5 — bounded retries, backoff, and cancellation` |
| Last completed item | `NNC8.4 — stale generation/epoch callbacks and restart eligibility`; the commit containing its completed ledger row is the durable item commit. |
| Next action | Start the NNC8.5 read-only retry-loop census. Freeze the permanent-failure, backoff, cancellation, and diagnostic matrix before product edits. |
| Current acceptance checkpoint | NNC8.4 K1-K14 pass. The one Sol/xhigh/fast review accepted only a corrected recovery-text P3; no narrow review was authorized. NNC8.5 implementation has not started. |
...
| Execution base | Current `origin/main` is `8877eaff43a36d9606a1feaa0ab31d0377539d9d`; the NNC8.3 checkpoint `fa10498777d5b44b5c698b7a91bb127b2aa23db4` is `0 behind / 167 ahead`. No merge or rebase is required. |
| Last checkpoint commit | `fa10498777d5b44b5c698b7a91bb127b2aa23db4` is the last addressable checkpoint; the commit containing this row becomes the durable NNC8.4 checkpoint. |
| Owned paths | NNC8.5 initially owns only its proof/ledger audit. Product paths remain closed until the frozen retry matrix identifies an expected-red behavior. |
| Latest dirty-state checkpoint | NNC8.4 closes three paths. Its review input tree was `683e68763289e3a706e63168abe7fcf611cb780d`, with patch SHA-256 `3841c550b9f29dddb1d1870e1c643505acf64c775b3f7fdbf27888d5023a42a2`. The sole accepted P3 changed only recovery text. |
| Last completed item review | NNC8.4 used one GPT-5.6 Sol/xhigh/fast review. It accepted one recovery-text P3 at `0.98`; the implementation and behavior were accepted. The correction changed no executable code, so no narrow review ran. |
...
| Last green | NNC8.4 focused matrix `145`; compute `477 + 1 ignored`; NNCV034 `86/86`; live architecture `38/38`; docs `108`; site `17/17`. |
| Latest verification checkpoint | NNC8.4 compute all-target/all-feature check and strict Clippy/Rustdoc, Rustfmt, Prettier, diff, and strict proof lint pass. The sole review accepted one recovery-text P3 and no executable finding. |
| Next | Commit the exact NNC8.4 item, record its hash as the NNC8.5 recovery base, and start the NNC8.5 read-only retry-loop census. |
| Blocker | none |
...
| NNC8 — recovery/fencing | `in_progress` | Failure table and crash/stale/ambiguity model fully closed. | NNC8.1-NNC8.4 are complete. NNC8.5 starts with a read-only retry, backoff, cancellation, and diagnostics audit. |
...
| NNC8.5 | `in_progress` | **Dependency:** NNC8.4 is complete in the commit containing the preceding row. **Evidence:** planned `proof/nimbus-network-control-plane/nnc8.5-bounded-retries-backoff-cancellation.md`. **State:** read-only audit next; implementation has not started. **Owned paths:** proof/ledger only until the retry-loop matrix and expected-red seam are frozen. **Last green:** NNC8.4 final gates pass; NNC8.5 has no executable change. **Next:** inventory every network-control-plane retry loop and classify bounds, backoff, cancellation, permanent failure, diagnostics, and effect ambiguity. **Blocker:** none. |
| NNC8.6 | `todo` | — |
| NNC9.1 | `todo` | — |
| NNC9.2 | `todo` | — |
| NNC9.3 | `todo` | — |
