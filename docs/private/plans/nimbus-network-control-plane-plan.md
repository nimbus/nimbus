...
| Next action | Implement the R2 compute-owned normalized reducer, confirmed command, dispatcher, driver, deterministic clock, and bounded watch against the durable portable/store seam. Then generalize the existing sandbox provider-command journal with a restart ordinal and earn the real Container/Krun capabilities. Keep the service route, SDK method, machine transport, scheduler deletion, caller cutover, and full live convergence in R3/R4. |
| Current acceptance checkpoint | R2 portable command and store state is durable at `14e6236d4e3d1199a7ae40674bcdedd50b98fd58`. Workloads owns exact initial claims, inspection state, authenticated-absence retry at the next dispatch epoch, correlated success receipts, terminal failure, and a clock-free global candidate predicate/page. The Engine adapter owns one derived `restartWatchCandidate` field and ordered index, rejects a crossed mirror, and returns bounded global pages. Workloads passes `165/165`; server workload-store behavior passes `52` with `5` declared child-only ignores; the four affected crates pass all-target checks and strict Clippy; warning-denied rustdoc, Rustfmt, Prettier, and diff checks pass. NNCV034 mutations pass `68/68`; the live contract exits `1` on exactly reducer, command, ambiguity, readiness, capabilities, service, watch, machine, scheduler, and behavior, with no path violation. No provider effect, route, SDK method, machine transport, scheduler deletion, or caller cutover entered this checkpoint. |
...
| Last checkpoint commit | `14e6236d4e3d1199a7ae40674bcdedd50b98fd58` is the durable R2 portable command/store product checkpoint. Its tree is `1b36f0de809d52b3b07ed42bce0686af24e7b50b`; its patch from the R2 fail-before checkpoint is SHA-256 `2287a498552e12b4b15d828e95b29bdc85bbcc6b08e9b800d5a5be7499d22b08` across `33` paths. |
| Audit dirty state | The owner worktree was clean at the R2 portable command/store checkpoint `14e6236d4e3d1199a7ae40674bcdedd50b98fd58` before this ledger update. The original checkout remains untouched with its user-owned `AGENTS.md`, concurrent-write benchmark, and browser bundle changes. |
| Latest dirty-state checkpoint | Only this plan, its routing index, and the NNC6.4a proof contain the post-product recovery update. R2 compute, provider, route, SDK, machine, and scheduler paths remain unchanged. |
...
| Latest verification checkpoint | R2 command-state tests pass `23/23`; portable candidate-page tests pass `4/4`; durable Engine candidate-query tests pass `4/4`; full workloads passes `165/165`; server workload-store tests pass `52` with `5` declared child-only ignores. Workloads, compute, server, and CLI pass all-target checks and strict Clippy; workloads/server pass warning-denied rustdoc; Rustfmt, Prettier, JavaScript syntax, and diff checks pass. NNCV034 passes `68/68` sole-diagnostic mutations. Its live contract exits `1` on exactly the ten intended R2-R4 diagnostics and has no path violation. Known vendored Brotli warnings are unchanged. R1 aggregate and docs/site evidence remains valid; no final R4 gate or item-completion claim is made. |
| Blocking decision | No blocker. The portable command/store dependencies are durable; the compute reducer/driver/watch slice is dependency-ready. |
...
| NNC6.4a | `in_progress` | **Dependency:** NNC6.4 complete at `6f4f909a06a20de1003d5aafc2f5ffcba43cf0bd`. **Audit:** `docs/private/plans/proof/nimbus-network-control-plane/nnc6.4a-fenced-restart-substitution-audit.md`. A1-A20 freeze one nested same-generation restart state, a separate execution-attempt fence, compute-owned schedule/count/watch and command authority, Container/Krun substitutions, service/SDK ingress, race/crash/recovery behavior, deletion gates, later-owner boundaries, and the candidate path allowlist. **R0:** durable at `6d8961bd6d4da819b2524128cb398e22e0a9382f`. **R1:** durable at `d117ba369eaf5acc5ede9ec3edad32a11ddfbeb2`; strict workloads-v4 policy/state/history/attempt/deadline/count content and pure recovery decisions are complete; behavior passes `17/17` and exact store tests pass `9/9`; the R1 NNCV034 baseline passes `39/39` and aggregate mutations pass `366/366`. **R2 fail-before:** `68/68` sole-diagnostic mutations pass; R1 and R2 path ranges are separate; the live contract has exactly the ten intended R2-R4 diagnostics. The seam audit requires record-owned claim/result transitions, one bounded clock-free global candidate query, one restart-ordinal extension to the existing provider journal, and small real Container/Krun substitutions. **Current step:** implement those R2 seams. **Owned paths:** exact R2 allowlist only; no service route, SDK, machine transport, scheduler deletion, or caller cutover. **Next:** start with the workloads transitions and store page before compute commands or provider effects. **Last green checkpoint:** R1-to-R2 ledger commit `d05b668c95ddb57a9351d25483a7b0c0c23d26a0`. **Review:** none; one full review only after A1-A20 are green. **Blocker:** none. |
