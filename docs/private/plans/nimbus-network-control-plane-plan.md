...
Status: `active; NNC4.6d complete; NNC4.6g in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC4.6d complete; NNC4.6g in progress` |
| Current band | `NNC4 — evidence-based capability registry and sovereignty profile` |
| Current item | `NNC4.6g — give standalone KV the typed local-node authority without coupling its lifecycle to start` |
| Last completed item | `NNC4.6d — compose CLI start/dev/Compose and server listeners under one local-node manager` |
| Next action | Commit the exact staged NNC4.6d payload, truth up its commit identity, then start NNC4.6g with the required read-only KV constructor/lifecycle audit and fail-before packet. |
| Current acceptance checkpoint | NNC4.6d D1-D14 pass. Full review thread `019fadf3-f7a8-7171-9fa8-949319748feb` accepted four defects; narrow correction thread `019fae14-1935-7a32-ac53-5e9b745469ab` confirmed three fixes and found one incomplete-UNC edge in the fourth. All five findings are corrected with exact fail-before evidence. Final network/operator 243/0, CLI 895/1, sandbox 722/24, server 601/28 under two exact inherited exclusions, affected check/strict Clippy/rustdoc, format/diff, dependency/census gates, verifier 16/16 plus self-test 51/51, docs 108, and site 17/17 pass. NNC4.6g has not begun executable work. |
...
| Last checkpoint commit | `d90199e94dfb722b1e80aa4e937cbbbf701d0364` — NNC4.6c one OCI process authority; tree `1ce12ff27fce`. |
| Audit dirty state | NNC4.6c is durably committed. NNC4.6d owns its canonical plan/routing/proof truth-up plus the operator root policy, CLI start/dev/standalone-Compose composition, sandbox OCI authentication visibility, and server retained-authority/composition implementation and tests. New concept-owned modules keep server listener tests and local composition out of their production roots. Standalone KV and machine-realm paths remain untouched for NNC4.6g/e. |
| Latest dirty-state checkpoint | HEAD remains NNC4.6c commit `d90199e94dfb722b1e80aa4e937cbbbf701d0364` until the next command commits NNC4.6d. All 59 NNC4.6d paths are staged with no unstaged paths. Original reviewed executable `a553219f0253703ffdbe892ed2c37da3e28984f4dc960db939b633f07dbede2f`; first correction `e62fae0f382234b6ccf8c9a0a7e504ea32e39431cb64095ed3f6173f92ce18c0`; narrow-review UNC correction `c84682cfaa0204b83ef73a564a0fd8383eee0dd3c467ec023821c0c2d8509953`; final executable `bdd7536009ff2f205377690a814c3c852d02986dca18cc17032f1eae80fdf064`. The branch remains unpushed with no PR. |
| Current item review | NNC4.6d full and narrow Sol/xhigh/fast reviews are fully dispositioned. The narrow review's incomplete-UNC finding failed 0/1 and passes after correction; the owner cadence permits no third review. NNC4.6g is a new independent value/review/commit unit and has no review. |
...
| Latest verification checkpoint | Final NNC4.6d aggregates: network/operator 243/0, sandbox 722/24, CLI 895/1, server 601/28 under two exact inherited exclusions. Five accepted-finding regressions and the source-order gate pass. Affected all-target check, strict Clippy, warning-denied rustdoc, format/diff, dependency/census gates, live verifier 16/16, self-test 51/51, docs 108, and site 17/17 pass. A Windows-target check was attempted but cannot build native C dependencies without the Visual Studio/Windows SDK; Windows execution is not claimed. |
| Blocking decision | No blocker. Commit NNC4.6d, then continue NNC4.6g. |
...
| NNC4 — capabilities/sovereignty | `in_progress` | Named negative matrix, evidence-based seams, machine modes, offline local profile. | NNC4.1-NNC4.6d are complete. NNC4.6g now owns standalone KV parity; NNC4.6e owns host/guest machine authority realms; NNC4.6f owns constructor/root/primitive-handle census closure. NNC4.7 remains the sovereignty tripwire owner. |
...
| NNC4.6d | `done` | Final evidence: `docs/private/plans/proof/nimbus-network-control-plane/nnc4.6d-local-node-production-composition.md`. D1-D14 pass: one typed project-independent node root feeds start/dev/standalone Compose; one staged manager and pointer-shared OCI process retain listener/backend authority; exact healthy attachment+ingress freezes positive capability while partial/forwarded/unavailable shapes freeze empty; alias retarget, divergent roots, pre-effect port conflicts, and lifecycle cleanup fail closed. Network/operator 243/0, sandbox 722/24, CLI 895/1, server 601/28 under two independently reproduced execution-base exclusions, affected check/strict Clippy/rustdoc, exact dependency/census, verifier 16/16 plus self-test 51/51, format/diff, docs 108, and site 17/17 pass. Full review thread `019fadf3-f7a8-7171-9fa8-949319748feb` and narrow correction thread `019fae14-1935-7a32-ac53-5e9b745469ab` produced five accepted findings, all reproduced and corrected. Final executable SHA-256 `bdd7536009ff2f205377690a814c3c852d02986dca18cc17032f1eae80fdf064`; exact commit identity is recorded by the following recovery truth-up. |
| NNC4.6g | `in_progress` | **Dependency:** NNC4.6d final executable is frozen and its exact item commit is the next command. **Owned paths:** standalone `nimbus-kv` plus narrow CLI KV composition, canonical plan/routing/proof, and exact census updates. Start/dev/Compose/server behavior, machine realms, cluster, provider effects, policy, naming, proxy, and NNC4.6e-f are forbidden. **Last green:** NNC4.6d final executable SHA-256 `bdd7536009ff2f205377690a814c3c852d02986dca18cc17032f1eae80fdf064`; exact commit identity follows in the next recovery truth-up. **Current:** no NNC4.6g executable edits; no review. **Next:** after the NNC4.6d commit/truth-up, read the standalone KV constructor, listener lifecycle, root resolution, observability, and server-conflict call graph; freeze acceptance and fail-before before implementation. **Acceptance:** typed root parity, retained manager-derived authority, alias/divergent-root fencing, durable server/KV pre-bind conflict, honestly empty KV-only registry, and post-bind listening evidence all pass with affected/static/docs gates, one candidate-frozen Sol/xhigh/fast item review, and one exact item commit. **Blocker:** none. |
| NNC4.6e | `todo` | Depends on NNC4.6d and NNC4.6g; separate parent-host and guest-node manager/provenance/publication lifetimes and close NNCF24-NNCF25. |
| NNC4.6f | `todo` | Depends on NNC4.6b-e plus NNC4.6g; close the constructor/root/primitive-handle census and named verifier condition. |
| NNC4.7 | `todo` | — |
| NNC5.1 | `todo` | — |
| NNC5.2 | `todo` | — |
| NNC5.2a | `todo` | — |
