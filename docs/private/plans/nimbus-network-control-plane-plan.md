...
Status: `active; NNC6.5f1 complete; NNC6.5f2 in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC6.5f1 complete; NNC6.5f2 in progress` |
| Current band | `NNC6 — compute orchestration and exact lifecycle order` |
| Current item | `NNC6.5f2 — Engine-backed Compose retirement and coarse guest-stop deletion` |
| Last completed item | `NNC6.5f1 — canonical forwarded composition and foreground retirement facade`; the commit containing this ledger transition is its durable item checkpoint. |
| Next action | Commit the exact NNC6.5f1 closeout. From that clean checkpoint, fetch and reconcile current `origin/main` before any NNC6.5f2 source edit. Then rerun the affected acceptance baseline and load the frozen f2 contract. |
| Current acceptance checkpoint | NNC6.5f1 K1-K20 pass. The full review's one accepted P2 error-classification regression is corrected: operational forwarded activation errors remain their exact `nimbus::Error` variant, while composition validation maps to `InvalidInput`. Its focused test passes `1/1`; full CLI is `996 + 3 ignored`; strict CLI Clippy passes. The one narrow Sol/xhigh/fast review is clean at confidence `0.99`. Server composition is `13/13`; full server serial is `722 + 33 ignored`; full sandbox is `1,155 + 32 ignored` plus `11` integration/binary passes and `16` environment-gated ignores; node is `121/121`. The byte-matched Rust 1.96.1 Linux K15 test passes `1/1`, with `1004` filtered. NNCV035 is `0/6`, its helper is `138/138`, aggregate mutations pass `552/552`, and the live aggregate is exact `35/36` with only NNCV035 red. Format, diff, strict proof lint, docs `108`, site `17/17`, and strict affected Clippy pass. NNC6.5f2 owns no source edit yet. |
...
| Last checkpoint commit | The commit containing this ledger transition is the durable NNC6.5f1 checkpoint. NNC6.5f is complete at `53e0b36ce94d6676875d51eb720a37373050311c`. |
| Audit dirty state | NNC6.5f1 owns its exact 18-path item commit: fourteen product/test paths, one source-derived census, proof, plan, and routing index. NNC6.5f2 owns no dirty path. No manifest, dependency, `nimbus-network`, Compose-down behavior, coarse-stop, physical-stop, compensation, tenant-retirement, or provider ownership changed in f1. The original checkout remains untouched. |
| Latest dirty-state checkpoint | `proof/nimbus-network-control-plane/nnc6.5f1-canonical-forwarded-composition.md` records green K1-K20 evidence, both review dispositions, exact owned paths, and the clean handoff to NNC6.5f2. |
| Last completed item review | NNC6.5f1 used one full and one narrow GPT-5.6 Sol/xhigh/fast review. The full review accepted one P2 error-classification regression. The correction passes focused `1/1`, full CLI `996 + 3 ignored`, and strict CLI Clippy. The narrow review is clean at confidence `0.99`; review cadence is exhausted. |
...
| Last verification | NNC6.5f1 server composition `13/13`; full CLI `996 + 3 ignored`; full server serial `722 + 33 ignored`; full sandbox `1,155 + 32 ignored` plus `11` integration/binary passes and `16` environment-gated ignores; node `121/121`; accepted P2 regression `1/1`; canonical source `1/1`; real forwarded execute/inspect `2/2`; missing authority `1/1`; delayed activation `1/1`; Linux K15 `1/1` with `1004` filtered; live NNCV035 `0/6`; helper `138/138`; aggregate mutations `552/552`; exact live aggregate `35/36` with only NNCV035 red; format; diff; strict proof lint; docs `108`; site `17/17`; and strict CLI/server/sandbox/node Clippy green. |
| Latest verification checkpoint | Completed NNC6.5f1 record: `proof/nimbus-network-control-plane/nnc6.5f1-canonical-forwarded-composition.md`. It records green K1-K20 evidence, exact owned paths, review disposition, and recovery state. |
| Blocking decision | No blocker. Commit NNC6.5f1, reconcile current main from the clean checkpoint, and resume NNC6.5f2. |
...
| NNC6.5f2 | `in_progress` | **Dependency:** NNC6.5f1 is complete in the commit containing the preceding row. **Owned paths:** none; do not edit source before main reconciliation and frozen-contract reload. **Last green:** NNC6.5f1 K1-K20 and its exact quality/review gates. **Next:** verify a clean checkpoint, fetch `origin/main`, reconcile the 73-commit snapshot divergence or its current value without dropping owner commits, rerun affected f1 acceptance, and load the frozen f2 contract. Then open the canonical Engine/store for local and forwarded Compose down, retire through compute, report durable identity/outcome, and atomically delete the coarse guest stop envelope. **Forbidden:** physical-machine stop/barrier, compensation, tenant retirement, provider ownership changes, and `nimbus-network` effects. **Blocker:** none. |
| NNC6.5f3 | `todo` | **Dependency:** NNC6.5f. Add the compute-owned desired-authority/provider-witness decision; provider-owned admission guard, durable barrier, and authentication; guarded initial/restart Engine CAS; barrier-first Engine scan; three provider-admission checks; and one shared fail-closed physical-stop gate across five real callers. Do not add provider-owned stop policy or a machine desired-state registry. |
