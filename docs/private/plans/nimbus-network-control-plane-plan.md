...
Status: `active; NNC6.5f2 complete; NNC6.5f3 in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC6.5f2 complete; NNC6.5f3 in progress` |
| Current band | `NNC6 — compute orchestration and exact lifecycle order` |
| Current item | `NNC6.5f3 — physical-machine admission barrier and fail-closed stop gate` |
| Last completed item | `NNC6.5f2 — Engine-backed Compose retirement and coarse guest-stop deletion`; the commit containing this ledger transition is its durable item checkpoint. |
| Next action | Commit the exact NNC6.5f2 candidate, then load NNC6.5f3 acceptance and owned paths without product edits. |
| Current acceptance checkpoint | NNC6.5f2 F2-01–F2-20 are green. Compose down uses the canonical Engine/store/runtime and `ComputeResourceRetirer`; the direct Compose stop path and coarse guest envelope are deleted. Frozen behavior passes `9/9`; full affected suites, strict Clippy, format/diff, live architecture `35/36` with only NNCV035 red, the pre-review aggregate passes `552/552`, the additive correction helper passes `139/139`, docs `108`, and site `17/17` pass. The one full and one narrow Sol/xhigh/fast review are fully dispositioned; cadence is exhausted. |
...
| Last checkpoint commit | Current-main reconciliation commit `0327e8d196e3a8bec885b164a88ec3ec9f8de6b5`; the staged NNC6.5f2 transition becomes the next durable checkpoint when committed. |
| Audit dirty state | The staged 50-path NNC6.5f2 closeout owns the workloads/compute carrier, Compose cutover, coarse machine-envelope deletion, tests, directly required Engine schema-idempotency correction, exact verifier census/contract reconciliation, and plan/proof/routing state. NNC6.5f3 product work has not started. The original checkout remains untouched. |
| Latest dirty-state checkpoint | `proof/nimbus-network-control-plane/nnc6.5f2-engine-compose-retirement.md` records the rebased baseline, frozen ownership, F2-01–F2-20 acceptance, and next action. |
| Current item review | NNC6.5f2 used one full and one narrow GPT-5.6 Sol/xhigh/fast review. The full review accepted a P2 typed-error regression and a P3 activation-owner contract gap. The narrow review accepted two P2 public-error/dataflow proof gaps. All are corrected and proven; review cadence is exhausted. |
...
| Last verification | F2-01–F2-20 are green. Frozen Compose retirement passes `9/9`; full CLI passes `995/995` with three declared skips. A deterministic Engine regression fails before at `16` writes for `16` concurrent equal schema declarations and passes after at one durable write. Final correction gates pass Compose `9/9`, NNCV035 `139/139`, direct expected-red `0/5`, strict CLI Clippy, format, syntax, and diff checks. |
| Latest verification checkpoint | Completed NNC6.5f2 record: `proof/nimbus-network-control-plane/nnc6.5f2-engine-compose-retirement.md`. |
| Blocking decision | No blocker. Commit NNC6.5f2, then begin the dependency-ready NNC6.5f3 read-only load. |
...
| NNC6 — compute choreography | `in_progress` | Exact provision/teardown observers across every workload/retirement path. | NNC6.1-NNC6.5f1 and NNC6.1e1 are complete. NNC6.5f2 is active and must reconcile current main before Compose/coarse guest cutover. NNC6.5f3 retains physical-machine fencing. NNC6.5g remains the later compensation, tenant-retirement, remaining deletion, and convergence unit. No completed item retains a temporary legacy authority. NNC6.6 retains resolver fencing; NNC6.1e2 is the final convergence gate. Provider effects remain with their named owners. |
...
| NNC6.5f3 | `in_progress` | **Dependency:** NNC6.5f2 is complete in the commit containing its row. Add the compute-owned desired-authority/provider-witness decision; provider-owned admission guard, durable barrier, and authentication; guarded initial/restart Engine CAS; barrier-first Engine scan; three provider-admission checks; and one shared fail-closed physical-stop gate across five real callers. Do not add provider-owned stop policy or a machine desired-state registry. **Last green:** predecessor NNC6.5f2 F2-01–F2-20 and all closeout gates pass; NNC6.5f3 product work has not started. **Next:** load the frozen acceptance and owned paths read-only after the NNC6.5f2 commit. **Blocker:** none. |
| NNC6.5g | `todo` | **Dependency:** NNC6.5e, NNC6.5f2, and NNC6.5f3. Failed-provision, tenant-retirement, remaining legacy-deletion, and final NNCV035 convergence gate. |
