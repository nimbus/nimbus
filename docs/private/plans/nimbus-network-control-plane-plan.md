...
Status: `active; NNC3.6 complete; NNC3.7 machine SSH/forwarding migration in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC3.6 complete; NNC3.7 machine SSH/forwarding migration in progress` |
| Current band | `NNC3 — cross-process host-global PortLease` |
| Current item | `NNC3.7 — migrate machine SSH/forwarding listeners` |
| Last completed item | `NNC3.6 — standalone nimbus-kv listener migration` |
| Next action | Begin NNC3.7 with a read-only machine SSH/forwarding bind and allocation call graph, reconcile the NNC0.2 external-binder-after-probe/drop baseline with the current machine provider paths, and identify the exact allocator/provider/server conflict seams. Install a deterministic fail-before proof before changing behavior. Do not start CLI dev/start-adapter work (NNC3.7a), census closure (NNC3.7b), producer-wide restart convergence (NNC3.8), old-authority deletion (NNC3.9), or structured sibling-task unwind (NNC7.1a). |
...
| Last checkpoint commit | `856c5834a6d59151aea525688501815cdc80d8de` — reviewed NNC3.5 completion and NNC3.6 activation. The reviewed NNC3.6 completion commit is being formed; after it lands, `HEAD` is the recovery proof. |
| Audit dirty state | The exact NNC3.6 completion candidate is staged: `Cargo.lock`; `crates/nimbus-cli/src/kv.rs`; `crates/nimbus-kv/{Cargo.toml,src/lib.rs,src/listener.rs,src/server.rs,tests/network_listener.rs,tests/resp_server.rs}`; `docs/private/plans/{README.md,nimbus-network-control-plane-plan.md}`; `docs/private/plans/proof/nimbus-network-control-plane/{nnc0.1-bind-owner-inventory.json,nnc3.6-kv-listener-port-migration.md}`. No machine, CLI dev/start-adapter, NNC3.8 reconciliation, NNC3.9 deletion, or NNC7.1a task-group path is admitted. |
...
| Last verification | NNC3.6 written acceptance passes 3/3. The focused listener suite passes 8/8 with one child-only ignore; full `nimbus-kv` passes 22/22 with two skips; full `nimbus-cli` passes 863/863 with two skips. Affected check, strict Clippy, warning-denied rustdoc, format/diff, verifier 16/16 self-tests plus expected live 14/1 solely later-owned NNCV005, docs 108 pages, and site 17/17 pass. One actual Sol/xhigh/fast review found two P2 closeout gaps: this proof/ledger correction and an ambiguous-drop behavioral test; both are resolved without production-byte changes, so no repeat review is required. Exact commands and dispositions are in `docs/private/plans/proof/nimbus-network-control-plane/nnc3.6-kv-listener-port-migration.md`. |
| Blocking decision | No blocker. NNC3.6 is complete and NNC3.7 is active. Machine SSH/forwarding is the only newly executable implementation scope; CLI dev/start decisions, census closure, fresh-process cleanup-pending convergence, and old-authority deletion remain owned by their later NNC3 items. |
...
| NNC3.7 | `in_progress` | Owned paths: none yet. Last green: NNC3.6 written acceptance 3/3, focused listener 8/8, KV 22/22, CLI 863/863, and all affected quality/static/docs gates pass as recorded in its final proof. Next: map the machine SSH/forwarding bind/allocation call graph and NNC0.2 probe/drop baseline, identify the exact fail-before seam, and freeze the deterministic machine/server conflict proof before mutation. Blocker: none. |
| NNC3.7a | `todo` | — |
| NNC3.7b | `todo` | — |
| NNC3.8 | `todo` | — |
| NNC3.9 | `todo` | — |
