...
| Next action | Finish the NNC6.4a R1 evidence and exact checkpoint commit, then start R2 confirmed commands and provider adapters. Do not add a service route, SDK method, or caller cutover before R3. |
| Current acceptance checkpoint | R1 is candidate-complete at `proof/nimbus-network-control-plane/nnc6.4a-fenced-restart-substitution-audit.md`. Workloads format v4 now owns the strict portable restart policy, nested state, complete admission history, same-generation attempt fences, deadline/count durability, and pure recovery decisions. Pure behavior passes `17/17`; exact Engine-store behavior passes `9/9`; full workloads passes `155/155`; server workload-store behavior passes `48/48` with `5` declared child-only ignores. Nine NNCV034 groups are green and ten planned R2-R4 groups remain expected red. NNCV034 mutations pass `39/39`; aggregate mutations pass `366/366`; the live aggregate keeps the required intermediate `34` passed and `1` expected NNCV034 failure. R1 adds no provider effect, route, SDK method, or caller cutover. |
...
| Last checkpoint commit | `6d8961bd6d4da819b2524128cb398e22e0a9382f` is the durable NNC6.4a R0 fail-before checkpoint. |
| Audit dirty state | The owner worktree contains only the candidate-complete R1 owned diff after R0 checkpoint `6d8961bd6d4da819b2524128cb398e22e0a9382f`. The original checkout remains untouched with its user-owned `AGENTS.md`, concurrent-write benchmark, and browser bundle changes. |
| Latest dirty-state checkpoint | R1 portable model, strict store, tests, static contract, proof, plan, and routing index are dirty and ready for final gates and the exact R1 commit. No R2 provider effect is present. |
...
| Latest verification checkpoint | R1 pure restart tests pass `17/17`; exact server-store tests pass `9/9`; full workloads passes `155/155`; server workload-store tests pass `48/48` with `5` declared child-only ignores; affected all-target check, strict Clippy, warning-denied rustdoc, Rustfmt, Prettier, diff, Bash syntax, and scoped ShellCheck pass. NNC6.2a passes `24` direct checks and `10/10` mutations. NNCV034 passes `39/39` mutations with exactly ten later-band groups still red. The live aggregate has the required intermediate shape at `34` passed and `1` expected NNCV034 failure; aggregate mutations pass `366/366` with zero failure markers. Proof lint has zero diagnostics; docs pass `108`; the site gate passes `17/17`. |
| Blocking decision | No blocker. R1 is candidate-complete and ready for its exact checkpoint after final gates. R2 is dependency-ready after that commit. |
...
| NNC6.4a | `in_progress` | **Dependency:** NNC6.4 complete at `6f4f909a06a20de1003d5aafc2f5ffcba43cf0bd`. **Audit:** `docs/private/plans/proof/nimbus-network-control-plane/nnc6.4a-fenced-restart-substitution-audit.md`. A1-A20 freeze one nested same-generation restart state, a separate execution-attempt fence, compute-owned schedule/count/watch and command authority, Container/Krun substitutions, service/SDK ingress, race/crash/recovery behavior, deletion gates, later-owner boundaries, and the candidate path allowlist. **R0:** durable at `6d8961bd6d4da819b2524128cb398e22e0a9382f`. **R1 candidate:** strict workloads-v4 policy/state/history/attempt/deadline/count content and pure recovery decisions are complete; behavior passes `17/17` and exact store tests pass `9/9`; NNCV034 mutations pass `39/39`; aggregate mutations pass `366/366`; nine groups are green and ten R2-R4 groups remain expected red. **Current step:** finish R1 closeout gates and exact commit. **Owned paths:** R1 allowlist only; no provider effect, route, SDK, or caller cutover. **Next:** R2 confirmed commands and provider adapters. **Last green product:** NNC6.4 item commit `6f4f909a06a20de1003d5aafc2f5ffcba43cf0bd`. **Review:** none; one full review only after A1-A20 are green. **Blocker:** none. |
