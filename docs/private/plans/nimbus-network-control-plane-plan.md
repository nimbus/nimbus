...
Status: `active; NNC6.5f3 complete; NNC6.5g in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC6.5f3 complete; NNC6.5g in progress` |
| Current band | `NNC6 — compute orchestration and exact lifecycle order` |
| Current item | `NNC6.5g — failed-provision compensation, tenant retirement, legacy deletion, and NNCV035 convergence` |
| Last completed item | `NNC6.5f3 — physical-machine admission barrier and fail-closed stop gate`; the commit containing this ledger transition is its durable item checkpoint. |
| Next action | Read the frozen NNC6.5 audit and current four NNCV035 diagnostics, then freeze the exact NNC6.5g authority census and acceptance ledger before product edits. |
| Current acceptance checkpoint | NNC6.5f3 F3-01–F3-24 are green. One compute policy owner, one Engine adapter, and one provider admission/barrier authority now fence all physical-machine stop and workload-admission races. Full CLI passes `1007` tests with four ignores. NNCV035 physical is `1/1`; helper mutations are `150/150`; aggregate mutations are `552/552`; live architecture is the exact `35/36`, with only the four NNC6.5g-owned service, tenant, compensation, and behavior diagnostics. Review cadence is exhausted. |
...
| Last checkpoint commit | NNC6.5f2 item commit `40fac9aa9dd0ea36e7b89bf209d8935e7db4b269`; the NNC6.5f3 item commit is the pending staged checkpoint. |
| Audit dirty state | The NNC6.5f3 item candidate owns all current staged paths. After its commit, NNC6.5g starts from a clean checkpoint with no product edit. Attribute any unexpected dirty path before G1. The original checkout remains untouched. |
| Latest dirty-state checkpoint | `proof/nimbus-network-control-plane/nnc6.5g-final-teardown-convergence.md` records the NNC6.5g recovery state and next read-only action. |
| Last completed item review | NNC6.5f3 used one full and one narrow GPT-5.6 Sol/xhigh/fast review. The owner accepted and corrected five full-review P2 findings and three narrow-review P2 findings. Exact restart succession and four new verifier mutations prove the final corrections. Review cadence is exhausted. |
...
| Last verification | NNC6.5f3 full CLI `1007 passed / 4 ignored`; strict CLI Clippy and Rustdoc; NNCV035 physical `1/1`; helper mutations `150/150`; aggregate mutations `552/552`; live verifier `35/36`; strict proof lint; docs `108`; site `17/17`; format, syntax, ShellCheck, Prettier, and diff checks pass. |
| Latest verification checkpoint | Completed NNC6.5f3 record: `proof/nimbus-network-control-plane/nnc6.5f3-physical-machine-admission-barrier.md`. Active NNC6.5g recovery: `proof/nimbus-network-control-plane/nnc6.5g-final-teardown-convergence.md`. |
| Blocking decision | No blocker. Commit NNC6.5f3, then begin the read-only NNC6.5g census. |
...
| NNC6 — compute choreography | `in_progress` | Exact provision/teardown observers across every workload/retirement path. | NNC6.1-NNC6.5f3 and NNC6.1e1 are complete. NNC6.5g owns failed-provision compensation, tenant retirement, remaining deletion, and NNCV035 convergence. No completed item retains a temporary legacy authority. NNC6.6 retains resolver fencing; NNC6.1e2 is the final convergence gate. Provider effects remain with their named owners. |
...
| NNC6.5g | `in_progress` | **Dependency:** NNC6.5e, NNC6.5f2, and NNC6.5f3 are complete. **Evidence:** `proof/nimbus-network-control-plane/nnc6.5g-final-teardown-convergence.md`. **Current state:** no product work has started. The four remaining NNCV035 diagnostics are service retirement, tenant retirement, failed-provision/restart compensation, and attributed end-to-end behavior. **Owned paths:** none until the read-only census freezes the exact product, test, verifier, proof, plan, and routing paths. **Last green:** NNC6.5f3 CLI `1007 + 4 ignored`, helper `150/150`, aggregate `552/552`, and live verifier `35/36`. **Next:** freeze the exact source authority census, deletion list, race matrix, and acceptance ledger before product edits. **Blocker:** none. |
