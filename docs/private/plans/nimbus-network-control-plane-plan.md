...
Status: `active; NNC3.5 complete; NNC3.6 standalone nimbus-kv migration in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC3.5 complete; NNC3.6 standalone nimbus-kv migration in progress` |
| Current band | `NNC3 — cross-process host-global PortLease` |
| Current item | `NNC3.6 — migrate standalone nimbus-kv` |
| Last completed item | `NNC3.5 — server main and sibling wire listener migration` |
| Next action | Begin NNC3.6 with a read-only standalone `nimbus-kv` listener call graph and source-census reconciliation, then install its fail-before separate-process contention, fixed-conflict owner-identity, and pre-bound adoption tests before mutating KV listener construction. Do not reopen NNC3.4/NNC3.5 or pull machine SSH/forwarding, CLI dev/start-adapter decisions, producer-wide restart convergence, old-authority deletion, or structured sibling-task unwind across their NNC3.7-NNC3.9/NNC7.1a dependency gates. |
...
| Last checkpoint commit | `5ddee8a1def0062e6bfa84e77b0ef54224eba264` — reviewed NNC3.4 completion and NNC3.5 activation. NNC3.5 completion is the current item commit being formed with this self-referential ledger checkpoint; after commit, `HEAD` is the durable recovery proof. |
| Audit dirty state | The exact NNC3.5 closeout set is staged for its one item commit: `Cargo.lock`; the CLI and server manifests; `crates/nimbus-cli/src/start/boot.rs`; `crates/nimbus-server/src/{construction.rs,lib.rs,listener_lease.rs}`; the routing index; this plan; and `docs/private/plans/proof/nimbus-network-control-plane/nnc3.5-server-listener-port-migration.md`. NNC3.6 has no implementation-owned path yet. No machine, CLI dev/start-adapter, NNC3.8 reconciliation, NNC3.9 deletion, or NNC7.1a task-group path is admitted. |
...
| Last verification | NNC3.5 fail-before proved the main listener lacked Active authority and a sibling bind reached its guard without a claim; both exact commands exited `101`. Written acceptance passes 3/3. Focused ownership cases pass 17/17: 4/4 server sequencing, 6/6 listener-adapter lifecycle, 6/6 CLI main-bind behavior, and 1/1 cleanup-error aggregation. Final disjoint affected suites pass 1,446/1,446: CLI 862/862 with 2 expected skips and server 584/584 with 28 expected skips after excluding exactly two unrelated NNC0.7 baseline failures that reproduce independently. All-target check, strict Clippy, warning-denied rustdoc, format, and diff checks pass. The first actual Sol/xhigh/fast review's six findings were fixed. The one bounded actual Sol/xhigh/fast repeat review reported three findings: two direct defects were fixed and proven; fresh-process external re-adoption is explicitly routed to NNC3.8. Verifier self-tests pass 16/16 and live passes 14/15, failing only later-owned NNCV005 while NNCV006 passes. Docs pass at 108 pages and 17/17 site conditions. Exact evidence: `docs/private/plans/proof/nimbus-network-control-plane/nnc3.5-server-listener-port-migration.md`. |
| Blocking decision | No blocker. NNC3.5 is complete and NNC3.6 is active. Standalone KV is the only newly executable migration scope; machine SSH/forwarding, CLI dev/start decisions, fresh-process cleanup-pending convergence, and old-authority deletion remain owned by their later NNC3 items. |
...
| NNC3.6 | `in_progress` | Owned paths: none; read-only standalone KV call-graph and census preparation begins after the NNC3.5 commit. Last green: NNC3.5 completion gates pass as recorded in its proof, and NNC3.6 dependencies NNC3.0-NNC3.5 are done. Next: identify the standalone `nimbus-kv` bind/composition owner, reconcile its NNC0.1 census row, and install fail-before separate-process contention, fixed-conflict owner-identity, and pre-bound adoption tests before implementation. Blocker: none. |
| NNC3.7 | `todo` | — |
| NNC3.7a | `todo` | — |
| NNC3.7b | `todo` | — |
| NNC3.8 | `todo` | — |
