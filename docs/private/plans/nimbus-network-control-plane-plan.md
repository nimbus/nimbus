...
| Next action | Route local and forwarded Compose restart through the same compute submission authority. Then add the fully fenced forwarded-machine command, delete provider-local scheduler state, and close the three remaining machine, scheduler, and behavior contract groups. Keep final A1-A20 convergence and the one item-level review in R4. |
| Current acceptance checkpoint | R2 is durable through product commit `5826dff6019d453f9eba575ecf67850ac3b19e6a`. R3 verifier extraction is durable at `e1c57ce9eba823b8dc7d0a5a4ab02d58fe4c0030`. The current R3 service/SDK candidate adds one compute-owned explicit restart submitter over the existing coordinator and retained supervisor, exact source-generation fencing, stable completed-request replay, an authorized native service route, and the matching Nimbus SDK method. Duplicate service submission returns the same restart epoch and internal request ID, provider convergence occurs through the retained supervisor, and the coarse stop path remains unused. Workloads pass `167/167`; compute passes `292` with one declared child-only ignore; the process-global network-authority Server lane passes `596` with `31` declared ignores under `--test-threads=1`; the new restart fixtures explicitly await shutdown. The Nimbus package build, test, and typecheck pass with `24`-route parity. NNCV034 remains `71/71`, and the live contract now exits `1` only on machine, scheduler, and behavior. Compose, forwarded-machine transport, scheduler deletion, and final process behavior remain open. |
...
| Last checkpoint commit | `e1c57ce9eba823b8dc7d0a5a4ab02d58fe4c0030` is the durable R3 verifier-fixture extraction. Its tree is `525ab0c5cc43c29dd2986ed9fd320e53cbb50d75`; its patch from the R2 recovery commit is SHA-256 `6d97796a59f4b3bc0ffe29797c47d93feab70d3654f1a774952f27cffa805753` across `2` script paths. R2 provider orchestration remains durable at `5826dff6019d453f9eba575ecf67850ac3b19e6a`; earlier R2 checkpoints remain `14e6236d4e3d1199a7ae40674bcdedd50b98fd58`, `8935e0c77dd188f50566b72c917b2005a213ecdd`, and `e1e9c95167972c2566a468d01aa0b91e559dd9be`. |
...
| Latest dirty-state checkpoint | The owned R3 service/SDK candidate spans workloads completed-replay state, compute explicit submission/runtime/service facade, server route and managed-workload proofs, Nimbus SDK route/types/client/contracts, and the exact NNCV034 service contract. The canonical plan, routing index, and proof are the only documentation paths being updated. Compose restart, machine restart transport, and scheduler-deletion paths remain unchanged. |
...
| Latest verification checkpoint | The R3 service/SDK candidate passes focused workloads replay `2/2`, compute submission `3/3`, server service restart `2/2`, full workloads `167/167`, full compute `292 + 1` declared ignore, serial process-global-authority Server `596 + 31` declared ignores, strict affected Clippy, Rustfmt, Prettier, diff, docs `108`, site `17/17`, Nimbus package build/test/typecheck with `24`-route parity, and NNCV034 `71/71`. A default-parallel Server diagnostic run produced `39` root-collision failures because independent tests cannot compose the deliberate process singleton concurrently; the required serial rerun is green, and the new restart fixtures now await exact server shutdown. The live contract exits `1` only on machine, scheduler, and behavior. No final aggregate/R4 or item-completion claim is made, and no structured review has run. |
| Blocking decision | No blocker. R3 service/SDK, Compose, machine, and scheduler cutover work is dependency-ready. |
...
| NNC6.4a | `in_progress` | **Dependency:** NNC6.4 complete at `6f4f909a06a20de1003d5aafc2f5ffcba43cf0bd`. **Audit:** `docs/private/plans/proof/nimbus-network-control-plane/nnc6.4a-fenced-restart-substitution-audit.md`. A1-A20 freeze one nested same-generation restart state, a separate execution-attempt fence, compute-owned schedule/count/watch and command authority, Container/Krun substitutions, service/SDK ingress, race/crash/recovery behavior, deletion gates, later-owner boundaries, and the candidate path allowlist. **R0:** durable at `6d8961bd6d4da819b2524128cb398e22e0a9382f`. **R1:** durable at `d117ba369eaf5acc5ede9ec3edad32a11ddfbeb2`. **R2 portable/store, admission, command:** durable at `14e6236d4e3d1199a7ae40674bcdedd50b98fd58`, `8935e0c77dd188f50566b72c917b2005a213ecdd`, and `e1e9c95167972c2566a468d01aa0b91e559dd9be`. **R2 provider orchestration:** durable at `5826dff6019d453f9eba575ecf67850ac3b19e6a`. **R3 verifier extraction:** durable at `e1c57ce9eba823b8dc7d0a5a4ab02d58fe4c0030`. **R3 service/SDK candidate:** one compute submitter, source-generation fence, completed replay, authorized service route, retained-supervisor handoff, and matching SDK are green; focused behavior is workloads `2/2`, compute `3/3`, server `2/2`, and SDK build/test/typecheck with `24` routes. NNCV034 is `71/71`; live red is exactly machine, scheduler, and behavior. **Current step:** Compose cutover, then machine, scheduler deletion, and final behavior. **Review:** none; one full review only after A1-A20 are green. **Blocker:** none. |
