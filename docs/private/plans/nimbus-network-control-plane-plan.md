...
Status: `active; NNC6.5d1 complete; NNC6.5d2 in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC6.5d1 complete; NNC6.5d2 in progress` |
| Current band | `NNC6 — compute orchestration and exact lifecycle order` |
| Current item | `NNC6.5d2 — exact Krun execution drain/stop over the shared sandbox teardown substrate` |
| Last completed item | `NNC6.5d1 — exact Container execution drain/stop and shared sandbox teardown command substrate`; the commit containing this checkpoint is its durable item commit. |
| Next action | Load only NNC6.5d2, its frozen NNC6.5d audit contract, the Krun lifecycle/provider paths, and the shared teardown substrate. Complete the read-only substitution and complexity audit, create the item proof with exact fail-before criteria, and make no product edit before that expected-red checkpoint is durable. |
| Current acceptance checkpoint | NNC6.5d1 C1-C25 are green. Container execution drain/stop now has strict exact retry-receipt lineage, stop-only authenticated next-epoch KILL recovery, fail-closed manifest teardown state, stale-live-claim exclusion, and atomic effect/result publication under one provider-journal lock. The full review's three defects, the manual audit's stale-claim defect, and the narrow review's publication-gap defect are corrected and proven. NNC6.5d2 has no product edit yet. Krun must earn the same contract without a second journal or any detach/release behavior. |
...
| Last checkpoint commit | The commit containing this checkpoint is the durable NNC6.5d item commit. Its parent, `bbb3098dae475e16b3d0a4a456267eaba6a7d62a`, is the durable NNC6.5c checkpoint and clean NNC6.5d audit base. |
| Audit dirty state | The containing commit owns the exact NNC6.5d1 compute, sandbox, two CLI exhaustive-mapping, inventory, proof, plan, and routing paths recorded in its proof. NNC6.5d2 has no product diff at this checkpoint. The original checkout remains untouched. |
| Latest dirty-state checkpoint | The commit containing this header is the clean durable NNC6.5d1 item checkpoint. Its parent is the clean NNC6.5d audit commit `b186074e105dd3f5636df4361b3dbdb0a2596887`. |
| Current item review | NNC6.5d1 review cadence is complete: one full GPT-5.6 Sol/xhigh/fast review, one manual same-scope audit, and one narrow GPT-5.6 Sol/xhigh/fast correction review. All accepted findings are corrected and proven. No third review ran. NNC6.5d2 has not reached candidate freeze and has no structured review. |
| Full-review input identity | Complete 281,204-byte staged NNC6.5d1 bundle; thread `019fe832-401c-78c2-8171-88ea3f38e089`; GPT-5.6 Sol/xhigh/fast; three accepted findings; overall incorrect confidence `0.99`. |
| Narrow-review input identity | Complete 367,583-byte corrected NNC6.5d1 bundle; thread `019fe889-679f-7672-8967-ac7c4952a09a`; GPT-5.6 Sol/xhigh/fast; one accepted P2 atomic-publication finding; overall incorrect confidence `0.94`. |
| Final closeout identity | The containing commit is the exact NNC6.5d1 item checkpoint. Final pre-ledger Rust patch: `36` files, `7,036` additions, `83` deletions, SHA-256 `e51a878979a44b3cd5577d5f9d6a37d985225c5d5fc323da55283c0d180bd10f`. |
...
| Last verification | Provider journal `19` plus one child-only ignore; Container teardown `19` plus one child-only ignore; compute adapter `6`; conmon runtime identity `8`; manifest durability `19`; generic compute-CAS process parents `2` plus one child-only ignore. Full compute is `352` plus one ignore; serialized full sandbox is `1,049` plus 44 declared ignores; the earlier unaffected full CLI gate is `948` plus one ignore. Consumer check, strict Clippy, warning-denied rustdoc, format/diff, exact `nimbus-network -> nimbus-core`, and forbidden-effect scans pass. NNCV035 self-test is `55/55`, direct is expected `0/7`, and aggregate is `35/36` with only NNCV035 red. Docs pass `108`; site passes `17/17`; proof lint is zero. |
| Latest verification checkpoint | Canonical record: `proof/nimbus-network-control-plane/nnc6.5d1-container-execution-teardown.md`. Prior NNC6.5d artifacts remain under `/tmp/nnc65d-*`. |
| Blocking decision | No blocker. Begin the NNC6.5d2 read-only Krun substitution audit. Do not reopen NNC6.5d1 or run another structured review for it. |
...
| NNC6.5d2 | Add exact Krun execution drain/stop adapters over the shared substrate. | Krun provides the same exact drain/stop contract and real compute substitution without a second journal. Creator, activation, and restart work settle before drain; raw PID never authorizes a signal; missing/corrupt/crossed provider evidence is ambiguous or definite-failure as specified; stop leaves attachment, PEP, listener, IPAM, and segment authority fenced. Replay, ambiguity, concurrency, and fresh-process TERM/KILL cuts converge exactly once. |
...
| NNC6 — compute choreography | `in_progress` | Exact provision/teardown observers across every workload/retirement path. | NNC6.1-NNC6.5d1 and NNC6.1e1 are complete. NNC6.5d2 is the active Krun execution provider unit. NNC6.5d3-NNC6.5d4 remain later detach/release and forwarded-machine provider units, and NNC6.5e-NNC6.5g remain later caller/convergence units. No completed item retains a temporary legacy authority. NNC6.6 retains resolver fencing; NNC6.1e2 is the final convergence gate. Provider effects remain with their named owners. |
...
| NNC6.5d2 | `in_progress` | **Dependency:** NNC6.5d1. Krun execution drain/stop over the same substrate, with no second journal or network teardown. **Owned paths:** `crates/nimbus-compute/src/workload_saga/teardown_sandbox/krun.rs` and attributed tests; `crates/nimbus-sandbox/src/backends/krun/vm/teardown.rs` and concept-owned children/tests; narrow Krun manifest, inspection, lifecycle, and composition-root edits; the future item proof, this plan, and routing index. Shared conmon identity changes are allowed only if the NNC6.5d1 mechanism is insufficient and the audit proves the need. Attachment detach/release, machine transport, caller cutover, coarse-stop deletion, and a second journal are forbidden. **Last green:** clean NNC6.5d1 candidate with compute `352` plus one ignore, serialized sandbox `1,049` plus 44 ignores, consumer check, strict quality gates, NNCV035 self-test `55/55`, expected direct `0/7`, aggregate `35/36` sole NNCV035 red, docs `108`, site `17/17`, and proof lint zero. **Next:** complete the read-only substitution and complexity audit, create the acceptance-bearing proof, and capture exact expected-red evidence before a product edit. **Blocker:** none. |
| NNC6.5d3 | `todo` | **Dependency:** NNC6.5d1 and NNC6.5d2. Shared host-managed Container/Krun detach and release with compound retained evidence and no premature reuse. |
| NNC6.5d4 | `todo` | **Dependency:** NNC6.5d3. End-to-end forwarded-machine provider adapters and exact guest phase envelopes without caller cutover. |
