...
Status: `active; NNC6.5b complete; NNC6.5c in progress`
...
| Field | Current value |
| --- | --- |
| Plan status | `active; NNC6.5b complete; NNC6.5c in progress` |
| Current band | `NNC6 — compute orchestration and exact lifecycle order` |
| Current item | `NNC6.5c — final-ingress and DirectProcess/Systemd teardown adapters` |
| Last completed item | `NNC6.5b — compute confirmed-command driver and registry`; the commit containing this checkpoint is its durable item commit. |
| Next action | Begin the read-only NNC6.5c substitution audit. Freeze its exact adapter, effect, test, and deletion boundaries before fail-before or product edits. |
| Current acceptance checkpoint | NNC6.5b B1-B24 are green. The exact roster is 36/36; workloads passes 216, compute passes 340 with one child-only ignore, and the server store slice passes 59 with seven child-only ignores. Two parents pass ten real-process cuts through the retained runtime. NNCV035 is 55/55 and exact 0/8; aggregate is 35/36 with only NNCV035 red. NNCV015 is 7/7 and NNCV027 is 10/10. Format, strict Clippy, warning-denied Rustdoc, dependency, effect, line-count, proof-lint, docs 108, and site 17/17 gates pass. The full review is dispositioned, and its one authorized narrow correction review is clean at 0.98. |
...
| Last checkpoint commit | `3629714b58ac03df7a9cf8457d79f0cdaeee9dee` is the durable NNC6.5b fail-before checkpoint. The commit containing this recovery row advances the item to its completed checkpoint. |
| Audit dirty state | The NNC6.5b commit contains only its compute concepts/tests, server test-only process handoff, neighboring static verifier corrections, and evidence/routing state. No manifest, provider adapter, product caller, `ComputeState`, or `nimbus-network` source changed. The owner worktree must be clean before NNC6.5c starts. The original checkout remains untouched with its user-owned `AGENTS.md`, concurrent-write benchmark, and browser bundle changes. |
...
| Current item review | The one full Sol/xhigh/fast review ran against staged tree `7bd2a42f7d3539294667b29a385d48c8cf643f5b` and reported three findings at overall confidence 0.96. Two executable findings were accepted and corrected; the process-frozen provider-report finding was a wording correction only. The one authorized narrow Sol/xhigh/fast review of staged tree `d88cbc08974e8a028ee60b9c0bcfc6785d02ded0` reported zero findings and classified the patch as correct at 0.98. No further NNC6.5b review is warranted. |
| Full-review input identity | Staged tree `7bd2a42f7d3539294667b29a385d48c8cf643f5b`; complete patch SHA-256 `c69d09b237dd70bce0127ba91db145221080520842512d36d974ea2f56c26bfc`; executable/script patch SHA-256 `8539cb6a1aea238d5adb9109bdb498c48999b13167a333eeb82100e113079d8a`. Actual reviewer: GPT-5.6 Sol, xhigh, fast. |
| Narrow-review input identity | Staged tree `d88cbc08974e8a028ee60b9c0bcfc6785d02ded0`; complete patch SHA-256 `4db62fc5513de1daea0c1143c400d0be45a129daa6b79fc7a01762f03b85c1f8`; executable/script patch SHA-256 `0e1a4af0bc6b6ec89b551ee4a6ccb90651701bb3fdc891649e03a84ed7973720`. Actual reviewer: GPT-5.6 Sol, xhigh, fast. |
| Final closeout identity | The containing commit is the NNC6.5b item checkpoint. Use its HEAD tree plus the identities above to recover the reviewed candidate. |
...
| Last verification | Workloads 216/216; compute 340 passed plus one ignored; server store slice 59 passed plus seven ignored; roster 36/36; ten process cuts; NNCV035 55/55 and 0/8; aggregate 35/36 sole NNCV035 red; NNCV015 7/7; NNCV027 10/10; format, strict affected Clippy, warning-denied Rustdoc, dependency, effect, line-count, proof lint, docs 108, and site 17/17 pass; narrow review is clean at 0.98. |
| Latest verification checkpoint | `/tmp/nnc65b-server-test-correction.out`, `/tmp/nnc65b-clippy-correction.out`, `/tmp/nnc65b-rustdoc-correction.out`, `/tmp/nnc65b-nnc61e-correction.out`, `/tmp/nnc65b-composition-correction.out`, `/tmp/nnc65b-correction-autoreview.out`, and `/tmp/nnc65b-correction-autoreview.json`. |
| Blocking decision | No blocker. Commit NNC6.5b, then start NNC6.5c with a read-only substitution audit. |
...
| NNC6 — compute choreography | `in_progress` | Exact provision/teardown observers across every workload/retirement path. | NNC6.1-NNC6.5b and NNC6.1e1 are complete. NNC6.5b closed the accepted exact-key retention and cancellation races, passed all B1-B24 gates, and completed its full plus one authorized narrow review. NNC6.5c is in progress with its read-only substitution audit first; NNC6.5d-NNC6.5g remain later units. No completed item retains a temporary legacy authority. NNC6.6 retains resolver fencing; NNC6.1e2 is the final convergence gate. Provider effects remain with their named owners. |
...
| NNC6.5c | `in_progress` | **Dependency:** NNC6.5b complete in the commit containing the preceding row. **Owned paths:** `crates/nimbus-server/src/workload_ingress.rs` and child, `listener_lease.rs` and child, server `lib.rs`; `crates/nimbus-node/src/direct_process.rs`, `host_lifecycle.rs` and child, `systemd_transient.rs` and child, node `lib.rs`; and `crates/nimbus-compute/src/workload_saga/teardown_node.rs` and child. The node reconciler caller is forbidden. **Last green:** NNC6.5b B1-B24, affected suites, NNCV015/NNCV027, NNCV035 self and exact expected-red, quality/docs gates, and the authorized reviews all pass. **Next:** complete a read-only substitution audit and freeze exact adapter, effect, test, and deletion boundaries before fail-before or product edits. **Blocker:** none. |
