...
| Next action | Perform the NNC6.4a read-only substitution audit. Inventory every Container, Krun, service, compute, node, inspection, exit, withdrawal, and restart authority. Freeze the fail-before contract and exact acceptance-bearing path allowlist before any product edit. |
| Current acceptance checkpoint | NNC6.4 E1-E35 are complete at item commit `6f4f909a06a20de1003d5aafc2f5ffcba43cf0bd`. Final proof is NNCV033 `40/40` and `50/50`, aggregate `34/34` and `327/327`, affected non-CLI `2,502/2,502` with `79` skips, CLI `936/936` with one skip, system `73/73`, listener `18/18`, and all written SDK, docs, quality, static, and modularity gates green. NNC6.4a has no acceptance claim yet. |
...
| Audit dirty state | The owner worktree is clean at NNC6.4 item commit `6f4f909a06a20de1003d5aafc2f5ffcba43cf0bd` before this routing transition. The original checkout remains untouched with its user-owned `AGENTS.md`, concurrent-write benchmark, and browser bundle changes. |
| Latest dirty-state checkpoint | Only this NNC6.4-to-NNC6.4a plan and routing transition is dirty. No NNC6.4a product path has changed. |
| Current item review | NNC6.4a has no review and no candidate. Its first step is a read-only substitution audit. The completed NNC6.4 review cadence is closed: one full Sol/xhigh/fast item review plus one narrow correction review, with no further NNC6.4 review warranted. |
...
| Latest verification checkpoint | NNC6.4 item commit `6f4f909a06a20de1003d5aafc2f5ffcba43cf0bd`. The full item review reported `41` findings at confidence `0.98`; disposition was `33` accepted, `4` rejected, and `4` routed to NNC6.5. The permitted narrow correction review reported six accepted findings at confidence `0.96`; every correction and affected proof is green. |
| Blocking decision | No blocker. NNC6.4a is active and dependency-ready for its read-only substitution audit. |
...
| NNC6.4a | `in_progress` | **Dependency:** NNC6.4 complete at `6f4f909a06a20de1003d5aafc2f5ffcba43cf0bd`. Owns eligible Container, Krun, and explicit service restart cutover through the same fenced saga. **Current step:** read-only substitution audit and fail-before contract. **Owned paths:** this plan, its routing row, and `docs/private/plans/proof/nimbus-network-control-plane/nnc6.4a-fenced-restart-substitution-audit.md` during the audit; no product path is owned before the audit freezes its allowlist. **Last green:** NNC6.4 item commit `6f4f909a06a20de1003d5aafc2f5ffcba43cf0bd` with NNCV033 `40/40` plus `50/50`, aggregate `34/34` plus `327/327`, affected behavior, SDK, docs, quality, static, and modularity gates green. **Next:** inventory every restart, exit, inspection, withdrawal, and recovery authority; prove current side effects and duplicated decisions; freeze exact source/target ownership, path allowlist, lifecycle state matrix, dependency constraints, and acceptance tests. **Blocker:** none. |
