...
| SWT0 Install diagnostics, then freeze same-session base and resources | `in_progress` | `codex/sqlite-write-throughput-p0-observability` / `nimbus-sqlite-write-throughput-p0`; later `p0-baseline` remains gated | SWT0.1 merged: PR #242 / `78c2a7f77` | Deterministic fail-before: 2 queued writer opens, per-record format/schema/table checks, 2 current-document encodes; N=256 diagnostic: 48 foreground commits, 1 automatic checkpoint, 1,024-frame WAL high water, 0.157% measured NOOP-probe share; post-run PASSIVE separate | SQLite filter 74/74 with documented external-fixture opt-out (incl. status-only NOOP probe guard); fmt and storage clippy pass; Engine benchmark check/clippy pass | `proof/sqlite-write-throughput/swt0/fail-before.md` | SWT0.1 complete (PR #242 merged, p0-observability worktree/branch removed); next: SWT0.2 freezes `B_ref` from fresh post-merge `origin/main` per D13 |
