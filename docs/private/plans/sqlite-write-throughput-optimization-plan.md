...
| SWT0 Install diagnostics, then freeze same-session base and resources | `complete` | SWT0.1 PR #242 / `78c2a7f77`; SWT0.2 PR #243 / `a326f124` | both merged; p0 worktrees/branches removed | `B_ref`=`2a1853dab` frozen; accepted quiet-host baselines: CRUD 2,018/16,135/27,165 (CV≤4.3%), hot-key 559/3,150/2,639 (CV≤3.3%), layered storage 42,806 all-lane CV≤5.4%; Engine-scale checkpoint diagnostic: 75 auto samples, 1,087-frame high water at N=256 | SWT0.1 fail-before proof merged; SWT0.2 attempt ledger complete with all rejected runs retained and hashed | `proof/sqlite-write-throughput/swt0/` | complete |
| SWT1 Prepared statements + batch-invariant apply context | `complete` | merged; p1 worktree/branch removed | PR #244 / `6c5299219` | N=256 paired +54.1% [CI +47.3,+61.0], candidate ~45,048 abs; N=32 +19.9%; storage lane +110.7%; N=1 −3.79% (in gate); hot-key N=32 −5.49% (accepted deviation, D15) | 428/428 storage; 253/253 Engine journal/publisher/direct/unit/fanout; fail-before RED/GREEN for both tasks; invalidation tests | `proof/sqlite-write-throughput/swt1/ab-acceptance.md` | complete |
...
| SWT4 Attribute and prove a guarded forward-apply optimization | `planned` | `codex/sqlite-write-throughput-p4-forward-apply` / `nimbus-sqlite-write-throughput-p4` | — | first isolate the combined ~7–11.5% cross-run lower-layer delta; implement only with ≥3% projected end-to-end safe gain | conditional-write + recovery full-validation corruption proof | `proof/sqlite-write-throughput/swt4/` | SWT4.1 next: attribution lanes landed on `codex/sqlite-write-throughput-p4-attribution` (`cf8b78c8b`); run measurement, then SWT4.2 decision |
...
| SWT0 frozen source/reference `B_ref` | `2a1853dab` | 2,018 | 16,135 | 27,165 | 26,523–27,807 / 4.3% | 3,150 (N=32; N=256 2,639) | 42,806 accepted layered | 668,368,896 B | 1,396,712 / 339 on layered fixture | frozen: binaries `1e7320c0…`/`9e378942…`; diagnostic reference run accepted (all lanes CV≤5.4%); reruns rebuild this exact commit |
...
| O6 | Checkpoint scheduling/tuning | Layered fixture remains at 337–341 frames; SWT0.1 Engine N=256 diagnostic observed one automatic checkpoint at a 1,024-frame high water and separately measured the passive probe | medium | no tuning decision from the diagnostic; reassess only after SWT0.2 freezes the accepted Engine/resource baseline |
...
| D14 | 2026-07-28 | Keep checkpoint observation opt-in and outside canonical timed runs | SWT0.1 measured the N=256 NOOP status-probe share at 0.157%; timed runs disable the probes, statement counters are `cfg(test)`, and the post-run passive probe has separate counters/time |
| D16 | 2026-07-28 | Reject SWT3 per its decision gate | Post-SWT2 N=256 ≈44k absolute with paired trajectory ≈1.6× over `B_ref`; the ≤5.8% CPU-encode ceiling cannot clear the ≥3% safe end-to-end bar against the remaining profile; SWT4.1 attribution proceeds as mandated |
...
| Final hashed layered binary has no quiet-host accepted report | SWT0 | same-session report and binary hashes recorded with every lane CV≤10% | closed 2026-07-28: `layered-raw-accepted.md` `065ffc11…` from hashed binary `1e7320c0…`, all lanes CV 1.1–5.4% |
| Fresh hot-key/resource baseline not captured in exact SWT0 session | SWT0 | accepted CV≤10% reports under `swt0/` | closed 2026-07-28: `hotkey-raw-accepted.md` `7544a346…` (CV 1.1–3.3%), RSS/cold-open/checkpoint in `swt0/resources.md` |
| `B_ref` cannot be frozen before observability merges | SWT0 | SWT0.1 fail-before and Engine-scale WAL/checkpoint counter seams merged to `origin/main`, then the exact post-merge commit frozen per D13 | closed 2026-07-28: PR #242 squash-merged as `78c2a7f77`; SWT0.2 freezes that or a later clean `origin/main` commit |
