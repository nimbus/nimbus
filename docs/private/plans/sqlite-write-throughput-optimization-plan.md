...
| SWT2 Resident embedded writer connection | `in_progress` | `codex/sqlite-write-throughput-p2-writer-residency` / `nimbus-sqlite-write-throughput-p2` | local `b34a20fb6` (pre-rebase); PR pending | target ≥5% storage gain and N=1 or N=32 gain; no N=256 loss >2% | two queued commits retained; route/fault/reopen parity | `proof/sqlite-write-throughput/swt2/` | after SWT1 merge/cleanup |
