...
| SWT0 Install diagnostics, then freeze same-session base and resources | `in_progress` | SWT0.1 merged (PR #242 / `78c2a7f77`); SWT0.2 on `codex/sqlite-write-throughput-p0-baseline` / `nimbus-sqlite-write-throughput-p0` | SWT0.2 evidence PR pending | `B_ref`=`2a1853dab` frozen; accepted quiet-host baselines: CRUD 2,018/16,135/27,165 (CV≤4.3%), hot-key 559/3,150/2,639 (CV≤3.3%), layered storage 42,806 all-lane CV≤5.4%; Engine-scale checkpoint diagnostic: 75 auto samples, 1,087-frame high water at N=256 | SWT0.1 fail-before proof merged; SWT0.2 attempt ledger complete with all rejected runs retained and hashed | `proof/sqlite-write-throughput/swt0/` | open SWT0.2 evidence PR, key-lane merge, then mark SWT0 complete and promote SWT1 |
