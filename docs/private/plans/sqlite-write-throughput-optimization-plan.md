...
| SWT1 Prepared statements + batch-invariant apply context | `in_progress` | `codex/sqlite-write-throughput-p1-batch-sql` / `nimbus-sqlite-write-throughput-p1` | — | target ≥5% paired N=256 gain; lower-layer delta lower CI >0 | queued/direct/unit parity + crash/replay + fail-before counters | `proof/sqlite-write-throughput/swt1/` | create `codex/sqlite-write-throughput-p1-batch-sql` branch + p1 worktree from fresh `origin/main`; SWT1.1 statement reuse first |
