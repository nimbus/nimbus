...
| SWT1 Prepared statements + batch-invariant apply context | `in_progress` | `codex/sqlite-write-throughput-p1-batch-sql` / `nimbus-sqlite-write-throughput-p1` | local `7fc79dbea`+`9a6d7b744`; PR pending | N=256 paired +54.1% [CI +47.3,+61.0], candidate ~45,048 abs; N=32 +19.9%; storage lane +110.7%; N=1 −3.79% (in gate); hot-key N=32 −5.49% (accepted deviation, D15) | 428/428 storage; 253/253 Engine journal/publisher/direct/unit/fanout; fail-before RED/GREEN for both tasks; invalidation tests | `proof/sqlite-write-throughput/swt1/ab-acceptance.md` | orchestrator review/push/PR/merge |
