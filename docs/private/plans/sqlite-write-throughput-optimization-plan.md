...
| SWT0 Install diagnostics, then freeze same-session base and resources | `in_progress` | `codex/sqlite-write-throughput-p0-observability` / `nimbus-sqlite-write-throughput-p0`; later `p0-baseline` remains gated | SWT0.1 local `1c9858984`, `5ca8a3988`, `dff400fad`, `ab8a87ed3`, `aaa4ee33f` (orchestrator NOOP-contract guard); PR pending | Deterministic fail-before: 2 queued writer opens, per-record format/schema/table checks, 2 current-document encodes; N=256 diagnostic: 48 foreground commits, 1 automatic checkpoint, 1,024-frame WAL high water, 0.157% measured NOOP-probe share; post-run PASSIVE separate | SQLite filter 74/74 with documented external-fixture opt-out (incl. status-only NOOP probe guard); fmt and storage clippy pass; Engine benchmark check/clippy pass | `proof/sqlite-write-throughput/swt0/fail-before.md` | SWT0.1 complete locally; orchestrator review/push/merge is next; do not begin SWT0.2 before merge |
...
| `B_ref` cannot be frozen before observability merges | SWT0 | SWT0.1 fail-before and Engine-scale WAL/checkpoint counter seams merged to `origin/main`, then the exact post-merge commit frozen per D13 | open pending orchestrator review/merge; SWT0.1 is complete locally through `ab8a87ed3` plus its proof/ledger commits |
