...
| SWT0 Install diagnostics, then freeze same-session base and resources | `in_progress` | `codex/sqlite-write-throughput-p0-observability` / `nimbus-sqlite-write-throughput-p0`; later `p0-baseline` remains gated | SWT0.1 local `1c9858984`, `5ca8a3988`, `dff400fad`, `ab8a87ed3`; no PR | Deterministic fail-before: 2 queued writer opens, per-record format/schema/table checks, 2 current-document encodes; N=256 diagnostic: 48 foreground commits, 1 automatic checkpoint, 1,024-frame WAL high water, 0.157% measured NOOP-probe share; post-run PASSIVE separate | SQLite filter 73/73 with documented external-fixture opt-out; fmt and storage clippy pass; Engine benchmark check/clippy pass | `proof/sqlite-write-throughput/swt0/fail-before.md` | SWT0.1 complete locally; orchestrator review/push/merge is next; do not begin SWT0.2 before merge |
