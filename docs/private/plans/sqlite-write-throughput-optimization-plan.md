...
| `B_ref` cannot be frozen before observability merges | SWT0 | SWT0.1 fail-before and Engine-scale WAL/checkpoint counter seams merged to `origin/main`, then the exact post-merge commit frozen per D13 | open pending orchestrator review/merge; SWT0.1 is complete locally at `dff400fad` plus its proof/ledger commit |
