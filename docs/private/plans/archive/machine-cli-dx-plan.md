...
Canonical execution plan for improving the nimbus CLI developer experience to
...
OrbStack CLIs, and assessed against the current nimbus CLI surface.

Reviewed against:

- `crates/nimbus-bin/src/main.rs` — top-level `Cli` struct and `Command` enum
- `crates/nimbus-bin/src/machine/mod.rs` — `MachineSubcommand` enum,
  `MachineInitCommand` args, `MachineStatusView`, `render_machine_view()`
- `crates/nimbus-bin/src/machine/manager.rs` — `build_ssh_command()`,
  `ssh_identity_path` usage throughout
- `crates/nimbus-bin/src/machine/bootstrap.rs` — ignition generation,
  `identity_path` / `public_key_path()` internals
- `crates/nimbus-bin/src/service/mod.rs` — `ServiceSubcommand` enum for DX
  parity reference
...
- `nimbus --version` now works via Clap's top-level `version` support and has
...
- On the macOS host-managed contract, `machine start` now auto-generates a
  machine-owned SSH identity under the Nimbus machine data root when no
...
**Nimbus current:** Users must manually construct `scp` commands using the
SSH identity, port, and user from `nimbus machine status`.
...
3. `crates/nimbus-bin/src/main.rs` and `crates/nimbus-bin/src/machine/mod.rs`
...
- This plan owns CLI surface ergonomics. It does not own machine architecture
  ...
  archived hardening plan, or machine guest content (`nimbus-machine-os` /
...
- `nimbus serve` or `nimbus service` DX changes
...
- **Version lane**
  - `nimbus --version` prints the version from `Cargo.toml`

- **Help text lane**
  - `nimbus machine --help` shows user-facing descriptions
  - `nimbus machine os --help` shows user-facing descriptions
...
- **Init+start lane**
  - `nimbus machine start` on uninitialized machine creates with defaults and
    starts
  - `nimbus machine init --now` initializes and starts in one command
  - `nimbus machine init` without `--now` still works (init only)
  - `nimbus machine start` on initialized machine still works (start only)

- **Format lane**
  - `nimbus machine status --format json` outputs valid JSON
  - `nimbus machine status --format yaml` outputs valid YAML
  - `nimbus machine status` defaults to table output

- **List lane**
  - `nimbus machine list` shows machine table
  - `nimbus machine ls` alias works
  - `--format json` and `--quiet` flags work

- **Inspect lane**
  - `nimbus machine inspect` outputs full config+state as JSON
  - `--format yaml` alternative works

- **Set lane**
  - `nimbus machine set --cpus 4` on a stopped machine updates the config
  - `nimbus machine set` on a running machine produces a clear error
  - updated config is visible in `inspect` and `status`

- **Machine name lane**
  - `nimbus machine start` operates on `default` (backwards compatible)
  - `nimbus machine start my-machine` operates on `my-machine`
  - all error messages include the machine name

- **Copy lane**
- `nimbus machine cp ./local default:/remote` copies to guest
- `nimbus machine cp default:/remote ./local` copies from guest
...
| --- | --- | --- | --- |
| DX1 | done | Added Clap's top-level `#[command(version)]` support on `Cli`; `nimbus --version` now prints the Cargo package version and has focused parser coverage | none |
...
- `nimbus --version` prints `nimbus {version}` where version matches
  `Cargo.toml`
...
- `nimbus --help` about text reflects the full CLI surface (not just "database")
- `nimbus machine --help` shows terse, user-facing descriptions
- `nimbus machine os --help` shows terse, user-facing descriptions
- `nimbus machine init --help` shows terse, user-facing flag descriptions
...
- error messages referencing old flag names updated (e.g., `manager.rs` line
  857: `re-run nimbus machine init --ssh-identity <path>` → `--identity`)

Acceptance criteria:

- old flag names produce "unexpected argument" errors
- new flag names work correctly
- short aliases work (`nimbus machine init -c 4 -m 4096 -d 40`)
- `nimbus machine init --help` shows new flag names with unit documentation
...
- `nimbus machine start` on a clean system initializes with defaults and starts
- `nimbus machine init --now --cpus 4` initializes with 4 CPUs and starts
- `nimbus machine start` on an already-initialized machine starts without
  re-initializing
- `nimbus machine init` without `--now` initializes without starting
...
- `nimbus machine list` and `nimbus machine ls` show machine table
...
- `nimbus machine inspect` outputs valid JSON combining config and state
...
- `nimbus machine set --cpus 4` on a stopped machine updates the config
- `nimbus machine set --memory 4096` on a stopped machine updates the config
- `nimbus machine set` on a running machine produces a clear error
- `nimbus machine set` with no flags produces a helpful usage message
...
- `nimbus machine status -q` prints the machine name only
- `nimbus machine status --format json -q` still prints only the machine name
- existing `nimbus machine list -q` behavior continues to print machine names,
  one per line
- `for m in $(nimbus machine ls -q); do echo $m; done` works in shell scripts
...
- `nimbus machine cp ./local-file default:/tmp/remote-file` copies to guest
- `nimbus machine cp default:/tmp/remote-file ./local-file` copies from guest
...
- `nimbus machine start` operates on the `default` machine (backwards
  compatible)
- `nimbus machine start my-machine` operates on `my-machine`
- `nimbus machine status my-machine` shows status for `my-machine`
- `nimbus machine rm my-machine` removes `my-machine` only
...
- 2026-04-16: Created this plan based on comparative DX audit against Podman
  ...
  the nimbus machine CLI surface against best-in-class tooling. Each gap has a
...
- 2026-04-16: Closed `DX1` by adding Clap's top-level `version` attribute to
  `crates/nimbus-bin/src/main.rs` and a focused parser regression proving
  `nimbus --version` now exits through Clap's display-version path with the
  package version from `Cargo.toml`. Verified with `cargo test -p nimbus-bin
  cli_supports_top_level_version_flag -- --test-threads=1` plus a direct
  `cargo run -p nimbus-bin -- --version` smoke check.
- 2026-04-18: Closed the lingering live-proof note for `DX1` on the current
  ...
  `/tmp/nimbus-v0.1.14-homebrew-proof/run`, the released `v0.1.14` darwin
  ...
  digest with no `NIMBUS_MACHINE_GUEST_BINARY` override. That run proved host
  `nimbus 0.1.14`, guest `nimbus 0.1.14` from `/usr/local/bin/nimbus
  ...
  Durable conclusion: the user-facing `nimbus --version` DX task is fully
...
- 2026-04-18: Audited this plan against the current worktree and shipped
  ...
  work. The audit removed the outdated claim that `nimbus --version` is still
...
- 2026-04-18: Closed `DX2` by rewriting the public machine help surface to
  operator-facing language. `crates/nimbus-bin/src/main.rs` now advertises the
  full machine/service orchestration scope at the top level, and
  `crates/nimbus-bin/src/machine/mod.rs` now uses terse Podman-style wording
  for `machine`, `machine os`, and `machine init` help text. Added focused
  help regression coverage for `nimbus --help`, `nimbus machine --help`,
  `nimbus machine os --help`, and `nimbus machine init --help`. Verified with
  `cargo fmt --all --check`, `cargo check -p nimbus-bin`, `cargo test -p
  nimbus-bin cli_supports_top_level_version_flag -- --nocapture`, `cargo test
  -p nimbus-bin cli_help_describes_machine_and_service_surface -- --nocapture`,
  `cargo test -p nimbus-bin machine_help_uses_user_facing_descriptions --
  --nocapture`, `cargo test -p nimbus-bin
  machine_os_help_uses_user_facing_descriptions -- --nocapture`, and `cargo
  test -p nimbus-bin machine_init_help_uses_user_facing_flag_descriptions --
  --nocapture`. Next item: `DX3`.
- 2026-04-18: Closed `DX3` by aligning `nimbus machine init` with Podman's
  ...
  set. Verified with `cargo fmt --all --check`, `cargo check -p nimbus-bin`,
  `cargo test -p nimbus-bin parses_machine_init_with_resource_overrides --
  --nocapture`, `cargo test -p nimbus-bin
  machine_init_accepts_short_flag_aliases -- --nocapture`, `cargo test -p
  nimbus-bin machine_init_rejects_legacy_flag_names -- --nocapture`, `cargo
  test -p nimbus-bin machine_init_help_uses_user_facing_flag_descriptions --
  --nocapture`, `cargo test -p nimbus-bin
  podman_machine_os_bootstrap_contract_requires_ssh_identity -- --nocapture`,
  and `bash scripts/verify-nimbus-machine-recreate-helper.sh`. Next item:
  `DX4`.
- 2026-04-18: Closed `DX4` by making `nimbus machine start` the primary
  create-or-start path and adding the Podman-style `nimbus machine init --now`
  shortcut. `crates/nimbus-bin/src/machine/mod.rs` now lets `start` inherit
  ...
  overrides once a machine already exists. `crates/nimbus-bin/src/machine/manager.rs`
  now auto-generates a machine-owned SSH keypair under the Nimbus machine data
  ...
  nimbus-bin`, `cargo test -p nimbus-bin machine_init_parses_now_flag --
  --nocapture`, `cargo test -p nimbus-bin
  machine_start_parses_create_if_missing_overrides -- --nocapture`, `cargo test
  -p nimbus-bin machine_help_uses_user_facing_descriptions -- --nocapture`,
  `cargo test -p nimbus-bin machine_init_help_uses_user_facing_flag_descriptions
  -- --nocapture`, `cargo test -p nimbus-bin
  ...
  test -p nimbus-bin machine_init_now_attempts_start_after_initialization --
  --nocapture`, `cargo test -p nimbus-bin
  machine_start_rejects_create_if_missing_overrides_when_machine_exists --
  --nocapture`, and `cargo test -p nimbus-bin
  ...
  at `/tmp/nimbus-dx4-proof.9uILev`: `target/debug/nimbus machine start` on a
  clean root succeeded with no explicit `--identity`, auto-created
  `/tmp/nimbus-dx4-proof.9uILev/home/.local/share/nimbus/machine/default/machine`
  plus `.pub`, converged the pinned Podman image digest, reported
  `machine_api.reachable: true` in `run/machine-status.txt`, returned
  `nimbus 0.1.14` from guest `run/guest-nimbus-version.txt`, returned `200`
...
- 2026-04-18: Closed `DX11` by threading optional `[NAME]` targeting through
  ...
  -p nimbus-bin`, and `cargo test -p nimbus-bin machine_ -- --nocapture
...
- 2026-04-18: Closed `DX5` by adding a real `MachineStatusOutputFormat`
  surface to `nimbus machine status`: `--format table` is now the default
  ...
  check -p nimbus-bin`, and `cargo test -p nimbus-bin machine_ -- --nocapture
  ...
  reused the isolated bundle rooted at `/tmp/nimbus-dx4-proof.9uILev`: after
  rebuilding `target/debug/nimbus`, the same isolated machine root produced
...
- 2026-04-18: Closed `DX8` by adding `nimbus machine set` as the stopped-only
  resource reconfiguration surface for the current machine contract.
  `crates/nimbus-bin/src/machine/mod.rs` now accepts optional `--cpus`,
  ...
  scope. Verified with `cargo fmt --all --check`, `cargo check -p nimbus-bin`,
  `cargo test -p nimbus-bin machine_set_ -- --nocapture --test-threads=1`, and
  `cargo test -p nimbus-bin machine_ -- --nocapture --test-threads=1`. Focused
  ...
  Next item: `DX6`.
- 2026-04-18: Closed `DX6` by adding `nimbus machine list` with a visible
  `ls` alias, `--format json|table`, and `--quiet` names-only output.
  `crates/nimbus-bin/src/machine/mod.rs` now enumerates initialized machine
  ...
  nimbus-bin`, `cargo test -p nimbus-bin machine_list_ -- --nocapture
  --test-threads=1`, and real local macOS CLI proof using the isolated bundle
  rooted at `/tmp/nimbus-dx4-proof.9uILev`, which produced
...
- 2026-04-18: Closed `DX7` by adding `nimbus machine inspect` as the direct
  config-and-state inspection surface for the current machine manager.
  `crates/nimbus-bin/src/machine/mod.rs` now accepts optional named-machine
  ...
  nimbus-bin`, `cargo test -p nimbus-bin machine_inspect_ -- --nocapture
  --test-threads=1`, and real local macOS CLI proof using the isolated bundle
  rooted at `/tmp/nimbus-dx4-proof.9uILev`, which produced
...
- 2026-04-18: Closed `DX9` by adding `--quiet` / `-q` to `nimbus machine
  status` and aligning its precedence rules with the already-landed
  `machine list --quiet` surface. `crates/nimbus-bin/src/machine/mod.rs` now
  ...
  -p nimbus-bin`, `cargo test -p nimbus-bin machine_status_ -- --nocapture
  --test-threads=1`, and real local macOS CLI proof using the isolated bundle
  rooted at `/tmp/nimbus-dx4-proof.9uILev`, which produced
  `run/machine-status-quiet.txt` from
  `target/debug/nimbus machine status --format json --quiet` and confirmed
...
- 2026-04-18: Closed `DX10` by adding `nimbus machine cp` as the Podman-style
  host↔guest transfer surface. `crates/nimbus-bin/src/machine/mod.rs` now
  ...
  layer in `crates/nimbus-bin/src/machine/manager.rs` now reuses the same
  ...
  `cargo fmt --all --check`, `cargo check -p nimbus-bin`, `cargo test -p
  nimbus-bin machine_cp_ -- --nocapture --test-threads=1`, `cargo test -p
  nimbus-bin scp_command_ -- --nocapture --test-threads=1`, and real local
  macOS proof using the isolated bundle rooted at `/tmp/nimbus-dx4-proof.9uILev`:
  `target/debug/nimbus machine start` outside the sandbox to use real host
  port binding and virtualization, a host→guest copy into
  `default:/tmp/nimbus-dx10-copy.txt`, a guest→host round-trip back into
...
- 2026-04-18: Completed the archived-plan release closeout on the shipped
  ...
  `/tmp/nimbus-v0.1.16-homebrew-proof`, exposed that the previously pinned
  ...
  `scripts/collect-nimbus-homebrew-cask-proof.sh` to request
  `nimbus machine status --format yaml` so packaged-release assertions stay
  stable across CLI presentation changes. Re-running the isolated proof with
  the fixed tree succeeded in `/tmp/nimbus-v0.1.16-homebrew-proof-fixed2`,
  proving `machine start`, structured `machine status`, guest SSH, guest
  `nimbus --version`, guest machine-API `/healthz` and `/capabilities`,
