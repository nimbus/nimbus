...
| 2026-05-21 | Bun/JSC Gate 3 | `designed` | Audited Bun's native build target plumbing and identified the smallest viable next patch: split process-neutral link roots out of `bun_bin`, parameterize the Rust archive emitter away from `-p bun_bin --lib`, add a non-default probe archive and generated C++ driver, then link a `check-bun-embed-probe` target against Bun's existing C++/WebKit/JSC object graph. The key new finding is that `bun_bin` still owns `bun_platform as _` force-linking plus seven remaining `phase_c_exports.rs` C ABI symbols, so the probe target may need a shared process-neutral link bridge before it can final-link without `bun_bin`. Next action: implement that proof target in the local Bun worktree and run `bun scripts/build.ts --profile=debug-no-asan --build-dir=/private/tmp/nimbus-bun-embed-native --cache-dir=/private/tmp/nimbus-bun-cache --target=check-bun-embed-probe`. | Source review only: inspected `scripts/build.ts`, `scripts/build/configure.ts`, `scripts/build/profiles.ts`, `scripts/build/bun.ts`, `scripts/build/rust.ts`, `scripts/build/compile.ts`, `scripts/glob-sources.ts`, `src/bun_bin/lib.rs`, `src/bun_bin/phase_c_exports.rs`, and `src/jsc/VirtualMachine.rs`; `rg -n -A2 "#\\[unsafe\\((no_mangle|export_name)" src/bun_bin/phase_c_exports.rs` listed seven remaining Bun-bin-owned C ABI bridge symbols; Bun worktree stayed clean. |
| 2026-05-21 | Bun/JSC Gate 4 | `done` | Implemented the local Bun proof target in Bun commit `ead332f17f`: moved remaining process-neutral C ABI link roots from `src/bun_bin/phase_c_exports.rs` into new `src/link_bridge`; made `bun_bin` depend on `bun_link_bridge`; added non-CLI `src/embed_probe` staticlib root; parameterized Bun's Rust archive emitter; generated and linked an opt-in `bun-embed-probe` driver behind `check-bun-embed-probe`. The first final-linked run proved execution reached `VirtualMachine::init` but crashed on unfinalized JSC options; adding the existing Bun pre-VM contract `bun_jsc::initialize(false)` fixed it. Next action: keep Bun/JSC proof-only and build the next gate around sync host-call transport from guest JS into a Nimbus-owned C/Rust ABI. | In Bun: `cargo fmt --all` passed; `RUSTUP_TOOLCHAIN=nightly-2026-05-06 CARGO_TARGET_DIR=/private/tmp/nimbus-bun-native-proof-target BUN_CODEGEN_DIR=/private/tmp/nimbus-bun-rust-only/codegen CARGO_ENCODED_RUSTFLAGS= cargo check -p bun_link_bridge -p bun_embed_probe --lib` passed in 0.84s after cache reuse with upstream warnings only; `bun scripts/build.ts --profile=debug-no-asan --build-dir=/private/tmp/nimbus-bun-embed-native --cache-dir=/private/tmp/nimbus-bun-cache --target=check-bun-embed-probe` passed and ran `bun-embed-probe`; `git diff --check` passed; `rg` against the generated build graph showed `bun-embed-probe` links `libbun_embed_probe.a`, not `libbun_rust.a`. Detailed transcript: `docs/plans/proof/runtime-engine/bun-jsc/gate-4-native-vm-run.md`. |
| 2026-05-21 | Bun/JSC Gate 5 | `done` | Implemented the local sync host-call proof in Bun commit `52788ee03b`: the generated driver now runs the VM construct/destroy probe and then a second probe that installs a Rust `#[bun_jsc::host_fn]` on `globalThis`, evaluates guest JS through `Bun__REPL__evaluate`, verifies the JS result is `42`, and verifies Rust observed one host call with payload `41`. A first attempt through `JSModuleLoader::evaluate` aborted on a null module-import promise, so module loading remains a later gate. The commit also fixes Bun's generator replay command so `/private/tmp` build dirs reconfigure from the repo root. Next action: keep Bun/JSC proof-only and build the next gate around async host-call/promise settlement and event-loop progress. | In Bun: `cargo fmt --all` passed; `RUSTUP_TOOLCHAIN=nightly-2026-05-06 CARGO_TARGET_DIR=/private/tmp/nimbus-bun-native-proof-target BUN_CODEGEN_DIR=/private/tmp/nimbus-bun-rust-only/codegen CARGO_ENCODED_RUSTFLAGS= cargo check -p bun_embed_probe --lib` passed in 0.60s after cache reuse with upstream warnings only; `bun scripts/build.ts --profile=debug-no-asan --build-dir=/private/tmp/nimbus-bun-embed-native --cache-dir=/private/tmp/nimbus-bun-cache --target=check-bun-embed-probe` passed and ran `bun-embed-probe`; `git diff --check` passed; `rg` against the generated build graph showed the `regen` rule now `cd`s to the Bun repo and the proof target remains `check-bun-embed-probe`. Detailed transcript: `docs/plans/proof/runtime-engine/bun-jsc/gate-5-sync-host-call.md`. |
| 2026-05-21 | Bun/JSC Gate 6 | `done` | Implemented the local async host-call and event-loop proof in Bun commit `68d246f19e`: the generated driver now also runs a third probe where a Rust `#[bun_jsc::host_fn]` returns a pending `JSPromise`, guest JS attaches a `.then` observer, Rust enqueues a `ManagedTask`, and the host drives `vm.wait_for_promise(AnyPromise::Normal(...))` until the promise fulfills with `42` and guest state records the observed value. The first native run aborted on `currentThreadIsHoldingAPILock()` while draining microtasks, proving the bare embedder must hold the JSC API lock across `wait_for_promise`; the passing proof also calls `ensure_waker()` before driving the non-CLI event loop. Next action: keep Bun/JSC proof-only and build the next gate around Nimbus-shaped bundle/module loading without Bun's CLI runner or the Gate 5 `JSModuleLoader::evaluate` failure path. | In Bun: `cargo fmt --all` passed; `RUSTUP_TOOLCHAIN=nightly-2026-05-06 CARGO_TARGET_DIR=/private/tmp/nimbus-bun-native-proof-target BUN_CODEGEN_DIR=/private/tmp/nimbus-bun-rust-only/codegen CARGO_ENCODED_RUSTFLAGS= cargo check -p bun_embed_probe --lib` passed in 0.80s after cache reuse with upstream warnings only; `bun scripts/build.ts --profile=debug-no-asan --build-dir=/private/tmp/nimbus-bun-embed-native --cache-dir=/private/tmp/nimbus-bun-cache --target=check-bun-embed-probe` passed and ran `bun-embed-probe`; `git diff --check` passed; `rg` against the generated build graph confirmed the `regen` command and proof target, and `sed -n '1,40p' /private/tmp/nimbus-bun-embed-native/embed-probe/driver.cpp` confirmed the generated driver declares and calls `nimbus_bun_embed_probe_async_host_call`. Detailed transcript: `docs/plans/proof/runtime-engine/bun-jsc/gate-6-async-host-call.md`. |
| 2026-05-21 | Bun/JSC Gate 7 | `done` | Implemented the local program-bundle load/invoke proof in Bun commit `08b993bce8`: the generated driver now also runs a fourth probe that loads a Nimbus-shaped self-contained program bundle through `Bun__REPL__evaluate`, installs `globalThis.__nimbusBundle.sync` and `globalThis.__nimbusBundle.asyncCall`, invokes both loaded exports, and proves the sync export uses the Rust sync host transport while the async export uses the Rust async transport through a bundle-level promise chain. This intentionally does not claim ESM module loading; Gate 5's `JSModuleLoader::evaluate` failure remains the ESM/module-loader risk. Next action: keep Bun/JSC proof-only and run an artifact-shape decision gate against real Nimbus-generated function artifacts, or proceed to timeout/cancel proofing. | In Bun: `cargo fmt --all` passed; `RUSTUP_TOOLCHAIN=nightly-2026-05-06 CARGO_TARGET_DIR=/private/tmp/nimbus-bun-native-proof-target BUN_CODEGEN_DIR=/private/tmp/nimbus-bun-rust-only/codegen CARGO_ENCODED_RUSTFLAGS= cargo check -p bun_embed_probe --lib` passed in 0.92s after cache reuse with upstream warnings only; `bun scripts/build.ts --profile=debug-no-asan --build-dir=/private/tmp/nimbus-bun-embed-native --cache-dir=/private/tmp/nimbus-bun-cache --target=check-bun-embed-probe` passed and ran `bun-embed-probe`; `git diff --check` passed; `rg` against the generated build graph confirmed the `regen` command and proof target, and `sed -n '1,50p' /private/tmp/nimbus-bun-embed-native/embed-probe/driver.cpp` confirmed the generated driver declares and calls `nimbus_bun_embed_probe_program_bundle_host_calls`. Detailed transcript: `docs/plans/proof/runtime-engine/bun-jsc/gate-7-program-bundle-load.md`. |
| 2026-05-21 | Bun/JSC Gate 8 | `done` | Audited the real Nimbus generated runtime bundle and selected the Bun/JSC near-term artifact lane: keep Deno/V8 on ESM, and prove Bun against an explicit self-contained JavaScript program wrapper that installs `globalThis.__nimbusInvoke`. Added an internal `generateRuntimeProgramBundle(...)` codegen proof path that shares the existing runtime preamble/execution/dispatch code but omits top-level module syntax, plus selftests that execute a real generated mutation/scheduler manifest and reject Node import descriptors. The runtime seam docs now call out JavaScript evaluation format as a separate metadata axis from content kind. Next action: feed this real generated program-wrapper source into the Bun embed target in place of the synthetic Gate 7 program bundle, then continue to timeout/cancel proofing. | `npm run test --workspace @nimbus/codegen` passed; `git diff --check -- packages/codegen/src/emit/runtime_bundle.mjs packages/codegen/src/emit/runtime_bundle_parts.mjs packages/codegen/src/emit/runtime_bundle_dispatch.mjs packages/codegen/src/emit/runtime_bundle_dispatch_global_invoke.mjs packages/codegen/src/selftest/runtime_fixtures.mjs docs/architecture/runtime/engine-seam.md docs/architecture/runtime/new-engine-proof-harness.md docs/plans/proof/runtime-engine/bun-jsc/gate-8-artifact-shape.md docs/plans/runtime-engine-seam-plan.md` passed. Detailed transcript: `docs/plans/proof/runtime-engine/bun-jsc/gate-8-artifact-shape.md`. |
| 2026-05-21 | Bun/JSC Gate 9 | `done` | Implemented the generated-artifact Bun proof in Bun commit `db4d1067b9`: replaced the synthetic Gate 7 program with a checked-in 17,459-byte Nimbus-generated program-wrapper fixture, installed a host-backed `__nimbusCreateContext`, invoked `messages:sendAndSchedule` through generated `globalThis.__nimbusInvoke`, and verified generated reference-tree materialization plus async `ctx.db.insert` and sync `ctx.scheduler.runAfter` host-call transport. Next action: use the generated-program path for timeout/cancel proofing so cancellation evidence covers real Nimbus dispatch. | In Bun: `cargo fmt --all` passed; `RUSTUP_TOOLCHAIN=nightly-2026-05-06 CARGO_TARGET_DIR=/private/tmp/nimbus-bun-native-proof-target BUN_CODEGEN_DIR=/private/tmp/nimbus-bun-rust-only/codegen CARGO_ENCODED_RUSTFLAGS= cargo check -p bun_embed_probe --lib` passed in 0.92s after cache reuse with upstream warnings only; `bun scripts/build.ts --profile=debug-no-asan --build-dir=/private/tmp/nimbus-bun-embed-native --cache-dir=/private/tmp/nimbus-bun-cache --target=check-bun-embed-probe` passed and ran `bun-embed-probe`; `git diff --check` passed; `rg` against the generated build graph confirmed the proof target and generated driver. Detailed transcript: `docs/plans/proof/runtime-engine/bun-jsc/gate-9-generated-program-bundle.md`. |
| 2026-05-21 | Bun/JSC Gate 10 | `done` | Implemented the generated-program timeout/cancel proof in Bun commit `ea677357e3`: added `messages:spinForever` to the generated fixture, invoked it through generated `globalThis.__nimbusInvoke`, interrupted the loop from a host deadline thread, cleared JSC termination state, proved same-VM recovery with `40 + 2`, then repeated the interruption through an explicit cancel thread. The proof found that cross-thread `VM::notifyNeedTermination()` must be preceded by owner-thread termination-exception priming, and that Bun/JSC `setExecutionTimeLimit()` interrupted the loop but left a debug watchdog trap that asserted on recovery, so Nimbus timeout semantics should use a host-owned deadline/cancellation handle for this embed path. Next action: keep Bun/JSC proof-only and investigate memory/permission containment or VM reuse versus fresh-VM discard policy. | In Bun: `cargo fmt --all` passed; `RUSTUP_TOOLCHAIN=nightly-2026-05-06 CARGO_TARGET_DIR=/private/tmp/nimbus-bun-native-proof-target BUN_CODEGEN_DIR=/private/tmp/nimbus-bun-rust-only/codegen CARGO_ENCODED_RUSTFLAGS= cargo check -p bun_embed_probe --lib` passed in 1.04s after cache reuse with upstream warnings only; `bun scripts/build.ts --profile=debug-no-asan --build-dir=/private/tmp/nimbus-bun-embed-native --cache-dir=/private/tmp/nimbus-bun-cache --target=check-bun-embed-probe` passed and ran `bun-embed-probe`; `git diff --check` passed. Detailed transcript: `docs/plans/proof/runtime-engine/bun-jsc/gate-10-timeout-cancel.md`. |
