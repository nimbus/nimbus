# Plan: `nimbus codegen` CLI Integration

Canonical execution plan for adding a first-party `nimbus codegen` subcommand
...
capability of the `nimbus` binary rather than a sideband Node.js script.

Successor to the completed source-root plan
(`docs/plans/archive/nimbus-source-root-plan.md`), which landed the
`nimbus/` vs `convex/` source-root detection and namespace-aware `_generated`
emission but explicitly deferred the CLI integration (`"Follow-on work, not
part of this story: add a first-party nimbus codegen command"`).
...
- **Activation gate:** none — can start immediately; all upstream inputs
  (source-root resolver, namespace-aware codegen, `@nimbus/codegen` package)
  are shipped
- **Parent plan:** none
- **Related plans and docs:**
  - `docs/plans/archive/nimbus-source-root-plan.md` — landed source-root
    resolver and namespace-aware `_generated` emission
  - `docs/stories/support-nimbus-source-directory.md` — product contract that
...
Today a developer working on a Nimbus project must context-switch between two
disconnected tools:

```bash
npx convex codegen --app ./my-app       # JS tool, Convex-branded
nimbus serve --convex-app-dir ./my-app  # Rust tool, Nimbus-branded
```

This workflow has four concrete problems:

1. **The primary tool has no codegen.** The `nimbus` binary is the developer's
   ...
   separate tool (`npx convex codegen` or `npx nimbus-codegen`) that lives
   outside the Rust binary's command tree.
2. **The command names contradict the product identity.** A developer who
   creates a `nimbus/` source root, writes Nimbus-native functions, and
   imports from `nimbus/server` must then run `npx convex codegen` to build
...
3. **The serve flag is misleading.** `--convex-app-dir` implies the serve
   ...
   with both `nimbus/` and `convex/` source roots after the source-root plan
   landed.
...
The goal is to make the first-party Nimbus CLI feel coherent and migration-
friendly:

- Nimbus users should not have to reach for a Convex-branded command just to
  generate Nimbus-native `_generated/` files.
- `nimbus serve --app-dir` should be able to perform one startup-time codegen
...
- a first-party `nimbus codegen` entrypoint
- a one-shot preflight codegen step on `nimbus serve --app-dir`
- clearer startup errors and better naming

It does **not** land:

- a `nimbus dev` watched loop
...
> The primary Nimbus CLI should own the common path from source to running
...
- The `nimbus` Rust binary now ships five subcommands: `serve`, `codegen`,
  `machine`, `service`, and `encryption`.
- The `@nimbus/codegen` JS package provides the full codegen pipeline and
  exports both a library API (`generateConvexArtifacts()`) and a CLI entrypoint
  (`nimbus-codegen`).
- The `convex` JS package wraps `@nimbus/codegen` behind `convex codegen`.
- Source-root detection (`nimbus/` vs `convex/`) is fully implemented in
...
- `nimbus serve --app-dir` now runs a one-shot preflight codegen pass before
...
- Workspace and hoisted installs are already a real local-development case in
  this repo. For example, the demo apps can resolve `@nimbus/codegen` through
  workspace Node resolution even though
  `{appDir}/node_modules/@nimbus/codegen/` does not exist beneath each app.
- The root workspace scripts, demo app scripts, and Makefile still run explicit
  ...
  watch loop.
- The live CLI/reference and compatibility docs now describe `nimbus codegen`,
...
- When the required manifest is missing or unreadable, the serve boot boundary
  ...
  `.nimbus/convex/functions.json`, includes the exact `nimbus codegen --app`
...
1. **Codegen logic stays in JavaScript.** The Rust `nimbus codegen` command
   spawns the `@nimbus/codegen` pipeline as a Node.js subprocess. It does not
...
2. **`convex codegen` stays.** It remains the compatibility entry point for
   ...
   pipeline.
3. **No `--convex` / `--nimbus` flag.** The chosen source-root directory
   (`nimbus/` or `convex/`) remains the signal.
4. **`.nimbus/convex/` internal artifacts stay unchanged.** This plan does not
...
6. **Node resolution must honor workspaces and hoisting.** Do not assume an
   app-local `node_modules/@nimbus/codegen/...` filesystem path exists.
7. **Serve-side auto-codegen is a one-shot preflight.** This plan does not add
   file watching, background regeneration, or a `nimbus dev` loop.
...
10. **Codegen failure must be a clear, blocking error.** When `nimbus serve`
...
```bash
# Codegen for the current directory (detects nimbus/ or convex/)
nimbus codegen
...
nimbus serve --app-dir ./my-app --skip-codegen
```

### Serve without an app directory

```bash
# Pure database server, no Convex/Nimbus functions
nimbus serve
```

### Migration Taste for Convex Users

After this plan, the Nimbus-native story should feel like:
...
### C1: `nimbus codegen` subcommand

Add a `codegen` subcommand to the Rust binary that invokes the existing
`@nimbus/codegen` pipeline through Node's module-resolution semantics.

**Implementation:**

- Add `crates/nimbus-bin/src/codegen.rs` with a `CodegenCommand` struct:
  ```rust
  #[derive(Debug, clap::Args)]
  #[command(
      about = "Generate _generated files and runtime bundle from nimbus/ or convex/ source",
      help_template = cli_ux::COMMAND_HELP_TEMPLATE,
  )]
  pub(crate) struct CodegenCommand {
      /// App directory containing the nimbus/ or convex/ source root.
...
- Implement `run_codegen_command()` that:
  ...
  3. invokes `@nimbus/codegen` through Node module resolution
     (`@nimbus/codegen/cli` or a tiny bootstrap that calls
     `runCliFromArgs()`), not through a hardcoded
     `{app_dir}/node_modules/@nimbus/codegen/...` path
...
- Add `nimbus codegen` to `ROOT_HELP_EXAMPLES`
...
- `cargo run -p nimbus-bin -- codegen --app ./demos/convex/html` succeeds from
  the repo root and produces `.nimbus/convex/` artifacts
- `cargo run -p nimbus-bin -- codegen --app ./demos/convex/node` succeeds from
  the repo root
- `nimbus codegen` in a directory with `nimbus/` or `convex/` source root works
- `nimbus codegen` in a directory with no source root fails with the existing
  JS resolver error
- `nimbus --help` shows `codegen` in the command list
...
- In `crates/nimbus-bin/src/serve/mod.rs`, rename the field:
  ```rust
  /// App directory containing generated .nimbus/convex/ runtime artifacts.
...
- `nimbus serve --app-dir ./my-app` works
- `nimbus serve --help` shows `--app-dir`
...
- no file watching
- no `nimbus dev` command

**Verification:**

- `nimbus serve --app-dir ./my-app` runs codegen once, then serves
- `nimbus serve --app-dir ./my-app --skip-codegen` skips codegen
- `nimbus serve` without an app dir does not attempt codegen
...
- In `load_convex_registry()` or the registry-loading boundary, detect the
  specific case where `.nimbus/convex/functions.json` is missing or unreadable
  and produce a structured error such as:
  ```text
  Error: No generated function manifest found at <app_dir>/.nimbus/convex/functions.json.

  Run "nimbus codegen --app <app_dir>" to generate it, or remove
...
- `nimbus serve --app-dir ./empty-dir --skip-codegen` gives the actionable
  error
- the error message includes the exact `nimbus codegen` command to run
...
- **`docs/reference/cli.md`:** Add `nimbus codegen`, update `nimbus serve`
...
- **`docs/convex/compatibility.md`:** Document that `convex codegen` and
  `nimbus codegen` are equivalent entry points to the same pipeline, and state
...
- **`ROOT_HELP_EXAMPLES` in `cli_ux.rs`:** Add `nimbus codegen`

**Verification:**

- doc examples are accurate and runnable
- `nimbus --help` and `nimbus serve --help` reflect the new surface
...
| File | What |
|------|------|
| `crates/nimbus-bin/src/codegen.rs` | `CodegenCommand` struct and `run_codegen_command()` |

### Files to modify

| File | Change |
|------|--------|
| `crates/nimbus-bin/src/main.rs` | Add `Codegen` variant, `mod codegen`, dispatch |
| `crates/nimbus-bin/src/serve/mod.rs` | Rename `convex_app_dir` → `app_dir` with no alias, add `skip_codegen` |
| `crates/nimbus-bin/src/serve/boot.rs` | Preflight codegen before registry load, updated field name, clearer load errors |
| `crates/nimbus-bin/src/serve/tests.rs` | Update parse and startup tests for `--app-dir`, `--skip-codegen`, and required-manifest error wording |
| `crates/nimbus-bin/src/cli_ux.rs` | Update help examples |
| `crates/nimbus-server/src/adapters/convex/registry/loading.rs` | Optional only if registry-layer error wording is the cleanest place to improve required-manifest diagnostics |
| `package.json` | Update root workflow scripts to `--app-dir` |
| `packages/convex/src/differential.mjs` | Update local Nimbus launch command to `--app-dir` |
| `Makefile` | Update demo targets |
| `docs/reference/cli.md` | Add `nimbus codegen`, update serve flags and preflight semantics |
...
| `packages/nimbus/**` | SDK surface unchanged |
...
```
                           ...
                           └──────────┬───────────┘
                                      │
                                      ▼
                       nimbus serve --app-dir . [--skip-codegen]
                           ┌──────────────────────┐
                           │  Runs one preflight  │
                           │  codegen pass unless │
                           │  --skip-codegen      │
...
```
┌──────────────────────────────────────────────────────────────────┐
│                     @nimbus/codegen pipeline                     │
│                   (single JS implementation)                     │
│                                                                  │
│  resolveSourceRoot() → parse → emit _generated/ + .nimbus/convex │
└──────┬───────────────────────┬────────────────────────┬──────────┘
       │                       │                        │
       ▼                       ▼                        ▼
  nimbus codegen         convex codegen          npx nimbus-codegen
  (Rust CLI,             (JS CLI,                (JS CLI,
   spawns Node)           compatibility)          direct)
       │                       │                        │
       │    All three invoke the same pipeline.         │
       │    All three detect nimbus/ vs convex/.        │
...
| C1 | `done` | 2026-04-22 | Added a first-party `nimbus codegen` subcommand in `crates/nimbus-bin`, invoking `@nimbus/codegen` through Node module resolution from the app directory instead of probing app-local package paths. Updated root/codegen help examples and CLI parse coverage. Verification: `cargo test -p nimbus-bin`; `cargo run -p nimbus-bin -- codegen --app ./demos/convex/html`; `cargo run -p nimbus-bin -- codegen --app ./demos/convex/node`. Next: rename `--convex-app-dir` to `--app-dir` across serve, tests, scripts, and docs. |
| C2 | `done` | 2026-04-22 | Renamed the serve app-directory flag to `--app-dir` with no alias, updated serve help/examples, and moved the owned package scripts, differential helper, and Makefile target to the new flag. Verification: `cargo test -p nimbus-bin`; targeted grep over `crates/nimbus-bin/src/serve`, `crates/nimbus-bin/src/cli_ux.rs`, `package.json`, `packages/convex/src/differential.mjs`, and `Makefile` confirmed the old flag only remains in explicit rejection tests. Next: add one-shot preflight codegen and `--skip-codegen` to `nimbus serve --app-dir`. |
| C3 | `done` | 2026-04-22 | Added one-shot serve preflight codegen plus `--skip-codegen`, reusing the first-party Rust wrapper before any listener or scheduler startup. Added CLI parse/help coverage and preflight tests that generate real artifacts in a repo-local temp app. Verification: `cargo test -p nimbus-bin`; `cargo run -p nimbus-bin -- serve --app-dir ./demos/convex/node --data-dir /tmp/nimbus-c3-data --control-data-dir /tmp/nimbus-c3-control --port 18091` then `curl -i -sS http://127.0.0.1:18091/health` (required escalated execution because sandboxed serve died after preflight with `Os { code: 1, kind: PermissionDenied, message: "Operation not permitted" }`); `cargo run -p nimbus-bin -- serve --app-dir ./demos/convex/node --skip-codegen --data-dir /tmp/nimbus-c3-skip-data --control-data-dir /tmp/nimbus-c3-skip-control --port 18093` then `curl -i -sS http://127.0.0.1:18093/health`; `cargo run -p nimbus-bin -- serve --data-dir /tmp/nimbus-c3-noapp-data --control-data-dir /tmp/nimbus-c3-noapp-control --port 18094` then `curl -i -sS http://127.0.0.1:18094/health`; `cargo run -p nimbus-bin -- serve --app-dir ./target/nimbus-c3-empty.zn65zE --port 18092` showed preflight blocking startup and surfaced the JS source-root error. Next: tighten startup diagnostics for missing required manifests while preserving optional bundle semantics. |
| C4 | `done` | 2026-04-22 | Added a serve-boundary required-manifest check for `.nimbus/convex/functions.json`, returning an actionable `nimbus codegen --app …` recovery message and a `--skip-codegen` hint when preflight was bypassed. Reordered serve startup so registry/service-manager validation happens before listener bind, preserving fail-fast behavior. Verification: `cargo test -p nimbus-bin`; `cargo test -p nimbus-server`. New focused tests cover the actionable missing-manifest error, manifest-only startup without `bundle.mjs`, and the existing `nimbus-server` suite continues to prove that a present `bundle.mjs` still requires `bundle.sha256`. Next: align remaining docs and developer-facing surfaces with the shipped command shape and scoped Convex migration story. |
| C5 | `done` | 2026-04-22 | Updated the remaining live docs and developer-facing surfaces: CLI reference, compatibility guide, HTTP/API reference, architecture note, package scripts, Makefile, and the differential helper now all describe `nimbus codegen`, `--app-dir`, `--skip-codegen`, checked-in generated files, and one-shot serve preflight honestly. Verification: `cargo fmt --all --check`; `make clippy`; `make test`; `npm run test --workspaces --if-present`; `npm run build --workspaces --if-present`; targeted grep over live docs/scripts confirmed the old flag is absent from shipped surfaces. Workstream complete. |
