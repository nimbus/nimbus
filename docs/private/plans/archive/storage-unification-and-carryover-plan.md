...
| SUC3.1 Provider facade extraction | `complete` (PRs #254, #257–#261: shared store core; libsql on the seam; U5 witness 3-of-4 paths + U8; U4 StorageCommit* ban + Journal* pins; five transaction-half modules; per-provider feature gates; 32 test bodies onto 14 shared scenarios) | delete the ~855-line triplicated blocks into one facade; sqlite (with #244/#245 optimizations) is the reference; provider feature gates so embedded builds/measures in isolation | ~−1,700 LoC net; conformance harness green for all providers; sqlite paired A/B unchanged | `suc3/facade.md` | after SUC2; sqlite fork is canonical, not blocked-on |
...
| U9 | 2026-07-30 | SUC3.1 LoC acceptance verdict: figure NOT met; duplication goals met | honest crates-only ledger across steps 1–6: +153, −182, +658, −366, +986, −993 = **net +256** vs the plan's ~−1,700 (gross −8,170 deletions against +8,426 of shared-core/witness/gate/cfg/test-scaffolding). The −1,700 assumed dedup without the witness/gate/feature machinery costs and without behavior-preserving declines (each published with arithmetic at decision time: table_lifecycle +17, document_versions no-orchestration, driver-lifetime session runs, wave-4 9-methods-for-1-scenario). What was bought: triplicated wrapper/transaction/test logic now has single sources of truth, a compile-time commit-effect witness, structural fault-point gates, and embedded builds free of remote-provider deps |
| U10 | 2026-07-30 | U7 owner-override exercised: SUC6.2 rejection now MEASURED on current main | retained SWT4.1 attribution instrumentation rerun on minicloud against main `22c5cdd62` (12 rounds, all lane CVs ≤0.6%): binding component 0.095 ms = 1.42% of guarded = 0.90% of the production storage lane; end-to-end strictly smaller → a-fortiori below the ≥3% bar. Same conclusion as U7's arithmetic, now with measurement; proof `proof/storage-follow-ups/fu7.md` |
