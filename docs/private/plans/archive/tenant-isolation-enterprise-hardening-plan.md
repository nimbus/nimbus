...
| EIH6 | `done` | Define workload identity shape. | `TenantWorkloadStableIdentity` renders stable Nimbus and SPIFFE-style audit IDs, includes node/machine location in the decision fingerprint, and is documented as the future service-identity source projection. |
...
- At the time of the EIH6 checkpoint, `docs/plans/secret-management-plan.md`
  and related secret-management research drafts were untracked in the local
  worktree, so the checkpoint intentionally recorded the provider-auth follow-up
  in the tracked tenant-isolation plan and architecture contract without
  staging those drafts. A later baseline-hardening pass tracks the follow-up
  plans and splits provider policy subjects from per-invocation audit claims.
...
| 2026-05-22 | EIH6 | `done` | `crates/nimbus-server/src/tenant_isolation.rs`, `crates/nimbus-server/src/lib.rs`, `docs/architecture/server/auth-runtime-trust.md`, `docs/plans/tenant-isolation-enterprise-hardening-plan.md` | Added the stable workload/audit identity projection, node/machine location in decision fingerprints, SPIFFE-style rendering, audit exposure via `workload_stable_id`, and architecture guidance for future secret-provider auth. A later service-identity baseline splits low-cardinality provider subjects from per-invocation audit claims. Next phase is EIH7 image provenance and signature admission. | `cargo test -p nimbus-server tenant_workload_stable_identity -- --nocapture` passed: 3 passed, 0 failed, 0 ignored, 750 filtered out; `cargo test -p nimbus-server tenant_isolation -- --nocapture` passed: 28 passed, 0 failed, 0 ignored, 725 filtered out; `cargo test -p nimbus-server runtime_execution_admission -- --nocapture` passed: 2 passed, 0 failed, 0 ignored, 751 filtered out; `cargo fmt --all --check` passed; `cargo clippy -p nimbus-server --all-targets` passed; `git diff --check` passed. |
