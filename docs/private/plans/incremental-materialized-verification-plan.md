...
Next action: publish IMV6 correction `0fe5db256`, rerun hosted provider
qualification, and merge `codex/imv6-provider-parity-faults-observability`.
...
| IMV6 | If required, complete six-provider parity, recovery faults, memory bounds, metrics, and operator controls. | `in_progress` | Work `3567ba656` adds provider-root parity, persistent-tamper failure, hard session bounds, fixed metrics, and operator controls. Correction `0fe5db256` makes replacement invalidation overlap-safe. Proof is in `imv6.md`; verifier `Summary: 14 passed, 2 failed`; final `make ci` and both Nimbus autoreviews pass. Hosted provider requalification and merge remain. |
...
- Owning seam and paths:
  ...
  `crates/nimbus-cli/src/app_manifest.rs`, one
...
- Steps:
  ...
  12. Delete every incorrect claim that `preserve_order` stays disabled. This
      includes the workspace manifest and `crates/nimbus-cli/src/app_manifest.rs`.
...
| 2026-08-19 | meta | refined | Applied the required Fable corrections. Led IMV1 with the shipped-graph `preserve_order` divergence and named the duplicate false claim in `crates/nimbus-cli/src/app_manifest.rs`. Recorded that periodic verification does not exist today. Required the threshold 1 margin and the 192-bytes-per-leaf derivation. No implementation started. |
...
| 2026-08-21 | IMV4 | completed | Correction PR #307 passed the archive build, canonical inventory, test-universe, taxonomy, and required hosted gates. It merged as `eb2997083b`. IMV4 is terminal. |
| 2026-08-21 | IMV5 | started | Created `codex/imv5-bounded-verification-sessions` from corrected main `eb2997083b`. IMV5 is the only `in_progress` task. |
| 2026-08-21 | IMV5 | implemented | Work `a7c678351` adds per-tenant bounded root sessions, exact applied-suffix advance, full-scrub anchors, explicit modes and escalation reasons, retention rebuild, mismatch failure, and count, byte, idle, and anchor limits. Focused lanes pass 7 consistency, 3 session, and 22 storage tests. The verifier reports `Summary: 11 passed, 5 failed`. Review and publication remain. |
| 2026-08-21 | IMV5 | verified | The required local CI gate passed format, workspace Clippy, dependency, runtime, and IMV-focused checks. Its non-runtime lane ran 7,572 tests: 7,571 passed, 110 skipped, and the unchanged MongoDB report test failed once. That test is outside the IMV5 paths and passed three isolated reruns. Hosted CI remains the merge source of truth. |
...
| 2026-08-21 | IMV6 | corrected | Hosted PostgreSQL and MySQL passed. The libSQL root-parity test passed, but four PPSC recovery cases exposed that the new single-writer invalidator could reject overlapping replica-cache work after durability. Correction `0fe5db256` uses a reference-counted non-current epoch, so derived verification state does not break valid storage concurrency. Focused storage, engine, libSQL-feature compile, and Clippy checks pass. Hosted requalification and review remain. |
