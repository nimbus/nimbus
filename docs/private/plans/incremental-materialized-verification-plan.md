...
| IMV3 | If required, implement the storage-owned deterministic Merkle treap and prove batch versus incremental equivalence. | `in_progress` | Work `139a50928`; proof `030802c52`. Eight focused tests and the million-leaf gate pass. Verifier: `Summary: 8 passed, 8 failed`. Pre-PR review, hosted checks, and merge remain. |
...
| 2026-08-21 | IMV3 | implemented | Work `139a50928` adds the opaque logical key, versioned verification position, domain-separated SHA-256 treap, update and delete operations, generated histories, and depth and memory counters. Proof `030802c52` records the dependency screen and a million-leaf depth of 55 at 145 measured and 160 budgeted bytes per leaf. The full storage lane passes 344 tests with 3 expected ignores. The verifier reports `Summary: 8 passed, 8 failed`. |
