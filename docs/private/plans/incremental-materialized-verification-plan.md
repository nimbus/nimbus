...
Next action: execute the IMV2 benchmark matrix from merged main, publish the
raw measurements and three-state verdict, and advance only the selected path.
...
| IMV2 | Run the benchmark matrix and decide `STREAMING_ACCEPTED`, `MERKLE_REQUIRED`, or `NO_ACCEPTABLE_DESIGN` from the ratified gate. | `in_progress` | Branch `codex/imv2-measurement-verdict` starts from merged main `9c807015a`. |
