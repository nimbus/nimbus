...
- **Activation gate:** the macOS prerequisite is **met.** Windows reuses the
  macOS hybrid host-resident control-plane pattern and forwarded machine-API
  seam (different VM provider + transport), and that pattern was stabilized by
  the archived MAC1-MAC7 closeout record. Windows therefore remains deferred
  **only** because this plan itself has not been promoted — it is not blocked on
  MAC5+ or any other plan. Promote it when Windows developer-machine support is
  actually scheduled.
- **Related plans:**
  - `docs/private/architecture/sandbox/macos-machine-flow.md` — current macOS developer-machine
    contract reference; Windows shares the hybrid control-plane architecture
    and the forwarded machine-API contract
  - `docs/private/plans/archive/macos-machine-support-plan.md` — completed macOS
    execution record with the exact proof and sequencing Windows can reuse
  - `docs/private/architecture/sandbox/microvm-service-baseline.md` — current landed Linux
    microVM and service-control baseline
  - `docs/private/plans/distribution-plan.md` — packaging/distribution umbrella; this
    plan owns the detailed execution of the Windows row
  - `docs/private/plans/archive/machine-lifecycle-hardening-plan.md` — completed
...
3. `docs/private/architecture/sandbox/macos-machine-flow.md` (architectural precedent)
4. `docs/private/architecture/sandbox/microvm-service-baseline.md`
5. `docs/private/plans/distribution-plan.md`
...
- a fresh agent can find this plan from `docs/private/plans/README.md`
