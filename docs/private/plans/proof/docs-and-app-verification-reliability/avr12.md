...
| [#280](https://github.com/nimbus/nimbus/pull/280) | `437c2a185` | Shared Rust setup now removes the failed Azure regional Ubuntu mirror and bounds apt network waits. |
...
Cleanup PR #278 run `32155982971` reproduced the Azure mirror outage in two
independent jobs before either reached project code. PR #280 added a fixture
for the official hosted-runner mirror format. It passed 50 hosted checks with
three expected skips and merged before the cleanup branch reconciled current
main. The replacement cleanup run is the remaining hosted gate.
...
| Cleanup pull request | [#278](https://github.com/nimbus/nimbus/pull/278) is open. The first hosted run was superseded by merged CI remediation [#280](https://github.com/nimbus/nimbus/pull/280); replacement checks and merge remain. |
