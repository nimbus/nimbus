...
`bd2a8a36496275e65672917f4e7d17983feff7a5`. The examples runner now has one
run lifetime owner and one process-group owner. Nimbus owns every listener and
port lease.
...
| `node scripts/examples-verify-lifetime-test.mjs` | Pass. 9/9 groups covered cross-case sentinels, concurrent binds, external binders, process trees, six cuts, cleanup retry, artifact retention, post-spawn record failure, and corrupt records. |
...
| `bash scripts/examples-verify-contract-test.sh --task AVR7` | Pass. AVRC19-AVRC20 are 2/2 and the lifetime groups are 9/9. |
