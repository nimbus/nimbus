...
# SUC3.1 Step 6 — Provider Test Dedupe

Brief: the provider test suites had drifted into three near-copies of the same
scenarios. Collapse the duplication without weakening provider coverage, under
four rulings: provider-*specific* tests stay per-provider; a shared scenario
must still run under each provider's cfg gate and fixture environment exactly
as before, fail-loud intact; test *names* stay discoverable per provider so the
official nextest filters keep working; and the test taxonomy rules still apply.

Delivered in three waves. 32 test bodies now delegate to 14 shared scenarios,
the tree is **1,371 lines smaller**, and all three per-provider filters select a
**byte-identical** list of test names to the ones they selected before the step.
This is the campaign's final change.

## What Changed

**Wave 1 — the data builders.** The three `*_provider/support.rs` files each
carried their own copy of the same fixture constructors. They now live once in
`tests/provider_support.rs` as 14 neutral-named builders, gated
`any(libsql, mysql, postgres)`.

**Wave 2 — the three-way scenarios.** Four scenarios that all three providers
run identically moved to `tests/provider_scenarios.rs`. Two of them — the
historical index scans — needed a decision recorded below, because the three
providers reach those scans by different routes.

**Wave 3 — the SQL pair.** Ten scenarios that only PostgreSQL and MySQL run
moved to `tests/sql_pair_scenarios.rs`, gated `any(mysql, postgres)`. These two
already share their entire write path through `SqlStoreCore`,
`SqlDurableJournalStore` and `SqlHistoricalIndexStore`, so these were literally
the same test written twice. libSQL does not run them because it has no
equivalent surface — which is why they are a separate module from wave 2's
rather than a wider gate on it.

**The shape of a converted test.** Every provider keeps its own `#[tokio::test]`
function under its own module, with its own `<provider>_` name prefix, its own
fixture bootstrap, and (for libSQL) its own `#[serial]`. Only the body is
delegated:
...
async fn postgres_document_versions_track_direct_write_history() {
    with_test_provider(|provider, _config| async move {
        let tenant = TenantId::new("versions").expect("tenant id should build");
        let opened = provider
...
Nothing about gating, fixture policy, or naming changed — which is what makes
the filter-identity proof below possible.

## LoC Delta

| Scope | Delta |
| --- | --- |
| 13 existing test files | **+93 −3022** |
| `tests/provider_support.rs` (new) | +469 |
| `tests/provider_scenarios.rs` (new) | +373 |
| `tests/sql_pair_scenarios.rs` (new) | +716 |
| **Net** | **−1371** |

No non-test file was touched. The whole step is confined to
`crates/nimbus-storage/src/tests/`.

| File | Delta |
| --- | --- |
| `mysql_provider/versions.rs` | +17 −468 |
| `postgres_provider/versions.rs` | +11 −468 |
| `libsql_provider/versions.rs` | +25 −378 |
| `postgres_provider/journal.rs` | +7 −225 |
| `mysql_provider/journal.rs` | +7 −221 |
| `mysql_provider/support.rs` | +4 −268 |
| `postgres_provider/support.rs` | +2 −268 |
| `libsql_provider/support.rs` | +2 −254 |
| `mysql_provider/execution_units.rs` | +4 −121 |
| `postgres_provider/execution_units.rs` | +4 −121 |
| `mysql_provider/schema.rs` | +2 −116 |
| `postgres_provider/schema.rs` | +2 −114 |
| `tests.rs` | +6 −0 |

## The Dedupe Boundary Is The Trait Surface, Not Text Similarity

The one methodological decision worth recording. Text-normalized similarity
scored the two `historical_index_scan_*` scenarios as perfect three-way matches,
and a bound chosen from that number alone would not have compiled.

PostgreSQL and MySQL reach the five historical scans through
`SqlHistoricalIndexStore`, via inherent forwarders generated by
`sql_historical_index_facade!` (`postgres/index_versions.rs:90`,
`mysql/index_versions.rs:91`). The libSQL replica store hand-writes its own at
`libsql/read.rs:424-561` and implements no such trait. There is therefore **no
existing trait all three share** to bound a shared scenario on.

What made the share possible anyway is that all three end up with *inherent*
methods of identical signature. So the shared module declares a test-only
forwarding trait over the inherent surface (`HistoricalIndexScanOracle`, 5
methods) and a macro that implements it for each provider store by naming the
store explicitly, so the inherent method is always the one selected and the impl
cannot recurse into itself. The same pattern covers `StorageHealthProbe`, whose
single method is generated by `impl_storage_health_diagnostic!` and is likewise
inherent rather than trait-backed.

Every scenario was checked signature-by-signature before its body was moved. The
compiler is the oracle here: under this workspace's `unused = "deny"` and
`clippy all = "deny"`, a wrong bound, an unused import left behind by a moved
body, or a field no provider reads is an **error**, not a warning.

Two consequences of that fell out during the work:

- `E0034` on `append_durable_records_batch`, `journal_progress` and
  `recover_durable_journal`: `SqlStoreCore` and `DurableJournal` both provide
  them, and since `MaterializedRebuild: DurableJournal` the collision is
  unavoidable in a bundled bound. Resolved with fully-qualified
  `SqlStoreCore::method(store, …)` — faithful, because the original
  concrete-typed calls resolved to inherent forwarders backed by `SqlStoreCore`.
- `DurableRecoveryFixture`'s three fields are read only by the libSQL wrapper,
  so every feature configuration *without* libSQL failed with "fields are never
  read". Fixed with `#[cfg_attr(not(feature = "libsql"), allow(dead_code))]` and
  a comment saying why, not with a blanket `allow`.

## A Divergence Check Caught A Real Behavior Difference

Before every replacement, the candidate bodies were diffed against each other
with provider tokens normalized away. That is how
`sql_pipeline_post_runner_cancellation_is_counted_once` was caught: PostgreSQL
asserts `provider_operation_count == 2`, MySQL asserts `== 1`, and the two
`expect_err` messages differ. That is a genuine provider-behaviour difference,
so under ruling (a) it stays two separate tests. It was **declined, not
parameterized** — collapsing it behind a per-provider expected-count argument
would have converted a visible behavioural difference into a table entry.

Three other pair candidates differed only cosmetically — `IndexDefinition` vs
`nimbus_core::IndexDefinition`, a `let updated_title` binding vs an inline
literal, and sequence number 19 vs 23 — and were shared safely.
`pipeline_barriers` differed only in an opaque barrier label, so the shared
helper mints a neutral `"sql-pipeline-{sequence}"`.

One assertion was rewritten in a clearer shape. Each provider defines its own
`IndexVersionInterval` struct, so rather than introduce an associated type the
probe flattens to `Vec<(DocumentId, SequenceNumber, Option<SequenceNumber>)>`,
and the shared scenario compares the whole vector at once:
...
This was initially written up here as a *stronger* assertion. The reviewer
refuted that, correctly: the originals asserted `len == 2` and then each
interval by position, which constrains exactly the same ordered tuples. The
rewrite is clearer and survives the per-provider struct difference; it is not
additional coverage, and the claim has been corrected rather than left standing.

## Declined Targets, With Numbers

| Target | Lines | Why declined |
| --- | --- | --- |
| `sql_pipeline_post_runner_cancellation_is_counted_once` | 2×60 | Real behaviour difference: `provider_operation_count` 2 vs 1, different error messages |
| `index_versions_*` **three-way** | — | Per-provider `IndexVersionInterval`; shared across the pair instead, where the tuple normalization is honest |
| `tenant_event_journal_replays_mixed_history` | 414 | libSQL calls `refresh_tenant_snapshot` mid-body; sharing needs an apply/assert split, which is a restructure, not a dedupe |
| `durable_replay_retires_recreated_table_identity` | 321 | Same shape — libSQL reopens with `open_existing_opened_tenant` mid-body |
| `table_lifecycle_activates_hidden_identity_and_diagnostics_track_layout` | — | Four inherent identity-staging methods with no shared trait |
| `provider_reloads_registry_after_reconnect` | — | Provider-specific reconnect semantics; this is a provider test, not a shared one |
| `execution_unit_batch_and_scheduler_state_round_trip` | 2×137 | Wave 4 — measured and declined, below |

### Wave 4 was conditional on hook size, and the hook loses

`execution_unit_batch_and_scheduler_state_round_trip` is the best remaining
candidate: 137 lines on each side, and the two bodies are **identical** once
provider tokens are normalized (the unified diff is the function-name line and
nothing else). It was still declined, on the measurement the ruling asked for.

Of the 13 methods the body calls, `apply_execution_unit_batch` is on
`SqlStoreCore`, `get` is on `TenantPointRead`, and `has_scheduled_work` /
`next_scheduled_work_at` are on `SchedulerStore`. The other **nine** —
`list_scheduled_jobs`, `claim_due_jobs`, `recover_running_jobs`,
`record_scheduled_job_result`, `complete_scheduled_job`,
`get_scheduled_job_result`, `save_cron_job`, `load_cron_jobs`,
`delete_cron_job` — are inherent methods on `postgres/read.rs` and
`mysql/read.rs` with no shared trait, so sharing this one scenario means a
nine-method forwarding trait plus its macro.

Compare the ratio to what wave 3 bought: `SqlPairProbe` is **7 methods serving
10 scenarios**. This would be **9 methods serving 1**. The abstraction would be
larger than the unique content it exists to host, and every future change to the
scheduler read surface would then touch the trait, the macro and the shared body
instead of one test. Net line saving is roughly 60 on a 1,371-line step. Not
worth the coupling; recorded here so the judgement can be revisited with the
numbers rather than re-derived.

## Verification

All lanes re-run against the exact committed tree. The three fixture containers
(`nimbus-external-provider-tests-{postgres,mysql,libsql}-1`) were `Up (healthy)`
throughout and were **not** torn down, with
`NIMBUS_REQUIRE_EXTERNAL_PROVIDER_FIXTURES=1` and the fixture URLs exported so a
missing fixture panics rather than skipping.

| Lane | Result |
| --- | --- |
| `cargo fmt --all --check` | RC=0 |
| `make clippy` (workspace, all targets) | RC=0 — only third-party `brotli`/`brotli-decompressor` warnings, zero first-party |
| `cargo clippy -p nimbus-storage --features libsql,mysql,postgres --all-targets` | RC=0 |
| `cargo clippy -p nimbus-storage --no-default-features --all-targets` | RC=0 |
| `scripts/test-taxonomy.py check` | RC=0 — `test taxonomy ok` |
| Feature matrix, `cargo check -p nimbus-storage --all-targets`, 8 configurations | **8/8 RC=0** |
| **Live fixtures** — `make test-external-provider PROVIDER=postgres` | **77 run, 77 passed**, 1103 skipped (85.8s) |
| **Live fixtures** — `make test-external-provider PROVIDER=mysql` | **47 run, 47 passed**, 1133 skipped (64.9s) |
| **Live fixtures** — `make test-external-provider PROVIDER=libsql` | **52 run, 52 passed**, 1128 skipped (103.4s) |
| **Live fixtures** — `nimbus-storage --features libsql,mysql,postgres` | **439 run, 439 passed**, 2 skipped (29.5s) |
| **Provider-free** — `nimbus-storage --no-default-features` | **297 run, 297 passed**, 2 skipped (5.8s) |

The 439 and 297 figures are the step-5 baselines unchanged, which is the point:
the step removed 1,371 lines of test source and changed neither the number of
tests nor their outcome.

The eight feature configurations are `(provider-free)`, `libsql`, `mysql`,
`postgres`, `libsql,mysql`, `libsql,postgres`, `mysql,postgres`, and
`libsql,mysql,postgres`. All eight matter here, because the two new shared
modules carry different gates (`any(libsql, mysql, postgres)` and
`any(mysql, postgres)`) and a dead-code error appears only in the
configurations that exclude a provider.

Every command run with `set -o pipefail`, redirected to a log file, and its
status read directly rather than through a pipe.

### The filter-identity proof

This is the ruling with the sharpest failure mode — a rename or a collapsed
wrapper would silently shrink what a provider lane runs — so it is proved on
names, not counts. For each provider, `cargo nextest list --message-format json`
was run with the lane's **official** filter from
`scripts/test-external-providers.sh`, and every test whose `filter-match.status`
is `"matches"` was collected into a sorted list. (`-E` does not prune the JSON;
counting rows instead of checking that field would have counted the whole
workspace.)

| Provider | Names selected | `nimbus-engine` | `nimbus-storage` | `nimbus-system` | vs. baseline |
| --- | --- | --- | --- | --- | --- |
| postgres | **77** | 41 | 35 | 1 | **IDENTICAL** |
| mysql | **47** | 11 | 35 | 1 | **IDENTICAL** |
| libsql | **52** | 18 | 33 | 1 | **IDENTICAL** |

`diff` against the pre-step baseline lists reports no difference for all three —
not merely the same count, the same names. The `<provider>_` prefix on every
wrapper is load-bearing for this, since the official filters are `^`-anchored
regexes over test names.

### The fixture manager's refusal was a port mismatch, and the first diagnosis was wrong

`make test-external-provider` initially failed RC=2 for all three providers:
PostgreSQL and MySQL with "refusing foreign or mismatched container; owner /
provider / image / config must match this fixture definition", libSQL with
"healthy owned libsql fixture already exists; set REUSE=1".

The fixture guard compares `com.docker.compose.config-hash` on the running
container against the hash of this worktree's compose file. Owner, provider and
image matched for all three; libSQL's hash matched, PostgreSQL's and MySQL's did
not.

**The first diagnosis was wrong, and the way it was wrong is the lesson.** This
worktree's `compose.test-external-providers.yaml` does differ from the primary
worktree's — it carries a five-line MySQL `--innodb-redo-log-capacity` block —
and that difference was a plausible enough explanation to stop at. It does not
survive contact with the evidence: it accounts for MySQL but not PostgreSQL,
whose service definition is byte-identical between the two files. A cause that
explains only some of the symptom is not the cause.

The real cause is that the compose file interpolates the published ports:

```
- "127.0.0.1:${NIMBUS_PROVIDER_FIXTURE_POSTGRES_PORT:-5432}:5432"
- "127.0.0.1:${NIMBUS_PROVIDER_FIXTURE_MYSQL_PORT:-3306}:3306"
- "127.0.0.1:${NIMBUS_PROVIDER_FIXTURE_LIBSQL_PORT:-18080}:8080"
```

The containers were created on 15432 / 13306 / 18080. `config --hash` computes
over the *resolved* config, so an invocation without those overrides exported
resolves to the 5432 / 3306 defaults and produces different hashes — while
libSQL matches regardless, because its default (18080) already equals its live
port. Supplying the overrides reproduces all three hashes exactly:

| Provider | Container label | Recomputed with live ports |
| --- | --- | --- |
| postgres | `2b5c39a9a21b52c8…` | `2b5c39a9a21b52c8…` |
| mysql | `e7507ad2fe3c94f6…` | `e7507ad2fe3c94f6…` |
| libsql | `bd1de6a612226865…` | `bd1de6a612226865…` |

So the guard was not detecting foreign containers; it was correctly refusing
containers whose published ports did not match the definition it had been asked
to resolve. With the port overrides and `REUSE=1 KEEP=1`, all three official
`make` lanes run and pass, and the containers stay up.

Two things follow. The guard is behaving correctly and should not be loosened —
it is exactly what prevents a lane from adopting a container built from someone
else's definition. And the near-miss is worth naming: the plausible-but-partial
first diagnosis would have been recorded as a permanent environmental caveat and
the three official lanes would have been reported as unrunnable, when they run
and pass. Tearing the containers down would have "fixed" it too, while
destroying the evidence and the standing instruction to keep them up.

## Step 6 — Review Disposition

**The reviewer ran for this step.** Steps 3 through 5 each recorded a reviewer
outage, and the first draft of this section copied that disclosure forward. It
was checked before being written down, and it was false: the Codex engine
(gpt-5.6-sol, xhigh) answers normally on this host as of 2026-07-30. The stale
outage claim was removed and a real review was run instead. Recording an outage
that has since cleared would have been the easiest paragraph in this document to
get wrong.

Verdict: **APPROVE**, no fixes required. The reviewer worked statically from
`git diff` and direct file reads — no cargo command, and no write to the tree.

All four rulings verified as holding, each against code rather than against my
description of it:

- **(a) provider-specific tests stay per-provider** — confirmed, including the
  PostgreSQL/MySQL operation-count divergence and the libSQL cache-refresh
  paths.
- **(b) gates and fixture policy unchanged** — provider modules keep their
  original feature gates (`tests.rs:43`), every scenario stays inside its
  provider's `with_test_provider`, and a missing fixture still panics
  (`provider_test_fixtures.rs:254`, `:280`). No skip-on-missing regression.
- **(c) names discoverable** — the `postgres_*` / `mysql_*` / `libsql_*`
  wrappers still match the anchored lane filters at
  `scripts/test-external-providers.sh:47`.
- **(d) taxonomy** — a full search of the test tree found no `env!` or
  `option_env!`; fixture reads remain runtime `std::env`.

Three of the four specific questions put to the reviewer came back clean, and
one corrected me:

1. **Assertion fidelity across all 14 shared scenarios: intact**, with ordering
   preserved. The MySQL 23→19 value change in the trigger-cursor scenario was
   checked and is coverage-neutral, because the write and the expected read
   changed together.
2. **No recursion risk** in the forwarding macros: both `impl_provider_read_oracles!`
   (`provider_support.rs:93`) and `impl_sql_pair_probe!` (`sql_pair_scenarios.rs:85`)
   invoke `<$store>::method`, naming the concrete type, so the inherent method
   is always the one selected. `SqlPairStore` carries no forwarding bodies at
   all.
3. **`SqlStoreCore::method(store, …)` confirmed semantically correct** — a no-op
   resolution change, not a behaviour change. Traced concretely: the inherent
   append facade delegates to `SqlStoreCore` (`store_core.rs:1233`), and both
   providers' `SqlStoreCore` impls forward `journal_progress` /
   `recover_durable_journal` to their own inherent implementations
   (`postgres/write.rs:114`, `mysql/write.rs:97`). `DurableJournal` forwards to
   that same inherent surface separately (`traits/provider_impls.rs:495`), so it
   was never the original target.
4. **The `cfg_attr` allowance is narrow enough** — it applies only when libSQL is
   absent, and all three fields are consumed by the libSQL wrapper. Field-level
   attributes would be textually narrower but semantically identical, and mask
   nothing.

The **one refutation** is recorded above where the claim was made rather than
only here: the index-version rewrite is clearer, not stronger, because the
originals already constrained the same ordered tuples via `len == 2` plus
per-position assertions. The proof text was corrected.

All seven dedupe exclusions were independently verified and agreed with,
including the wave-4 call — the reviewer separately confirmed the identity-
staging and reconnect scenarios diverge on real per-provider fields
(`schema_name` / `database_name` / `namespace`).
