...
Status: `audit complete. fail-before packet pending. no product edits`
...
The compiler derives identity from the tenant-qualified admitted workload UID.

```text
NetworkPlanId
  <- TenantId + TenantWorkloadUid

NetworkAttachmentId
  <- TenantWorkloadUid + attachment name

ListenerId
  <- TenantId + TenantWorkloadUid + listener name

PortLeaseId
  <- ListenerId

PublishedEndpointId
  <- TenantWorkloadUid + endpoint name

IngressRouteId
  <- TenantWorkloadUid + service name + route name
```
...
An IP address, host port, guest port, provider ID, provider handle, process ID,
or backend-generated `SandboxId` never forms workload identity. Changing an
address or requested port changes desired content, but it does not change the
stable resource ID.
...
| C3 | Source correlation is exact. | Crossed tenant, kind, name, owner, backend, generation, and node cases return named errors. |
...
| C18 | Quality and review gates pass once. | Focused and full affected tests, checks, strict Clippy, rustdoc, verifier, docs, and one candidate-frozen Sol review pass. |
...
11. No IP address, port, provider handle, or `SandboxId` forms a stable ID.
...
14. Every completed acceptance row has a recorded command and result.

The helper includes mutation tests for each item-local check. A missing input
or empty scan target fails.
...
cargo clippy -p nimbus-network -p nimbus-tenant -p nimbus-sandbox -p nimbus-workloads -p nimbus-compute --all-targets --all-features -- -D warnings
RUSTDOCFLAGS="-D warnings" cargo doc -p nimbus-network -p nimbus-tenant -p nimbus-sandbox -p nimbus-workloads -p nimbus-compute --no-deps
cargo fmt --all --check
...
| Expected-red tests | `pending` | Must land before product implementation. |
| Static verifier expected red | `pending` | Must record exact missing targets and OCI caller baseline. |
| Compiler implementation | `pending` | No product edit exists. |
| Acceptance C1-C17 | `pending` | No criterion is claimed early. |
| Candidate gates | `pending` | Run after implementation converges. |
| Full structured review | `pending` | Run once after candidate freeze. |
