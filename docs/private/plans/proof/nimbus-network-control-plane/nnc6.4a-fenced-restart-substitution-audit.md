...
| `crates/nimbus-cli/src/machine/backend/provision.rs` | 1,640 | One parent-host exact-phase adapter authenticates compute-confirmed commands and owns parent ingress lease/lifetime reconciliation. Restart-specific mapping and tests live in the `provision/restart.rs` and `provision/restart/tests.rs` children. Compute owns lifecycle order. The guest owns workload and forwarding effects. Extract the intact parent-publication state machine before this owner reaches 2,000 lines. |
...
| `crates/nimbus-sandbox/src/backends/container/runtime/machine_port_publication.rs` | 1,732 | One external-publication journal and exact command/authority authentication state machine. NNC6.4a adds planned-lease withdrawal and authenticated-absence reconciliation; NNC6.5d4 adds read-only publication classification and an exact complete-absence witness without a second authority. Machine transport and provider selection remain outside. |
| `crates/nimbus-sandbox/src/backends/oci/port_lifecycle.rs` | 1,589 | One OCI port transition state machine. Authority construction, batch state, machine-specific behavior, and planned-Netavark lifetime remain in the concept-owned `port_lifecycle/authority.rs`, `port_lifecycle/batch_state.rs`, `port_lifecycle/machine.rs`, and `port_lifecycle/netavark_lifetime.rs` children. |
...
| Exact item identity | Commit `a37a87f86ee80252812fda66d33d23f05e73d0d4`; tree `84f80479f3fea331283e5239c7f228f57ac76ae8`; complete item patch SHA-256 `af56a260caac24d1b1a167b669fb64511c41e54291a9e777e4697b6bd6159827`; complete executable/script patch SHA-256 `45a0323f8323a27062ac2a6a2d77c9307170d64e2fdff7eea149a6b6f018f797`; final staged patch SHA-256 `773fe1c8cc78016dce77a3d7e5ea3f21e9495c984c08e7b01292c8b6d045c974`. |
...
| Acceptance | A1-A20 are green. NNC6.4a is complete at exact item commit `a37a87f86ee80252812fda66d33d23f05e73d0d4`. NNC6.5 is the sole active teardown owner. |
