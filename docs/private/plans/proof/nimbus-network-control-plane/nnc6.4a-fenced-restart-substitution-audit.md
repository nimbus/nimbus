...
| `crates/nimbus-cli/src/machine/backend/provision.rs` | 1,697 | One parent-host exact-phase adapter authenticates compute-confirmed commands and owns parent ingress lease/lifetime reconciliation. Restart-specific mapping and tests live in the `provision/restart.rs` and `provision/restart/tests.rs` children. Compute owns lifecycle order. The guest owns workload and forwarding effects. Extract the intact parent-publication state machine before this owner reaches 2,000 lines. |
