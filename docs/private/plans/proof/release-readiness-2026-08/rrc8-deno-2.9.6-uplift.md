...
| U3 | Audit the 21 Deno carries and replay only product-required concepts on upstream 2.9.6. | `complete` | Seven local commits end at exact reviewed correction `8d48dc4a68df8e083ed4b17855440b1df6405620`. The paired Nimbus cleanup remains at baseline `e0cbb5937d5390d44a597b6ef45ed7003e267a03` for the U6 repin commit. Controlled A/B, observed-mode crossover, full local CI, macOS, Linux debug-profile, application, desktop, and final Sol review evidence pass. The 8 GiB Linux full-LTO attempt is recorded as resource-blocked. |
...
| U6 | Repin Nimbus, update fork policy, and run the exact release replay. | `in_progress` | `Cargo.toml` and `Cargo.lock` resolve 41 Deno packages at `v2.9.6-nimbus.1#6c37e683` and rusty_v8 at `v150.4.0-nimbus.1#961a76d0`. Fork provenance, upstream policy, standardization, the exact-tag all-target runtime check, the canonical runtime gate, and full repository CI pass. Release-critical smoke, artifact replay, and the higher-memory Linux release build remain. |
