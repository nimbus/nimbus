...
Next action: SIC2 merged to `main` as `09d1003d3` and the plan is current through blocker B4. Next is SIC3, cut from updated `main` as `codex/sic-sic3`: turn the SIC0 writer census into one checked ownership matrix so no writer can omit an effect through a default, an option, or an opaque callback.
