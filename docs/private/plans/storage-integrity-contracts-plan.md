...
Next action: SIC3 is `merged` (#285, `6de99e977`). Cut `codex/sic-sic4` from updated `main` and execute SIC4, the canonical materialized position: one storage-owned `MaterializedPosition` over applied sequence plus a logical state digest with durable head kept separate, the engine canonicalizer in `crates/nimbus-engine/src/verification.rs` deleted, and PITR targets, journal bootstraps, shadow checkpoints, replica comparison, and diagnostic reports bound to the position. Verifier conditions 9 through 11 are the gate.
