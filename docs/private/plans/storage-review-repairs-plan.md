...
Next action: add the SRR1 fail-before tests for resource bindings and the trigger cursor
...
| SRR1 | Extend canonical materialized identity through full and incremental verification. | `in_progress` | |
| SRR2 | Make nonzero-base PITR import atomic and seed MVCC anchors. | `todo` | |
