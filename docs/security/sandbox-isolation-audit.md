...
`neovex-sandbox`, including the patched `crun` runtime and the OCI bundle
configuration generated by the krun backend.

## Scope

- OCI bundle generation (`crates/neovex-sandbox/src/backends/krun/bundle.rs`)
- Patched crun with TSI port-map annotation support
- Host-side process privileges and namespace configuration
- Guest-side user switching via `neovex-guest-user-switch`
- Buildah image mount trust boundary

Out of scope: V8 isolate security (process-level, not hardware), the
`neovex-runtime` HostBridge trait surface, and application-layer auth
enforcement in `neovex-engine`.
...
```
buildah unshare -> conmon -> /usr/libexec/neovex/crun (patched) -> libkrun -> KVM microVM
```
...
inside the guest, `neovex-guest-user-switch` drops privileges to the image's
declared `USER` before starting the service process. The helper binary is
bind-mounted read-only at `/.neovex/neovex-guest-user-switch`.
...
`crates/neovex-sandbox/src/backends/krun/bundle.rs`. Add
...
field on `SandboxPortBinding` (`crates/neovex-sandbox/src/spec.rs:193`) and
...
Neovex already maintains a patched crun
...
This approach keeps neovex on the standard OCI runtime interface while
...
`.github/workflows/verify-neovex-crun-patch.yml`), build scripts
(`scripts/build-neovex-crun.sh`), and pinned versions (crun 1.27, libkrun
...
Neovex maintains patched forks of components in the krun launch chain. The
...
Build scripts: `scripts/build-neovex-crun.sh`, `scripts/verify-crun-patch.sh`
CI: `.github/workflows/verify-neovex-crun-patch.yml`
