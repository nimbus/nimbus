...
When an action needs Node built-ins, opt that module into the Node.js-compatible
runtime with `"use node"`. Examples include `node:crypto` and `node:buffer`.
This also applies to an npm SDK that expects Node APIs.
...
Node26 is a Current/non-LTS compatibility target. Node20 remains selectable
only as a legacy-grace target after its 2026-04-30 end of life. Select a
version with `node.nodeVersion` in `convex.json`. See
...
not grant host access. The runtime permission mode and explicit grants control
filesystem, network, environment, subprocess, and other resource access. See
...
- [Packages and bundling](/developers/runtimes/nodejs/packages-and-bundling/)
  explains how to use npm packages in Node actions.
